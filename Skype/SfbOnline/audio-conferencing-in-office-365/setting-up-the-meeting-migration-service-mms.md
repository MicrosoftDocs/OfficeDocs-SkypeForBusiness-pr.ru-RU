...
- Когда пользователь в Сети обновляется только Teams или если в режиме пользователя в TeamsUpgradePolicy установлено имя SfBwithTeamsCollabAndMeetings
...
Это самый распространенный сценарий, в котором MMS помогает создать более плавный переход для пользователей. Без переноса собраний существующие собрания, организованные пользователем Skype для бизнеса Server локальной сети, больше не будут работать после его переноса в интернет-службу. Поэтому при использовании средств локального администрирования (или панели управления администратором) для перемещения пользователя в облако существующие собрания автоматически перемещаются в облако следующим `Move-CsUser` образом:

- Если задан переключатель, собрания переносятся непосредственно в Teams и пользователь будет работать `MoveToTeams` `Move-CsUser` в режиме TeamsOnly. Для использования этого переключателя требуется Skype для бизнеса Server 2015 с cu8 или более поздней. Эти пользователи по-прежнему могут присоединяться Skype для бизнеса собраниям, на которые они могут быть приглашены, используя клиент Skype для бизнеса или приложение Skype для собраний.
...
- Если номер телефона для моста аудиоконференции не назначен. Это сложный сценарий, который требует дополнительных действий. Дополнительные сведения см. в сведениях Об изменении номеров телефонов на мосте [аудиоконференции.](/MicrosoftTeams/change-the-phone-numbers-on-your-audio-conferencing-bridge)
...
**SourceMeetingType:**
- Использование означает, что Skype для бизнеса собрания (как локального, так и `SourceMeetingType SfB` сетевого) должны обновляться.
- Использование означает, что Teams только собрания `SourceMeetingType Teams` должны обновляться.
...
- Чтобы получить сводное состояние для всех миграций MMS, запустите следующую команду, которая предоставляет табли представление всех штатов миграции:
...
- Чтобы проверить состояние миграции для определенного пользователя, используйте `Identity` параметр. Например, следующая команда получит состояние переноса для пользователя ashaw@contoso.com:
    ...
    ```
Если вы видите, что миграция завершилась сбойом, как можно скорее примите меры для устранения этих проблем, так как пользователи не смогут звонить на собрания, организованные этими пользователями, пока вы не уберите их. Если `Get-CsMeetingMigrationStatus` показана любая миграция в состоянии сбойного выполнения, выполните следующие действия:
...
Чтобы узнать, включена ли MMS для вашей организации, запустите следующую команду: MMS включена, если `MeetingMigrationEnabled` задан параметр `$true` .
```PowerShell
