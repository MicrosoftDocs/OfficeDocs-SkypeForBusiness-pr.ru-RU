...
- Если пользователь переносится из локальной сети в облако (Skype для бизнеса Online или TeamsOnly).
- Когда администратор вносит изменения в настройки голосовой конференции пользователя, 
- Если пользователь в сети обновляется только для Teams или режим пользователя в TeamsUpgradePolicy имеет значение SfBwithTeamsCollabAndMeetings
...
- Когда администратор вносит изменения в настройки голосовой конференции пользователя, 
- Если пользовательским режимом в TeamsUpgradePolicy задано значение TeamsOnly или SfBWithTeamsCollabAndMeetings (с помощью PowerShell или портала администрирования Teams)
...
- Если `MoveToTeams` указан параметр in in `Move-CsUser` , собрания переносятся непосредственно в Teams, и пользователь переходит в режим TeamsOnly. Для использования этого переключателя требуется Skype для бизнеса Server 2015 с обновления HF1 или более поздней версии. Эти пользователи по-прежнему могут присоединиться к собранию Skype для бизнеса, на которое они могут быть приглашены, с помощью клиента Skype для бизнеса или из приложения "собрания Skype".
...
- Миграция собрания вызывается только при предоставлении `TeamsUpgradePolicy` определенного пользователя. Если вы предоставляете `TeamsUpgradePolicy` `mode=TeamsOnly` или `mode=SfBWithTeamsCollabAndMeetings` на *уровне клиента* , то миграция собрания не вызывается.
- Пользователь может предоставить режим TeamsOnly только в том случае, если пользователь подключен к сети. Локальные пользователи должны перемещаться с помощью `Move-CsUser` описанных выше функций.
- Предоставление режима, отличного от TeamsOnly или SfBWithTeamsCollabAndMeetings, не приводит к преобразованию существующих собраний Teams в собрания Skype для бизнеса.
...
Помимо автоматических миграций собраний, администраторы могут вручную инициировать миграцию собраний для пользователя, запустив командлет `Start-CsExMeetingMigration` . Этот командлет помещает в очередь запрос на перенос для указанного пользователя.  В дополнение к обязательному `Identity` параметру принимаются два необязательных параметра, `SourceMeetingType` `TargetMeetingType` которые позволяют задать способ миграции собраний.

**TargetMeetingType:**

- Если вы `TargetMeetingType Current` указываете, что собрания Skype для бизнеса останутся на собраниях Skype для бизнеса, а собрания Teams остались на собраниях Teams. Однако возможно изменение координат голосовой конференции и перенос локальных собраний Skype для бизнеса в Skype для бизнеса Online. Это значение по умолчанию для TargetMeetingType.
...
- Использование `SourceMeetingType All` указывает на то, что вы должны обновить как собрания Skype для бизнеса, так и собрания Teams. Это значение по умолчанию для SourceMeetingType.
...
Вы можете `Get-CsMeetingMigrationStatus` проверить состояние миграции собраний с помощью командлета. Ниже приведено несколько примеров.
...
- Чтобы получить сводное состояние всех миграций MMS, выполните следующую команду, которая обеспечивает представление всех состояний миграции в виде таблицы.
    ...
    Succeeded   131
    ```
- Чтобы получить полные сведения обо всех миграции за определенный период времени, используйте `StartTime` `EndTime` Параметры и. Например, следующая команда возвращает полные сведения обо всех миграции, произошедших с 1 октября 2018 г. на 8 октября 2018 г.
...
- Отключить только для тех изменений, которые относятся к голосовой конференции. В этом случае MMS будет выполняться, когда пользователь переносится из локальной сети в облако или когда вы предоставляете режим TeamsOnly или SfBWithTeamsCollabAndMeetings `TeamsUpgradePolicy` .
...
Чтобы проверить, включена ли поддержка MMS для вашей организации, выполните следующую команду: MMS включен, если `MeetingMigrationEnabled` параметр имеет значение `$true` .
```PowerShell
