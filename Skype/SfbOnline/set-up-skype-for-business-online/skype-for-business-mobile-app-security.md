...
|**Android** <br/> |Учетные данные хранятся в учетных записях Android. Кроме того, мы шифруем учетные данные, прежде чем сохранять их в учетных записях. Для шифрования используется алгоритм **AES/CBC/PKCS5Padding** " . <br/> |Мы храним в зашифрованной SQL с помощью библиотеки [sqlcipher.](https://www.zetetic.net/sqlcipher/design/) Мы используем алгоритм 256-битной версии AES по умолчанию в режиме CBC. Хранимые данные всегда шифруются в файле базы данных и расшифровываются только при переходе между переменной памятью и стопкой вызовов приложения. Кроме того, файлы голосовой почты шифруются таким же способом, что и имя пользователя и пароль (они не хранятся в DB). Голосовая почта временно расшифрована на диске, чтобы разрешить воспроизведение.  <br/> |Эта информация не шифруется.  <br/> |
