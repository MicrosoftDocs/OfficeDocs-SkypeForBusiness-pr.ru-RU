...
Из этого и других отчетов по Skype для бизнеса вы можете подробно и подробно известить об использовании услуг, в том числе звонков, в вашей организации. Эти сведения полезны при исследование, планировании и принятии других решений на предприятии, а также при настройке кредитов на [связь.](/microsoftteams/what-are-communications-credits)
...
- Перейдите в Центр администрирования > **центрах** администрирования Skype для бизнеса, в которых отчеты об использовании  >    >    >  **услуг ННР.**
...
[![Отчет об использовании ОКП в Skype для бизнеса ](../images/79d7aadf-c69e-4d6a-8179-ab69dbbb2472.png)](../images/79d7aadf-c69e-4d6a-8179-ab69dbbb2472.png#lightbox)
...
*    **Тип звонка**  сведения о том, выполнен ли входящий или исходящий звонок по ТСОП, а также информация о типе звонка (например, звонок выполнен пользователем или по аудиоконференции). Могут отображаться следующие типы звонка: 
     ...
     *    **ucap_in (входящий** вызов через ННР в приложение для службыuc, например автоотвщик или очередь вызовов) 
     *    **ucap_out** (исходящие вызовы через STN из приложения дляuc, например автоотетаря или очереди вызовов)
         > [!NOTE]
         > Звонки, которые были переданы пользователю из приложения дляuc, например автоответ или очередь звонков, не отображаются в отчете об использовании ЗВОНКОВ, так как эти вызовы являются одноранговые (P2P) аудиозвонками. Вы можете получать доступ к звонкам P2P в Центре администрирования Skype для бизнеса в области "Инструменты > Аналитика звонков Skype для бизнеса" и искать по имени пользователя или SIP-адресу, соотносяющий звонок с датой, временем и/или именем пользователя (ИД строки звонков). 
...
> [!NOTE]
> Если вы хотите, чтобы в отчет включались только звонки с поминутной оплатой, не включенные в подписку на звонки или телефонные услуги, отфильтруйте отчет с помощью функции MCOPSTNPP. При этом будут предоставляться позиции для всех звонков с поминутной оплатой.  Для аудиоконференции с поминутной оплатой отфильтруем по функции MCOMEETACPEA вместо MCOPSTNPP.  

> [!NOTE]
> В некоторых полях также может от нее от быть "нет данных". "Нет данных" означает, что это поле не относится к типу или возможности звонка. 
...
Нажатие кнопки "Экспорт в **Excel"** позволяет скачать отчет об использовании ДНР. Вы можете экспортировать данные сроком до одного года с текущей даты, если только в отдельных странах их хранение запрещено в течение 12 месяцев.
...
Если у вас есть как планы звонков, так и прямая маршрутия, экспортный файл может содержать данные для обоих продуктов. Файл отчета об использовании ДНР будет иметь имя файла **"PSTN.calls.[ `UTC date` ]. csv**". В дополнение к файлам ННР и прямой маршрутингу архив содержит файл **"parameters.js**", а также выбранный диапазон времени экспорта и возможности (если таковой имеется).
...
- [В отчете](pstn-minute-pools-report.md) по пулам минут ЗВОНКОВ по ПС Skype для бизнеса можно увидеть количество минут, посещаемых в организации за текущий месяц.
