...
- Перейдите в центр администрирования > **центры**  >  **администрирования Skype для бизнеса Отчеты** об использовании  >    >  **ОКП**.
...
*    **Тип звонка**  сведения о том, выполнен ли входящий или исходящий звонок по ТСОП, а также информация о типе звонка (например, звонок выполнен пользователем или по аудиоконференции). Могут отображаться следующие типы звонка: 
     ...
     *    **conf_out** (исходящие вызовы с моста аудиоконференции, обычно для добавления к конференции номера ДНР). Для записей такого типа звонка пользователь, указанный в столбце **ИД** пользователя, соответствует организатору собрания.

     **Приложения объединенных коммуникаций (UCAP)** 
     *    **ucap_in** (входящие вызовы через ОКП, такие как автозаказ или очередь вызовов) 
     *    **ucap_out** (исходящие вызовы через ПС из приложения дляuc, например автозавода или очереди вызовов)
         > [!NOTE]
         > Звонки, которые были перенаправлены пользователю из приложения для UC, например автоответ или очередь звонков, не отображаются в отчете об использовании ЗВОНКОВ по ННР, так как они являются одноранговые (P2P) аудиозвонками. Вы можете получать доступ к звонкам P2P в Центре администрирования Skype для бизнеса в области "Средства > Skype для бизнеса Аналитика звонков" и искать по имени пользователя или SIP-адресу, соотносящем звонок по дате, времени и/или источнику CLID (ИД строки звонков). 
...
> [!NOTE]
> Если вы хотите запустить отчет, включив в него только звонки поминутно, которые не включены в подписку на звонки или услуги для связи, отфильтруйте отчет с помощью функции "MCOPSTNPP". При этом будет засвечиваться все звонки с поминутной оплатой.  Для аудиоконференций с поминутной оплатой отфильтруем по полю MCOMEETACPEA вместо MCOPSTNPP.  

> [!NOTE]
> В некоторых полях также может быть "нет данных". "Нет данных" означает, что поле не относится к типу или возможности звонка. 
...
Нажав кнопку **Экспорт в Excel,** можно скачать отчет об использовании ННР. Вы можете экспортировать данные сроком до одного года с текущей даты, если только в отдельных странах не запрещено хранение данных в течение 12 месяцев.
