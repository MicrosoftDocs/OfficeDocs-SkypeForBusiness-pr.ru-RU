...
Skype для бизнеса планы звонков в Интернете теперь поддерживают блокирование входящие вызовы из телефонной сети общего звонков (STN). Эта функция позволяет определить глобальный список шаблонов номеров клиента, чтобы в списке на совпадение можно было проверить ИД звонящая для каждого входящих вызовов по ДНР в клиенте. Если совпадение выполнено, входящий звонок отклоняется.
...
Шаблоны номеров можно управлять с ```CsInboundBlockedNumberPattern``` помощью команд ```New```, ```Get```, ```Set```и ```Remove```. Вы можете управлять заданным шаблоном с помощью этих cmdlets, включая возможность переключения активации заданного шаблона.
...
Просмотром и активацией всей функции блокировки зовов управляется с помощью ```CsTenantBlockingCallingNumbers``` ```Get``` команд и ```Set```.
...
В этом примере параметры ```-Enabled``` и ```-Description``` параметры необязательны:
...
В этом примере параметр ```-Identity``` является требуемой:

```powershell
Remove-CsInboundBlockedNumberPattern -Identity “<identity>”
```
 
Если удостоверение неизвестно, используйте его, ```Get-CsInboundBlockedNumberPattern``` чтобы сначала найти нужный шаблон и заметить удостоверение. Затем запустите этот ```Remove-CsTenantBlockedNumberPattern``` cmdlet и передатите соответствующее значение удостоверения.
...
Вы можете добавить исключения для заблокированных ```CsTenantBlockNumberExceptionPattern``` шаблонов с помощью команд, ```New```, ```Get```, и ```Set``````Remove```.
...
В этом примере создается шаблон исключения для номеров, который по умолчанию добавляет шаблон как включенный. Параметры ```-Enabled``` ```-Description``` и параметры необязательны.
...
В этом примере параметр -Identity является обязательным. Он ```Set-CsTenantBlockedNumberExceptionPattern``` позволяет изменить один или несколько параметров для идентификатора заданного числового шаблона.
...
В этом примере ```-Identity``` параметр является требуемой. Этот cmdlet удалит заданный шаблон номера из списка клиента.  Если удостоверение неизвестно, используйте его, ```Get-CsInboundBlockedNumberPattern``` чтобы сначала найти нужный шаблон и заметить удостоверение. Затем запустите этот ```Remove-CsTenantBlockedNumberExceptionPattern``` cmdlet и передатите соответствующее значение удостоверения.Разрешить время репликации перед проверкой и проверкой.  
...
Используйте его ```Test-CsInboundBlockedNumberPattern``` для проверки того, заблокирован ли номер в клиенте.
 
В этом примере необходимы ```-Phonenumber``` параметры ```-Tenant``` и параметры. Параметр ```-PhoneNumber``` должен быть числовой строкой без дополнительных символов, таких как +или -. В TRPS это ```-Tenant parameter``` необязательный вариант. Результат возвращает значение ```isNumberBlocked``` True, если число заблокировано в клиенте, и False, если оно не заблокировано.
...
Как говорилось ранее, соответствие шаблона для блокирования вызывающих вызовов делается с помощью Regex. В Интернете доступно несколько инструментов для проверки совпадения с шаблоном Regex. Если вы не знакомы с шаблонами Regex, советуем ознакомиться с основами в течение некоторого времени. Чтобы получить ожидаемые результаты, используйте инструмент для проверки совпадений с шаблонами перед добавлением новых заблокированных номеров в клиент. 
