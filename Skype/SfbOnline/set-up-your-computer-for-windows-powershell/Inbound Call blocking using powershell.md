...
---
# <a name="inbound-pstn-call-blocking-for-calling-plans"></a>Входящий звонок через ТСОП блокировки для Тарифные планы

Скайп для бизнеса планы Online вызов теперь поддерживает блокирование для входящих вызовов из телефонной сети общего пользования (PSTN). Эта функция позволяет клиента глобального списка шаблоны номеров можно определить, поэтому Звонящего каждого входящего вызова ТСОП для клиента могут быть установлено списка для соответствия. Входящий звонок отклоняется, если совпадение. 
...
В этом примере параметр identity при необходимости (is?):`Remove-CsInboundBlockedNumberPattern -Identity “<identity>”`
