...
Если вы развертываете Skype для бизнеса Online в составе Microsoft 365, Office 365, Skype для бизнеса Server Hybrid или Skype для бизнеса Cloud Connector Edition, необходимо понимать взаимодействие между клиентом и серверами Skype для бизнеса, а также поток зовов, чтобы эффективно планировать, развертывать, использовать и устранять неполадки веб-служб Skype для бизнеса.
 ...
 **ExpressRoute** Это сетевой сегмент, который является частью вашей общей сети и обеспечивает выделенное, частное подключение к сети Майкрософт. Это рекомендуемый вариант для подключения сети к сети Майкрософт (Microsoft 365 или Office 365 центрам обработки данных) для всех рабочих нагрузок, зависящих от скорости и производительности сети, например для связи Skype для бизнеса Online в режиме реального времени. Подключения ExpressRoute между вашей сетью и сетью Майкрософт используют поставщики подключения [ExpressRoute](/azure/expressroute/expressroute-locations) для обеспечения закрытой и управляемой сети, обеспечивая 99,9 % простоя и поддержку качества обслуживания (QoS), что может повысить производительность мультимедиа в режиме реального времени в периоды перегрузки сети.

 **Сеть Майкрософт** Это сетевой сегмент, который является частью общей сети, которая поддерживает Microsoft 365 и Office 365 служб. К ним относятся все связи между серверами Online для Microsoft 365 или Office 365. Сюда может входить трафик, который проходит через магистраль сети Майкрософт и передается между географическими регионами.
...
Назначения для этого трафика находятся в Office 365 [URL-адресах](https://support.office.com/article/8548a211-3fe7-47cb-abb1-355ea5aa88a2) и диапазонах IP-Microsoft 365 или Office 365 службах. Для каждого URL-адреса указывается, может ли эта часть трафика проходить через ExpressRoute для Microsoft 365 или Office 365. Схемы, которые показывают, что Интернет по-прежнему используется для некоторого трафика, если включена служба [ExpressRoute,](https://support.office.com/article/6d2534a2-c19c-4a99-be5e-33a0cee5d3bd)см. в этой Office 365. Важно понимать, что даже URL-адреса, которые указаны как маршрутируемые по ExpressRoute, также можно маршрутить через Интернет. Это означает, что в некоторых случаях определение того, будет ли использоваться Интернет или ExpressRoute, зависит от расположения клиента и конфигурации прокси-серверов и брандмауэров. Кроме того, важно понимать, что не все URL-адреса, связанные с Microsoft 365 или Office 365, могут использовать ExpressRoute, подключение к Интернету требуется даже при покупке ExpressRoute у партнера ExpressRoute.

Трафик, который можно передавать только через Интернет, включает распространенные интернет-зависимости, такие как списки отзыва сертификатов (CRLS), DNS-запросы и разрешение имен, URL-адреса общих служб Microsoft 365 или Office 365, например служб Центр администрирования Microsoft 365, а также некоторые функции связи Skype для бизнеса Online, не в реальном времени, такие как телеметрия и федерация для взаимодействия с Skype потребителем, а также мультимедиа, потоковая передача данных для трансляции собрания Skype. При планировании сетевой маршрутизации ознакомьтесь с разделом [Маршрутизация с помощью ExpressRoute для Office 365](https://support.office.com/article/e1da26c6-2d39-4379-af6f-4da213218408), в котором приведены различные факторы и условия.
...
1. Конференция Skype для бизнеса в том же регионе, где находится организатор конференции. Эта возможность находится в облаке, если организатором является пользователь Online, или в локальном центрах обработки данных, если организатор собрания является пользователем локальной сети.
...
Когда пользователь гибридной сети присоединяется к конференции, которая проходит через Интернет, мы знаем, что сигнальный сигнал и мультимедиа будут направляться в облако Microsoft 365 или Office 365, а так как пользователь присоединяется из Интернета, обычно идет прямой путь к Интернету. Однако в некоторых случаях, например вследствие ограничений брандмауэра, прямой маршрут через Интернет недоступен. В этом случае при ретрансляции медиапотока на локальном edge-сервере трафик мультимедиа может возвращаться к локальной сети перед переходом через канал ExpressRoute в облако.
 ...
 **Звонок через ПСС через телефонная система в Microsoft 365 или Office 365 и облачной соединители**
...
> [!NOTE]
> ExpressRoute будет использоваться на пути мультимедиа от пользователя, расположенного в корпоративной сети, до онлайн-сервера, но не будет использоваться, если в локальном развертывании другого клиента используется edge-сервер.

### <a name="call-flows-for-skype-for-business-hybrid"></a>Потоки вызовов для гибридный Skype для бизнеса

Гибридные потоки зовов применяются при развертывании Skype для бизнеса с по крайней мере некоторыми пользователями, которые размещены на локальной основе. Потоки звонков в этом разделе включают как конференции на локальной основе, так и одноранговую или ЗВОНКОВ по СТП, включив по крайней мере одного локального пользователя.
...
Все пользователи, которые будут подключаться к Cloud Connector Edition, находятся на домашней платформе Online. Это означает, что конференции будут проходить в сети, а сигнальный сигнал будет следовать тем же шаблонам, что и для пользователей Online. В сценариях, кроме звонков по ЗВОНКОВ по ЗВОНКОВ по ННР, поток звонков будет точно таким же, как описано выше для Skype для бизнеса Online.



|**Сценарий использования** <br/> |**Конечные точки** <br/> |**Путь сигнального трафика** <br/> |**Маршрут мультимедиа** <br/> |**Пример потока** <br/> |**Примечания** <br/> |
|:-----|:-----|:-----|:-----|:-----|:-----|
|Вызов ТСОП  <br/> |Пользователь Online в своей сети, использующий Cloud Connector Edition.  <br/> |локальный  <br/> |локальный  <br/> |[Звонок через ПС SKYPE ДЛЯ БИЗНЕСА CLOUD CONNECTOR EDITION](call-flow-using-expressroute.md#bk_Figure6) <br/> ||
|Вызов ТСОП  <br/> |Пользователь Online, использующий Интернет с помощью Cloud Connector Edition.  <br/> |Интернет  <br/> |Интернет  <br/> |Сочетание [локального edge-сервера](call-flow-using-expressroute.md#bk_Figure5) с Microsoft 365 или Office 365 конференций и [звонками по STN](call-flow-using-expressroute.md#bk_Figure6)с помощью Skype для бизнеса Cloud Connector Edition.  <br/> |Пользователи Интернета подключаются через edge-сервер, который входит в cloud Connector, а Cloud Connector — к сети STN.  <br/> |
