...
Если вы развертываете Skype для бизнеса Online в составе Microsoft 365 или Office 365, Skype для бизнеса Server Hybrid или Skype для бизнеса Cloud Connector Edition, необходимо понимать взаимодействие между клиентом и серверами Skype для бизнеса и поток зовов, чтобы эффективно планировать, развертывать, использовать и устранять неполадки веб-служб Skype для бизнеса.
...
Когда пользователь гибридной сети присоединяется к конференции, которая проходит через Интернет, мы знаем, что сигнальный сигнал и мультимедиа будут направляться в облако Microsoft 365 или Office 365, а так как пользователь присоединяется из Интернета, обычно идет прямой путь к Интернету. Однако в некоторых случаях, например вследствие ограничений брандмауэра, прямой маршрут через Интернет недоступен. В этом случае при ретрансляции трафика мультимедиа может быть ретранслятор локального edge-сервера, что приводит к возврату трафика мультимедиа к локальной сети перед переходом через канал ExpressRoute в облако.
...
Гибридные потоки зовов применяются Skype для бизнеса развертывания, включаемого по крайней мере некоторых пользователей, размещенных на локальной основе. Потоки звонков в этом разделе включают как конференции, так и однорангые вызовы или вызовы через ОКП хотя бы для одного локального пользователя.
...
|Вызов ТСОП  <br/> |Пользователь Online, использующий Интернет с помощью Cloud Connector Edition.  <br/> |Интернет  <br/> |Интернет  <br/> |Сочетание [локального edge-сервера](call-flow-using-expressroute.md#bk_Figure5) с Microsoft 365 или Office 365 конференций и вызовами [через ПС](call-flow-using-expressroute.md#bk_Figure6)SKYPE ДЛЯ БИЗНЕСА CLOUD CONNECTOR EDITION.  <br/> |Пользователи Интернета подключаются через edge-сервер, который входит в cloud Connector, а Cloud Connector — к сети STN.  <br/> |
