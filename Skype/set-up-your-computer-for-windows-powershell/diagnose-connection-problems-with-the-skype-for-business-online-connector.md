...
- **Решение**: часто эта ошибка означает, что не работает Microsoft Online Services помощник по входу. Можно проверить состояние этой службы, выполнив следующую команду в командной строке PowerShell: 
    ```
...
- **Решение**: закрыть один или несколько предыдущих подключений является единственным способом для устранения этой проблемы. По окончании с Скайп для бизнеса в Интернет сеанса, рекомендуется использовать командлет **Remove-PSSession** для прерывания сеанса. Это поможет вам для предотвращения этой проблемы.
...
- **Решение**: закрыть один или несколько предыдущих подключений является единственным способом для устранения этой проблемы. По окончании с Скайп для бизнеса в Интернет сеанса, рекомендуется использовать командлет **Remove-PSSession** для завершения этого сеанса. Это поможет вам для предотвращения этой проблемы.  
