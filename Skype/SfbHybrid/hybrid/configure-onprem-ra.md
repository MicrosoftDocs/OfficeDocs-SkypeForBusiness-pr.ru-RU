...
8. Связать учетную запись ресурса с автоотчетом или очередью вызовов: (New-CsApplicationInstanceAssociation).
...
Если создаваемая очередь вызовов или автоспутник телефонной системы будут вложены и не требуют номера телефона, процесс будет:
...
6. Назначьте номер службы учетной записи ресурса. Используйте `Set-CsHybridApplicationEndpoint` команду, чтобы назначить номер телефона (с параметром -LineURI) учетной записи ресурса.
   ...
   Учетной записи ресурса потребуется соответствующий номер телефона, если он будет назначен автоотчету верхнего уровня или очереди звонков. Учетной записи ресурса не могут быть назначены телефонные номера пользователей (подписчиков), можно использовать только платные или бесплатные номера телефонов служб.
...
В этом разделе обсуждается создание локальной учетной записи ресурса. Создание учетной записи ресурса, которая находится в Сети, обсуждается на веб-сайте "Управление учетными записями [ресурсов" в Microsoft Teams.](/MicrosoftTeams/manage-resource-accounts)
...
Для миграции с exchange UM на телефонную систему потребуется повторное воссоздание очереди вызовов и структуры автозахваждения, прямой перенос с одного на другой не поддерживается. Для повторной реализации набора очередей вызовов и автоспутников:
...
2. Для каждой из перечисленных очередей вызовов или автозахватаря exchange следует обратить внимание на ее место в структуре, параметрах и получить копии связанных звуковых или текстовых файлов (guid в выходных данных будет именем папки, в которой хранятся файлы). Эти сведения можно получить с помощью команды:
...
3. Создайте новые конечные точки локально, как описано выше.
   Назначьте автоотводу верхнего уровня временный номер для тестирования.

4. Настройте автоотвод или очередь вызовов телефонной системы, использующую конечные точки, как описано выше.

5. Протестировать автосватаря телефонной системы или очередь звонков.
...
[Управление учетными записями ресурсов в Microsoft Teams](/MicrosoftTeams/manage-resource-accounts)  -  \( создание учетных записей ресурсов, которые были в сети\)
