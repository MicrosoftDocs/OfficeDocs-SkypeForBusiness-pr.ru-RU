...
Обратитесь к данной статье представлен список [измерений](dimensions-and-measures-available-in-call-quality-dashboard.md)и мер, доступные в вызов качества панели мониторинга для групп Майкрософт и Скайп для бизнеса в Интернет.
  
> [!NOTE]
> Сведения об обновлениях и изменения в панели мониторинга можно найти, нажав кнопку **хорошо!** в панели мониторинга. Перейдите к [панели мониторинга качество звонка](https://aka.ms/CQDOnline). 
...
Потоки классифицируются в три группы: хороший низкого уровня и Неклассифицированные. Существует также рассчитываются *низкого уровня %* со значениями, позволяющие вам отношение числа потоков рассматривать как * плохих * количеству общее секретная потока. С момента *низкого уровня % = низкий потоков / (низкого уровня потоков + хороший потоков) * 100* , это делает *низкого уровня %* влияет на сведения о присутствии с несколькими потоками *несекретными* . Для чего используется для классификации потока как низкого уровня или хороший обратитесь к [Пороговые значения качества звонков](https://aka.ms/cqd_quality_thresholds).
  
Используйте масштаб слева оценить количество вызовов значений.
...
Общий поток том является важным фактором при выборе этих как соответствующие вычисляемые низкого уровня процентные доли являются. Меньшего размера тома общий поток, менее надежные, переданные низкого уровня процентные значения.
...
В подробных отчетов измерения группы можно использовать для фильтрации данных групп Майкрософт или Скайп для бизнеса в Интернет данных как часть определения отчета.
...
2. После выбора типа файла данных, нажмите кнопку **Обзор** , чтобы выбрать файл данных.
  ...
  - Файл данных должен находиться файл .tsv (вкладка-запятыми) или в CSV-файл (CSV-). Если с помощью CSV-файла, любое поле, которое содержит запятую должно содержать кавычки или иметь запятой, которая удалена. К примеру Если построение называется Нюнорск Йорке, в CSV-файл его ввода как «Нюнорск Йорке».
...
5. Если не происходит ошибки во время проверки, отправка файла будет успешно. Может появиться файл загруженные данные в таблицу **Мои передача** , в которой показан полный список всех загружаемых файлов для текущего клиента в нижней части страницы.
    ...
    Каждого записывать один отправленный клиентом данные файла, тип файла, время последнего обновления, периода времени, описание, удаление и значок загрузки. Чтобы удалить файл, выберите значок корзины корзины в таблице. Чтобы загрузить файл, выберите значок загрузки в столбце **загрузки** таблицы.
...
- Для каждого столбца, если тип данных string, данных может быть пустым (, но по-прежнему должны быть разделены соответствующие запятые (то есть вкладки или запятой). Это просто присваивает значение пустое строковое значение.
    
- Должен быть 14 столбцов для каждой строки и столбцы должны иметь следующие данные типа и столбцы должны находиться в том порядке, перечисленные в следующей таблице:
