...
- Мы рекомендуем использовать общий кластер для общего доступа к файлам. Если вы уже используете один из них, вы должны использовать кластер Windows Server 2012 или более новой версии.

> [!Note]
> **Зачем нужна новейшая версия Windows?** В более ранних версиях могут отсутствовать разрешения, необходимые для включения всех функций. Вы можете использовать администратор кластеров для создания общих файловых ресурсов. Более подробную информацию можно найти в статье поддержка в разделе [Создание общих файловых ресурсов на кластере](https://support.microsoft.com/help/224967) .
    
> [!CAUTION]
> Обратите внимание, что использование сетевого хранилища (NAS) не поддерживается в качестве общего файлового ресурса, поэтому следует использовать один из перечисленных ниже вариантов. Это ограничение на поддержку вызвано тем, что архитектура устройств NAS обеспечивает адаптируемость файловой системы для компьютера под управлением Windows, который получает доступ к общей файловой системе устройства.
