...
---
# <a name="deploy-a-resource-forest-topology"></a>Развертывание топологии с лесом ресурсов
...
- Поддерживаемые версии Lync Server и Скайп для сервера в гибридной конфигурации в разделе [требования к версии сервера](plan-hybrid-connectivity.md#server-version-requirements) в [планировании гибридного подключения между Скайп Business Server и Office 365](plan-hybrid-connectivity.md).
...
В топологии с лесом ресурсов леса ресурсов, размещение Скайп для Business Server должен доверять каждого леса учетная запись, которая содержит учетные записи пользователей, которые будут получить к нему доступ. При наличии нескольких лесов пользователей для включения проверки подлинности между лесами важно включить режим "Маршрутизация суффикса имен" применительно к каждому из этих отношений доверия. Инструкции см. в разделе [Управление отношениями доверия между лесами](https://technet.microsoft.com/en-us/library/cc772440.aspx). Если у вас есть Exchange Server, развернутые в другом лесу и предоставляет функциональные возможности для Скайп для бизнес-пользователей, леса, размещения Exchange необходимо обеспечить доверие размещение Скайп для Business Server. Например если Exchange были развернуты в лесу, это будет эффективно означает, что в этой конфигурации требуется двусторонним доверием между учетной записи и Скайп для бизнеса лесов.
...
В топологиях с лесом ресурсов это необходимо, что атрибуты пользователя из леса ресурсов и любой учетной записи леса (s) синхронизируются в Azure AD. Простой и рекомендуемый способ сделать это — иметь Azure AD подключить синхронизация и объединение удостоверения пользователей из *всех* лесах с включенным учетных записей пользователей и лес, содержащий Скайп для бизнеса. Дополнительные сведения см [Подключить Настройка Azure AD для Скайп для бизнеса и рабочих групп](configure-azure-ad-connect.md).

Обратите внимание на то, что подключение AAD не предоставляет синхронизации при локальном между лесами учетных записей и ресурсов. Который необходимо отдельно настроить с помощью Microsoft Identity Manager или аналогичный продукт, как описано выше.
