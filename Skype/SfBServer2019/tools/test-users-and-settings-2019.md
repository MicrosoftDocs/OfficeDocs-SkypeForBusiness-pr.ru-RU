...
Искусственные транзакции кажутся очень полезными при определении проблем с системой. Например, Test-CsRegistration может оповещать администраторов о том, что пользователи с трудом регистрируются в Skype для бизнеса Server. Однако для определения фактической причины сбоя могут потребоваться дополнительные сведения.
