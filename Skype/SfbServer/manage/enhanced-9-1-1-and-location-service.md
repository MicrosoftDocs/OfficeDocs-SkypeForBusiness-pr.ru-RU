---
title: Управление расширенными версиями 9-1-1 и службой location
ms.reviewer: ''
ms.author: serdars
author: SerdarSoysal
manager: serdars
...
description: Skype для бизнеса Server поддерживает расширенные вызовы 9-1-1 (E9-1-1) Skype для бизнеса клиентов. При настройке Skype для бизнеса Server для E9-1-1 экстренные вызовы из Skype для бизнеса включают сведения о расположении аварийного реагирования (ERL) из базы данных службы сведений о расположении.
ms.openlocfilehash: 5b002e4043cb400b5f22a4b11bc70941a88abc2a
ms.sourcegitcommit: 296862e02b548f0212c9c70504e65b467d459cc3
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 05/25/2022
ms.locfileid: "65676011"
---
# <a name="manage-enhanced-9-1-1-and-the-location-service-in-skype-for-busines-server"></a>Управление расширенными версиями 9-1-1 и службой location в Skype для Busines Server

Skype для бизнеса Server поддерживает расширенные вызовы 9-1-1 (E9-1-1) Skype для бизнеса клиентов. При настройке Skype для бизнеса Server для E9-1-1 экстренные вызовы из Skype для бизнеса включают сведения о расположении аварийного реагирования (ERL) из базы данных службы сведений о расположении. Используйте процедуры, описанные в этой статье, для управления политикой расположения.

> [!Note]
> Дополнительные сведения о развертывании расширенных Корпоративная голосовая связь, таких как E9-1-1 и служба сведений о расположении, см. в статье "Развертывание расширенных Корпоративная голосовая связь [функций"](../deploy/deploy-enterprise-voice/deploy-advanced-enterprise-voice-features.md).
...
Политики расположения можно настроить из группы **конфигурации** сети в Skype для бизнеса Server панель управления. В Skype для бизнеса Server панель управления можно просматривать, создавать, изменять или удалять политики расположения. Используйте следующую процедуру для просмотра сведений о политиках расположения. 
...
3.  На панели навигации слева щелкните " **Конфигурация сети**" и выберите пункт **"Политика расположения"**.
...
5.  В меню **Правка** щелкните **Подробнее**.
    ...
    > [!NOTE]  
    > Вы можете просматривать сведения только об одной политике расположения за раз.

По умолчанию существует одна политика с именем Global, которая не может быть удалена или переименована. Однако вы можете изменить глобальную политику. Эта политика будет применяться ко всем пользователям и контактам, если вы не создаете политики сайта или политики для отдельных пользователей. Политики для отдельных пользователей должны применяться к определенным пользователям.


## <a name="create-or-modify-a-location-policy"></a>Создание или изменение политики расположения 

В Skype для бизнеса Server можно переопределить время по умолчанию между запросами клиента на обновление расположения из службы сведений о расположении. Значение по умолчанию: 4 часа. Для переопределения значения по умолчанию используется командлет **Set-CsLocationPolicy** с параметром LocationRefreshInterval.


### <a name="to-create-a-new-location-policy-in-the-skype-for-business-server-control-panel"></a>Создание новой политики расположения в Skype для бизнеса Server панель управления
...
5.  Заполните остальные поля следующим образом:
      ...
      - **Включение расширенных экстренных служб**   Установите этот флажок, чтобы включить пользователей, связанных с этой политикой, для E9-1-1. Если службы экстренного реагирования включены, Skype для бизнеса Server клиенты будут получать сведения о расположении при регистрации и включать эти сведения при вызове экстренных служб.
      ...
      - **Расположение**   Укажите одно из следующих значений:
          ...
          - **Отказ от ответственности**   Этот параметр совпадает с обязательным, за исключением того, что пользователь не может закрыть запрос без ввода сведений о расположении. Пользователь по-прежнему может выполнить экстренный вызов, но никакие другие вызовы не могут быть выполнены без ввода информации. Кроме того, пользователю показывается текст отказа, который может предупреждать о последствиях отказа от ввода информации о местонахождении. Чтобы задать текст заявления об отказе, необходимо использовать командную консоль Skype для бизнеса Server для запуска **командлета Set-CsLocationPolicy** или **командлета New-CsLocationPolicy** с параметром EnhancedEmergencyServiceDisclaimer. Дополнительные сведения см [. в разделе Set-CsLocationPolicy](/powershell/module/skype/Set-CsLocationPolicy) или [New-CsLocationPolicy](/powershell/module/skype/New-CsLocationPolicy).
          
    
      - **Используйте расположение только** для экстренных служб, Skype для бизнеса могут использовать сведения о расположении по различным причинам (например, для уведомления коллег о текущем расположении). Установите этот флажок, чтобы убедиться, что сведения о расположении доступны только для использования с экстренным вызовом.
...
6.  Нажмите кнопку **Зафиксировать**.
    ...
    > [!IMPORTANT]  
    > Когда создают политику на уровне пользователя, первоначально эта политика не применяется к каким-либо пользователям или сетевым сайтам. Чтобы применить политику к пользователю, щелкните **Пользователи** на левой панели навигации. Найдите пользователя, к которому хотите применить политику. В меню **Правка** щелкните **Подробнее**. На странице **"Изменение пользователя сервера**" выберите новую политику расположения в раскрывающемся списке "Политика расположения" и нажмите кнопку **"Зафиксировать"**.<BR>Чтобы применить политику к сайту сети, щелкните **Конфигурация сети** на левой панели навигации, а затем щелкните **Сайт**. Найдите сайт сети, к которому хотите применить политику. В меню **Правка** щелкните **Подробнее**. В окне **Изменить сайт** выберите новую политику расположения в раскрывающемся списке **Политика расположения**, а затем щелкните **Зафиксировать**.


### <a name="to-modify-a-location-policy-in-the-skype-for-business-server-control-panel"></a>Изменение политики расположения в Skype для бизнеса Server панель управления
