...
- AdapterType — позволяет указать тип адаптера. Адаптер — это сторонний продукт, который преобразует данные в базе данных соответствия в определенный формат. XML по умолчанию.

- OneChatRoomPerOutputFile — этот параметр позволяет указать, что для каждой комнаты чата создаются отдельные отчеты.
...
- Identity — этот параметр позволяет устанавливать область действия параметров соответствия для определенной коллекции, включая глобальные уровни, уровни сайтов и служб. Значение по умолчанию — глобальный уровень. 
...
|:-----|:-----|:-----|
|Тип  <br/> |Указывает тип сообщения. Типы сообщений описаны в таблице типов сообщений элемента Message.  <br/> |Обязательно  <br/> |
|Контент  <br/> |Представляет собой содержимое сообщения. Для сообщений с типом Join или Part этот атрибут не используется.  <br/> |Необязательный  <br/> |
|Идентификатор  <br/> |Указывает уникальный идентификатор содержимого. Этот атрибут используется только с сообщениями, имеющими тип Chat.  <br/> |Необязательный  <br/> |
...
|**Атрибут**|**Описание**|**Необязательный/необходимый**|
|:-----|:-----|:-----|
|Имя пользователя  <br/> |Имя отправителя.  <br/> |Необязательный  <br/> |
|Идентификатор  <br/> |Уникальный ИД отправитель.  <br/> |Обязательно  <br/> |
|Электронный адрес  <br/> |Электронный адрес отправителя.  <br/> |Необязательный  <br/> |
