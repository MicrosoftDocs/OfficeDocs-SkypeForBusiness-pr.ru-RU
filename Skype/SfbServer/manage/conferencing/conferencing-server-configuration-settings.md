...
Первая команда использует командлет **New-CsConferencingConfiguration** для создания новой коллекции параметров, хранимых в переменной $x. Параметр InMemory указывает, что коллекция должна создаваться в памяти, а не немедленно применяться к сайту Redmond.
