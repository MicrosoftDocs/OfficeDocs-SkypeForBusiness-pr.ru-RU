...
- Skype для бизнеса Server на каждом сервере Skype для бизнеса Server выпуск Enterprise или выпуск Standard сервере.
    
- Вы можете обновить содержимое справки Skype для бизнеса Server управления с помощью команды [Update-Help](/powershell/module/microsoft.powershell.core/update-help). Командлет Update-Help и устанавливает самые новые файлы справки, доступные для всех модулей, установленных на компьютере, включая обновления Skype для бизнеса командлетов.
    
    По умолчанию **командлет Update-Help** обновляет все модули, установленные на Skype для бизнеса Server. Если требуется обновить только определенные модули, можно использовать параметр _Module_ , чтобы ограничить область действия командлета. В следующем примере обновляется только Skype для бизнеса модуль.
  ...
  ```
    ...
    Дополнительные сведения см. в [справке о updatable Help](/powershell/module/microsoft.powershell.core/about/about_updatable_help).
