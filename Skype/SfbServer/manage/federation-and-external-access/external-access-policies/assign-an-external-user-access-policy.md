...
## <a name="assigning-per-user-external-access-policies-by-using-windows-powershell-cmdlets"></a>Назначение политик Per-User внешнего доступа с помощью Windows PowerShell управления

Политики внешнего доступа для каждого пользователя можно начертать с помощью Windows PowerShell и Grant-CsExternalAccessPolicy управления. Этот cmdlet можно запустить либо из оболочки управления Skype для бизнеса Server, либо из удаленного сеанса Windows PowerShell. 

## <a name="to-assign-a-per-user-external-access-policy-to-a-single-user"></a>Назначение политики внешнего доступа на пользователя одному пользователю
