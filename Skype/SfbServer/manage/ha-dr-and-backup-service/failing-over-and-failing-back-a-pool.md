...
1. Откройте топологию Builder, щелкните правой кнопкой мыши пул Edge, который необходимо изменить, и выберите **Изменить свойства**.
...
1. Найдите пул хозяйского сервера для центрального сервера управления, введя на Front-End в Pool2 следующие команды:
    ...
    Результаты этого команды показывают, в котором пуле в настоящее время размещен Центральный сервер управления. В остальной части этой процедуры этот пул называется CMSPool\_.

2. Используйте Топологию Builder, чтобы найти версию Skype для бизнеса Server в CMSPool\_. Если он работает Skype для бизнеса Server, используйте следующий cmdlet, чтобы найти резервный пул пула пула 1.

    ```powershell
    Get-CsPoolBackupRelationship -PoolFQDN <CMS_Pool FQDN>
    ```

    Позвольте BackupPool\_ быть пулом резервного копирования.
...
3. Проверьте состояние центрального магазина управления следующим cmdlet:
    ...
    Этот комлет должен показать, что как ActiveMasterFQDN, так и ActiveFileTransferAgents указывают на FQDN CMSPool\_. Если они пусты, центральный сервер управления недопустим, и его необходимо сбой.
...
5.  Чтобы не сбой в центральном хранилище управления в пуле с Skype для бизнеса Server, сделайте следующее:
    ...
    1. Сначала проверьте, Back-End сервер в BackupPool\_ запускает основной экземпляр магазина централизованного управления, введя следующие данные:

        ```powershell
        Get-CsDatabaseMirrorState -DatabaseType Centralmgmt -PoolFqdn <Backup_Pool Fqdn>
        ```
    
    1. Если основной сервер Back-End в BackupPool\_ является основным, введите:

        ```powershell        
        Invoke-CSManagementServerFailover -BackupSQLServerFqdn <Backup_Pool Primary BackEnd Server FQDN> -BackupSQLInstanceName <Backup_Pool Primary SQL Instance Name>
        ```
        
    1. Если зеркальный Back-End в BackupPool\_ является основным, введите:
    ...
    1. Проверьте, завершен ли сбой центрального сервера управления. Введите следующую команду:
        ...
        Убедитесь, что и ActiveMasterFQDN, и ActiveFileTransferAgents указывают на FQDN BackupPool\_.
...
6.  Установка центра управления на сервере Back End в BackupPool\_.
    ...
    1. Запустите следующую команду на одном из серверов\_ резервного копирования, чтобы заставить переместить центральный магазин управления:
    ...
    1. Установите службу Центрального сервера управления на остальных серверах переднего конца в BackupPool\_. Для этого запустите следующую команду на всех серверах переднего конца, за исключением команды, используемой при принуждении к переходу центра управления в начале этой процедуры:
...
1.  На интерфейсном сервере откройте построитель топологии. **Расширите пулы** Edge и нажмите правой кнопкой мыши на сервере Edge или в пуле edge server, который в настоящее время настроен для Федерации. Выберите пункт **Изменить свойства**.
...
3.  **Расширьте пулы** Edge, а затем нажмите правой кнопкой мыши в пуле edge server или Edge Server, которые вы теперь хотите использовать для Федерации. Выберите пункт **Изменить свойства**.
...
5.  Выберите **Действие**, выберите **топологию**, выберите **Публикацию**. Если вам будет **предложено опубликовать топологию**, выберите **Далее**. После завершения публикации выберите **Finish**.

6.  На сервере Edge откройте мастер Skype для бизнеса Server развертывания. Выберите **установку или обновление Skype для бизнеса Server,** а затем установите или **удалите Skype для бизнеса Server компоненты**. Выберите **Выполнить снова**.
...
2.  Если вы хотите отойтки Skype для бизнеса Server федерации, чтобы использовать восстановленный edge Server, сделайте следующее:
    ...
    1. На интерфейсном сервере откройте построитель топологии. **Расширьте пулы** edge, а затем нажмите правой кнопкой мыши на сервере Edge или в пуле edge server, который в настоящее время настроен для Федерации. Выберите пункт **Изменить свойства**.
    ...
    1. **Разойдитесь** по пулам Edge, щелкните правой кнопкой мыши исходный пул edge server или edge server, который вы снова хотите использовать для Федерации. Выберите пункт **Изменить свойства**.
    ...
    1. Выберите **Действие**, выберите **топологию**, выберите **Публикацию**. Если вам будет **предложено опубликовать топологию**, выберите **Далее**. После завершения публикации выберите **Finish**.
    
    1. На сервере Edge откройте мастер Skype для бизнеса Server развертывания. Выберите **установку или обновление Skype для бизнеса Server,** а затем установите или **удалите Skype для бизнеса Server компоненты**. Выберите **Выполнить снова**.
