...
В следующих разделах описываются действия, необходимые для настройки двух факторов проверки подлинности для развертывания. Дополнительные сведения о двухфакторной проверке подлинности см. в публикации [Enabling Office 365 multi-factor authentication for online administrators - Grid User Post.](https://go.microsoft.com/fwlink/p/?LinkId=313332)

## <a name="configure-an-enterprise-root-certificate-authority-to-support-smart-card-authentication"></a>Настройка органа корпоративного корневого сертификата для поддержки проверки подлинности смарт-карт

Ниже описано, как настроить корпоративный корневой ЦС для поддержки проверки подлинности смарт-карт:

Сведения об установке корпоративного корневого ЦС см. в [примере Install an Enterprise Root Certification Authority](/previous-versions/windows/it-pro/windows-server-2003/cc776709(v=ws.10)).

1. Войдите на компьютер корпоративной системы администрирования с помощью учетной записи администратора домена.
...
1. Войдите на Windows 8 с помощью учетных данных пользователя с поддержкой Skype для бизнеса.

2. На экране Windows 8 начните перемещение курсора в нижний правый угол экрана.
...
1. Войдите на Windows 8 рабочей станции с помощью учетных данных пользователя с поддержкой Skype для бизнеса.
...
> [!NOTE]
> При установке AD FS 2.0 не используйте диспетчер Windows Server для добавления роли Служб Федерации Active Directory. Вместо этого скачайте и установите [пакет Active Directory Federation Services 2.0 RTW.](https://go.microsoft.com/fwlink/p/?LinkId=313375)
...
7. Убедитесь, что для вашего сервера Skype для бизнеса создано новое доверие.
...
Ниже описано, как создать настраиваемую конфигурацию веб-служб для серверов Directors, Корпоративные пулы и Standard Edition, которые будут включены для пассивной проверки подлинности.
...
3. В командной строке Командной строки Skype для бизнес-серверов создайте новую конфигурацию веб-службы для каждого сервера Director, Enterprise Pool и Standard Edition, которая будет включена для пассивной проверки подлинности с помощью следующей команды:
...
5. Для клиентов пассивная проверка подлинности является наименее предпочтительным методом проверки подлинности для проверки подлинности вебтикета. Для всех серверов директоров, корпоративных пулов и стандартных выпусков, которые будут включены для пассивной проверки подлинности, все другие типы проверки подлинности должны быть отключены в Веб-службах Skype для бизнеса, используя следующий кодлет:
...
Если проверка подлинности сертификатов отключена для веб-служб Skype для бизнеса, клиент Skype для бизнеса будет использовать менее предпочтительный тип проверки подлинности, например Kerberos или NTLM, для проверки подлинности в службу Регистратора. Проверка подлинности сертификатов по-прежнему необходима для того, чтобы позволить клиенту получить вебтикет, однако Kerberos и NTLM должны быть отключены для службы Регистратора.

Ниже описано, как создать настраиваемую конфигурацию прокси для серверов Edge Pools, Корпоративные пулы и Standard Edition, которые будут включены для пассивной проверки подлинности.

### <a name="to-create-a-custom-proxy-configuration"></a>Создание настраиваемой конфигурации прокси

1. В командной строке Командной строки Skype для бизнес-серверов создайте новую прокси-конфигурацию для каждого окантовки Skype для бизнес-сервера, корпоративного пула и сервера Standard Edition, которая будет включена для пассивной проверки подлинности с помощью следующих команд:
...
[Использование двух факторов проверки подлинности с клиентом Skype для бизнеса и Skype для бизнеса Server](use-two-factor.md)