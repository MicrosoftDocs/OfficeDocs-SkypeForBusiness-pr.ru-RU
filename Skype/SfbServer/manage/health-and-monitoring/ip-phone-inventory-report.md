...
---
# <a name="ip-phone-inventory-report-in-skype-for-business-server"></a>IP-телефонам в Скайп для Business Server
 
**Сводка:** Сведения о IP-телефонам в Скайп для Business Server.
...
|**Версия оборудования** <br/> |Номер версии IP-телефон; с помощью производитель и фильтры версия оборудования можно однозначно identity определенного типа телефона. Значения для данного фильтра заполняются автоматически на основе данных об IP-телефонах, хранящихся в базе данных.  <br/> |
|**Агент пользователя** <br/> |Идентификатор программного обеспечения, используемого IP-телефоном. Значения для данного фильтра заполняются автоматически на основе данных об IP-телефонах, хранящихся в базе данных.  <br/> |
|**MAC-адрес** <br/> |Уникальный идентификатор сетевого интерфейса IP-телефона. Обычно MAC-адрес задается производителем и не поддерживает изменение.  <br/> Чтобы найти записи, относящиеся к определенному MAC-адресу, введите требуемый MAC-адрес. Пример:  <br/> 00-08-5D-16-16-48  <br/> Требуется вводить полные адреса. При вводе части адреса (например, 00-08-5D) данные не будут возвращены.  <br/> |
|**Последние действия за период** <br/> | Выберите одно из следующих значений: <br/>  [All] (Все) <br/>  10 <br/>  20 <br/>  30 <br/> |
|**Последние выходы за период** <br/> | Выберите одно из следующих значений: <br/>  [All] (Все) <br/>  10 <br/>  20 <br/>  30 <br/> |
|**Префикс URI пользователя** <br/> |SIP-адрес пользователя, который использует IP-телефон.  <br/> |
...
|**Версия оборудования** <br/> |Да  <br/> |Номер версии IP-телефона.  <br/> |
|**MAC-адрес** <br/> |Да  <br/> |Уникальный идентификатор сетевого интерфейса IP-телефона. Обычно MAC-адрес задается производителем и не поддерживает изменение.  <br/> |
|**URI пользователя** <br/> |Да  <br/> |SIP-адрес пользователя, который использует IP-телефон.  <br/> |
|**Агент пользователя** <br/> |Да  <br/> |Идентификатор программного обеспечения, используемого IP-телефоном.  <br/> |
|**Время последнего входа** <br/> |Да  <br/> |Дата и время, IP-телефона последнего входа на Скайп для Business Server.  <br/> |
|**Last logoff time (Время последнего выхода)** <br/> |Да  <br/> |Дата и время, IP-телефона последнего выхода из Скайп для Business Server.  <br/> |
