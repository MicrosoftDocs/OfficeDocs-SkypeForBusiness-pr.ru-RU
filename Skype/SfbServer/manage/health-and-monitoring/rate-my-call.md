...
ms.locfileid: "58586989"
---
# <a name="rate-my-call-in-skype-for-business-server"></a>Оцените мой вызов в Skype для бизнеса Server

**Сводка:** Узнайте о функции Rate My Call в Skype для бизнеса Server.

Rate My Call — это новая функция Skype для бизнеса 2015 и 2016 Windows, которая предоставляет предприятиям возможность получать отзывы от конечных пользователей.

Окно Rate My Call предлагает систему оценки "звезда" и предопределяемы маркеры для аудио- и видеозвонков. Кроме того, администраторы могут включить настраиваемую область для предоставления отзывов.

Собранные данные rate My Call в настоящее время не включены ни в один существующий отчет мониторинга, но он имеет отдельный отчет мониторинга. Данные собираются в SQL таблицах, к которые можно получить доступ с помощью SQL запросов.

## <a name="rate-my-call-prerequisites"></a>Оцените условия вызова

Прежде чем пользователи в Skype для бизнеса Server смогут получить доступ к функциям Rate My Call, необходимо развернуть и настроить следующий набор компонентов:

-  Необходимо установить Skype для бизнеса Server (версия 9160 или выше).

- Установите и обновите пользователей до последней версии Skype для бизнеса, а также попросите их использовать Skype для бизнеса пользовательского интерфейса.

- Пользователи должны быть дома в пуле Skype для бизнеса Server переднего конца.

- Должна быть развернута Skype для бизнеса Server база данных мониторинга, связанная с Skype для бизнеса Server пулами.

- Рекомендуется развертывать панель мониторинга качества вызовов (CQD).

## <a name="configure-rate-my-call"></a>Настройка rate my Call

Функция Rate My Call включена по умолчанию в политике клиента со следующими настройками:

- Rate My Call Display Percentage - 10%

- Оцените возможность настраиваемой обратной связи пользователей с отключением

Существует никаких действий, необходимых для включить базовую функцию, однако, если вы хотите пользовательские отзывы необходимо включить его отдельно. Следующий Windows PowerShell является примером включения пользовательских отзывов конечных пользователей и изменения интервала с 10% до 80%.

```PowerShell
Set-CSClientPolicy -Identity <PolicyIdentity> -RateMyCallDisplayPercentage 80 -RateMyCallAllowCustomUserFeedback $true 
```

## <a name="accessing-rate-my-call-data"></a>Доступ к скорости мои данные вызовов
 ...
 **[QoeMetrics]. [dbo]. [CallQualityFeedbackTokenDef]** — в этой таблице содержатся определения маркеров.

Определения маркеров закодироваться следующим образом:
