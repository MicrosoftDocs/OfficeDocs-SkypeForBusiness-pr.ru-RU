...
В большинстве развертывании использование ЦП UCWA в среднем должно быть меньше 15 процентов. Использование памяти должно подпадать под ограничения, описанные в описании монитора для ограничения емкости памяти сервера [в Skype для бизнеса Server.](server-memory-capacity-limits.md)
...
- **ASP.NET v2.0.50727\Requests Current** показывает количество веб-запросов сервера в очереди. Когда этот счетчик достигает 5000, последующие запросы не будут отправляться с сообщением об ошибке "503 — служба недоступна".
