...
После изменения параметров порта следует остановить и перезапустить каждую службу, затронутую изменениями.
