...
Это важно, так как, когда это свойство задавалось false, Skype для бизнеса клиенты будут использовать любой доступный порт между портами 1024 и 65535 при вовлечении в сеанс связи; это верно независимо от любых других параметров порта (например, ClientMediaPort или ClientVideoPort). Если вы хотите ограничить использование определенного набора портов (и это то, что вы хотите сделать, если вы планируете реализовать качество службы), то сначала необходимо включить диапазоны клиентских медиа-портов. Это можно сделать с помощью следующей Windows PowerShell:
...
5.  В редакторе управления групповой политикой **расширйте** конфигурацию **компьютера,** Windows Параметры, щелкните правой кнопкой мыши **QoS** на основе политики и нажмите **кнопку Создать новую политику**.
...
7.  На следующей странице выберите только приложения с **этим исполняемым** именем **,** введитеLync.exeимя, а затем нажмите **кнопку Далее**. Этот параметр предписывает политике только приоритизировать соответствие трафику Skype для бизнеса клиента.
...
Если вы решили создать политику управления трафиком общего доступа к приложениям, сделайте эти замены:
  ...
  - Используйте ранее настроенный диапазон портов для видео-трафика. Например, если вы зарезервировали порты 42000 до 42019 для общего доступа к приложениям, установите диапазон портов таким образом: **42000:42019**.
...
Для политики передачи файлов
  ...
  - Используйте ранее настроенный диапазон портов для приложения. Например, если для общего доступа к приложениям зарезервированы порты 42020-42039, установите диапазон портов: **42020:42039**.
