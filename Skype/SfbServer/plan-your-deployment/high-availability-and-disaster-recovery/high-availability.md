...
- Потеря кворума на уровне группы маршрутов, вызванная недостаточном количеством серверов реплик для определенной группы маршрутов. Группа маршрутов — это набор пользователей, которые были в пуле. Каждая группа маршрутов имеет три реплики в пуле: одну первичную и две вторичные реплики.
...
- Если по какой-либо причине требуется отключить оба сервера одновременно, после окончания простоя пула выполните следующие действия:
  ...
  - Если вы не можете вернуть их в этом порядке, используйте следующий cmdlet перед тем, как сделать резервную резервную порядок пула:  `Reset-CsPoolRegistrarState -ResetType QuorumLossRecovery -PoolFQDN <FQDN>`
