...
В предыдущей таблице "первые серверы" — это серверы, которые были впервые, в хронологическом порядке, когда пул был запущен впервые. Чтобы определить эти серверы, можно использовать cmdlet  `Get-CsComputer` с параметром `-PoolFqdn` . В этом комлете будут отображаться серверы в том порядке, в который они отображаются в топологии, а в верхней части списка находятся первые серверы.
  
> [!IMPORTANT]
> В 2019 г. максимальное число [серверов переднего Skype для бизнеса Server увеличено до 16.](../../../SfBServer2019/plan/user-model-2019.md)
> 
