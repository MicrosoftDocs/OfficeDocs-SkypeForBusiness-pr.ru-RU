...
Сервер-посредник по умолчанию находится на сервере Standard Edition или переднем сервере в пуле переднего конца на центральных сайтах. Количество вызовов общедоступных телефонных сетей (PSTN), которые можно обрабатывать, и количество машин, необходимых в пуле, будет зависеть от:
...
Вы можете использовать средство планирования бизнеса Skype для оценки того, может ли объединение сервера-посредника с вашим пулом переднего плана обрабатывать нагрузку. Если среда не может соответствовать этим требованиям, необходимо развернуть автономный пул серверов-посредников.
...
> [!NOTE]
> Чтобы улучшить производительность автономного сервера-посредника, следует включить масштабирование на принимающей стороне (RSS) в сетевых адаптерах на этих серверах. RSS позволяет обрабатывать входящие пакеты параллельно несколькими процессорами на сервере. Дополнительные сведения см. в материале["Расширение](/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/hh997036(v=ws.11))масштабирования на стороне получения в Windows Server". Сведения о включении RSS см. в документации вашего сетевого адаптера. 
