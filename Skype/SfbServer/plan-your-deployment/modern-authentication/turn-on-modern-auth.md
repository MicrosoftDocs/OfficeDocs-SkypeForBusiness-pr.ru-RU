...
__Тип 5 Description:__ *Внешние*ваши современные клиенты ADAL будут использовать MA и все клиенты, не поддерживающие ADAL, будут использовать устаревшие методы проверки подлинности. Но *internally* *все клиенты* будут использовать устаревшую проверку подлинности (включая всех клиентов с поддержкой ADAL).
...
`Set-CsAuthConfig`Настройка эффектов командлетов для ролей регистратора и веб-служб.
...
> [!IMPORTANT] 
> Некоторые веб-приложения Skype для бизнеса не поддерживают MA. Таким образом, с помощью сценария Блокквиндовсаусекстерналляндинтерналли вы не сможете получить доступ к этим приложениям. Приложения без поддержки MA — это веб-планировщик, страница телефонного подключения, панель управления Skype для бизнеса (КСКП) и параметры группы ответа. 
