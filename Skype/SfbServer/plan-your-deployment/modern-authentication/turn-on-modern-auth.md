...
В этой статье вы сможете заткнуть отверстия, которые были использованы для атак службы отказа в обслуживании (DOS) в Skype для бизнес-серверов, отключив старые методы, используемые для проверки подлинности, внешне, внутренне или обоих, в сеть. Например, одним из способов, которые помогут остановить атаки DOS, является отключение комплексной проверки подлинности Windows (включая NTLM и Kerberos). Отключение внешнего доступа к NTLM и проверка подлинности на основе сертификатов помогает защитить пароли от воздействия. Это потому, что NTLM использует учетные данные паролей для проверки подлинности пользователей, но проверка подлинности на основе сертификатов , включенная в Modern Auth, — нет. Это означает, что одним из идеальных вариантов сокращения атак DOS является блокировка внешнего NTLM и использование только проверки подлинности на основе сертификатов.

Хорошо, давайте начнем.

## <a name="what-would-you-be-changing"></a>Что бы вы меняли? 

Эти команды работают как для точек доступа SIP, так и для веб-служб. Хотя эти два канала используют различные методы доступа, используя гамму от NTLM и Kerberos до анонимного доступа, все стандартные методы, используемые Skype для бизнеса, были приняты во внимание.

## <a name="how-to-get-the-get--and-set-csauthconfig-cmdlets"></a>Как получить комлеты Get и Set-CsAuthConfig

Эти cmdlets будут установлены только после июля 2018 кумулятивного обновления (6.0.9319.534) для Microsoft Skype для бизнес-сервера 2015, а затем у вас есть различные topologies, которые можно выкатить для вашего сервера Skype для бизнеса.
...
> [!IMPORTANT]
> В приведенной ниже таблице  и описаниях современная проверка подлинности сокращена так как аутентификация __ma__ и *Windows Integrated Authentication* сокращена как __Win__. Напомним, что интегрированная проверка подлинности Windows состоит из двух методов: проверки подлинности NTLM и Kerberos. Вам нужно знать это, чтобы правильно прочитать таблицу!
...
Довольно легко уследить за целью защиты паролей в доступных вариантах. Помните, что идеальная ситуация — использовать ma внешне (например, путем настройки auth на основе сертификатов), чтобы избежать атак DOS. Если вы будете использовать его внутренне для современных клиентов, вы также сможете использовать свою сеть в будущем в отношении атак DOS Skype для бизнеса Server.
...
> [!IMPORTANT] 
> Некоторые веб-приложения Skype для бизнеса не поддерживают ma. Таким образом, с помощью сценария BlockWindowsAuthExternallyAndInternally вы не сможете получить доступ к этим приложениям. Приложения без поддержки MA являются веб-планировщиком, страницей dial-in, панелью управления Skype для бизнеса (CSCP) и страницей параметров группы ответов. 
...
- Дополнительные инструкции по использованию команд или ТС, необходимых для их установки:
    ...
    - Июль [2018 Года Skype для бизнеса Server 2015, Cu](https://support.microsoft.com/help/4340903/july-2018-cumulative-update-6-0-9319-534-for-skype-for-business-server) Core Components (6.0.9319.534)
