...
С другой стороны спуфинг идентификатор звонящего вступает определив SIP-магистраль между поставщиком, шлюз ТСОП или другой системе УАТС и Скайп для Business Server. В таких случаях Скайп для Business Server не обеспечивает защиту для предотвращения от спуфинг идентификатор звонящего. Это означает, что Скайп для корпоративных пользователей можно получить вызов магистрали SIP с идентификатором подложный вызывающего абонента, отображение телефонный номер другого Скайп для бизнес-пользователя. На стороне поставщика требуется применить защиту на этот шлюз ТСОП или УАТС.
