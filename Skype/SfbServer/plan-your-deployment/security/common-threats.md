...
## <a name="identity-spoofing-ip-address-and-caller-id-spoofing"></a>Спуфинг удостоверений (IP-адреса и вызывающего спуфинг Id)

Удостоверение подделки адреса происходит, когда он определяет и использует номер телефона допустимого пользователя (звонящего) или IP-адрес сети, компьютера или сетевой компонент без авторизованных для этого. Атаки позволяет злоумышленнику работать как в случае злоумышленник сущности, обычно определяется номер телефона (звонящего) или IP-адрес.

В контексте Скайп для Business Server подмена IP-адреса вступает в действие только в том случае, если администратору выполненная оба следующих:
...
С другой стороны спуфинг идентификатор звонящего вступает определив SIP-магистраль между поставщиком, шлюз ТСОП или другой системе УАТС и Скайп для Business Server. В таких случаях Скайп для Business Server не обеспечивает защиту для предотвращения от спуфинг идентификатор звонящего. Это означает, что Скайп для бизнес-пользователь может получить вызов магистрали SIP с идентификатором подложный вызывающего абонента, отображение номер телефона или отображаемое имя (если применимо обратный поиск номера) из другой Скайп для бизнес-пользователя. На стороне поставщика требуется применить защиту на этот шлюз ТСОП или УАТС.
