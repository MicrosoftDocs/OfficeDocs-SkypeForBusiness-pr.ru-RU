...
Развертывание магистральных каналов SIP может быть важным шагом в упрощении телекоммуникационных систем организации и подготовке к усовершенствованию коммуникаций в режиме реального времени. Одно из основных преимуществ SIP-магистрали заключается в том, что вы можете консолидировать подключения организации к телефонной сети общего перейти на центральный сайт, в отличие от предыдущей магистрали TDM, для которой обычно требуется отдельная магистраль от каждого сайта филиала.
...
Развертывание SIP-магистралей зачастую осуществляется ради функций голосовой, однако, поддержка голосовой связи — только первый этап. С помощью магистрали SIP можно расширить возможности VoIP и предоставить Skype для бизнеса Server более богатый набор услуг. Например:
...
- Экстренные вызовы E9-1-1 позволяют органам, которые отвечают на вызовы по номеру 911, определять местоположение вызываемого по номеру телефона.
...
Подробные сведения о серверах-посредниках см. в сведениях о компоненте [сервера-посредника в Skype для бизнеса Server.](mediation-server.md)
...
Распределенные магистрали SIP — это модель развертывания, в которой реализуются локальные магистрали SIP на одном или более сайтах филиалов. VoIP-трафик затем маршрутизся с сайта филиала непосредственно поставщику услуг без прокрутки центрального сайта.
...
- Устойчивость требуется между двумя центральными сайтами. Необходимо убедиться, что магистраль SIP завершает работу на каждом центральном сайте. Например, если у вас есть центральные сайты Dublin и Tukwila и оба используют только одну магистраль SIP сайта, если магистраль выходит из нее, пользователи другого сайта не могут делать вызовы STN.
