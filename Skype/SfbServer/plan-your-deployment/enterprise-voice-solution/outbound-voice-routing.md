...
Можно указать префикс внешнего доступа не более четырех символов (#, и 0–9), если пользователям нужно набрать одну или несколько цифр в качестве ведущего \* номера (например, 9), чтобы получить внешнюю линию.
...
Skype для бизнеса Server обеспечивает гибкость в маршруте вызовов в STN. Маршрут голосовых вызовов указывает набор магистральных линий связи, используемых для связи с ТСОП, которая может использоваться для конкретного голосового звонка. Магистраль связывает сервер-посредник и номер порта со шлюзом ТСТС и номером порта прослушивания. Эта логическая связь позволяет связывать сервер-посредник с несколькими шлюзами и иметь несколько подключений к одному шлюзу. При определении маршрута вызовов указываются магистрали, связанные с этим маршрутом, но не указываются серверы-посредники, связанные с маршрутом. Чтобы создать магистрали, определив связи между серверами-посредниками и шлюзами STN, IP-PBX и пограничными контроллерами сеансов (SCS), используйте построитель топологий.
...
Skype для бизнеса Server позволяет управлять ИД вызываемой почты для исходящие вызовы. Например, если организации требуется скрыть расширения прямого набора номера сотрудников и заменить их общим корпоративным или отделовым номером, администратор может сделать это с помощью панели управления Skype для бизнеса Server, чтобы скрыть и заменить его указанным альтернативным ИД вызываемого. При планировании логики маршрутов учитывайте, для каких пользователей, групп и сайтов вы хотите использовать этот параметр ( возможно, даже для всех сотрудников).
