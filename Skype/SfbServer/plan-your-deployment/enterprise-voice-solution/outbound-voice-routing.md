---
title: Планирование исходящие маршруты голосовой маршрутики в Skype для бизнеса Server
ms.reviewer: ''
ms.author: v-cichur
author: cichur
manager: serdars
...
description: Узнайте о маршрутизации исходяющих голосовых данных в skype for Business Server Корпоративная голосовая связь, включая параметры маршрутизации вызовов, планы набора голосовых номеров, правила нормализации, политики голосовой поддержки, записи использования PSTN и маршруты голосовой почты.
ms.openlocfilehash: 70681e995068a0999324694a78806421e0f2d129
ms.sourcegitcommit: 01087be29daa3abce7d3b03a55ba5ef8db4ca161
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 03/23/2021
ms.locfileid: "51101285"
---
# <a name="plan-for-outbound-voice-routing-in-skype-for-business-server"></a>Планирование исходящие маршруты голосовой маршрутики в Skype для бизнеса Server
 
Узнайте о маршрутизации исходяющих голосовых данных в skype for Business Server Корпоративная голосовая связь, включая параметры маршрутизации вызовов, планы набора голосовых номеров, правила нормализации, политики голосовой поддержки, записи использования PSTN и маршруты голосовой почты.
  
Маршрутика исходящие вызовы применяется Корпоративная голосовая связь вызовов, предназначенных для шлюза, магистрали или частного филиала (PBX) для шлюза, магистрали или частного филиала. Когда пользователь Skype для бизнеса помещает вызов, сервер при необходимости нормализует номер телефона до формата E.164 и пытается связать его с SIP URI. Если сопоставление не удается, то сервер применяет логику маршрутизации исходящих вызовов на основе представленной строки набора. Эта логика задается путем настройки параметров сервера, описанных в следующей таблице.
  
**Параметры маршрутинга исходящие вызовы Skype для бизнес-сервера**
...
Правила нормализации используют регулярные выражения .NET Framework для указания шаблонов совпадения номеров, используемых сервером для преобразования строк набора номера в формат E.164 с целью выполнения обратного поиска номеров. Вы создаете правила нормализации в панели управления Skype для бизнес-серверов, вводя выражения вручную или вводя начальные цифры и длину соответствующих строк набора и позволяя панели управления Skype для бизнес-серверов создавать соответствующее регулярное выражение для вас. В любом случае после завершения вы можете ввести тестовый номер, чтобы убедиться в правильной работе правил нормализации.
  
Подробные сведения об использовании платформа .NET Framework регулярных выражений см. в материале ["платформа .NET Framework регулярные выражения".](/dotnet/standard/base-types/regular-expressions)
...
Политики голосовой поддержки Skype для бизнеса Server определяют следующие правила для каждого пользователя, сайта или организации:
...
Следующие действия помогут спланировать голосовые политики, необходимые для развертывания Корпоративная голосовая связь:
...
Область политики голосовых служб определяет иерархический уровень, на котором применяется политика. В Skype для бизнеса Server можно настроить голосовые политики со следующими уровнями области (перечисленными от наиболее специфических до наиболее общих).
...
- **Отслеживание вредоносных** вызовов позволяет пользователям сообщать о вредоносных вызовах с помощью клиента Skype для бизнеса, а затем пометить такие вызовы в записях детализации вызовов. По умолчанию параметр отключен.
    
- Выход **голосовой** почты предотвращает немедленное перенастройка вызовов в систему голосовой почты мобильного телефона пользователя при настройке одновременного звонка и отключении телефона из батареи или в диапазоне, а также на основе значения времени. Этот параметр включает или отключает таймер и задает его значение. Его можно настроить только с помощью оболочки управления Skype для бизнес-серверов. По умолчанию параметр отключен.
    
- Использование PSTN-вызовов и одновременный вызов позволяет администраторам указать то же самое использование **PSTN,** что и голосовая политика для переадправления вызовов и одновременного звонка, ограничить переадправление вызовов и одновременный звонок только внутренним пользователям Skype для бизнеса или указать настраиваемый использование PSTN, которое отличается от использования PSTN политики голосовой политики. По умолчанию используется такое же использование PSTN, как и голосовая политика для переададки вызовов и одновременного звонка.
...
Маршруты вызовов можно указать в панели управления Skype для бизнес-серверов. Эти маршруты вызовов заполняют таблицу маршрутизов серверов, которую Skype для бизнеса Server использует для маршрутных вызовов, предназначенных для PSTN.
  
### <a name="mn-trunk-support"></a>Поддержка магистрали M:N

Skype для бизнеса Server обеспечивает гибкость в маршруте вызовов в PSTN. Маршрут голосовых вызовов указывает набор магистральных линий связи, используемых для связи с ТСОП, которая может использоваться для конкретного голосового звонка. Магистраль связывает сервер-посредник и номер порта с шлюзом PSTN и номером порта прослушивания. Эта логическая связь позволяет связывать сервер-посредник с несколькими шлюзами и иметь несколько подключений к одному шлюзу. При определении маршрута вызова укажите магистрали, связанные с этим маршрутом, но не указываете, какие серверы-посредники связаны с маршрутом. Чтобы создать магистрали, определяя связи между серверами-посредниками и шлюзами PSTN, ip-PBXs и контроллерами границ сеансов (SBCs), используйте конструктор топологии.
...
Skype для бизнеса Server предоставляет способ управления ИД вызываемой звонков для исходящие вызовы. Например, если организация хочет замаскировать расширения прямого номера сотрудников и заменить их общим корпоративным или ведомствальным номером, администратор может сделать это с помощью панели управления Skype для бизнес-серверов для подавления ИД вызываемого лица и его замены на указанный альтернативный ИД вызываемого. При планировании логики маршрутивки учитывайте, какие отдельные лица, группы, сайты необходимо использовать для всех сотрудников, возможно, даже для них.
