...
Прямые подключения SIP поддерживаются между Skype для бизнеса Server и шлюзами PSTN и IP-PBX в Корпоративная голосовая связь.

Вы можете использовать прямые SIP-подключения для подключения Skype для бизнес-сервера к следующему:
...
Если в организации используется одно из развертывание, описанное в этом разделе, вы можете использовать Skype для бизнеса Server в качестве единственного решения телефонии для части или всей организации. В этом разделе подробно описаны следующие развертывания:

- **Постепенное развертывание:** Этот параметр предполагает, что у вас есть существующая инфраструктура частного филиала exchange (PBX), и вы намерены постепенно внедрять Корпоративная голосовая связь для небольших групп или групп в вашей организации.
...
При постепенном развертывании Skype для бизнеса Server является единственным решением телефонии для отдельных групп или отделов, в то время как остальные пользователи в организации продолжают использовать PBX. Эта стратегия постепенного развертывания предоставляет один из способов внедрения IP-телефонии в ваше предприятие с помощью управляемых пилотных программ. Группы, чьи потребности в связи лучше всего обслуживаются Microsoft Unified Communications, перемещаются в Корпоративная голосовая связь, в то время как другие пользователи остаются на существующем PBX. Дополнительные группы можно перенести в Корпоративная голосовая связь при необходимости.
...
> [!NOTE]
> Если вы подключали развертывание Skype для бизнеса Server к сертифицированным партнерам direct SIP, то не требуется шлюз для открытой телефонной сети (PSTN) между сервером-посредником и PBX. Список сертифицированных партнеров прямого SIP см. в программе открытой межоперабельности [Microsoft Unified Communications.](../../../SfbPartnerCertification/lync-cert/qualified-ip-pbx-gateway.md)
...
В этой топологии для Корпоративная голосовая связь. Шлюз PSTN связывает группу голосовой связи с протоколом Интернета (VoIP) с поддержкой PBX. Пользователи, которые включены для Корпоративная голосовая связь, включая удаленных сотрудников, общаются через IP-сеть. Вызовы Корпоративная голосовая связь пользователей в PSTN и коллегам, которые не включены для Корпоративная голосовая связь, будут перенастроен в соответствующий шлюз PSTN. Звонки от коллег, которые по-прежнему находятся в системе PBX, или от звонивших в PSTN, перенанапряжяются в шлюз PSTN, который передает вызовы в Skype для бизнеса Server для маршрутинга.
...
Когда Корпоративная голосовая связь развертывается перед PBX, все вызовы поступают на шлюз PSTN, который передает вызовы Корпоративная голосовая связь пользователей в Skype для бизнеса Server и звонки для пользователей PBX в PBX. Вызовы в PSTN как Корпоративная голосовая связь, так и пользователей PBX перенапрягаются через ip-сеть в наиболее экономичный шлюз PSTN. В следующей таблице показаны преимущества и недостатки этой конфигурации.
...
|Корпоративная голосовая связь пользователи держат те же номера телефонов.  <br/> | <br/> |
...
В этом развертывании все вызовы будут проходить по ip-сети. Вызовы в PSTN перенастрояются в соответствующий шлюз PSTN. Skype для бизнеса или Lync Phone Edition служит в качестве софтфона. Удаленный контроль вызовов недоступен и не является необходимым, так как для управления пользователями отсутствуют телефоны PBX. Службы голосовой почты и автообслуживщика доступны через необязательный развертывание единой системы обмена сообщениями Exchange (единой системы обмена сообщениями).
...
Шлюзы общедоступных переключеных телефонных сетей (PSTN) — это сторонние компоненты оборудования, которые переводят сигналы и носители между инфраструктурой Корпоративная голосовая связь и PSTN непосредственно или через подключение к магистральм SIP. При любой топологии канал PSTN оканчивается шлюзом. Шлюз изолирован в собственной подсети и подключен к корпоративной сети с помощью сервера-посредника.
...
Сведения о планировании нескольких шлюзов см. в материале [M:N trunk in Skype for Business Server.](m-n-trunk.md)
