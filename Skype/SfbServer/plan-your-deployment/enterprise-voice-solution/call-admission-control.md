...
Несмотря на то, что служба политики пропускной способности в Skype для бизнеса Server обеспечивает ограничение пропускной способности для аудио и видео, это ограничение не выполняется в сетевом маршрутизаторе (уровень 2 и 3). Cac не может запретить приложению данных, например, использование всей пропускной способности сети на канале WAN, включая пропускную способность, зарезервированную для аудио- и видеосвязи политикой CAC. Чтобы защитить необходимую пропускную способность сети, можно развернуть протокол качества обслуживания (QoS), такой как дифференциалные службы (DiffServ). Поэтому лучше всего координировать политики пропускной способности CAC, которые вы определяете, с любыми развертываемой настройкой QoS.
...
Кодек G.722, который по умолчанию является кодековом кодеку для skype для бизнеса Server, полностью отличается от кодеков G.722.1 и Siren.
