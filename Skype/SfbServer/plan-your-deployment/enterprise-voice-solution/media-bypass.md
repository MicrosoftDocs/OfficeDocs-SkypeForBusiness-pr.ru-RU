...
Обход сервера-посредника может улучшить качество звука, поскольку уменьшается задержка, потребность в преобразовании, вероятность потери пакетов и число точек вероятного сбоя. Может улучшиться масштабируемость, поскольку устранение обработки мультимедиа для вызовов с обходом сервера-посредника уменьшает нагрузку на сервер-посредник. Это сокращение нагрузки дополняет возможность сервера-посредника управлять несколькими шлюзами.
 ...
 Если сайт филиала без сервера-посредника подключен к центральному сайту с помощью одной или более каналов WAN с ограниченной пропускной способностью, обход сервера-посредника снижает требования к пропускной способности, позволяя клиенту на сайте филиала переходить непосредственно к локальному шлюзу без необходимости сначала переходить через канал WAN к серверу-посреднику на центральном сайте и обратно.

За счет снятия с сервера-посредника обработки мультимедиа обход сервера-посредника также может уменьшить число серверов-посредников, необходимых Корпоративная голосовая связь инфраструктуре. Общая рекомендация состоит в том, чтобы включать обход сервера-посредника везде, где это возможно.
...
- Одноранговой сервер-посредник должен поддерживать необходимые возможности для обхода сервера-посредника, что является наиболее важной возможностью обработки нескольких разметки ответов (известных как "ранние диалоги"). Обратитесь к производителю шлюза или ITSP, чтобы получить значение максимального количества ранних диалогов, которые могут поддерживать шлюз, PBX или SBC

- Одноранговой сервер-посредник должен принимать трафик мультимедиа непосредственно из конечных точек Skype для бизнеса. Многие itsPs позволяют SBC получать трафик только с сервера-посредника. Обратитесь к its ITSP, чтобы определить, принимает ли SBC трафик мультимедиа непосредственно из конечных точек Skype для бизнеса.
