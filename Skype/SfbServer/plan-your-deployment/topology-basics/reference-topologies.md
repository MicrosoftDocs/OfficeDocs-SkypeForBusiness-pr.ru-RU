...
Справочные топологии для Skype для бизнеса Server, в том числе схемы и решения, которые необходимо принять для крупных, средних и малых организаций.

Лучшая топология Skype для бизнеса Server для вас зависит от размера организации, рабочих нагрузок, которые вы хотите развернуть, и ваших предпочтений для высокой доступности и стоимости инвестиций.
...
Эталонная топология для небольших организаций показывает, как можно развернуть надежное и доступное решение, развернув только три сервера под управлением Skype для бизнеса Server.
...
- **Развертывание пары серверов стандартного выпуска** Эта организация имеет 4000 пользователей на своем центральном сайте. Они развернули два сервера Standard Edition и спарили их для обеспечения высокой доступности и аварийного восстановления. На каждом сервере находится 2000 пользователей, но сведения обо всех пользователях синхронизируются между двумя серверами. В случае отказа одного сервера администратор может переключить соответствующих пользователей на обслуживание другим сервером, с минимальным влиянием на работу пользователей. Дополнительные сведения о высокой доступности и аварийном восстановлении функций в Skype для бизнеса Server см. в см. в проекте Plan [for high availability and disaster recovery in Skype for Business Server.](../../plan-your-deployment/high-availability-and-disaster-recovery/high-availability-and-disaster-recovery.md)

- **Рекомендуется использовать развертывание с пограничным сервером.** Хотя развертывание пограничного сервера не является обязательным, для внутреннего обмена мгновенными сообщениями, предоставления сведений о присутствии и конференций мы рекомендуем сделать это даже для развертываний малого размера. Вы можете увеличить инвестиции Skype для бизнеса Server, разверив edge Server для предоставления услуг пользователям, которые в настоящее время находятся вне брандмауэров организации. Ниже перечислены связанные с этим преимущества.

  - Собственные пользователи организации могут использовать функции Skype для бизнеса Server, если они работают из дома или находятся в пути.

  - Ваши пользователи смогут приглашать внешних пользователей для участия в собраниях.

  - Если у вас есть партнер, поставщик или клиентская организация, которая также использует Skype для бизнеса Server, вы можете сформировать федератные отношения с этой организацией. Затем развертывание Skype для бизнеса Server распознает пользователей из федераированной организации, что позволит улучшить совместную работу.
...
- **Жизнеспособность узла филиала.** В этой организации запущена пилотная программа Корпоративная голосовая связь Skype для бизнеса Server. Некоторые пользователи используют Skype для бизнеса Server в качестве единственного голосового решения. Некоторые из Корпоративная голосовая связь пользователей находятся на сайте филиала. Сайт филиала не имеет надежной связи с широкой сетью области (WAN) на центральном сайте, поэтому там развернуто устройство для ветвей. При сбое связи с глобальной сетью пользователи филиала смогут принимать и осуществлять вызовы (звонки в организации и по ТСОП), использовать функции голосовой почты и общаться с помощью двухсторонних мгновенных сообщений. В случае нарушения связи с глобальной сетью также будет доступна проверка подлинности пользователей. Дополнительные сведения см. в Корпоративная голосовая связь в [Skype для бизнеса Server.](../../plan-your-deployment/enterprise-voice-solution/enterprise-voice-resiliency.md)

- **Развертывание единой системы обмена сообщениями Exchange.** Эта эталонная топология включает сервер Exchange Unified Messaging (UM), который запускается Microsoft Exchange Server, а не Skype для бизнеса Server.

- **Сервер Office Web Apps.** Рекомендуется развернуть сервер Office Web Apps или ферму серверов Office Web Apps в каждом организации, использующей веб-конференции. Office Web Apps Server позволяет представлять слайды PowerPoint на веб-конференциях.
...
- **Можно добавить аварийное восстановление.** Для этой организации высокая доступность для служб Skype для бизнеса Server является необходимой функцией, но аварийное восстановление не является. Пул развернутых серверов front end обеспечивает высокую доступность.
...
- **Варианты развертывания узлов филиалов.** В этой топологии организация в качестве решения голосовой связи развернула решение Enterprise Voice. Сайт Branch Site 1 не имеет устойчивой связи с широкой сетью области (WAN) на центральном сайте, поэтому для обслуживания многих функций Skype для бизнес-сервера развернута поддерживаемая связь WAN с центральным сайтом. У узла филиала 2 есть надежная связь по глобальной сети с центральным узлом, поэтому требуется только шлюз ТСОП. Развернутый шлюз ТСОП поддерживает обход сервера-посредника, поэтому в узле филиала 2 сервер-посредник не требуется. Дополнительные сведения см. в Корпоративная голосовая связь в [Skype для бизнеса Server.](../../plan-your-deployment/enterprise-voice-solution/enterprise-voice-resiliency.md)
...
- **Рекомендуется использовать system Center Operations Manager.** Рекомендуется отслеживать работоспособность развертывания Skype для бизнеса Server, чтобы обеспечить доступность служб для конечных пользователей. Вы можете использовать пакет управления диспетчером системных центров для Skype для бизнеса, доступный в качестве бесплатной загрузки из Microsoft. С пакетом управления Skype для бизнеса вы можете получать оповещения в режиме реального времени при возникновения проблем, запускать синтетические транзакции для тестирования функций Skype для бизнеса, получения отчетов о доступности служб и так далее. Это позволит упреждающее реагировать на неполадки в развертывании, прежде чем эти неполадки будут замечены пользователями.
...
- **Развертывание единой системы обмена сообщениями Exchange.** Skype для бизнеса Server работает как с локальной развертыванием единой системы обмена сообщениями Exchange (единой системы обмена сообщениями), так и с размещением единой системы обмена сообщениями Exchange. Центральный сайт A включает сервер единой системы обмена сообщениями Exchange, который выполняется Microsoft Exchange Server, а не Skype для бизнеса Server. Функции обмена данными Exchange для Skype для бизнеса Server выполняется в пуле переднего конца.
    ...
    Сведения о единой системы обмена сообщениями Exchange см. [](/previous-versions/office/lync-server-2013/lync-server-2013-hosted-exchange-unified-messaging-integration) в документации по планированию интеграции единой системы обмена сообщениями [Exchange](/previous-versions/office/lync-server-2013/lync-server-2013-planning-for-exchange-unified-messaging-integration) и интеграции единой системы обмена сообщениями.
