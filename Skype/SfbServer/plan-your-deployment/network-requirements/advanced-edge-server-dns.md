...
- Внешний доступ: \<ExtPoolFQDN\> https:///Autodiscover/autodiscoverservice.svc/Root
    
- Для внутреннего доступа: https:// \<IntPoolFQDN\> /AutoDiscover/AutoDiscover.svc/Root
    
Мы рекомендуем использовать автоматическое обнаружение, а не ручное обнаружение. Но если вы занимаетесь устранением неполадок или тестированием, могут быть полезны параметры вручную.
...
Эта ситуация представляет некоторые проблемы. Самое важное, что DNS с разделенным мозгом не **поддерживается для** Mobility. Это происходит из-за записей DNS LyncDiscover и LyncDiscoverInternal (LyncDiscover необходимо определить на внешнем DNS-сервере, а LyncDiscoverInternal — на внутреннем DNS-сервере).
...
- Этот внутренний contoso.com содержит:
  ...
  - Записи DNS A и AAAA (если вы используете адреса IPv6) и записи SRV для внутренней Skype для бизнеса Server клиентской автоконфигурации (которая **необязательна).**
...
Вы определяете и настраивает дополнительные записи DNS host A (AAAA при использовании IPv6) для внутреннего и внешнего разрешения веб-служб у поставщика GeoDNS. В следующих подробностях предполагается, что пулы с географическим разбросом распределены, а geoDNS, поддерживаемый вашим поставщиком, имеет круглую **DNS** или настроена на использование Пул1 в качестве основного и не может быть отключена в Pool2 в случае потери связи или отказа питания. 
...
- Клиент пытается установить подключение TCP к одному из IP-адресов. Если это не удается, он будет пытаться следующий IP-адрес кэшировали из этого списка.
...
В этом случае учитывается вес. В этом случае с высокой вероятностью следует получить более крупные весы. Администраторы DNS должны использовать вес 0, если не существует выбора сервера. В присутствии записей, содержащих вес более 0, записи с весом 0 имеют очень небольшой шанс принести выбран.
