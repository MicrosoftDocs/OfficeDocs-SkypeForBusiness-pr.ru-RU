...
Скайп для Business Server поддерживает следующие три простых URL-адресов: провести-связь, а администратор. Требуется настройка простых URL-адресов для собраний и телефонных и простых URL-адрес администрирования является необязательным. Записи доменных имен (DNS), которые должны поддерживать простые URL-адреса зависят от того, как было определено этих простых URL-адресов, и требуется ли поддержка аварийного восстановления для простых URL-адресов. 
...
|:---------------------|:---------------------------------------------------------------------------------------------------------------------|
| Провести  <br/>          | https://meet.contoso.com, https://meet.fabrikam.comи так далее (по одному для каждого домена SIP в организации)  <br/> |
...
| **Простой URL-адрес** <br/> | **Пример** <br/>                                                                                                                    |
|:---------------------|:-------------------------------------------------------------------------------------------------------------------------------------|
| Провести  <br/>          | https://SfB2015.contoso.com/Meet, https://SfB2015.fabrikam.com/Meetи так далее (по одному для каждого домена SIP в организации)  <br/> |
...
| **Простой URL-адрес** <br/> | **Пример** <br/>                                                                                                      |
|:---------------------|:-----------------------------------------------------------------------------------------------------------------------|
| Провести  <br/>          | <https://SfB2015.contoso.com/contosoSIPdomain/Meet>  <br/> <https://SfB2015.contoso.com/fabrikamSIPdomain/Meet>  <br/> |
| Телефонные подключения  <br/>       | <https://SfB2015.contoso.com/Dialin>  <br/>                                                                            |
| Admin  <br/>         | <https://SfB2015.contoso.com/Admin>  <br/>                                                                             |
...
Если у вас есть несколько сайтов, которые содержат пулов переднего плана и поставщика DNS поддерживает GeoDNS, можно настроить записи DNS для простых URL-адресов для поддержки аварийного восстановления, чтобы функциональность простой URL-адрес по-прежнему производится даже в том случае, если отключается один весь пул переднего плана. Эта функция аварийного восстановления поддерживает собраний и простых URL-адресов.

Чтобы настроить это, создайте два адреса GeoDNS. Каждый адрес имеет две записи DNS-записи A или CNAME, которые разрешают двух пулов, которые сочетании друг с другом для аварийного восстановления. Один адрес GeoDNS используется для внутреннего доступа и разрешает внутреннее полное доменное имя или нагрузки балансировки IP-адрес для двух пулов. Адрес GeoDNS используется для внешнего доступа и разрешает внешнее полное доменное имя или нагрузки балансировки IP-адрес для двух пулов. Ниже приведен пример для собраний простых URL-адрес, с помощью полные доменные имена для пулов. 
...
Затем создайте записи CNAME, которые разрешают собраний простых URL-адрес (например, meet.contoso.com) в два адреса GeoDNS.
...
При использовании этого метода можно настроить каждого адреса GeoDNS использовать метод циклического перебора для распространения запросов на двух пулов или для подключения в первую очередь pool (например, пул, расположенной ближе географически) и использовать только в случае из другого пула Сбой подключения. 

Можно настроить та же самая конфигурация-связи простых URL-адреса. Для этого создайте дополнительные записи как в предыдущем примере, подставляя `dialin` для `meet` в DNS-записей. Для простых URL-адрес администрирования используйте один из трех вариантов, перечисленных выше в этом разделе.

После настройки этой конфигурации мониторинга приложений необходимо использовать для настройки мониторинга HTTP для наблюдение за сбоев. Для внешнего доступа монитор, чтобы убедиться в том, lyncdiscover получить HTTPS.<sipdomain> запросы во внешнее полное доменное имя или нагрузки балансировки IP-адрес для двух пулов являются успешными. Например, следующие запросы не должно содержать любые заголовков **ACCEPT** и должен возвращать **200 OK**.
...
Для внутреннего доступа необходимо отслеживать порт 5061 на полное доменное имя внутренней сети или загрузкой балансировки IP-адрес для двух пулов. В случае обнаружения ошибок подключения к виртуальный IP-адрес для этих пулов необходимо закрыть порты 80, 443 и 4443.
