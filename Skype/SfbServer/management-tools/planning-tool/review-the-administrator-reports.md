...
Отчет администратора Skype для бизнеса — это первый из четырех ценных отчетов, подробно документировать проект. Сведения в этом отчете и трех других связанных отчетах полезны для ваших команд информационных технологий:
...
Конструктор и каждая из команд, которые будут развертывать инфраструктуру, управлять и поддерживать ее, должны просмотреть сводный отчет на точность и убедиться в том, что ошибки находятся как минимум на уровне.
...
- Шифрование — шифрование (протокол SSL, протокол TLS или протокол TLS) является важнейшим средством обеспечения безопасности коммуникаций, обеспечения конфиденциальности и создания надежной системы взаимодействия и совместной работы.

## <a name="review-the-firewall-report"></a>Просмотр отчета по брандмауэру
<a name="Firewall_report"> </a>

Skype для бизнеса Server 2015 имеет потенциально сложный набор правил брандмауэра. Средство планирования снижает эту сложность, создавая отчет, который подробно определяет все требования брандмауэра на основе критериев ввода конструктора. Администратор брандмауэра ИТ-инфраструктуры сможет использовать этот отчет для настройки и определения необходимых правил.
...
В отчете DNS, который входит в отчет администратора, подробно докаются все рекомендуемые и известные записи для службы доменных имен (DNS) во внутренней, демилиаторской и внешней сетях. Если конструктор выполнил изменения сетевой схемы, а все IP-адреса и полное доменное имя (FQDNs) определены в производственных значениях, отчет DNS предоставляет отличный ресурс конфигурации. Этот отчет также может служить рабочим документом по устранению неполадок.

![Отчет администратора DNS](../../media/DNS_Report_Admin_Report.png)

Группа управления DNS должна тщательно изучить отчет DNS, чтобы убедиться, что нет ошибок, которые могут вызвать трудности во время развертывания или усложнить сеанс устранения неполадок.
