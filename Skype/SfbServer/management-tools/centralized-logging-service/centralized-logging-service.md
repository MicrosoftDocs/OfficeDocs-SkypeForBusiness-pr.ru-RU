...
- Централизованный агент службы ведения журнала ClsAgent выполняется на каждой машине с Skype для бизнеса Server развернутой. Он прослушивает (в портах **TCP 50001-50003)** команды clsController над WCF и отправляет ответы обратно контроллеру. Он управляет сеансами журналов (start/stop/update) и поиском журналов. Он также выполняет операции по домашнему хозяйству, такие как архивация журналов и очистка. 
