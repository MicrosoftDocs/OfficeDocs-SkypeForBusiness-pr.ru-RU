...
**Сводка:** Ознакомьтесь с учебными материалами и примерами разработки для панели мониторинга качества звонков. Панель мониторинга качества звонков — это средство для Skype для бизнеса Server.
...
1. Служба данных, доступная для пользователей, прошедших проверку подлинности, с помощью [API-интерфейса данных для панели мониторинга качества звонков (CQD) в Skype для бизнеса Server](data-api.md).

2. Служба репозитория, доступная для пользователей с проверкой подлинности через [API репозитория для панели мониторинга качества звонков (CQD) в Skype для бизнеса Server](repository-api.md).
...
При переходе на домашнюю страницу CQD (например http://localhost/cqd), наборы отчетов и соответствующие отчеты для авторизованного пользователя, прошедшего проверку подлинности и уполномоченного, будут извлекаться из службы репозитория. Полный URL-адрес будет создан на основе идентификатора набора отчетов и года (код набора отчетов — это целое число после раздела "/#/" в URL-адресе, а по умолчанию — к концу кода набора отчетов после косой черты). Определения отчетов хранятся в формате JSON и после извлечения из службы репозитория вводятся в службу данных. В службе данных на основе введенных данных формируются запросы с многомерными выражениями (MDX), которые затем выполняются применительно к кубу для извлечения данных по каждому отчету. 
...
1. Создайте запрос (в примере он определен в переменной "запрос"). Запрос определяется как объект JSON, включающий в себя следующие данные.

   a) Измерения (не обязательно). Каждое измерение обозначается параметром DataModelName.
...
2. Отправка запроса в службу данных посредством вызова AJAX. Необходимо указать следующие параметры запроса.
   ...
   a) url (в качестве URL-адреса следует указать http://[имя_сервера]/QoEDataService/RunQuery).

   б) Data (это строковое представление объекта JSON, определенного в переменной "запрос"). Служба данных возвращает результаты запроса в виде параметра функции обратного вызова для успешного выполнения.

   в. Type (for Коедатасервице, Рункуери принимает только запросы POST).

   г. async (признак, указывающий, должен ли вызов AJAX быть синхронным или асинхронным).

   учеб. contentType (должно быть "Application/JSON").

   н. success (функция обратного вызова в случае успешного завершения вызова AJAX).

   финансовой. error (функция обработки ошибок в случае сбоя вызова AJAX).
...
Результатом приведенного выше кода является веб-страница, подобная показанной на рисунке (без определения отчета при первом посещении). Получение идентификатора Set отчета из CQD Portal (он указан после "/#/" входа в CQD URL-адрес портала (например, на первой фигуре идентификатор набора отчетов — 3024), и этот идентификатор набора отчетов будет помещен в раздел input (вход) на веб-странице. Нажмите кнопку "Загрузить" и ознакомьтесь с полным определением (измерениями, измерениями, списками фильтров) набора отчетов.

В сводке вы можете быстро получить полное определение отчета/набора отчетов. Для этого требуется выполнить следующие действия:

1. Перейдите на портал и с помощью редактора запросов настройте отчет (нажмите кнопку "Изменить" над отчетом, чтобы изменить, добавить, удалить измерения/измерения и фильтры, а затем сохраните отчет).

2. Получите идентификатор Set отчета с URL-адреса (целое число после "/#/" вход в URL-адрес).
...
1. Обновите измерения в переменной "запрос" с `[Measures].[Audio Good Streams JPDR Count]` и `[Measures].[Audio Poor Streams JPDR Count]` до `[Measures].[AudioPoorJPDRPercentage]`. 
...
4. Выполните синтаксический разбор результатов вызовов AJAX и поместите их в правильное положение в таблице. Поскольку эта операция выполняется преимущественно средствами HTML и JavaScript, ее детали здесь опущены. Соответствующий код приведен в приложении A.
    ...
    > [!NOTE]
    >  Если включен режим общего доступа к ресурсам (CORS), пользователи могут столкнуться с такими ошибками, как заголовок "нет", "нет", "нет", в запрошенном ресурсе нет заголовка "нет". Для происхождения "null" не разрешен доступ ". Чтобы устранить эту проблему, поместите HTML-файл в папку, в которую установлен портал (по умолчанию это должно быть `%SystemDrive%\Program Files\Skype for Business 2015 CQD\CQD)`. Затем можно получить доступ к HTML-документу через `http://<servername>/cqd/<html_file_name>`любой браузер с URL-адресом. (URL-адрес по умолчанию для локальной `http://<servername>/cqd.`панели мониторинга CQD) 
