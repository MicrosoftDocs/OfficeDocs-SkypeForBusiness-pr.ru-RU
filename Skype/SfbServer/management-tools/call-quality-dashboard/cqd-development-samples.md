...
При переходе к домашней странице CQD (например, набор отчетов и соответствующие отчеты для пользователя, подлинность и авторизованного пользователя будут извлечены из службы репозитория. http://localhost/cqd) Полный URL-адрес будет создан из ИД набора отчетов и год-месяц (ИД набора отчетов — это целый номер после раздела "/#/" в URL-адресе, а по умолчанию текущий год-месяц будет в конце ИД набора отчетов после косой черты). Определения отчетов хранятся в формате JSON и при извлечении из службы репозитория будут использоваться в качестве входных данных для службы данных. Служба данных создает запросы многомерных выражений (MDX) на основе входных данных и затем вызывает эти запросы многомерных выражений к кубу, чтобы получить данные для каждого отчета. 
...
1. Создание запроса (в примере он определен в переменной "query"). Запрос определяется как объект JSON, который включает следующие данные:
   ...
   - Операнд (тип сравнения, 0 означает "Равно").
     ...
     в. Один или несколько показателей.
...
2. Отправьте запрос в службу данных с помощью вызова AJAX. Должны быть предоставлены следующие параметры запроса:
   ...
   б. data (это строка, представленная объектом JSON, определенным в переменной "query"). Служба данных возвращает результаты запроса в качестве параметра функции обратного вызова для успешного выполнения.
   ...
   е. success (функция обратного вызова для успешного завершения вызова AJAX).
...
2. Получите ид набора отчетов по URL-адресу (это будет integer после URL-адреса для знака "/#/").
...
3. Создайте один набор фильтров для каждого сценария. Каждая строка системы показателей, представленная на рисунке, представляет разные сценарии, которые будут представлять собой разные фильтры (в то время как размеры и показатели остаются одинаковыми). 
