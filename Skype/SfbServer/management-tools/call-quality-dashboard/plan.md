...
- У пользователя, у которого уже есть участник безопасности для входа в базу данных метрик QoE (с полномочиями дб_датареадер) и участником безопасности входа в QoE архивном экземпляре SQL Server (требуется создать объект связанного сервера во время настройки). Эта учетная запись будет использоваться для запуска задания "QoE Archive Data" агента SQL Server.
