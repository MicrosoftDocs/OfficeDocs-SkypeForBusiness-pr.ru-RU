...
8. На странице Конфигурация куба введите следующие сведения:
   ...
   - **Сервер анализа кубов:** Имя экземпляра службы SQL Server Analysis Service для места создания куба. Это может быть другой компьютер, но пользователь, выполняющий установку, должен быть членом группы администраторов сервера в целевом экземпляре службы SQL Server Analysis Service.
     ...
     > [!NOTE]
     >  Дополнительные сведения о настройке разрешений администратора сервера служб Analysis Services см. в разделе [предоставление разрешений администратора сервера (службы Analysis Services)](https://msdn.microsoft.com/library/ms174561.aspx)
...
1. Настройка уровня Secure Sockets Layer в службах IIS: [Настройка уровня защищенных сокетов в службах IIS 7](https://technet.microsoft.com/library/cc771438%28v=ws.10%29.aspx). После этого замените `http` на `https`.
