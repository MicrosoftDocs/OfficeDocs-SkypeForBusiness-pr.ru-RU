...
- **Расположение файлов журнала сервера** Расположение папки, в которой хранятся журналы сервера политики пропускной способности. Обычно это филесервер \<\>\\_лт_чоице на Fe\>\аппсерверфилес\пдп.
...
- **Режим конференции** Отчет о подробных данных для конкретной конференции, включая сведения о времени расписания для Конференции, список участников, список типов мультимедиа, разрешенных для Конференции, активный Мкус (единицы управления MultiPoint), активный список участников и каждый сигнальное состояние участника.
...
Средство Мстурнпинг позволяет администратору коммуникационного программного обеспечения Skype для Business Server 2015 проверять состояние серверов, использующих службы голосовой и видеосвязи, голосовые и видеозвонки, а также серверы, на которых работает политика пропускной способности. Службы в топологии.
...
Сохранить и сохранить схему топологии сети CAC в формате JPG или BMP: Skype для бизнеса Server 2015 администраторы могут сохранить конфигурацию топологии сети CAC в графическом формате (форматы файлов JPG и BMP) с помощью команды **сохранить схему конфигурации сети как параметр рисунка** , как показано ниже.

![Сохранение конфигурации сети как изображение.](../media/Reskit_2012_Tools_Documentation_Image26.jpg)

 <strong>Просмотр данных конфигурации топологии сети CAC:</strong> Skype для бизнеса Server 2015 администраторы могут просматривать данные о конфигурации сети, сетевые сайты, профили пропускной способности и IP-адреса подсети сайтов в текстовом формате с помощью параметра просмотра данных конфигурации сети, как показано ниже. следующие.
...
2. A trusted application needs to be defined in the topology for the SEFAUtil tool. Чтобы определить Сефаутил как новое надежное приложение, используйте командную консоль управления Skype для бизнеса Server и выполните следующий командлет:
   ...
   ```
    ...
    > [!NOTE]
    > Полное доменное имя пула: полное доменное имя (FQDN) сервера или пула, на котором будет размещено приложение Сефаутил (обычно это серверный интерфейс _Гт_ или пул Skype для бизнеса).
...
- **Перенос параметров конфигурации в новую версию Skype для бизнеса Server:** Компания Contoso выполняет миграцию на Skype для бизнеса Server 2015 и в рамках процесса миграции администратор Skype для бизнеса Server хотел бы переместить неназначенные номера, настроенные приложением-объявлением из Lync Сервер 2013 — развертывание на новой странице развертывания в Skype для бизнеса Server 2015. Для перемещения параметров конфигурации администратор сервера Skype для бизнеса может использовать средство миграции "неназначенные номера объявлений".
