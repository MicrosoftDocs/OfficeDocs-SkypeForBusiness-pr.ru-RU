...
- Для использования функции контроля допуска звонков необходимо указать профиль политики пропускной способности для каждого сайта, на который распространяются ограничения полосы пропускания WAN. Если вы планируете развернуть управление приемом вызовов, необходимо создать профили политики [пропускной](create-bandwidth-policy-profiles.md) способности в Skype для бизнеса Server перед настройкой сетевых сайтов.

- Для использования E9-1-1 необходимо указать политику расположения для каждого сайта. Если планируется развертывание E9-1-1, необходимо создать политики расположения в Skype для бизнеса Server перед [настройкой](create-location-policies.md) сетевых сайтов.
...
3. Запустите следующий cmdlet для импорта **subnet.csv,** а затем храните его содержимое в магазине управления Lync Server:
