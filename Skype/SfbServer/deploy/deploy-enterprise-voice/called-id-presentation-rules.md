...
2. Чтобы приступить к определению правила перевода, выполните [](configure-trunk-with-media-bypass.md) действия в Настройка магистрали с обходом мультимедиа в Skype для бизнеса Server шаге 10 или настройка магистрали без обхода мультимедиа в Skype для бизнеса Server шаге 9.[](configure-trunk-without-media-bypass.md)
...
5. В разделе **Build a Translation Rule** (Создание правила преобразования) введите следующие значения:
   ...
   - **Цифры для удаления**: (Необязательно) Укажите количество начальных цифр, которые необходимо удалить. Например, введите 1, чтобы отострить +с начала номера.
...
6. В диалоговом окне **Ввод регулярных выражений** введите следующее.
   ...
   - В поле **Правило преобразования** укажите шаблон для формата преобразованных номеров.
     ...
     Например, если ввести ^\+(\d\d{9}+)$ в соответствие этому шаблону и 011$1 в правиле **перевода, правило** переведет +441235551010 на 011441235551010.
