...
Агент, который должен войти в группу и выйти из нее, не входя в Skype для бизнеса или выходя из нее, называется формальным агентом. Официальные агенты должны войти в группу, прежде чем они смогут получать направленные в нее звонки. Это может быть удобно для агентов, которые отвечают на звонки из группы неполный рабочий день. Официальные агенты входят и выходят из группы, щелкнув пункт меню в Skype для бизнеса, чтобы открыть браузер Windows Internet Explorer и отобразить консоль веб-страницы.
  
Агент, который не входит в группу и не выходит из нее, называется неофициальным агентом. Неформальные агенты автоматически включались в группу при входе в Skype для бизнеса, и они не могут выйти из группы.
...
### <a name="to-use-skype-for-business-server-control-panel-to-create-or-modify-an-agent-group"></a>Использование панели управления Skype для бизнес-серверов для создания или изменения группы агентов
...
9. В разделе **Метод маршрутизации** выберите метод маршрутизации вызовов для агентов, входящих в группу, следующим образом:
   ...
   - Чтобы сначала предложить новый вызов агенту, который больше всего простаивает  (больше всего в Skype для бизнеса имеется наличие доступных или неактивных), нажмите кнопку **Longest idle**.  
   ...
   - Чтобы предложить новый вызов всем агентам, которые подписаны в Skype для бизнеса и в приложение Группы ответа одновременно, независимо от их текущего присутствия, щелкните **Attendant**. Пользователи, настроенные в качестве агентов, могут видеть все вызовы, ожидающих и отвечающих на вызовы ожидания в любом порядке. Вызов отправляется первому агенту, который принимает его, после чего другие агенты больше не видят вызов.
...
### <a name="to-use-skype-for-business-server-management-shell-to-create-or-modify-an-agent-group"></a>Использование оболочки управления Skype для бизнес-серверов для создания или изменения группы агентов
