...
> [!IMPORTANT]
>  Перед обновлением сохраните копию топологии. После обновления вы не сможете понизить уровень топологии. _Гт_, если ваши службы находятся на том же сервере, что и базы данных, например служба "сохраняемый чат" находится на том же компьютере, что и в базе данных сохраняемого чата, пропустите этот шаг и перейдите к действию 4. После остановки служб запустите на каждом сервере настройку обновления на месте для обновления локальных баз данных.
...
> [!NOTE]
>  Перед обновлением установите все необходимые компоненты, необходимые для работы Skype для бизнеса Server 2015, которые включают: _Гт_ не менее 32 ГБ свободного места перед попыткой обновления. Кроме того, убедитесь, что диск является фиксированным локальным диском, а не подключен USB или FireWire, отформатирован с помощью файловой системы NTFS, не сжимается и не содержит файла подкачки. _Гт_ PowerShell версии 6.2.9200.0 или более поздней. _Гт_ новейшее приложение Lync Server 2013 Установлено накопительное обновление. _Гт_ SQL Server 2012 SP1 установлен. _Гт_ (устанавливается автоматически при использовании Microsoft Update): _Гт_ Windows Server 2008 R2-[KB2533623](https://support.microsoft.com/kb/2533623)_Гт_ windows Server 2012-[KB2858668](https://support.microsoft.com/kb/2858668)_гт_ Windows Сервер 2012 R2-[KB2982006](https://support.microsoft.com/kb/2982006)
...
1. На каждом сервере запустите **файл Setup. exe** из **OCS_Volume\Setup\amd64** на установочном носителе Skype для бизнеса Server 2015.
...
Если при обновлении на месте происходит сбой **проверки готовности** к обновлению или **установки недостающих предварительных условий**, убедитесь, что на сервере установлены все последние обновления Windows Server, Lync Server и SQL Server, а все необходимое программное обеспечение и роли установлена. Чтобы получить список необходимых данных, ознакомьтесь со сведениями [о серверных требованиях для Skype для бизнеса server 2015](../plan-your-deployment/requirements-for-your-environment/server-requirements.md) и [установите необходимые условия для Skype для бизнеса Server 2015](install/install-prerequisites.md).
