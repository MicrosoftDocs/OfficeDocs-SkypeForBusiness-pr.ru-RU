...
1. Используйте Internet Explorer для открытия **SQL Server Reporting Services** домашней страницы. URL-адрес домашней страницы Службы отчетов включает в себя:
   ...
   - Имя экземпляра базы данных, где установлены отчеты мониторинга (например, **archinst**).
     ...
     Например, если SQL Server Reporting Services `atl-sql-001.litwareinc.com` установлен на компьютере, а отчеты мониторинга используют сводку экземпляра базы данных, URL-адрес домашней страницы будет выглядеть так:
...
4. На странице **CDRDB** на вкладке **Свойства** и посмотрите строку подключения текстового окна с **меткой Connection**. Текущая строка подключения будет выглядеть примерно так:
...
7. На странице **CDRDB** щелкните **Reports_Content** ссылку. Щелкните **источник данных QMSDB** и отредактировать строку подключения для базы данных QoE. Например:
