...
Необходимо установить пакет управления Operations рабочей области для запуска сбор данных в журналах с системами комнаты Скайп устройств. Рабочая область — уникальный анализа журнала среды с собственными хранилище данных, источники данных и решения. Если у вас уже есть существующей рабочей области для анализа журнала, его можно использовать для отслеживания развертывания систем комнаты Скайп или можно создать должно выделенного анализа журнала рабочую область специально для систем комнаты Скайп наблюдение за.
...
### <a name="configure-operations-management-suite-to-collect-skype-room-systems-event-logs"></a>Настройка операций Management Suite собирать журналы событий системы Скайп комнаты
...
Необходимо настроить пакет управления операции собирать журналы, необходимые для наблюдения за состоянием системы комнаты Скайп устройств и приложений. Устройства версии 2 Скайп комнаты систем используют журнал событий Скайп комнаты систем.

Настройка набора операций управления для сбора событий Скайп комнаты систем, в статье [источников данных журнала событий Windows в аналитике журнала](https://docs.microsoft.com/azure/log-analytics/log-analytics-data-sources-windows-events)
...
Необходимо подготовить набор операций управления должны иметь возможность отслеживания событий, связанных с системами Скайп комнаты. Со, необходимые для развертывания агентов Operations Management Suite только один или два систем комнаты Скайп устройств имеют физический доступ к и передавать эти устройства проверки создания некоторых данных и записывать его в рабочую область аналитических журнала.
...
Развертывание агента Operations Management Suite тестовых устройств с помощью инструкций, приведенных в [компьютеры Windows подключиться к службе анализа журнала в Azure](https://docs.microsoft.com/azure/log-analytics/log-analytics-agent-windows). В этой статье приводятся подробные сведения о действиях, которые для развертывания Microsoft мониторинга агента для Windows, инструкции для получения идентификатора операции управления набора приложений рабочей области и первичный ключ для получения систем комнаты Скайп устройств, подключенных к вашей Операции управления набора приложений развертывания и действия для проверки подключения агента анализа журнала.
...
2.  Перечислены события, создаваемые устройства Скайп комнаты систем:
...
4.  Создание проблема оборудования и проверить, что необходимые события регистрируются в пакет управления Operations.
    1.  Отключите один из периферийных устройств на тестовых систем комнаты Скайп системы. Это может быть камера, устройство громкой связи, микрофон или отображения передний план комнаты
...
5.  Создание проблем приложений и проверки необходимых событий.
    1.  Изменение конфигурации приложения Скайп комнаты систем и введите пара неправильный адрес и пароль Session Initiation Protocol (SIP).
    2.  Подождите 10 минут для журнала событий, которые следует добавить в пакет управления Operations.
...
3.  Имя плитку **Скайп комнаты систем**.
...
4.  Определение свойств **заголовков** :<br>
    ...
    **Заголовков запроса:**```Event | where EventLog == "Skype Room System" and SRSOperationName_CF == "Heartbeat" and TimeGenerated > ago(20m) | summarize AggregatedValue = count() by Computer | count```
...
3.  Определение свойств **заголовков** :<br>
    ...
    **Заголовков запроса:**```Event | where EventLog == "Skype Room System" and SRSOperationName_CF == "Heartbeat" | summarize LastHB = max(TimeGenerated) by Computer | where LastHB < ago(20m) | count```
...
1.  Выберите **номер & списка** из коллекции, а затем добавьте новый фрагмент.
2.  Определение **общих** свойств:<br>
    **Групповой заголовок:** Оборудование<br>
    **Новую группу:** Выбранные
3.  Определение свойств **заголовков** :<br>
    **Условные обозначения:** Устройства, на которых произошла ошибка оборудования за последний час <br>
    **Заголовков запроса:**```Event | where EventLog == "Skype Room System" and EventLevelName == "Error" and EventID == "3001" and TimeGenerated > ago(1h) | summarize AggregatedValue = count() by Computer | count```
4.  Определение свойств **списка** :<br>
...
### <a name="create-a-tile-that-displays-skype-room-systems-application-versions"></a>Создать элемент, который отображает версии приложений системы Скайп комнаты
...
3.  Определение свойств **заголовков** .<br>
    ...
    **Заголовков запроса:**```Event | where EventLog == "Skype Room System" and EventLevelName == "Error" and EventID == "2001" and TimeGenerated > ago(1h) | summarize AggregatedValue = count() by Computer | count```
4.  Определение свойств **списка** .<br>
...
3.  Определение свойств **заголовков** .<br>
    ...
    **Заголовков запроса:**```Event | where EventLog == "Skype Room System" and EventID == "4000" and TimeGenerated > ago(24h) | summarize AggregatedValue = count() by Computer | count```
4.  Определение свойств **списка** .<br>
...
<a name="Alerts"></a> Устройства при систем комнаты Скайп обнаруживает ошибку, пакет управления Microsoft Operations может создать оповещения для уведомления администраторов подробные сведения о проблеме.
...
> [!NOTE]
> В приведенных ниже примерах отправлять оповещения по электронной почте при устройства систем комнаты Скайп приводит к возникновению ошибки оборудования или ошибки приложения. 


### <a name="configure-an-email-alert-for-skype-room-systems-hardware-issues"></a>Настройка оповещения по электронной почте для систем комнаты Скайп неполадки с оборудованием

Настройте правило оповещения, которое проверяет наличие систем комнаты Скайп устройств, которые были применены неполадки с оборудованием за последний час.
1.  Войдите в [портал Microsoft Operations Management Suite](http://aka.ms/omsportal).
...
5.  Настройка параметров оповещений с помощью следующие сведения:<br>
    **Имя правила:** Оповещение о сбоя оборудования систем Скайп комнаты<br>
    ...
    **Электронной почты субъекта:** Оповещение о сбоя оборудования систем Скайп комнаты<br>
    **Получателей:** Укажите адреса электронной почты, используя точку с запятой в качестве разделителей<br>

6.  Нажмите кнопку **Сохранить**.

### <a name="configure-an-email-alert-for-skype-room-systems-application-issues"></a>Настройка оповещения по электронной почте для систем комнаты Скайп проблем приложений

Настройка оповещений правила, которое проверяет наличие систем комнаты Скайп устройств, которые были применены проблем приложений за последний час.
...
4.  Настройка параметров оповещений с помощью следующие сведения:<br>
    **Имя правила:** Оповещение о сбое при Скайп комнаты систем приложений<br>
    ...
    **Частота оповещений:** 1 час<br>
    **Количество результатов:** Больше 0<br>
    **Электронной почты субъекта:** Оповещение о сбое при Скайп комнаты систем приложений<br>
...
<a name="configure_all_devices"></a> После настройки панели мониторинга и оповещения можно Установка и настройка агенты управления набор операций на все устройства Скайп комнаты системы для выполнения мониторинга развертывания.
...
Если вы уже развернули устройство Скайп комнаты систем перед реализацией операций Management Suite, предоставленного сценария можно использовать для установки и настройки агентов с помощью групповых политик Active Directory.
...
4.  Создание нового объекта групповой политики и назначения ее подразделение, где хранятся учетные записи Скайп комнаты системы компьютера.
...
7.  Систем комнаты Скайп устройств следует Установка и настройка агента мониторинг Microsoft с второй перезагрузке.
