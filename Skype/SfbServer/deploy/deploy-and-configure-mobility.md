...
Просмотрев статью "Планирование мобильности для Skype для бизнеса [Server",](../plan-your-deployment/mobility.md) вы должны быть готовы приступить к приведенной ниже работе, чтобы развернуть мобильность в среде Skype для бизнеса Server. Ниже следуют шаги (и мы включаем в эту таблицу список разрешений):
...
2. Нажмите **кнопку "Пуск"** и  выберите "Администрирование" **(может** потребоваться найти его, если это не параметр из меню "Пуск"), а затем щелкните **DNS,** чтобы открыть оснастку администратора DNS.
...
4. У вас есть время, чтобы узнать, что из следующего у вас есть:
   ...
   - Любые записи A или AAAA для сервера переднего сервера (standard или Enterprise) или пулов переднего сервера.
    
   - Любые записи A или AAAA для директора или пула директоров (дополнительная конфигурация, которая может быть в развертывании).
    
5. После этого щелкните правой кнопкой мыши имя домена SIP и выберите в меню новый псевдоним **(CNAME).**
    
6. В **текстовом ящике** псевдонима введите lyncdiscoverinternal для имени хоста для внутреннего URL-адреса службы автообнаружия.
...
7. Возможно, вам потребуется сохранить здесь или создать дополнительные записи CNAME в зоне forward lookup каждого домена SIP в среде Skype для бизнеса Server, это необходимо сделать, но когда все будет готово, сохраните свою работу.
...
7. Вам может потребоваться сохранить здесь или если вам нужно создать дополнительные записи A или AAAA в зоне forward lookup каждого домена SIP для вашей среды Skype для бизнеса Server, это необходимо сделать, но когда все будет готово, сохраните свою работу.
...
4. Сведения из шага 3 будут уникальными для вас. Вам необходимо найти его, чтобы определить, назначен ли вам один сертификат для нескольких вещей или для различных компонентов, которые в них нуждаются. Параметр **Use** покажет, как используется сертификат, а параметр **Thumbprint** – один или несколько сертификатов.
    
5. Если в нашем разделе планирования рекомендуется использовать записи SAN, все будет хорошо. В этом случае вам потребуется запросить новый сертификат или несколько сертификатов (в зависимости от конфигурации) в вашем сертификате.
...
1. Проверив, какие записи SAN у вас есть, вы знаете, что у вас есть один сертификат **(после** проверки с помощью вышеперечисленной проверки) и узнали, что у вас нет всех необходимых записей. В ваш ЦС должен быть сделан новый запрос на сертификат. Откройте Skype для бизнеса Server PowerShell:
   ...
   - Для отсутствующих san службы автонаружения (заменив параметр -Ca на собственный путь к ЦС):
...
- В зависимости от того, что вы нашли в разделе "Требуются ли новые сертификаты" выше, необходимо выполнить **одно** из следующих запусков.
  ...
  - Если у вас есть один сертификат для всего (отпечаток идентичен), необходимо выполнить этот запуск:
    
  ```powershell
  Set-CsCertificate -Type <certificate(s) from the Use parameter> -Thumbprint <unique identifier>
  ```

  - Если у вас есть разные сертификаты для различных вещей (отпечатки отличаются), запустите этот проект:
...
7. Вам также потребуется проверить записи **альтернативного имени** субъекта. Вы найдете один или несколько из следующих ок.
   ...
   - Внутренние и внешние имена веб-служб (например, webpool01.contoso.net, webpool01.contoso.com), основанные на вариантах, сделанных в построитель топологий и выборе веб-служб.
    
   - Если он уже назначен, lyncdiscover.\<sipdomain\> и lyncdiscoverinternal.\<sipdomain\> записи.
    
     Вам потребуется проверить несколько сертификатов, если вам назначено несколько сертификатов (см. примечание выше).
    
8. Поэтому, если вы нашли lyncdiscover.\<sipdomain\> и lyncdiscoverinternal.\<sipdomain\> records, you've got this configured already. Можно закрыть MMC.
...
Следующие шаги не предназначены для точного следованию. Это потому, что в предыдущих версиях продукта мы походили бы на вас, например, на настройку шлюза управления угрозами (TMG), и если вы не использовали его, вам потребуется разработать собственную версию оттуда.
...
3. Как правило, необходимо ввести следующие сведения:
   ...
   - В качестве пути для публикации необходимо ввести _, но также необходимо **/\\** перенаад значитеть _*исходный заглавный_загос.*.
...
5. Необходимо **также настроить** порты веб-сервера, а также сделать следующее:
    
   - **Запросы на перенаправление на ПОРТ HTTP,** а номер порта должен быть **8080.**
...
После развертывания Skype для бизнеса Server Mobility Service и службы автообнаружия Skype для бизнеса Server необходимо выполнить тестовую транзакцию, чтобы убедиться, что развертывание работает правильно. Вы можете запустить **Test-CsUcwaConference,** чтобы проверить возможность двух пользователей создавать, присоединяться и общаться в конференции. Для этого тестирования вам потребуется два пользователя (реальные или тестовые) и полные учетные данные. Эта команда будет работать как для клиентов Skype для бизнеса, так и для клиентов Lync Server 2013.
  
For Lync Server 2010 clients on Skype for Business Server 2015, you'll need to run **Test-CsMcxP2PIM** to test. Ваши пользователи Lync Server 2010 по-прежнему должны быть фактическими или предварительно заранее задав пользователей, и вам потребуется их пароль.
...
1. Войдите с учетной записью, которая является членом роли **CsAdministrator,** на компьютере, на котором установлены Skype для бизнеса **Server Management Shell** и **Ocscore.**
...
3. Отключите доступ к мобильности и вызову с помощью work глобально, введя:
   ...
   ```
    ...
    > [!NOTE]
    > Вы можете отключить "Позвонить с работы", не отключая доступ к мобильной работе. But you can't turn off Mobility without also turning off Call via Work. 
...
3. Можно создать политику на уровне сайта, отключить VoIP и видео, включить функцию "Требовать Wi-Fi для IP-аудио" и "Требовать Wi-Fi для IP-видео по сайту". Тип:
