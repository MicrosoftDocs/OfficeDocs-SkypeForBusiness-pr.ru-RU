...
3. Создание кластера Windows сервера (WSFC).
   ...
   - В **мастере управления** кластерами неудачных сбой нажмите правой кнопкой мыши управление кластером **failover**, а затем нажмите **кнопку Создать кластер**.
...
4. Рекомендуется настроить параметры кластерного кворума для использования свидетеля обмена файлами. Для этого используйте следующие действия:
   ...
   - На странице **Выбор параметра конфигурации кворума** нажмите **кнопку Выберите свидетель кворума.**
...
5. На каждом сервере кластера встроим функцию AG в диспетчер конфигурации SQL Server.
   ...
   - Откройте диспетчер конфигураций SQL Server. На дереве в левой части экрана нажмите кнопку **SQL Server службы,** а затем дважды щелкните SQL Server службу. 
...
7. Создание группы доступности.
   ...
   - Нажмите **кнопку Добавить,** а затем в **поле IPv4 Адрес,** устройте предпочтительный виртуальный IP-адрес, а затем нажмите **кнопку ОК**.
    
   - На странице **Выбор** начальной синхронизации данных выберите Полный и укажите папку, доступную репликам, и чтобы учетная запись службы SQL Server, используемая обеими репликами, имеет разрешения на запись. Затем нажмите **Далее**.
   ...
   - На странице Проверка убедитесь, что все проверки проверки являются успешными, а затем нажмите **кнопку Далее**.
...
8. После развертывания пула и ag выполните некоторые заключительные действия, чтобы убедиться, что SQL логины находятся на каждой из реплик в группе доступности AlwaysOn. 
   ...
   - Расширяйте Skype для бизнеса Server, расширяйте топологию и **расширяйте SQL Server Магазины.** Щелкните правой кнопкой мыши SQL магазина новой группы доступности AlwaysOn и нажмите **кнопку Изменить свойства**.
     ...
     - В нижней части страницы в поле **SQL Server FQDN** измените значение на значение FQDN слушателя AG.
   ...
   - Повторите два предыдущих шага (сбой над группой до вторичной реплики, а затем  `Install-CsDatabase -Update` использовать) для каждой реплики в группе.
...
2. Используйте Topology Builder для удаления зеркального зеркала баз данных из пула.
   ...
   - Откройте построитель топологий. В топологии расширите **выпуск Enterprise** пулы переднего конца, щелкните правой кнопкой мыши имя пула и нажмите **кнопку Изменить свойства**.
...
4. Используйте SQL Server Management Studio, чтобы разбить зеркало.
   ...
   - Откройте SQL Server Management Studio, перейдите к базам данных, щелкните правой кнопкой мыши **Задачи** и нажмите **зеркало**. Затем нажмите **кнопку Удалить зеркальное отражение** и нажмите **кнопку ОК**.
...
11. Создайте новый магазин с указанием прослушивателем AG и указанием основного старого зеркала в качестве основного узла AG.
    ...
    - Откройте построитель топологий. В топологии разйдитесь по разделу **Общие** компоненты, щелкните правой кнопкой **мыши SQL Server Магазины** и нажмите кнопку **SQL Server Store**.
    ...
    - В поле SQL Server доступности **FQDN** введите FQDN слушателя, созданного при создания группы доступности.
...
14. Выполните некоторые заключительные шаги, чтобы убедиться, что SQL логины находятся на каждой из реплик в группе доступности AlwaysOn.
    ...
    - Расширяйте Skype для бизнеса Server, расширяйте топологию и **расширяйте SQL Server Магазины.** Щелкните правой кнопкой мыши SQL магазина новой AG и нажмите **кнопку Изменить свойства**.
...
6. Создание группы доступности.
   ...
   - На странице Выбор баз данных выберите базы данных, которые необходимо включить в ag. Затем нажмите **Далее**.
     ...
     Не включайте в AG базы данных **ReportServer,** **ReportServerTempDB** или Persistent Chat, так как они не поддерживаются в этом сценарии. Вы можете включить все Skype для бизнеса Server базы данных в AG.
