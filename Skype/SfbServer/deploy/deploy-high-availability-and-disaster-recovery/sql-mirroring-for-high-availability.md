...
---
# <a name="deploy-sql-mirroring-for-back-end-server-high-availability-in-skype-for-business-server-2015"></a>Развертывание SQL зеркального зеркала для серверов заднего конца в Skype для бизнеса server 2015


Для развертывания зеркального SQL серверы должны запускать минимум SQL Server 2008 R2. Эта версия должна работать на всех участвующих серверах: первичном, зеркальном и свидетельном. Подробные сведения см. в материале Накопительный пакет обновления [9 для SQL Server 2008 Пакет обновления 1](https://go.microsoft.com/fwlink/p/?linkid=3052&amp;kbid=2083921).
...
> [!NOTE]
> Для записи зеркальных файлов необходимо создать файл файла SQL Server и SQL агента. Если служба SQL Server работает в контексте сетевой службы, можно добавить к разрешениям на совместное использование<\<Domain\> \\ SQLSERVERNAME $ как главных, так и зеркальных серверов \> SQL. $ важно определить, что это учетная запись компьютера.
...
Вы можете использовать Topology Builder для редактирования свойств уже существующего пула, чтобы включить SQL зеркальное отражение.

## <a name="to-add-sql-mirroring-to-an-existing-front-end-pool-in-topology-builder"></a>Добавление зеркального SQL к существующему пулу переднего конца в Topology Builder
...
9. Опубликуйте топологию. После этого появится запрос на установку базы данных.
    ...
    В процессе публикации топологии вам будет предложено определить путь к совместной обработке файлов. Серверы SQL, которые участвуют в зеркальном зеркальном окантовыве, должны иметь доступ к этой папке для чтения и записи, чтобы создать зеркало.
...
## <a name="using-skype-for-business-server-2015-management-shell-cmdlets-to-set-up-sql-mirroring"></a>Использование команды "Команды для управления Skype для бизнеса Server 2015" для SQL зеркального SQL
...
1. Откройте окно "Оболочка управления Skype для бизнеса Server 2015" и запустите следующий cmdlet:
...
2. Проверьте следующее:
   ...
   - Учетные записи SQL серверов на всех первичных и зеркальных SQL серверах имеют разрешение на чтение и запись в файле \\ E04-OCS\csdatabackup
...
Этот параметр приводит к удалению затронутых баз данных  `-DropExistingDatabasesOnMirror` из зеркала.
...
3. Опубликуйте топологию.
   ...
   ```
    ...
    Однако не следуйте этому шагу и не введите, так как это позволит удалить всю  `Uninstall-CsMirrorDatabase` конфигурацию зеркального зеркала.

4. Чтобы удалить только свидетеля из конфигурации SQL Server, следуйте инструкциям в Удалите свидетеля из сеанса зеркального зеркального [SQL Server).](/sql/database-engine/database-mirroring/remove-the-witness-from-a-database-mirroring-session-sql-server)