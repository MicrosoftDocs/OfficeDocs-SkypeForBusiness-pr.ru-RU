...
**Сводка:** Настройте роль сервера видеосвязи (VIS) в Skype для бизнеса Server.
...
Используйте следующие Windows PowerShell, чтобы указать конфигурацию видеоманкетов и телефонную связь, которые должны быть связаны с новой магистралью, определенной в документе топологии между VIS и всеми видео шлюзами. Все эти параметры можно установить на глобальном уровне, уровне сайта или службы (видео шлюза). 
 ...
 **GatewaySendsRtcpForCallsOnHold** Этот параметр определяет, по-прежнему ли пакеты RTCP отправляются по магистрали для вызовов, помещенных на удержание, и пакеты мультимедиа не должны поступать в любом направлении. VIS может завершить вызов, если пакеты RTCP, которые перетекают из VTC в VIS, пока вызов находится на удержании. По умолчанию используется значение **True**. Если протокол SIPTransport имеет TCP, этот параметр игнорируется.
