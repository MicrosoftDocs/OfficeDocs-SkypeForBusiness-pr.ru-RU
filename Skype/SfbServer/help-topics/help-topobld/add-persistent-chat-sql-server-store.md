...
**Щелкните Новое**, чтобы определить новый SQL Server для свидетеля необязательный экземпляр для свидетеля сохраняемой SQL Server зеркального.
  
Нажмите кнопку **Назад**, чтобы вернуться в предыдущее диалоговое окно определения пула.
  
Нажмите **кнопку** Далее после завершения ввода параметров конфигурации SQL Server этого пула и перейти к определению пула сохраняемой системы чата.
