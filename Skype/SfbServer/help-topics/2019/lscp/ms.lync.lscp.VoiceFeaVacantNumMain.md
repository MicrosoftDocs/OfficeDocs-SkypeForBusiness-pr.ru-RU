...
- **Перемещение вверх** Перемещает выбранный ненанаданный диапазон номеров в списке, чтобы Skype для бизнеса Server нашел его раньше и применял указанное действие, прежде чем применять действия, указанные для других диапазонов в списке.

    > [!NOTE]
    > Skype для бизнеса Server выполняет поиск по ненаназвершенной таблице номеров сверху вниз и использует первый диапазон, соответствующий ненаназоватому номеру. Например, если у вас есть диапазон, который задает действие, выполняемое в исключительных обстоятельствах, убедитесь, что этот диапазон находится внизу списка.
