...
Надстройки используются для расширения доступных в комнате возможностей. Типичная надстройки могут содержать URL-адрес приложение Silverlight, перехватывает при Финансовый символ представляется чата и отображает журнал запасов в области расширяемости. Другие примеры внедрения URL-адреса OneNote в комнате чата в качестве надстройки для включения некоторые общего контекста, например «В начало виду» или «Раздел дня».
