...
- **Включить для архивных** данных возможность с помощью этой возможности Выберите этот параметр, чтобы включить функцию и указать параметры, включающие следующие параметры:
  ...
  - Purging after the archiving data has been exported (which includes data that has been uploaded to Exchange, if you enable Microsoft Exchange integration).
    ...
    > [!NOTE]
    > Если включить интеграцию с Microsoft Exchange, exchange будет управлять процессом с помощью Exchange для пользователей, размещенных в Exchange, и их почтовых ящиковIn-Place удержанием. Единственное исключение — файлы для аудиоконференций, хранимые в файловом хранилище Lync Server. Эти файлы удаляются из файлового хранилища только после того, как эти файлы экспортированы (отправлены в Exchange), если выбран параметр удаления данных после экспортирования архивных данных; или после заданного числа дней, если задан параметр числа дней хранения. 
