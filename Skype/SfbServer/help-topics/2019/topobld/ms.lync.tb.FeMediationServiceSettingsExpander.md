...
Если вы коллокируете сервер-посредник на переднем конце пула или на сервере Standard Edition, выберите поле с включенным сервером-посредником **collocated.** Если вы решите не коллокировать сервер-посредник, в этом разделе нет вызывающих параметров.
