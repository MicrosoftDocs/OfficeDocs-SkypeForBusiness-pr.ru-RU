...
- **Агент пользователя** В списке можно выбрать тип клиента. В следующей таблице определяются коды агентов пользователя.
...
- **Действие** Вы можете указать действие, выполняемые при выполнении критериев, которые были выполнены в предыдущих шагах. Доступны следующие действия:
  ...
  - **Блокировка и обновление** Предотвращает вход клиента в систему и позволяет клиенту получать обновления из службы обновления Windows Server или Обновления Майкрософт. Это действие доступно, только если выбрано приложение **OC** агента пользователя.
