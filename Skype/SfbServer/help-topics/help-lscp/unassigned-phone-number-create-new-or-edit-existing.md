...
- **Диапазон номеров** В первом поле введите начальское число ненаназшенного диапазона номеров. Во втором поле введите конечное число диапазона.
  ...
  - Номер должен соответствовать регулярному выражению `(tel:)?(\+)?[1-9]\d{0,17}(;ext=[1-9]\d{0,9})?`. Это означает `tel:` , что номер может начинаться со строки (если вы не указываете, что строка будет автоматически добавлена для вас), знак плюс (+) и цифру 1-9. Номер телефона может быть длиной до 17 цифр, и в конце может быть указан добавочный номер в формате ";ext=добавочный_номер".
