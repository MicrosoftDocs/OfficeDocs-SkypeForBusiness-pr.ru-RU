...
- **Имя** Введите описательное имя, идентифицирующие диапазон ненаписаных номеров. После сохранения диапазона это имя нельзя изменить.
...
- **Диапазон номеров** В первом поле введите первый номер диапазона ненаписаных номеров. Во втором поле введите конец диапазона.
  ...
  - Номер должен соответствовать регулярному выражению (tel:)? \+ ()? [1-9]\d {0,17} (;ext=[1-9]\d {0,9} )?. Это означает, что номер может начинаться со строки tel: (если не указать эту строку, она будет автоматически добавлена для вас), знак плюса (+) и цифры от 1 до 9. Номер телефона может быть длиной до 17 цифр, и в конце может быть указан добавочный номер в формате ";ext=добавочный_номер".

- **Служба объявлений** Выберите **"Извеение",** чтобы приложение  "Извещая" обрабатывала входящий вызов или чтобы входящие вызовы обрабатывались с автосекретарь exchange UM.
