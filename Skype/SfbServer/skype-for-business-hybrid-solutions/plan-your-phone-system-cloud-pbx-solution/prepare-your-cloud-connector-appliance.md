...
Изменив файл с помощью значений для своей среды, сохраните его как CloudConnector.ini в **каталоге устройств**. Можно запустить **Get-CcApplianceDirectory** для определения пути в **Каталог устройства**.
  
При внесении изменений в INI-файл учтите следующие аспекты.
  
> [!NOTE]
> В этом разделе рассматриваются не все значения для файла INI, только ее с особое внимание можно найти здесь. Полный список, обратитесь к разделу [параметров развертывания Determine](plan-skype-for-business-cloud-connector-edition.md#BKMK_SiteParams) [Планирование Скайп облаке соединитель для выпуска для бизнеса](plan-skype-for-business-cloud-connector-edition.md) тема. > Дополнительные сведения о что значения необходимо изменить для дополнительных устройств или новых сайтов можно [ Один сайт с высокой доступности (высокой ДОСТУПНОСТИ) по сравнению с развертывания нескольких сайтах](deploy-multiple-sites-in-cloud-connector.md#BKMK_SingleSitecomparedtomulti-site) в разделе[Развертывание нескольких сайтах в облаке соединителя](deploy-multiple-sites-in-cloud-connector.md). 
...
- **Конкретных настроек виртуальной Машины base:** Параметры в этом разделе, должны быть настроены для командлета **Convert-CcIsoToVhdx** .
