...
9. На странице **буква авторизации** установите флажок каждого из следующих полей. В поле **подпись** введите человека, права на изменение для учетной записи. Это же имя, которое используется на странице **Сведения об учетной записи** > **Authorizing человека**. Затем нажмите кнопку **Далее**.
...
Запрос порядке порт будет установлен и ежедневно, и вы будете получать уведомление о ходе выполнения и состояние по электронной почте. Если запрос отклонен, вам будет предложено откройте билетов поддержки и, в которой поддержки билет, который мы попросите предоставить **Идентификатор заказа на порт**. Порядок порт, код можно найти в Скайп для бизнеса центра администрирования в разделе **голосовой связи** > **Заказы порт** > столбец**Идентификатора порядке** .
