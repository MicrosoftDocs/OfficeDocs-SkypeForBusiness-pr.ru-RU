...
1.  **Log временная**   шкала — временная шкала, к которой должен быть создан отчет.

2.  **Укажите, какие каталоги**   файлов предоставляют сведения о расположении файлов.
...
Пользователь может указать следующие каталоги файлов, как показано ниже.
  ...
  - **Расположение файлов журнала сервера** Расположение папки, в которой хранятся журналы сервера политики пропускной способности. Как правило, это \<филесерверный\>\\\<выбор Fe\>\\аппсерверфилес\\PDP.
...
2.  При открытии папки с файлами выберите файл консолидировать. CSV из расположения, указанного в поле сообщение, как показано ниже. В нем также указано расположение в формате **C\\: Temp**.
...
Все параметры являются взаимоисключающими. Синтаксис командной строки выглядит следующим образом:
  ...
  - **параметр – \<r\> n** — содержит \<n\> последних приприпаркованных вызовов. Отображаются следующие сведения:
...
C:\\Program Files\\Microsoft Lync Server\\2013\\ResKit\> сторажесервице импортсторажесервицедата. exe
...
При выполнении LookupUserConsole. exe откроется Командная строка, которая принимает адреса SIP и пытается отобразить внутреннюю информацию о маршрутизации Lync Server. Введите **Exit** , чтобы завершить работу средства LookupUserConsole.
...
Ниже приведены некоторые ключевые сценарии, в которых можно использовать это средство:
  ...
  - Майк — это новый сотрудник в компании Contoso, который присоединяется к новой команде, для которой все участники настроены для работы с командой, при включении Microsoft Lync администратор может настроить параметры группы звонков группы для включения всех своих новых участников группы, а также администратор добавляет пользователя Mike в качестве участника группы звонков группы для каждого участника в своей команде.
...
SYSPrep. ps1 это сценарий Windows PowerShell, который установит следующие необходимые компоненты Lync Server 2013 на компьютер с операционной системой Windows Server 2008.
...
Перед выполнением сценария SYSPrep. ps1 необходимо скопировать файлы необходимых компонентов в локальную папку на компьютере с операционной системой Windows Server 2008 (например, **D:\\Setup)**. Эта папка также должна содержать копию файлов Lync Server 2013, в частности, **Setup. exe.** Файлы необходимых компонентов можно скачать из следующих расположений:
...
<thead>
<tr class="header">
<th>Предварительные требования</th>
<th>Расположение</th>
</tr>
</thead>
<tbody>
