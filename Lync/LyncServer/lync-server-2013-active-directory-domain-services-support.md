...
Lync Server не поддерживает домены с одной меткой. Например, лес с корневым доменом **contoso.local** будет поддерживаться, а с корневым доменом **local** — нет. Дополнительные сведения см. в статье 300684 базы знаний Майкрософт "сведения о настройке Windows для доменов с DNS-именем, сопоставленными с одной меткой" по адресу " [https://go.microsoft.com/fwlink/p/?linkId=143752](https://go.microsoft.com/fwlink/p/?linkid=143752) .
