...
Конференц-связь с телефонным подключением позволяет пользователям использовать обычный телефон (то есть устройство в телефонной сети общего пользования), чтобы присоединиться к звуковой части Конференции.

<div>

## <a name="dial-in-conferencing-cmdlets"></a>Dial-In Conferencing Cmdlets

Если вы не хотите разрешить Конференц-связь с телефонным подключением, эту возможность можно отключить с помощью командлета Set-CsConferencingPolicy и задав для свойства ЕнабледиалинконференЦинг значение false. По умолчанию всем пользователям разрешается размещать собрания, которые включают конференц-связь с телефонным подключением.
