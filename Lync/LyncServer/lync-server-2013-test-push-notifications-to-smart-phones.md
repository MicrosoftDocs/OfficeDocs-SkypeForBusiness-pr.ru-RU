...
Если Test-CsMcxPushNotification успешно, командлет вернет результат теста Success:
...
Если Test-CsMcxPushNotification не удается подключиться к расчетной палате push-уведомлений, командлет не будет возвращать тестовый результат сбоя. Вместо этого команда обычно завершается с ошибками полностью. Например:
...
Кроме того, может произойти сбой Test-CsMcxConfiguration с указанным сообщением об ошибке:
