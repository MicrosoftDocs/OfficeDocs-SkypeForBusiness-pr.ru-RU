...
Test-CsP2PAV не проводит функцию "аудио"/"V". Обмен мультимедийными данными между тестовыми пользователями не осуществляется. Вместо этого командлет проверяет, могут ли быть выполнены соответствующие подключения, и могут ли два пользователя совершать такой вызов.
...
Если не удается выполнить команду Test-CsP2PAV, вам может потребоваться повторить проверку, в том числе параметр verbose:

Test-CsP2PAV-TargetFqdn "atl-cs-001.litwareinc.com"-verbose

Когда включается параметр Verbose, командлет Test-CsP2PAV возвращает пошаговое описание каждого выполняемого действия, как проверяло возможность входа в систему на Lync Server для указанного пользователя. Например, предположим, что тест завершился со следующей диагностикой:

ErrorCode = 6003, Source = ATL-CS-001. litwareinc. com, Reason = не поддерживается в диалоговом окне запроса

Если вы перезапустите Test-CsP2PAV и включили параметр Verbose, вы получите результат, подобный следующему:
