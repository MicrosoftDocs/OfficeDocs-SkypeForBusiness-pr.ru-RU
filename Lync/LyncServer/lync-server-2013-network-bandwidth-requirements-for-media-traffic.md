...
---
# <a name="network-bandwidth-requirements-for-media-traffic-in-lync-server-2013"></a>Требования к пропускной способности сети для трафика мультимедиа в Lync Server 2013
...
_**Последнее изменение темы:** 2015-09-24_
...
### <a name="audio-codec-bandwidth"></a>Полоса пропускания аудиокодека
...
<td><p>"Peer-to-peer" (Одноранговый сеанс);</p></td>
<td><p>29,0</p></td>
<td><p>45,0</p></td>
<td><p>57,0</p></td>
<td><p>86,0</p></td>
...
<tr class="odd">
<td><p>G. 711</p></td>
<td><p>PSTN, Конференц-связь</p></td>
<td><p>64,0</p></td>
<td><p>80,0</p></td>
<td><p>92,0</p></td>
<td><p>156,0</p></td>
...
Стерео версия кодека G. 722 используется системами на основе системы комнат Lync, что позволяет прослушивателям лучше различать в комнате для собраний несколько выпусков.

Для видео кодеком по умолчанию является стандарт H.264/MPEG-4 Part 10 Advanced Video Coding, а также его масштабируемые расширения для временного масштабирования. Чтобы обеспечить взаимодействие с клиентами Lync 2010 или Office Communicator 2007 R2, кодек RTVideo по-прежнему используется для одноранговых вызовов между Lync 2013 и устаревшими клиентами. В сеансах конференц-связи с клиентами Lync 2013 и устаревшими клиентами конечная точка Lync 2013 может кодировать видео с помощью видеокодеков и отправлять битстреам. 264 в Lync 2013 и RTVideo битстреам в Lync 2010 или Office Communicator 2007 R2.
...
Требуемая полоса пропускания зависит от разрешения, качества и частоты кадров. Для каждого разрешения существует две важных скорости передачи данных:
  ...
  - **Максимальная скорость**     передачи полезных данных Это скорость, с которой конечная точка Lync 2013 будет использовать для разрешения с максимальной частотой кадров, поддерживаемой для этого решения. Это значение важно, так как оно обеспечивает наилучшее качество и максимальную частоту кадров для видео.

  - **Минимальная скорость**     передачи полезных данных Это скорость, с которой конечная точка Lync 2013 будет переключаться на следующее более низкое разрешение. Чтобы гарантировать определенное разрешение, доступная скорость передачи полезной нагрузки видео не должна быть меньше этой минимальной скорости для данного разрешения. Это значение важно, потому что вы можете понять минимальное допустимое значение, когда максимальная скорость передачи недоступна или не требуется. Для некоторых пользователей видео с такой низкой скоростью передачи может считать неприемлемым, поэтому будьте осторожны при рассмотрении таких скоростей. Обратите внимание, что для видеосцен без движения или с небольшими перемещениями пользователя фактическая скорость передачи данных может временно становиться меньше минимальной скорости.

Lync 2013 поддерживает множество дополнительных разрешений. Это позволяет вам задавать разную полосу пропускания сети и возможности приема клиентов. В дополнение к пропорциям по умолчанию для Lync 2013 изменено на 16:9. Отношение 4:3 все еще поддерживается для веб-камер, которые не записывают видео с отношением сторон 16:9.
...
Следующие две полосы пропускания представляют интерес для планирования мощности:
  ...
  - **Максимальная пропускная способность с FEC**     Максимальная пропускная способность, потребляемая потоком, включая типичные действия потока и типичный кодек, используемый в сценарии с FEC. Это полоса пропускания при 100% активности потока с потерей пакетов, приводящей к использованию FEC для улучшения качества. Это значение применяется для вычисления полосы пропускания, которую нужно выделить, чтобы использовать кодек в данном сценарии и применить FEC для сохранения качества при потере пакетов. 

В следующих таблицах также содержится дополнительное значение пропускной способности, **Типичная пропускная способность**. Это среднее значение пропускной способности, потребляемой потоком, включая типичную активность потока и кодек, используемые в сценарии. Это значение можно использовать для определения примерного уровня пропускной способности, потребляемой в данный момент трафиком данных, однако его нельзя использовать при планировании емкости, так как отдельные вызовы будут превосходить это значение при повышении активности относительно среднего уровня. Типичная пропускная способность видеопотока в таблицах ниже основана на смешанном разрешении видео, как показано в измеренных данных клиента. Например, в одноранговых сеансах большинство пользователей используют окно обработки видео по умолчанию, в то время как некоторый процент пользователей увеличит или максимизирует приложение Lync, чтобы обеспечить более высокое разрешение для видео.
...
<tr class="even">
<td><p>Основное видео при вызове конечных точек Lync 2010 или Office Communicator 2007 R2</p></td>
<td><p>RTVideo</p></td>
<td><p>460</p></td>
...
<tr class="even">
<td><p>Панорамное видео при вызове конечных точек Lync 2010 или Office Communicator 2007 R2</p></td>
<td><p>RTVideo</p></td>
<td><p>190</p></td>
...
<tr class="odd">
<td><p>"Audio" (Аудио);</p></td>
<td><p>G. 711 (сюда входят участники PSTN в конференциях)</p></td>
<td><p>64,8</p></td>
<td><p>97</p></td>
<td><p>161</p></td>
</tr>
