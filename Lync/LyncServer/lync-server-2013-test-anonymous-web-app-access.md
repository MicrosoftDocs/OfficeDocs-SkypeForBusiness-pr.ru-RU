...
Командлет Test-Ксвебаппанонимаус проверяет, что анонимный пользователь может присоединиться к конференциям Lync Server с помощью веб-приложения Lync. При выполнении командлета Test-Ксвебаппанонимаус обращается к службе веб-билета, чтобы получить веб-билет для анонимного пользователя. Если командлету удалось получить этот билет, Test-Ксвебаппанонимаус будет обращаться к Lync Server и пытаться установить отдельные конференции для обмена мгновенными сообщениями, общего доступа к приложениям и совместной работы с данными.

Обратите внимание, что Test-Ксвебаппанонимаус проверяет только интерфейсы API и подключения, используемые для создания этих конференций. В действительности командлет не создает и не проводит никакие Конференции.
...
Для получения дополнительных сведений обратитесь к разделу "Справка" для командлета Test-Ксвебаппанонимаус. Обратите внимание, что командлет Test-Ксвебаппанонимаус является устаревшим для использования в Lync Server 2013.
...
Если Test-Ксвебаппанонимаус может присоединить анонимного пользователя к его конференциям, командлет вернет результат теста Success:
...
Если анонимный пользователь не может присоединиться к необходимым конференциям, результат теста будет помечен как сбой. Как правило, Test-Ксвебаппанонимаус также будет сообщать о подробном сообщении об ошибке и диагностике:
...
Ошибки Test-Ксвебаппанонимаус обычно связаны с ошибками проверки подлинности пользователей: необходимо выполнить тест с использованием действительной учетной записи пользователя, несмотря на то, что командлет проверяет возможность подключения анонимного пользователя к Lync Server. Если Test-Ксвебаппанонимаус завершается с ошибкой, необходимо убедиться, что указанный пользователь имеет действительную учетную запись пользователя Lync Server. Вы можете получить сведения об учетной записи Lync Server с помощью следующей команды:
...
Проблемы с конфигурацией сервера Office Web Apps также могут привести к сбою Test-Ксвебаппанонимаус; Это часто случается, если вы получаете следующую диагностику:
