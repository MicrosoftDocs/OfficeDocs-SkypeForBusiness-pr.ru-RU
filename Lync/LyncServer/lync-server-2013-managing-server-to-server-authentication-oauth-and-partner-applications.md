...
Lync Server 2013 поддерживает три сценария проверки подлинности между серверами. С помощью Lync Server 2013 вы можете:
  ...
  - Настройка межсерверной проверки подлинности между двумя компонентами Microsoft 365 (например, между Microsoft Exchange и Microsoft Lync Server или между Microsoft Lync Server и Microsoft SharePoint).

  - Настройка межсерверной проверки подлинности в распределенной среде (то есть межсерверной проверкой подлинности между локальным сервером и компонентом Microsoft 365).
...
Для Microsoft 365 версии Lync Server для обмена данными с аналогом Exchange Server 2013 сначала необходимо получить маркер безопасности от сервера авторизации. Затем Lync Server использует этот маркер безопасности для идентификации в Exchange. Для связи с Lync Server 2013 необходимо выполнить один и тот же процесс в Microsoft 365 версии Exchange.
