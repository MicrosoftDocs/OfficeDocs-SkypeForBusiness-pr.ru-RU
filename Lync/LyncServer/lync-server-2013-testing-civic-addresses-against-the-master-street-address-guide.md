...
Командлет Test-КслисЦивикаддресс используется для проверки расположений, добавленных в базу данных службы сведений о местоположении (LIS). Командлет выполняет сравнение расположений с местоположениями в главном руководстве по адресам (MSAG), которые относятся к поставщику маршрутизации сети E9-1-1. Если у вас нет поставщика сетевой маршрутизации или если поставщик недоступен, то тесты завершатся с ошибками.
...
Командлет Test-КслисЦивикаддресс можно использовать для проверки отдельных адресов или для проверки нескольких адресов. Например, эта команда проверяет один адрес, расположенный в Редмонде, WA:
...
Test-КслисЦивикаддресс будет сообщать об успешном выполнении или сбое для указанных адресов. Если не удается найти адрес или не удается связаться с поставщиком услуг, тест адреса завершится ошибкой.
...
Ниже приведены некоторые распространенные причины, по которым может произойти ошибка Test-КслисЦивикаддресс:
