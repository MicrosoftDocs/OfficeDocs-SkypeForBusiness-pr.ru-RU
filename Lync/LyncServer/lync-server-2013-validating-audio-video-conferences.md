...
Обратите внимание, что командлет Test-Ксавконференце не выполняет фактическую конференцию аудио-и видеосвязи между двумя тестовыми пользователями. Вместо этого командлет проверяет, могут ли два пользователя выполнить все подключения, необходимые для проведения такой конференции.
...
Если Test-Ксавконференце завершается с ошибкой, вам может потребоваться повторно выполнить проверку, включая параметр verbose:

    Test-CsAVConference -TargetFqdn "atl-cs-001.litwareinc.com" -Verbose

Если параметр verbose включен, Test-Ксавконференце будет возвращать пошаговые учетные записи каждого выполняемого действия при проверке возможности участия указанных пользователей в конференции AV. Например, предположим, что тест завершается сбоем и вы получаете следующую диагностику:
...
Ниже приведены некоторые распространенные причины, по которым может произойти ошибка Test-Ксавконференце:
