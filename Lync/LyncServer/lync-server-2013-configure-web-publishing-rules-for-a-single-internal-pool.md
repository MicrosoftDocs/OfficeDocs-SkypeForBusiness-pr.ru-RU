...
14. В диалоговом окне **перезаписи URL-адреса** с использованием webext.contoso.com в качестве примера полное имя правила, которое отображается, — это **\_arr webext.contoso.com\_лоадбаланце\_SSL**.
...
15. Повторите процедуру, определенную на шаге 14, для каждого определенного правила повторной записи SSL, для каждого из которых указан URL-адрес фермы серверов.
    ...
    > [!WARNING]  
    > По умолчанию правила HTTP также создаются и обозначаются аналогичным именованию правил SSL. Для нашего текущего примера правило HTTP будет называться <STRONG>арр_вебекст. contoso. com _лоадбаланце</STRONG>. Эти правила не требуют никаких изменений, и их можно игнорировать.
