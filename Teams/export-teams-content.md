...
- Массовый экспорт сообщения **Teams:** Teams API экспорта поддерживают до 200 RPS для каждого приложения на одного клиента и 600 RPS для приложения, при таких ограничениях вы сможете массово экспортировать Teams сообщений.
...
- Разрешения приложений используются приложениями, которые работают без участия пользователя, выписаного из учетной записи. разрешения на использование приложений может получить только администратор. Необходимы следующие разрешения:
    ...
    - *Chat.Read.All*: позволяет получать доступ ко всем чатам 1:1, групповым чатам и чатам собраний. 
