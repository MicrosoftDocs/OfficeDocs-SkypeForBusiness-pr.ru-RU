...
- **Пример 1.** Если вы включили Microsoft Teams в своей организации и хотите экспортировать все Microsoft Teams сообщения на дату программным путем передачи диапазона дат для данного пользователя или группы.
...
- Microsoft Teams API в Microsoft Graph доступ к конфиденциальным данным считается защищенным API. Чтобы можно было использовать API-функции экспорта, вам потребуется дополнительная проверка, помимо разрешений и согласия. Чтобы запросить доступ к этим защищенным API, заполните [форму запроса](https://aka.ms/teamsgraph/requestaccess).
