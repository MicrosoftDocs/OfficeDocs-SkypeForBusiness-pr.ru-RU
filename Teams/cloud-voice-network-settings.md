...
Узнайте о сетевых регионах, сетевых сайтах, сетевых подсетях и доверенных IP-адресах. Эти термины и понятия используются во всей облачной голосовой документации [](location-based-routing-plan.md) для маршрутизов на основе местоположения для прямой маршрутации и динамических [экстренных вызовов](configure-dynamic-emergency-calling.md). При развертывании этих облачных функций в организации необходимо настроить параметры сети для использования с этими функциями в Microsoft Teams.
...
Эти же сетевые регионы совместно Location-Based маршруты для прямой маршрутии и улучшенные экстренные службы. Если вы уже создали сетевые регионы для одной функции, создавать для нее новые сетевые регионы не нужно.
...
Сведения из подсети используются для определения сетевого сайта, на котором находится конечная точка, при иницике нового сеанса. Когда известно о расположении каждой из сторон в сеансе, облачная голосовая функция может применять эти сведения, чтобы определить, как управлять настройкой и маршрутизовой обработкой звонка.

Для каждого сайта сети определите, какие IP-подсети назначены каждому сетевому сайту. В нашем примере сайту "Нью-Йорк" в регионе "Северная Америка" назначены следующие IP-подсети: 172.29.80.0/23, 157.57.216.0/25, 172.29.91.0/23, 172.29.81.0/24. Если Г-н Мяк, который обычно работает в Москве, переходит в офис в Нью-Йорке для обучения, включает свой компьютер и подключается к сети, его компьютер получает IP-адрес в одном из четырех диапазонов, выделенных для Москвы, например 172.29.80.103.
