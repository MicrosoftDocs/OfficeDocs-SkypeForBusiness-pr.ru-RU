...
>
> Хотя определение **оператора** является необязательным, рекомендуется.  Автосекретари перенаправляют вызовы к оператору, если вызывающий не делает выбор в меню, неоднократно выбирает недопустимые параметры или набирает по имени или номеру несколько раз сбой.  Если оператор не определен, автосекретарь откатит вызов.
