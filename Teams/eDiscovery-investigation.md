...
|:---------------|:--------|
|Аудиозаписи||
|Содержимое карточки|[Дополнительные сведения см. в разделе "](#search-for-card-content)Поиск содержимого карточки".|
|Ссылки чата||
|Сообщения чата|К ним относится содержимое стандартных каналов Teams, чаты 1:1, групповые чаты 1:N и чаты с участниками гостевого пользователя.|
|Фрагменты кода||
|Измененные сообщения|Если пользователь находится на удержании, предыдущие версии измененных сообщений также сохраняются.|
|Эмодзи, GIF и наклейки||
|Встроенные изображения||
|Компоненты цикла|Содержимое компонента цикла сохраняется в .fluid-файле, который хранится в OneDrive для бизнеса учетной записи пользователя, отправившего компонент цикла. Это означает, что при поиске содержимого в компонентах цикла необходимо включить OneDrive в качестве источника данных.|
|Мгновенные беседы собраний||
|Метаданные собрания<sup>1</sup>||
|Имя канала||
|Котировки|Содержимое в кавычках поддерживает поиск. Однако результаты поиска не указывают на то, что содержимое было заключено в кавычки.|
|Реакции (например, "нравится", "уют" и другие реакции)|Реакции поддерживаются для всех коммерческих клиентов после 1 июня 2022 г. Реакции до этой даты недоступны для обнаружения электронных данных. Планируется поддержка облака для государственных организаций. Для реакций не поддерживается удержание по юридическим причинам.|
|Тема||
|Таблицы||
|Видеоклип Teams (TVC)|Найдите TVC с ключевым словом Video-Clip и сохраните как .mp4-файл для каждого вложения TVC, щелкнув правой кнопкой мыши предварительный просмотр (поиск по ключевому слову будет доступен в октябре 2022 г.). Данные TVC можно обнаружить в наборах проверки обнаружения [электронных данных](/microsoft-365/compliance/add-data-to-review-set).
