...
1. Подключение Azure AD PowerShell. Инструкции см. в разделе "Подключение с Azure Active Directory PowerShell" Подключение, [чтобы Microsoft 365 с Помощью PowerShell](/microsoft-365/enterprise/connect-to-microsoft-365-powershell#connect-with-the-azure-active-directory-powershell-for-graph-module). Обязательно выполните шаги 1 и 2 в предыдущей статье.
...
> [!NOTE]
> Чтобы отобразить изображения из содержимого карточки в результатах поиска (например, флажки на предыдущем снимке экрана), необходимо выполнить вход в Teams (<https://teams.microsoft.com>в) на другой вкладке в том же сеансе браузера, который используется для просмотра результатов поиска. В противном случае отображаются заполнители изображений.
