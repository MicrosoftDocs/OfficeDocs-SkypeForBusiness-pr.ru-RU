...
На вкладку **файлы** в группах аналогичны представления документов SharePoint. На вкладке **файлы** пользователи могут:

- Просмотрите дополнительные параметры в меню Файл **New** .
- Синхронизация файлов на локальном диске.
- В меню **Все документы** переключитесь из представления **списка** в **компактном список** **заголовков** представление.
- Определение файлов, которые требуют внимания или иметь вредоносных программ.
- Сразу просмотреть ли файл только для чтения или установленное состояние ожидания.
- Извлечение и возврат в файлах.
- Прикрепление и открепление измените порядок сортировки файлов.
- Определите, какие файлы должны метаданных
- Выберите один из многих дополнительных параметров фильтра.
- Группируйте файлы на основании заголовков столбцов.
- Измените параметры столбца (влево или вправо, скрыть) и ширина столбца.

## <a name="default-link-type-setting"></a>Настройка типа ссылок по умолчанию

SharePoint и OneDrive у параметра для указания типа связи по умолчанию для ссылок, которые создаются для файла. Команды использует такой же подход, повторное использование параметров, которые администратор устанавливает для SharePoint и OneDrive. Дополнительные сведения о такой подход, описаны в [Изменить тип связи по умолчанию, когда пользователи получают ссылки для общего доступа к](https://docs.microsoft.com/sharepoint/change-default-sharing-link). 
