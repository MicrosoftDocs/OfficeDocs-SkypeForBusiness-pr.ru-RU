---
title: Инструменты для обновления до Teams из локального Skype для бизнеса развертывания
author: dstrome
ms.author: dstrome
manager: serdars
ms.topic: article
...
В этой статье описаны средства обновления до Teams с Skype для бизнеса. 
...
> [!NOTE]
> Если вы используете Skype для бизнеса Online Connector для управления службами, вам потребуется перейти в модуль Teams PowerShell и обновить существующие сценарии PowerShell. Дополнительные сведения см. в Skype для бизнеса [Online Connector в модуле Teams PowerShell.](teams-powershell-move-from-sfbo.md)
...
Пользователям, которые работают в локальной Skype для бизнеса TeamsUpgradePolicy, можно назначить любой режим TeamsUpgradePolicy, кроме Режима TeamsOnly. И наоборот, пользователям, которые работают в облаке, может быть назначен только режим TeamsOnly. **Режим TeamsOnly** можно на назначении только пользователю, который уже находится в облаке, так как функциональность Skype для бизнеса и федерации с пользователями Skype для бизнеса и Microsoft 365 телефонная система возможна только в том случае, если он находится в Skype для бизнеса Online.  Кроме того, вы не можете назначить **режим TeamsOnly** стандартным для всего клиента, если у вас есть локальное развертывание Skype для бизнеса (которое обнаруживается наличием записи DNS lyncdiscover, которая указывает на другое расположение, кроме Office 365. Подробные сведения см. в теме Отключение [гибридной конфигурации для завершения миграции только Teams миграции.](/SkypeForBusiness/hybrid/cloud-consolidation-disabling-hybrid)

Пользователей с Skype для бизнеса локальной учетной записью необходимо перетасковыть в режим Teams только Move-CsUser в локальном Skype для бизнеса. [](/SkypeForBusiness/hybrid/move-users-from-on-premises-to-teams) 
...
Если ваши пользователи находятся в локальной Skype для бизнеса Server, вам потребуется использовать локальное решение для Skype для бизнеса Server 2019 или CU8 для Skype для бизнеса Server 2015. Для пользователей, Skype для бизнеса Server локальной сети, свойство mode веб-экземпляра TeamsUpgradePolicy будет соблюдаться, но свойство NotifySfbUsers — нет. При желании необходимо создать локальное экземпляр TeamsUpgradePolicy, чтобы контролировать поведение клиента. 
