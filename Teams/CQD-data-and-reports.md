...
Аналогично на вкладке Client-Client пять разделов:
...
### <a name="multi-select-filters"></a>Фильтрация с несколькими выборами
...
В некоторые отчеты CQD добавлены фильтры на уровне панели мониторинга, что делает их более простыми для фильтрации по общим параметрам. Эти фильтры отображаются за пределами обычных вкладок отчетов и непосредственно под фильтром Product и применяются во всех фильтрах панели мониторинга.
...
CQD поддерживает добавление фильтров к URL-адресу. Это позволяет легко делиться CQD-запросом или делать его закладкой. В URL-адресе можно определить параметры, такие как "Новый месяц", "ИД клиента" или "Язык". Вы также можете добавить фильтры на уровне продукта или панели мониторинга к URL-адресу.
