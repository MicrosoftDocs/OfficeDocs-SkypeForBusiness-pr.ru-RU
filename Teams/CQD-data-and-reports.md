...
Параметр "Дата URL-адреса" принимает поле "День". В отчетах о развертывание дней в качестве последнего дня тренда используются даты, указанные в формате ММ-ММ-ДД. Параметр Дата URL-адреса "00" означает "сегодня".
...
Если вы хотите использовать Power BI для анализа данных CQD, прочитайте статью Использование Power BI для анализа [данных CQD для](CQD-Power-BI-query-templates.md)Teams.
...
Общий объем потоков помогает определить, насколько релевантны вычисленные проценты низкого качества. Чем меньше общий объем потоков, тем менее надежными являются процентные значения низкого качества.
...
При проверке на Server-Client только клиентская конечная точка. Так как серверы всегда находятся за пределами точки зрения пользователя, это не учитывается в тесте.
...
|Отчеты о качестве     | Даты и времени: по регионам, расположениям, подсетям, часам и пользователям.        |
|Отчеты о сбоях     | Даты и времени: по регионам, расположениям, подсетям, часам и пользователям.        |
...
Навести указатель на ли графики и линии тренда в отчете, чтобы отобразить подробные значения. В отчете, в фокусе будет меню действий: **Изменить**, **Клонировать**, **Удалить**, **Скачать** и **Экспорт дерева отчетов**.
...
| Только просмотр внутри  | Используется для фильтрации отчета по управляемым (внутри) или неуправляемой (за пределами). Управляемый шаблон CQD уже предварительно сконфигурировали эти фильтры.       | Second Inside Corp = Inside        |
...
Помимо функции уточнения, CQD также поддерживает указание фильтров с несколькими значениями (ИЛИ фильтров).
...
CQD поддерживает добавление фильтров к URL-адресу. Это позволяет легко делиться CQD-запросом или делать его закладкой. В URL-адресе можно определить параметры, такие как "Новый месяц", "ИД клиента" или "Язык". К URL-адресу также можно добавить фильтры уровня Product (Продукт) или Dashboard (Панель мониторинга).
...
Например, чтобы применить значение фильтра продукта Microsoft Teams, нужно добавить следующее:
...
Чтобы применить ФИЛЬТРы URL-адресов с несколькими значениями, разделять каждое значение по черты (| ) символа. Например:
...
| Только просмотр внутри  | Используется для фильтрации отчета по управляемой (внутри) или неуправляемой (за пределами). Управляемый шаблон CQD уже предварительно сконфигурировали эти фильтры.       | Second Inside Corp = Inside        |
...
При проверке данных могут появиться различия между данными между Teams и Skype для бизнеса. Некоторые причины:
- Различия в механизмах обеспечения производительности и надежности.
  - Teams автоматическое переподключение и быстрое роуминг. Skype для бизнеса не так.
  - Teams имеет динамическое управление пропускной способностью. Skype для бизнеса не так.
- Различия в [диапазонах IP-адресов](Office-365-URLs-IP-address-ranges.md) между Teams и Skype для бизнеса. Диапазоны Teams более новые, что может привести к проблемам с подключением в брандмауэре.
