---
title: Ограничения на бесплатный набор номеров в Италия
ms.author: serdars
author: SerdarSoysal
manager: serdars
ms.reviewer: 'davlick, krkilaru'
ms.topic: reference
ms.tgt.pltfrm: cloud
ms.service: msteams
ms.collection:
  - M365-collaboration
  - M365-voice
search.appverid: MET150
audience: Admin
appliesto:
  - Skype for Business
  - Microsoft Teams
ms.localizationpriority: medium
ROBOTS: 'NOINDEX, NOFOLLOW'
f1.keywords:
  - NOCSH
ms.custom:
  - Calling Plans
description: 'Это поможет вам узнать, как набирать бесплатные номера в каждой стране или регионе. После выбора страны или региона вы будете перенараны на страницу, которая содержит подробные сведения, ограничения и ограничения доступности бесплатной службы, на которой доступна бесплатная служба. Форматы набора показывают коды доступа, необходимые для набора бесплатных номеров в каждой стране или регионе.'
---

# <a name="toll-free-dialing-restrictions-in-italy"></a>Ограничения на бесплатный набор номеров в Италия

[Найдите другую страну или регион](../toll-free-dialing-limitations-and-restrictions.md) или узнайте больше об ограничениях и ограничениях для бесплатного набора номера.


|**Сведения**|**Описание**|
|:-----|:-----|
|Доступны ли бесплатные вызовы?  <br/> |Да  <br/> |
|Формат набора  <br/> | 800 <br>803 <br>|
|Код страны или региона по ISO  <br/> |IT  <br/> |
|Охватываемая территория  <br/> |Неприменимо  <br/> |
|Неохватываемая территория  <br/> | Неприменимо  <br/>  |
|Доступность проводных и наземных линий связи  <br/> |Да  <br/> |
|Доступность мобильных сетей  <br/> | Да (некоторые числа могут быть премиум-) <br/> |
|Примечания, ограничения и проблемы с доступом  <br/> |Нет  <br/> |

## <a name="related-topics"></a>Статьи по теме

[Ограничения и запреты для бесплатных номеров](../toll-free-dialing-limitations-and-restrictions.md)

[Получение служебных номеров телефонов](../getting-service-phone-numbers.md)