...
В большинстве случаев сеть, соединяющая предприятие с облаком, является неуправляемой сетью, в которой вы не сможете надежно настраивать параметры качества обслуживания. Один из вариантов, доступных для обеспечения комплексного качества обслуживания, — это [Azure ExpressRoute](https://azure.microsoft.com/documentation/articles/expressroute-introduction/), но мы рекомендуем использовать QoS в локальной сети. Это повысит качество рабочей нагрузки для взаимодействия в реальном времени во время развертывания и чокепоинтс.

## <a name="verify-your-network-is-ready"></a>Проверка готовности сети

Если вы просматриваете реализацию QoS, вы уже должны определить требования к пропускной способности и другие [сетевые требования](prepare-network.md). Вычисление пропускной способности для Microsoft Teams является сложным и поможет вам создать калькулятор. Чтобы получить доступ к калькулятору, перейдите в [планировщик сети](https://aka.ms/bwcalc/) в мядвисор.
...
> [!NOTE]
> Удаленные пользователи, подключаемые к VPN, должны использовать раздельное туннелирование, чтобы максимально повысить качество взаимодействия с пользователем. Рекомендации по развертыванию и раздельному [каналу VPN](https://myadvisor.fasttrack.microsoft.com/CloudVoice/Downloads?SelectedIDs=5_1_0_9 ) доступны в мядвисор и содержат дополнительные сведения.
...
![Иллюстрация очередей QoS] (media/Qos-in-Teams-Image2.png "Общая доступная пропускная способность делится между несколькими очередями, звуком, видео и другим трафиком, которым назначены разные приоритеты.")
...
4. В поле **Фильтр отображения** используйте IP-адрес источника компьютера, который сделал звонок, а затем уточните фильтр, ОПРЕДЕЛИВ значение DSCP 46 (Hex 0xb8) в качестве условий поиска, как показано в следующем примере:
