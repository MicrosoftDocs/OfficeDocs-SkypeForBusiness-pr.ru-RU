...
- Хотя клиент Mac использует одинаковые диапазоны портов, он также использует жестко заданные значения для звука (EF) и видео (AF41). Эти значения нельзя настроить.
