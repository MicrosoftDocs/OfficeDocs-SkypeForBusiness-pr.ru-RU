...
ms.locfileid: "60536760"
---
# <a name="survivable-branch-appliance-sba-for-direct-routing"></a>Ветвное устройство (SBA) для прямой маршрутии


Иногда на сайте клиента с использованием прямой маршрутии для подключения к Телефон (Майкрософт) система может возникнуть с простоем в Интернете.
...
- Порт 5061 (или тот, который настроен на SBC) используется сервером Microsoft SBA Server для связи с SBC и должен быть разрешен через брандмауэр. 
- Порт UDP 123 используется сервером Microsoft SBA Server для связи с сервером NTP и должен быть разрешен через брандмауэр.
- Порт 443 используется сервером Microsoft SBA Server для связи с Microsoft 365 и должен быть разрешен через брандмауэр.
...
Вся настройка делается с Skype для бизнеса PowerShell Online. (Центр администрирования Teams пока не поддерживает функцию SBA прямой маршрутии.) 
...
### <a name="create-the-teams-branch-survivability-policy"></a>Создание политики Teams ветвей ветвей 
...
В приложении SBA помните следующее: 
...
Пошаговая инструкция по настройке геймпада границы сеанса с помощью внедренного ветвного устройства с возможностью "Ветвь", см. в документации, предоставленной поставщиком SBC: 
