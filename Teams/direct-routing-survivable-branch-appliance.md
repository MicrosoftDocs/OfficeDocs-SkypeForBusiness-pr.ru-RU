...
---
# <a name="survivable-branch-appliance-sba-for-direct-routing---public-preview"></a>Устройство с ветвью (SBA) для прямой маршрутки — общедоступный предварительный просмотр


> [!NOTE]
> Это общедоступный предварительный выпуск.

В редких случаях сайт клиента, использующий прямую маршрутику для подключения к телефонной системе Майкрософт, может привести к с отключению интернета.

Предположим, что сайт клиента ,называемый ветвью, временно не может подключиться к Облаку Майкрософт с помощью прямой маршрутинга. Однако интрасеть внутри ветви по-прежнему работает полностью, и пользователи могут подключаться к контроллеру границы сеанса (SBC), который обеспечивает подключение по ННР.

В этой статье описано, как с помощью устройства SBA (Скайп с возможностью сбоя) продолжать звонить и принимать звонки по телефонной сети общего пользования (МСК) с помощью телефонной сети Майкрософт (SBA).

## <a name="prerequisites"></a>Необходимые компоненты

Распространяемый код SBA предоставляется корпорацией Майкрософт поставщикам SBC, которые затем внедрят код в свои программы или распространят его отдельно, чтобы SBA запускался на отдельном VM-оборудовании. 
...
- Получение звонков по ДНР через местные службы SBA или SBC с мультимедиа, которые проходят через SBC. 

- Удержание и возобновление звонков по ННР.

## <a name="configuration"></a>Конфигурация

Чтобы функция SBA работала, клиент Teams должен знать, какие SBAs доступны на каждом сайте филиала и какие SBAs назначены пользователям на этом сайте. Конфигурация может выполняться следующим образом:
...
| Описание | Текст с бесплатным форматированием |
|||
...
Для создания политики используется New-CsTeamsSurvivableBranchAppliancePolicy. Этот cmdlet имеет следующие параметры: Обратите внимание, что политика может содержать одно или несколько SBAs.
...
Вы можете добавлять и удалять SBAs из политики с помощью Set-CsTeamsSurvivableBranchAppliancePolicy-управления. Например: 
...
- [Зарегистрируйте приложение на платформе удостоверений Майкрософт.](https://docs.microsoft.com/azure/active-directory/develop/quickstart-register-app)  
...
## <a name="session-border-controller-configuration"></a>Настройка граничного контроллера сеанса 
...
- Когда вы назначаете пользователю политику "Сетчивая устройство", может потребоваться некоторое время, прежде чем SBA будет показан в выходе Get-CsOnlineUser. 

- Обратный просмотр номеров для контактов Azure AD не выполняется. 

- SBA не поддерживает параметры переадваровки параметров. 
