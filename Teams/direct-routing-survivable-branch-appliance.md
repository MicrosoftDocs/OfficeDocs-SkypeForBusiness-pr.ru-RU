...
---
# <a name="survivable-branch-appliance-sba-for-direct-routing---public-preview"></a>Устройства ветвей (SBA) для прямой маршрутки — общедоступный предварительный просмотр
...
В этой статье описано, как использовать устройство с ветвным устройством (SBA) для обеспечения работы телефонной системы Майкрософт и ее приемов в случае сбоя.
...
Чтобы получить последнюю версию встроенного ПО для граничного геймпада сеанса с внедренным устройством Ветвной ветвной линии Скайп, обратитесь к поставщику SBC. Кроме того, требуется следующее:
...
- Неявное предоставление маркеров = маркеры доступа и токены ИД. 
...
Сообщайте о проблемах в службе поддержки поставщика SBC. Сообщая о проблеме, указать, что у вас установлена ветвная устройство с возможностью ветвь Скайп.

## <a name="known-issues"></a>Известные проблемы

- После добавления новых устройств с ветвями с использованием ветвных устройств может потребоваться некоторое время.
