...
Чтобы добавить новое настраиваемое приложение в каталог приложений клиента, нажмите кнопку **загрузить новое приложение** , чтобы отправить пакет приложения в формате ZIP. Приложение не выделено после отправки, поэтому для его поиска вам потребуется выполнить поиск по каталогу клиентов.

Вы также можете отправлять приложения [с помощью настольного клиента Teams](tenant-apps-catalog-teams.md#go-to-the-tenant-app-catalog). Дополнительные сведения можно найти [в разделе Управление бизнес-приложениями в Teams](manage-your-lob-apps.md).
