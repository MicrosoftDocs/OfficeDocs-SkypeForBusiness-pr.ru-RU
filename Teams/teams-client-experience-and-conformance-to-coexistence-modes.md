...
Так как присутствие указывает на доступность через чат и вызовы, то, когда чат и вызовы отключены, самообнаправление в Teams (т. е. отображение собственного присутствия в клиенте Teams на изображении пользователя) также скрыто. 
...
На снимках экрана ниже показано, чем отличаются **режимы Teams и** **"Только** острова". Обратите внимание, что значки чата и  звонков по умолчанию доступны в режиме "Только **Teams"** или "Острова" (снимок слева), но не в других режимах (правый снимок экрана):
...
При использовании PowerShell командлет проверяет конфигурацию соответствующих параметров `Grant-CsTeamsUpgradePolicy` в TeamsMessagingPolicy, TeamsCallingPolicy и TeamsMeetingPolicy, чтобы определить, будут ли эти параметры отключены teamsUpgradePolicy и, если да, в PowerShell выведется информационное сообщение.  Как было отмечено выше, больше не нужно настраивать эти другие параметры политики. Ниже приводится пример предупреждения PowerShell.
