...
Приложение "Задачи" обеспечивает согласованное управление задачами в Microsoft Teams, объединяя отдельные задачи на основе [Microsoft To-Do](https://todo.microsoft.com/tasks/) и командные задачи на основе Планировщика в одном месте. Пользователи могут получить доступ к приложению "Задачи" в левой части Teams, а к вкладке "Задачи" на канале — в отдельных командах. С помощью **моих задач** и **общих планов** пользователи могут просматривать и управлять всеми своими отдельными и командными задачами, а также определять приоритеты своей работы. "Задачи" доступны в классической версии Teams, в Интернете и на мобильных устройствах.
...
2. В списке приложений выполните одно из следующих действий.
    ...
    - Чтобы отключить задачи для организации, найдите приложение "Задачи", выберите его и выберите **Блокировать**.
    - Чтобы включить задачи для организации, найдите приложение "Задачи", выберите его и выберите **Разрешить**.
...
Чтобы разрешить или запретить определенным пользователям в вашей организации использовать задачи, убедитесь, что задачи для вашей организации включено на странице [Управление приложениями](manage-apps.md) , а затем создайте настраиваемую политику для разрешений приложений и назначьте ее этим пользователям. Дополнительные сведения см. в статье [Управление политиками и параметрами пользовательских приложений в Teams](teams-app-permission-policies.md).

### <a name="pin-tasks-to-teams"></a>Закрепление задач в Teams

#### <a name="use-the-tailored-frontline-app-experience-to-pin-tasks-and-other-apps-to-teams"></a>Использование специализированного интерфейса приложения для закрепления задач и других приложений в Teams

Специализированное интерфейсное приложение в Teams закрепляет наиболее релевантные приложения в Teams для пользователей с [лицензией F](https://www.microsoft.com/microsoft-365/enterprise/frontline#office-SKUChooser-0dbn8nt). Закрепленные приложения включают задачи, рации, смены и утверждения. По умолчанию эта функция включена, что позволяет сотрудникам, работающим с клиентами, работать в соответствии с их потребностями.
...
Чтобы закрепить приложение "Задачи" для пользователей, можно изменить глобальную политику (по умолчанию для всей организации) или создать и назначить настраиваемую политику в политике настройки приложения. Дополнительные сведения см. в статье [Управление политиками и параметрами пользовательских приложений в Teams](teams-app-setup-policies.md).
...
Если вы не хотите, чтобы пользователь видел раздел **Мои задачи**, вы можете скрыть его. Чтобы скрыть **мои задачи**, [удалите лицензию пользователя на Exchange Online](/microsoft-365/admin/manage/remove-licenses-from-users). Важно знать, что после удаления лицензии на Exchange Online пользователь больше не будет иметь доступ к своему почтовому ящику.  Данные почтового ящика хранятся в течение 30 дней, после чего данные будут удалены и не смогут быть восстановлены, если для почтового ящика не будет установлено [удержание на месте или удержание для судебного разбирательства](/exchange/security-and-compliance/in-place-and-litigation-holds).

Мы не рекомендуем удалять лицензию на Exchange Online для информационных работников, но могут существовать некоторые сценарии, в которых вы можете скрыть **мои задачи** таким образом, например для сотрудников переднего плана, которые не зависят от электронной почты.
...
Группа маркетинга предоставляет сведения о рекламной акции и соответствующем списке задач с помощью менеджера по связям с общественностью по вопросам розничной торговли. Менеджер по розничным коммуникациям, который выступает в качестве привратника для магазинов, проверяет информацию. Затем они создают список задач для повышения и создают задачу для каждой единицы работы, которую необходимо выполнить затронутым хранилищам. После завершения списка задач необходимо выбрать магазины, которые должны завершить работу. В этом случае акция распространяется только на магазины в США, в которых есть собственный ресторан. В разделе Задачи они фильтруют список магазинов по атрибуту ресторана в магазине, выбирают соответствующие США расположения в иерархии, а затем публикуют список задач в этих хранилищах.
...
Рядовые сотрудники в каждом магазине теперь имеют список приоритетных задач в "Задачах" на своем мобильном устройстве. После завершения задачи они помечают их как завершенные. Они даже могут отправить и прикрепить фотографию к задаче, чтобы показать свою работу.
