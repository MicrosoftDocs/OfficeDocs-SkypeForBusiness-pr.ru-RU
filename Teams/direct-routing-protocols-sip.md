...
- Приоритет 2. Заглавная связь с контактом. Если Record-Route не существует, прокси-сервер SIP подберет значение для заглавного адреса контакта, чтобы получить исходящие подключения. (Рекомендуемая конфигурация.)
...
Если звонок передается одному пользователю Teams другому через SBC, ожидается, что SBC выдадут новое приглашение (запустить новое диалоговое окно) для целевого средства передачи (пользователя Teams), используя сведения, полученные в сообщении refer. 
