...
1. Проверьте сертификат. По первоначальному подключению служба прямой маршрутизовывания принимает имя FQDN, которое представлено в заглавной части контакта, и совпадает с именем "Общее имя" или "Тема альтернативы" для представленного сертификата. Имя SBC должно соответствовать одному из следующих вариантов:
   ...
   - Вариант 2. Часть домена имени FQDN, представленная в заглавной части контакта (например, adatum.biz имени FQDN sbc1.adatum.biz), должна соответствовать поддеревным знакам в названии "Общее имя" или "Тема" (например, *.adatum.biz).
...
Для всех входящих звонков для совпадения номера телефона с пользователем используется URI запроса.   
...
- Приоритет 2. Заглавная связь с контактом. Если Record-Route не существует, прокси-сервер SIP будет искать значение в заглавной части контакта, чтобы сделать исходящие соединения. (Рекомендуемая конфигурация.)

Если используются как contact, так и Record-Route, администратор SBC должен поддерживать одинаковые значения, что приводит к накладным расходовам администратора. 
...
- Если IP-адрес представлен в record-route или Contact, проверка сертификата не проходит, а звонок не проходит.
...
Запрос на ссылку, который приходит от клиента, будет прекращен на прокси-сервере SIP. (На приведенной ниже схеме запрос на передачу от клиента отображается как "Передача вызовов Для Вас".  Дополнительные сведения см. в разделе 7.1 [RFC 3892.](https://www.ietf.org/rfc/rfc3892.txt) 
...
Если звонок перенаправится от одного Teams пользователя к другому через SBC, предполагается, что SBC будет отправлять новое приглашение (начать новое диалоговое окно) для целевого Teams, используя сведения, полученные в сообщении. 
...
Если центр обработки данных Direct Routing занят, служба может Retry-After сообщение с интервалом в одну секунду на SBC. Когда SBC получает сообщение 503 с Retry-After в ответ на приглашение, он должен прервать это подключение и попробовать следующий доступный центр обработки данных Майкрософт.
...
Перезапуск в прямой маршрутике выполняется в соответствии со следующими абзаями RFC:
