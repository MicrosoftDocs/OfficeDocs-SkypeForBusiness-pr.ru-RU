...
2. Попробуйте найти клиента, используя полное имя FQDN, которое представлено в заглавном тексте контакта.  
   ...
   Проверьте, зарегистрировано ли имя FQDN из sbc1.adatum.biz контакта в любой организации Microsoft 365 или Office 365. При обнаружении выполняется подсмотр пользователя в клиенте, в доменном имени которого зарегистрированО FQDN SBC. Если она не найдена, применяется шаг 3.   
...
У Teams может быть несколько конечных точек одновременно. Например, Teams для Windows, Teams клиента iPhone и Teams Телефон (Teams android). Каждая конечная точка может сигнализировать об отстаке HTTP следующим образом:
