...
Если у вас другая среда (например, Skype для бизнеса Cloud Connector Edition), гибридная среда не настроена или пользователи не могут использовать звонки по ННР, матрица поддержки будет другой.  
...
**Определения:**   
- **Корпоративная голосовая связь:** Возможность предоставления доступа к ОКП пользователям с локальной учетной записью Skype для бизнеса пользователей. На локальном сервере Skype для бизнеса-посредника обеспечивается взаимодействие с ДНР.  
...
|Локальное AD  |Локальная| Microsoft Teams| Корпоративная голосовая связь/гибридное голосовое подключение/прямая маршрутия   |Нет, Microsoft Teams не поддерживается для локального Skype для бизнеса |     
