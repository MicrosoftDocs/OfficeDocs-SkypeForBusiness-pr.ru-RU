...
- Имеют или ранее имели платную, бесплатную или пробную лицензию на Teams.
...
Предложение Microsoft Teams Exploratory действует без дополнительной платы в течение 12 месяцев (с начальной регистрации пользователя) плюс дополнительный 30-дневный льготный период. В это время пользователям, имеющим лицензию на Microsoft Exploratory, нужно будет перейти на платную лицензию, включающую Teams. Ко всем пользователям в одном клиенте применяется одинаковая дата окончания 12-месячного периода, который начинается с даты регистрации первого пользователя.

### <a name="what-happens-if-an-end-user-initiates-the-microsoft-teams-exploratory-experience-just-before-the-anniversary-or-renewal-date"></a>Что произойдет, если пользователь оформит лицензию Microsoft Teams Exploratory непосредственно перед истечением очередного года со дня заключения моего соглашения или перед его продлением
