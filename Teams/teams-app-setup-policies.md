...
- **Закрепление приложений:** С помощью политик настройки приложений можно выбирать приложения для закрепления, устанавливать порядок их показа пользователям на панели приложения Teams или в области сообщений, а также управлять возможностью закрепления приложений пользователями. Дополнительные сведения см. в теме [Закрепление приложений](#pin-apps).
- **Установка приложений:** С помощью политик настройки приложений вы можете устанавливать приложения от имени пользователей, когда они Teams и во время собраний. Дополнительные сведения см. в [установке приложений](#install-apps).
...
Приложения закреплены в:

- Строка приложения, которая находится в левой части Teams и в нижней части мобильных клиентов Teams (iOS и Android).

- Область расширений сообщений в нижней части области "Сообщение".
...
1. Войдите в Microsoft Teams [администрирования](https://admin.teams.microsoft.com).

1. В левой области перейдите к Teams **AppsSetup** >  **policies**.

1. Нажмите **Добавить**.

1. Введите имя и описание для политики.

1. **Включит закрепление пользователей**.

   > [!NOTE]
   > Параметр **Закрепление** пользователей доступен в Центре администрирования Teams в Microsoft 365 облако сообщества для государственных организаций (GCC) (GCC, GCC High и DoD), но в настоящее время он не действует.

1. В **области Закрепленные приложения** выберите **Добавить приложения**.

1. В области **Добавление закрепленных приложений** найди приложения, которые вы хотите добавить, и выберите **Добавить**. Вы также можете фильтровать приложения с помощью политики разрешений приложений.

1. Нажмите **Добавить**.

1. На панели **приложений или** в **расширениях** сообщений расположить приложения в том порядке, в которых они должны отображаться в Teams.

   ![Раздел Закрепленные приложения.](media/pin-messaging-extensions.png)

1. Нажмите **Сохранить**.

## <a name="install-apps"></a>Установка приложений

Вы можете выбрать приложения, установленные по умолчанию для пользователей в личной среде Teams, установить приложения в качестве расширений для обмена [сообщениями и](/microsoftteams/platform/messaging-extensions/what-are-messaging-extensions) назначить приложения для установки на собрания.
...
> [!NOTE]
> Пользователи по-прежнему могут устанавливать приложения самостоятельно, [](teams-app-permission-policies.md) если это разрешает назначенная им политика разрешений.
...
4. Включите или **отключите Upload приложения** в зависимости от того, хотите ли вы разрешать пользователям загружать пользовательские приложения в Teams.

> [!NOTE]
> Этот параметр нельзя изменить, если сторонние  приложения отключены в параметрах приложений для [всей организации](manage-apps.md#manage-org-wide-app-settings).
...
![страница политик настройки приложений.](media/app-setup-policies-update.png)

### <a name="edit-an-app-setup-policy"></a>Изменение политики настройки приложения

Центр администрирования Microsoft Teams для изменения политики, в том числе глобальной (по умолчанию в организации) и настраиваемой политики, которую вы создаете.

1. В левой области навигации центра администрирования Microsoft Teams перейдите к Teams **appsSetup** >  **policies**.

2. Выберите политику, которая вы хотите изменить, и выберите **изменить**.

3. Внести нужные изменения.

4. Нажмите **Сохранить**.

### <a name="assign-a-custom-app-setup-policy-to-users-and-groups"></a>Назначение настраиваемой политики настройки приложений пользователям и группам

Дополнительные сведения о назначении политик пользователям и группам см. в сведениях о назначении политик [пользователям и группам](assign-policies-users-and-groups.md).
...
- **FrontlineWorker**: эта политика для сотрудников с frontline. Вы можете назначить ее сотрудникам, работающим с телефоном в организации. Важно знать, что, как и настраиваемые политики, которые вы создаете, необходимо назначить политику пользователям, чтобы параметры были активными. Дополнительные сведения можно найти в разделе [Назначение](#assign-a-custom-app-setup-policy-to-users-and-groups) настраиваемой политики настройки приложений пользователям этой статьи.
...
Помните, что в Teams app Store перечислены все Teams приложения. В **области Добавить закрепленные** приложения доступны только приложения, которые можно Teams с помощью политики.
...
Сторонние приложения (которые можно скачать из Teams Store) должны быть утверждены, прежде чем они будут доступны на мобильном устройстве. Если администратор закрепит приложение, которое не используется корпорацией Майкрософт для мобильных устройств, оно будет отстрячено на рабочем столе Teams, но не на мобильном устройстве. [Дополнительные сведения см. в](/microsoftteams/platform/tabs/what-are-tabs#mobile-clients) мобильных клиентах.
...
Пользователи могут изменять порядок закрепленных приложений на Teams клиентах для настольных компьютеров и мобильных устройств,  если включен параметр Закрепление пользователей. Пользователи не могут изменить порядок закрепленных приложений в Teams клиентах.

#### <a name="does-user-pinning-take-precedence"></a>Имеет ли приоритет закрепление пользователей

Пин-коды администраторов всегда имеют приоритет. Если параметр **Закрепление** пользователей включен, то пользователи будут хранить закрепленные приложения под закрепленным администратором приложениями. Если параметр **Закрепление** пользователей отключен, пользователи потеряют свои предварительно установленные пин-коды, а на панели приложений будут доступны только приложения, закрепленные администратором.
