...
Шаблоны номеров можно настроить с помощью новых, **get-**, **set-**, and **Remove-CsInboundBlockedNumberPattern**. Вы можете управлять заданным шаблоном с помощью этих cmdlets, включая возможность переключения активации заданного шаблона.
...
Просмотром и активацией всей функции блокировки зовов управляется с помощью элементов **Get-** и **Set-CsTenantBlockingCallingNumbers** .
...
В следующем примере администратор клиента хочет заблокировать все звонки из диапазона 1 (312) 555-0000-1 (312) 555-9999. Шаблон чисел создается таким образом, что совпадают как числа в диапазоне с префиксом +, так и числа в диапазоне без префикса +. Вам не нужно включать символы – и () в номера телефонов, так как система размыкает эти символы перед соответствием.  Чтобы включить шаблон номера, для **параметра Enabled** задано true. Чтобы отключить этот шаблон номера, задан параметр False.

```PowerShell
New-CsInboundBlockedNumberPattern -Name "BlockRange1" -Enabled $True -Description "Block Contoso" -Pattern "^\+?1312555\d{4}$"
```

В следующем примере администратор клиента хочет заблокировать все звонки, исходят из номера 1 (412) 555-1234. Чтобы включить шаблон номера, для **параметра Enabled** задано true.
...
Совпадения с шаблонами можно использовать с помощью регулярных выражений (Regex). Дополнительные сведения см. [в том, как использовать Regex](#using-regex).

Разрешить время репликации перед проверкой и проверкой. 

#### <a name="allow-a-number"></a>Разрешить номер

Вы можете разрешить звонок на номер, удалив шаблон заблокированного номера. В следующем примере администратор клиента хочет разрешить 1 (412) 555-1234-звонков для повторного вызова.
...
Исключения для заблокированных шаблонов номеров можно добавить с помощью нью **-**, **get-**, **Set-и** **Remove-CsInboundExemptNumberPattern** .
...
- [Remove-CsInboundExemptNumberPattern](/powershell/module/skype/Remove-CsInboundExemptNumberPattern) удаляет шаблон исключения из списка клиента.

### <a name="examples"></a>Примеры

#### <a name="add-a-number-exception"></a>Добавление числого исключения

В следующем примере администратор клиента хочет разрешить телефонные номера 1 (312) 555-8882 и 1 (312) 555-8883 для звонков в клиент, даже если эти два номера телефонов находятся в диапазоне, заблокированном в примере выше. Чтобы включить эту функцию, создается новый шаблон исключения для числов:
...
Используйте для проверки, заблокирован ли номер в клиенте, с помощью макроса **Test-CsInboundBlockedNumberPattern** .
 
Параметр **PhoneNumber** является required и должен быть числовой строкой без дополнительных символов, таких как +, - или (). Результат **параметра IsNumberBlocked** возвращает значение True, если число заблокировано в клиенте; параметр возвращает false, если он не заблокирован.
...
В этих примерах можно увидеть, что номер телефона 1 (312) 555-8884 заблокирован так, как должен быть из-за того, что он находится в диапазоне заблокированных выше, а номер телефона 1 (312) 555-8883 можно звонить, исходя из созданного выше исключения.
