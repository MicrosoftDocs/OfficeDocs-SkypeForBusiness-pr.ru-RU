...
ms.locfileid: "66789894"
---
# <a name="using-the-cqd-pstn-direct-routing-report"></a>Использование отчета о прямой маршрутизации по ТСОП CQD

В марте 2020 г. мы добавили отчет о прямой маршрутизации ТСОП в Microsoft Teams в скачиваемые шаблоны запросов [Power BI для CQD](https://github.com/MicrosoftDocs/OfficeDocs-SkypeForBusiness/blob/live/Teams/downloads/CQD-Power-BI-query-templates.zip?raw=true). 
...
2.  Если на карте отзывов пользователей отображается увеличение красных точек, можно перейти на страницу "Коэффициент эффективности сети" и "Сетевой параметр", чтобы узнать, есть ли аномалии, и вы можете отправить запрос с помощью MS Service Desk.
...
Если на любой из четырех диаграмм (задержка, дрожание, частота потери пакетов, задержка после набора) отображается пиковая задержка для определенной даты, например задержка 14.02.2020, щелкните точку даты. А почасовая диаграмма тренда внизу обновится, чтобы отобразить почасовой номер. Вы можете проверить контроллеры SBC или создать запрос с помощью MS Service Desk.
