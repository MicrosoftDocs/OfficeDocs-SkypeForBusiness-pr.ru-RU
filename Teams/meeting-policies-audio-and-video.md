...
Управление камерой — это политика, которую можно назначить Комнаты Teams в учетных записях ресурсов Windows. Это позволяет управлять PTZ-камерами, подключенными к комнате Teams, участниками собрания в клиентском приложении Teams во время собраний.
...
Чтобы указать, кто может использовать управление камерой в собрании, создайте и назначьте новую политику учетной записи ресурса Комнаты Teams с помощью [командлета New-CsTeamsMeetingPolicy](/powershell/module/skype/new-csteamsmeetingpolicy?view=skype-ps) или используйте [Set-CsTeamsMeetingPolicy](/powershell/module/skype/set-csteamsmeetingpolicy) для изменения существующего. Задайте для `TeamsCameraFarEndPTZMode` параметра одно из следующих значений:

|Значение параметра|Поведение|
|---|---|
|Отключено|Это параметр по умолчанию. Если задано значение "отключено", никто не сможет использовать элементы управления камерой PTZ.|
...
Поддерживается любая камера с механическими элементами управления PTZ и UVC. Список камер, сертифицированных для Teams, включая PTZ- и не-PTZ-камеры, см. в разделе [Сертифицированные версии встроенного ПО для аудио- и видео периферийных устройств USB](rooms/requirements.md#certified-firmware-versions-for-usb-audio-and-video-peripherals). Эта функция пока не поддерживается на камерах с цифровыми элементами управления PTZ или на Комнаты Teams на Android.  
