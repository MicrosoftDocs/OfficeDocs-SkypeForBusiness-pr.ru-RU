...
- **Пометка** по сменам. С помощью этой функции в приложении "Смены" автоматически присваиваются теги, соответствующие их расписанию и Teams. [](https://support.microsoft.com/office/get-started-in-shifts-5f3e30d8-1821-4904-be26-c3cd25a497d6#bkmk_openshiftsappdesktop) Например, тег "EngineerOnCall" достигает всех инженеров, запланированных в сменах для работы в то время, когда тег используется в чате или сообщении канала. При пометки по смене Teams не зная имена сотрудников, которые работают в смене, когда пользователям нужно быстро ретранслировать информацию. Кроме того, теги по сменам можно использовать в основных системах управления трудовыми ресурсами, например JDA, Kronos и AMiON, путем их интеграции со Teams. Дополнительные информацию о том, как настроить эту функцию, см. в этой [теме.](#set-up-tagging-by-shift)

> [!NOTE]
> Теги пока не поддерживаются в закрытых каналах. Теги недоступны в организациях, в которые в настоящее время не доступны теги. 
...
По умолчанию владельцы команд могут добавлять настраиваемые теги. Вы можете изменить этот параметр, чтобы владельцы и участники группы могли создавать, изменять, удалять теги и управлять ими, а также отключать теги для своей организации.
...
2. В **зависимости от** потребностей организации в области Маркировка установите следующие метки:
    ...
    - **Пусть создаются настраиваемые** теги: включите этот параметр, чтобы позволить другим людям добавлять теги, не рекомендуемые по умолчанию. Если этот режим отключен, люди могут использовать только предлагаемые теги по умолчанию. Если вы отключите этот режим, убедитесь, что вы добавили один или несколько тегов по умолчанию.
...
Пометка по сменам позволяет пользователям связаться с пользователями в режиме реального времени. Teams автоматически назначает пользователям теги, совпадающие с их расписанием и названием группы смены, из приложения "Смены", что позволяет динамически обмен сообщениями на основе ролей. Уведомления отправляются только тем людям, которые находятся в смене в момент, когда тег используется для начала чата или публикации в канале. 
