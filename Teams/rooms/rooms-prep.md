...
1. Подготовьте учетную запись устройства для каждой консоли комнат Microsoft Teams. Дополнительные [сведения см. в сведениях о](rooms-deploy.md) развертывании комнат Microsoft Teams.
...
2. Коснитесь параметров, а затем нажмите кнопку "Перейти в **Windows",** затем  коснитесь кнопки "Перейти к входу администратора" и нажмите кнопку "Администратор" (если компьютер присоединился к домену, выберите "Другой  пользователь", а затем используйте в качестве имени пользователя .\admin).  
...
6. Перейдите к папке **C:\Users\Skype** и введите имя файла N УАСТЕР.dat и нажмите кнопку "Открыть".
...
> [!NOTE]
> Поддиавный знак для contoso.com и broadcast.skype.com представляет длинный список узлов, которые используются только для Microsoft 365 или Office 365. 
...
Для комнат Microsoft Teams по умолчанию установлен пароль "sfb". Пароль можно изменить локально, перейдите в параметры Windows перейти в Windows или в файл AutoUnattend.xml (измените XML-файл с помощью диспетчера изображений Системы Windows из \> ADK).
