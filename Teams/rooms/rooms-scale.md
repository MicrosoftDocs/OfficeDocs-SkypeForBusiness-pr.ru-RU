...
Используйте подход, который приведен ниже, чтобы пролицать конфигурацию Configuration Manager и настроить образцы пакетов и сценариев, предоставленные в рамках этого руководства, при необходимости для вашей организации.
...
-   В вашей сети должен быть сервер DHCP, настроенный для автоматического распространения IP-адресов на подсеть, в которых Комнаты Microsoft Teams будут развернуты единицы.
    ...
    > [!NOTE]
    > Длительность аренду DHCP должна быть установлена на более длительный срок, чем длительность развертывания изображений. В противном случае развертывание может привести к сбойу.
...
-   Комнаты Microsoft Teams настроены для работы в группе, а не в домене.
...
3.  Распространение пакетов операционной системы.
    ...
    1.  В консоли Configuration Manager перейдите к изображению **операционной** системы библиотеки \>  \> **программного обеспечения .** Выберите все изображения операционной системы, которые вы хотите распространить, а затем выберите **Распространение содержимого**.
...
2. Выполните каждый шаг и выполните рекомендуемые обновления.
   ...
   6. **Применить сетевой Параметры:** на этом этапе можно указать имя группы или домена Active Directory и подразделения.
      > [!NOTE]
      > Дополнительные [Skype](domain-joining-considerations.md) о рекомендуемых действиях, которые необходимо принять при развертывании Комнаты Microsoft Teams как участников домена Actve Directory, см. в Skype домене "Система комнат".
...
>
> **Настройка приложения SRS** должна быть последним этапом в последовательности задач, в противном случае может произойти сбой развертывания.
