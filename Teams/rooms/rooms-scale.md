...
В этой статье предполагается, что у вас уже настроено здоровое развертывание Configuration Manager, а также не подробно о шагах, необходимых для развертывания и настройки Configuration Manager с нуля. Документация [и рекомендации по настройке](https://docs.microsoft.com/configmgr/) в Диспетчере конфигураций конечной точки Майкрософт — отличный ресурс. рекомендуем начать с этих ресурсов, если вы еще не развернули Configuration Manager.
...
6.  Выберите последовательность задач, импортируемую ранее, и выберите **"Далее".**
