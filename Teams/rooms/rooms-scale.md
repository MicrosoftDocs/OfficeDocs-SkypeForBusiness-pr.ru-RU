...
3.  Просмотрите [поддержку Windows 10 в Configuration Manager;](https://docs.microsoft.com/configmgr/core/plan-design/configs/support-for-windows-10#windows-10-as-a-client) если вам нужно обновить развертывание, выберите обновление, а затем выберите **"Скачать".**
...
2. Скопируйте ниже сценарий **вInstall-SRSv2-OS-Updates.ps1** сценарий. Вы также можете скачать сценарий Install-SRSv2-OS-Updates.ps1 [здесь.](https://github.com/MicrosoftDocs/OfficeDocs-SkypeForBusiness/blob/live/Skype/SfbOnline/downloads/Skype-Room-Systems-v2/SRS-v2-Configuration-Manager-Files.zip?raw=true)
   ```
...
4.  Чтобы создать пакет, введите следующие данные:
    ...
    -   Выберите этот **пакет содержит исходные файлы,** введите путь к **SRS v2 - Set-SRSComputerName package** folder, а затем выберите **Далее.**
...
1.  Получите мультимедиа Windows 10 Корпоративный x64 и скопируйте **файл install.wim** в папку Операционных систем **Windows \\ 10 Enterprise.**
...
2. Выполните все этапы и выполните рекомендуемые обновления.
   ...
   17. **Настройка приложения SRS:** этот шаг позволяет настроить установку приложения "Комнаты Microsoft Teams" для следующего загрузки операционной системы.
...
1.  Подключите тестовое устройство к проводной сети с помощью одного из поддерживаемых адаптеров Ethernet или док-станции Surface. Если функциональность загрузки PXE не настроена для вашей среды, вы можете [](https://docs.microsoft.com/configmgr/osd/deploy-use/create-bootable-media) использовать изображение загрузки на USB-устройстве флэш-памяти, созданном ранее, для загрузки с USB и подключения к Configuration Manager.
...
Полный список файлов журнала, которые можно использовать для дальнейшего устранения неполадок при [](https://docs.microsoft.com/configmgr/core/plan-design/hierarchy/log-files)установке Configuration Manager, см. в справочнике по файлу журнала Диспетчера конфигураций Microsoft Endpoint.
