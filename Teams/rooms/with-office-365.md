...
В этой статье содержатся инструкции по созданию учетных записей ресурсов для общих пространств и устройств, а также по настройке учетных записей ресурсов для Комнаты Microsoft Teams в Windows, Комнаты Teams в Android, Комнаты Teams на Surface Hub и в службе "Горячее обслуживание" Teams  Отображает.
...
- &mdash;Если вы используете только учетную запись ресурса для резервации ресурса,&mdash; приглашайте ресурс на собрание и хотите, чтобы он автоматически принял или отклонил приглашение, не нужно назначать лицензию учетной записи ресурса, и вам нужно выполнить шаг 1 в следующем обзоре.  
...
5. По умолчанию учетные записи ресурсов настраиваются со следующими параметрами:
    ...
    Если вы хотите изменить их, выберите " **Задать параметры планирования** " перед нажми кнопку **"Закрыть"**. Если вы хотите изменить их позже, перейдите в **ResourcesRooms** >  **& оборудования**, выберите учетную запись ресурса. Затем в разделе **"Параметры резервирования**" выберите " **Изменить"**.

6. Перейдите **к пользователям** **UsersActive** >  и выберите комнату, которую вы создали, чтобы открыть панель свойств.
...
>
> Перейдите к следующему разделу, только если учетная запись ресурса предназначена для Комнаты Teams на Windows, Комнаты Teams в Android, Комнаты Teams на Surface Hub или Teams с горячим обслуживанием.
...
Сведения о назначении лицензий с Центр администрирования Microsoft 365 см. в статье ["Назначение лицензий пользователям"](/microsoft-365/admin/manage/assign-licenses-to-users). Сведения о назначении лицензий с помощью Azure AD см. на одной из следующих вкладок:
