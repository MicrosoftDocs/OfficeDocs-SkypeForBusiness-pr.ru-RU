...
Чтобы обеспечить успешную проверку подлинности на локальном сервере Exchange и (или) в Skype для бизнеса, необходимо убедиться, что учетная запись ресурса, используемая с комнатами Microsoft Teams, настроена для получения авторизации через Azure AD. 
...
Однако поскольку в комнатах Microsoft Teams используется авторизация паролей владельцев ресурсов и АPI REST для современной проверки подлинности, следует помнить о следующих важных отличиях, которые являются характерными для комнат Microsoft Teams. [](https://tools.ietf.org/html/rfc6749#section-1.3.3)
