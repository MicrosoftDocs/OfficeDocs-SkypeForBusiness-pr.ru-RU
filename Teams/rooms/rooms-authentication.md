...
ms.locfileid: "61306304"
---
# <a name="authentication-in-microsoft-teams-rooms"></a>Проверка подлинности в Комнаты Microsoft Teams

Управление учетной записью Комнаты Microsoft Teams устройствами обрабатывается на уровне приложения. Приложение подключается к Microsoft Teams, Skype для бизнеса и Exchange, чтобы получить ресурсы для учетной записи комнаты для звонков и собраний. Устройство использует специальную учетную запись ресурса, чтобы обеспечить постоянное использование возможностей, сценарии звонков (для устройств, настроенных с помощью плана звонков) и пользовательские механизмы блокировки, реализованные на этих устройствах. Это означает, что проверка подлинности для этих устройств происходит не так, как на устройствах конечных пользователей.  
...
Чтобы Комнаты Microsoft Teams современную проверку подлинности с Skype для бизнеса и Exchange, в этом Комнаты Microsoft Teams. Это можно сделать в параметрах устройства или в XML-файле.
...
Однако поскольку в Комнаты Microsoft Teams [](https://tools.ietf.org/html/rfc6749#section-1.3.3) для современной проверки подлинности используются авторизация паролей владельцев ресурсов и aPI REST, следует помнить о следующих Комнаты Microsoft Teams.
