...
Чтобы Комнаты Microsoft Teams современную проверку подлинности с Skype для бизнеса и Exchange, в этой Комнаты Microsoft Teams клиента. Это можно сделать в параметрах устройства или в XML-файле.
...
1. На устройстве Microsoft Team Rooms перейдите в **more** **(...**).
...
В SkypeSettings.xml xML-элементе современной проверки подлинности установите для элемента **True**(Истина), как поется в этом примере.

```
<ModernAuthEnabled>True</ModernAuthEnabled>
```

Чтобы применить этот параметр, см. статью Удаленное управление Комнаты Microsoft Teams консоли с [помощью XML-файла конфигурации.](xml-config-file.md)

## <a name="prepare-your-environment-for-modern-authentication"></a>Подготовка среды к современной проверке подлинности

Прежде чем начать, убедитесь, что вы понимаете модели удостоверений, которые нужно использовать с Office 365 и Azure AD. Дополнительные сведения можно найти на Office 365 и Azure Active Directory [и](/Office365/Enterprise/about-office-365-identity) в гибридных удостоверениях и синхронизации каталогов для Microsoft 365 [или Office 365.](/Office365/Enterprise/plan-for-directory-synchronization)
...
Мы рекомендуем не удалять основные политики проверки подлинности для Exchange Online и не отключать базовую проверку подлинности для клиента, пока не будет проверено, что устройства Комнаты Microsoft Teams смогут успешно войти с помощью Exchange Online, Teams и Skype для бизнеса Online.
...
Чтобы обеспечить успешную проверку подлинности на локальном сервере Exchange и (или) Skype для бизнеса-сервере, необходимо убедиться, что учетная запись ресурса, которая используется в Комнаты Microsoft Teams, настроена для получения авторизации из Azure AD. 
...
Необходимые условия для современной проверки подлинности в гибридной топологии можно найти в обзоре гибридной современной проверки подлинности и необходимые условия для ее использования с локальной Skype для бизнеса и [Exchange серверами.](/office365/enterprise/hybrid-modern-auth-overview) Применяются все необходимые условия, рассмотренные в этой статье.

Однако поскольку в Комнаты Microsoft Teams [](https://tools.ietf.org/html/rfc6749#section-1.3.3) для современной проверки подлинности используются авторизация паролей владельца ресурса и а также API REST, следует помнить о следующих важных различиях, которые Комнаты Microsoft Teams.
...
Чтобы включить гибридную современную проверку подлинности с Skype для бизнеса Server, см. Skype для бизнеса локальном использовании [гибридной современной проверки подлинности.](/Office365/Enterprise/configure-exchange-server-for-hybrid-modern-authentication)

### <a name="remove-or-disable-skype-for-business-and-exchange"></a>Удаление и отключение Skype для бизнеса Exchange
...
### <a name="azure-ad-conditional-access"></a>Условный доступ к Azure AD
