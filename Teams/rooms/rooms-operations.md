...
> [!NOTE]
> Если вы хотите, чтобы экран комнаты автоматически переключался на активный источник видео (например, консоль MTR) при переходе из режима ожидания, должны быть выполнены определенные условия. Эта функция необязательна, но Комнаты Microsoft Teams программное обеспечение, которое поддерживает оборудование. Телевизор, используемый в качестве переднего экрана комнаты, должен поддерживать функцию управления потребительскими электронными устройствами (CEC) HDMI.  В зависимости от выбранной док-станции или консоли (которая может не поддерживать CEC, обратитесь к документации производителя) для обеспечения желаемого поведения может потребоваться геймпад [HD-RX-201-C-E](https://www.crestron.com/Products/Video/HDMI-Solutions/HDMI-Extenders/HD-RX-201-C-E) из КомпанииГриголь или [Exаа HD CTL 100](https://www.extron.com/article/hdctl100ad) из Exstation. 
  
## <a name="microsoft-teams-rooms-reset-factory-restore"></a>Комнаты Microsoft Teams Сброс (восстановление до заводских цехов)
<a name="Reset"> </a>

Если Комнаты Microsoft Teams не работает, возможно, вам поможет сброс до заводских населений. Для этого воспользуйтесь средством восстановления Microsoft Teams [комнат](recovery-tool.md) и следуйте инструкциям по восстановлению до заводских насайтов.

> [!NOTE]
> Известная проблема, из-за которой Комнаты Microsoft Teams может стать неиспользоваемой, если во время сброса параметр Сохранить мои файлы : удаление приложений и параметров **Windows,** но сохранение личных файлов. Не *используйте* этот параметр.
...
В этом разделе параметров системы, которые Комнаты Microsoft Teams правильно работают. При присоединении Комнаты Microsoft Teams к домену убедитесь, что групповая политика не переопределяет параметры в таблице ниже.
...
По умолчанию Комнаты Microsoft Teams подключается к Windows обновления операционной системы и USB-периферийного устройства и устанавливает их за нестройное время. Чтобы указать часы работы, войдите в учетную запись администратора и запустите приложение "Параметры".
...
Консоль снова находится в обычном режиме работы. Если клавиатура еще не вложена, к устройству необходимо прикрепить клавиатуру. 
