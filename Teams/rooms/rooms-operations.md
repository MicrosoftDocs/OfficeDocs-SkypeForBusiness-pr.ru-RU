...
> [!NOTE]
> Известная проблема может привести к невозможности Комнаты Microsoft Teams, если во время сброса параметр Сохранить мои файлы : удаление приложений и параметров **Windows,** но сохранение личных файлов. Не *используйте* этот параметр.
...
В этом разделе параметров системы, Комнаты Microsoft Teams от которых должны работать правильно. При присоединении Комнаты Microsoft Teams к домену убедитесь, что групповая политика не переопределяет параметры в таблице ниже.
...
> [!NOTE]
> Если Комнаты Microsoft Teams совместим со следующей версией Windows 10 OS, оно автоматически обновляется до следующей версии с помощью Windows Update. Комнаты Microsoft Teams устройство не должно быть обновлено до следующего выпуска Windows 10 вручную или с помощью включения групповых политик Windows Update for Business (WUFB) "Выбор уровня готовности Windows для получения обновлений" и "Выбор при получении предварительных сборков и обновлений функций" с помощью GPO. Известно, что на устройстве с включенными групповыми политиками могут Windows 10 с обновлением ОС Комнаты Microsoft Teams приложением.
...
4. Откройте локонную политику безопасности  и добавьте группу безопасности Администраторы в группу безопасности Параметры  >  **локальных** политик. Назначение прав пользователя Доступ к  >    >  **этому** компьютеру из сети .
...
2. Запустите следующий сценарий для Комнаты Microsoft Teams устройств, при необходимости сменив \<share\> его на "Поделиться":
...
11. Вернись Комнаты Microsoft Teams, выбрав значок учетной записи пользователя в левой части экрана и выбрав Skype **.**
    ...
    Если **Skype** нет в списке, вам может  потребоваться выбрать другого пользователя и ввести в качестве имени пользователя **.\skype** и войти в нее.
    
Консоль снова находится в обычном режиме работы. Чтобы прикрепить клавиатуру к устройству, если она еще не вложена, следуя этой процедуре. 
