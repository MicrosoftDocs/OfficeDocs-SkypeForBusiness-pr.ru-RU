...
Отчет Power BI по использованию Teams состоит из двух основных отчетов: **["Сводка](#call-count-summary-report)** по подсчету вызовов" и **["Итоги аудиосвязи в минутах".](#audio-minutes-summary-report)** Отчеты ["Ежедневное](#daily-usage)использование", [](#user-list) "Региональные аудиоконференции", "Сведения о конференции" и "Список пользователей" вступает в силу, когда пользователь получает возможность воспользоваться детализанизу отчетами, которые описаны ниже. [](#regional-audio-details) [](#conference-details)
...
В отчете об использовании звуковых минут общая сумма минут предоставляется в нескольких различных представлениях. 
...
Чтобы показать разницу в звуке на P2P и конференции, на нижней левой диаграмме берется весь звук за последний год и он раздвеывается между двумя типами.
...
Отчет о ежедневном использовании позволяет администратору определить периоды пикового потребления в течение дня. Кроме использования, мы также можем фиксировать общее тональность пользователей и отзывы за этот день.

![Снимок экрана: отчеты об использовании Teams](media/CQD-teams-utilization-report12.png)

В отчете о ежедневном использовании отображается количество звуковых, видеофайлов и экранных ресурсов за выбранный день с добавленной возможностью отличить внутреннее и внешнее подключение. Разбивка между одноранговой конференцией и одноранговой разбивкой находится справа от итога модалисти. В правой верхней части отчета содержится список конференций, связанных с их ИД и участниками на день. Кроме того, в списке конференций есть дополнительные возможности для деталисции отчета о конференции. РИСУНОК "ЗАМЕНИТЬ"
...
Справа от гограммы отзывы пользователей представлены в визуальном формате. Хотя тональность пользователей может быть неохожна, она дает представление, которое можно использовать для выявления потенциальных проблем.

В нижней таблице содержится диапазон метрик для дня. Неудовлетворительные процентные показатели и коэффициенты сбоев могут предоставить администратору возможности улучшения. Каждый час также можно выбрать отдельно, как показано ниже.
...
![Снимок экрана: отчеты об использовании Teams](media/CQD-teams-utilization-report14.png)
  ...
  1.  В таблице под диаграммой отобразятся показатели за этот час. Его можно отсортировать по любому заглавию столбца. тем не менее мы хотели бы заинтересовались поиском проблемных областей.  
    ![Снимок экрана: отчеты об использовании Teams](media/CQD-teams-utilization-report15.png)
    
  2.  Мы видим, что в регионе IND наблюдается низкая производительность видео во время конференций в этот период времени. Впоследствии можно использовать отчеты Корпорации Майкрософт для QER для CQD, чтобы сузить проблемное расположение по мере определения региона и замера времени.
...
Региональные тенденции аудиосвязи можно использовать для демонстрации влияния внешних факторов на пользователей в мире. В частности, в настоящее время мы ожидаем, что внешний уровень использования регионов EMEA и APAC увеличится, и люди будут работать удаленно.
...
В правом верхнемправо на графике показано количество звонков, сделанных каждым пользователем в течение часа.
