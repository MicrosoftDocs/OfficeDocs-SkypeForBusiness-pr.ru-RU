...
Музыка по умолчанию, Microsoft Teams в сценариях  вызовов через ПСС, не должна быть больше, чем за роялти, которую оплачивает ваша организация.

Обратите внимание на то, что в других сценариях звоня люди, вызыватели ПСОП, могут прослушивать музыку при удержании. Например, если звонок находится в очереди облачных  вызовов или его звонок Microsoft Teams пользователем. Эти сценарии не охватываются функциями, упомянутыми в этой статье, и не контролируются этими возможностями.
...
Если у Teams пользователя есть политика Teams звонка с отключенной музыкой при удержании, то при Teams пользователь помещает звонок на удержание.
...
Чтобы настроить настраиваемую музыку при удержании, используйте командлеты PowerShell New/Get/Set/Grant/Remove-CsTeamsCallHoldPolicy и Import/Get/Remove/Export-CsOnlineAudioFile в модуле Teams PowerShell 3.0.0 или более поздней.

Поддерживаемые форматы звука и максимальный размер файла см. в [импорте CsOnlineAudioFile](/powershell/module/skype/import-csonlineaudiofile).
...
3. Создайте политику Teams удержания  вызовов со ссылкой на настраиваемый звуковой файл и назначьте Teams пользователю.

### <a name="upload-the-custom-audio-file"></a>Upload настраиваемый звуковой файл

Настройка пользовательской музыки при удержании начинается с отправки звукового файла. Для этого используется cmdlet [Import-CsOnlineAudioFile](/powershell/module/skype/import-csonlineaudiofile) PowerShell.

Ниже показан пример отправки звукового файла MP3 с помощью интерфейса PowerShell.
...
Чтобы экспортировать добавленный звуковой файл, воспользуйтесь Export-CsOnlineAudioFile видео.
...
| :------------| :------- | :------- | :------- | :------- | :------- |
| Удержание звонка через ПСС 1:1 | -Музыка при удержании<br>-Custom Music on Hold | -Музыка при удержании<br>-Custom Music on Hold | -Музыка при удержании<br>-Custom Music on Hold | -Музыка при удержании<br>-Custom Music on Hold | -Музыка при удержании<br>-Custom Music on Hold |
| Удержание для консультационной передачи при вызове 1:1 по ДНР |-Музыка при удержании<br>-Custom Music on Hold | | -Музыка при удержании<br>-Custom Music on Hold | -Музыка при удержании<br>-Custom Music on Hold | |

## <a name="restrictions"></a>Ограничения

- Музыка при удержании доступна только в коммерческих и GCC облачных экземплярах.
