...
ms.locfileid: "41233340"
---
# <a name="powershell-script-sample---reset-the-autostart-setting-in-teams"></a><span data-ttu-id="d879c-103">Пример сценария PowerShell: Сброс параметров автозапуска в Teams</span><span class="sxs-lookup"><span data-stu-id="d879c-103">PowerShell script sample - Reset the autostart setting in Teams</span></span>

<span data-ttu-id="d879c-104">Используйте этот сценарий, чтобы сбросить параметры автоматического запуска команд для каждого пользователя.</span><span class="sxs-lookup"><span data-stu-id="d879c-104">Use this script to reset the Teams autostart setting on a per-user basis.</span></span> <span data-ttu-id="d879c-105">Сюда входят любые значения, заданные пользователем или приложением Teams.</span><span class="sxs-lookup"><span data-stu-id="d879c-105">This includes any values set by the user or the Teams app.</span></span> <span data-ttu-id="d879c-106">По умолчанию команды запускаются автоматически, когда пользователь входит в систему на своем компьютере после установки.</span><span class="sxs-lookup"><span data-stu-id="d879c-106">By default, Teams automatically starts when a user logs in to their computer after it's installed.</span></span>

<span data-ttu-id="d879c-107">Если вы уже развернули команды и хотите отключить [Автоматический запуск Microsoft Teams после настройки групповой политики](../msi-deployment.md#use-group-policy-recommended) для отключения команды автозапуска, необходимо сначала задать для параметра групповой политики нужное значение, а затем выполнить этот сценарий.</span><span class="sxs-lookup"><span data-stu-id="d879c-107">If you've already deployed Teams and want to set the [Prevent Microsoft Teams from starting automatically after installation Group Policy setting](../msi-deployment.md#use-group-policy-recommended) to disable Teams autostart, you'll need to first set the Group Policy setting to the value you want, and then run this script.</span></span>

<span data-ttu-id="d879c-108">После запуска Teams для пользователя параметры автозапуска нельзя отключить с помощью групповой политики.</span><span class="sxs-lookup"><span data-stu-id="d879c-108">After Teams is started for a user, the autostart settings can't be disabled by using Group Policy.</span></span>

## <a name="sample-script"></a><span data-ttu-id="d879c-109">Пример сценария</span><span class="sxs-lookup"><span data-stu-id="d879c-109">Sample script</span></span>
...
## <a name="related-topics"></a><span data-ttu-id="d879c-110">См. также</span><span class="sxs-lookup"><span data-stu-id="d879c-110">Related topics</span></span>

- [<span data-ttu-id="d879c-111">Установка Teams с помощью MSI</span><span class="sxs-lookup"><span data-stu-id="d879c-111">Install Teams using MSI</span></span>](../msi-deployment.md)
- [<span data-ttu-id="d879c-112">Развертывание групп с помощью Office 365 ProPlus</span><span class="sxs-lookup"><span data-stu-id="d879c-112">Deploy Teams with Office 365 ProPlus</span></span>](https://docs.microsoft.com/deployoffice/teams-install)
