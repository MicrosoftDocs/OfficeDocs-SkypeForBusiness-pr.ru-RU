...
Live событий в Microsoft 365 позволяют пользователям вещание видео и содержимого для крупных online аудиторий.  Событий в реальном времени Microsoft 365 перевести видео потоковой передачи на новый уровень стимулирования подключения на протяжении жизненного цикла всей сотрудничества человек до, во время и после событий в реальном времени. Можно создать live события везде, где аудитории, группы или сообщества находится, с помощью Microsoft потока, группами Майкрософт или Yammer.  
...
Интерфейса участников является самой важной частью событий в реальном времени и очень важно, участники могут принимать участие в режиме реального времени событие без проблем. Интерфейса участника использует проигрывателем Windows Azure и работает на рабочий стол, браузеров и мобильных устройств (операций ввода-вывода, Android). Office 365 предоставляет Yammer и рабочих групп как два концентраторов совместной работы и live attendee качества интегрирована в эти средства совместной работы. События live на основе внешнего кодировщика также могут быть доступны с участников на портале Microsoft потока.
...
## <a name="planning--setup"></a>Планирование и настройка
В этой статье объясняется, как можно настроить пользователей с группами событий в реальном времени в вашей организации.

1. Установите флажок изучить области, которые событий в реальном времени, в настоящее время доступны в [региональных доступности для групп live события](#configure-live-events) .
...
#### <a name="purchase--setup-your-solution-outside-of-microsoft-teams"></a>Приобретение программы установки решения за пределами группами Майкрософт
...
Следующие решения SDN/eCDN предварительно встроенные и может быть программы установки для использования с Microsoft потока. Просмотрите сведения из поставщиков ниже:
...
1. Получение лицензии идентификатор и API URL-адрес шаблона из куста контакт. 
...
Командлет [Set-CsTeamsMeetingBroadcastConfiguration](https://docs.microsoft.com/en-us/powershell/module/skype/set-csteamsmeetingbroadcastconfiguration?view=skype-ps) PowerShell можно использовать для настройки поставщика eCDN. 
1. Получение маркера API и URL-адрес шаблона API из контактов Kollective. 
2. Выполните следующий командлет PowerShell
```
...
- Когда появляется в Windows PowerShell, он все сведения об управлении пользователями и какие пользователи могут или не действующими для них. С помощью Windows PowerShell вы можете управлять Office 365 и Skype для бизнеса online, используя единый центр администрирования, который упростит выполнение ваших повседневных задач. Чтобы начать работу с Windows PowerShell, ознакомьтесь с приведенными ниже разделами.
