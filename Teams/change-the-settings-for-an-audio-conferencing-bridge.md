...
## <a name="an-icon-showing-the-skype-for-business-logo--using-the-skype-for-business-admin-center"></a>![Значок с логотипом Skype для бизнеса](media/sfb-logo-30x30.png)  Использование Центра администрирования Skype для бизнеса
...
2. На странице **параметров моста Microsoft** в области **"Присоединиться к собранию"** выберите:
    
   - **Enable meeting entry and exit notifications to be turned on** This is selected by default. Если его свести, то пользователи, уже присоединившиеся к собранию, не будут уведомлены о том, что кто-то присоединился к собранию или покинул его.
    
   - Если включить **уведомления** о входе и выходе из собрания, вы можете выбрать эти параметры в списке типов извещений о входе и **выходе:**
    
   - **Имена или номера телефонов** Когда пользователи присоединяются к собранию по телефону, их номера телефонов будут играть, когда они присоединятся к нему.
    
   - **Мелодии** Когда пользователь присоединяется к собранию по телефонной сети, его мелодия звукового сигнала зазвучит.
  
   - **Попросите звоняющих записать свое имя перед присоединением к собранию** Он выбран по умолчанию. Если этот поле не зафиксировать, то перед тем как присоединяться к собранию, не будет предложено записать свое имя.
...
2. Перейдите в **Центр администрирования Microsoft 365**  >  **Skype для бизнеса.**
...
4. На странице **параметров** моста Microsoft в области "Безопасность" введите количество цифр ПИН-кода в списке длины **ПИН-кода** и нажмите кнопку **"Сохранить".**
    
    > [!IMPORTANT]
    > ПИН-код должен включать в себя от 4 до 12 цифр. 
  
**Выбор отправки электронной почты пользователям**
...
4. На странице **параметров** моста Microsoft выберите или отключите автоматическое отправку электронных писем пользователям в случае изменения данных для телефонного дозвона, а затем нажмите кнопку **"Сохранить".**
