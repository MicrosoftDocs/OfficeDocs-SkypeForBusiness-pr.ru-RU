...
Потоки, какая часть звонка была неудовлетворительной — исходящая или входящий. При анализе аналитики вызовов для неудовлетворительного звонка определите, был ли неудовлетворительный звонок вызван потоком (исходящим) или потоком (входящий звонок) вызываемого. Для конференций еще важнее определить, какой поток влияет на качество звонка. Если вы видите только данные о звонках, вы увидите, сколько конференций участвует человек, но вы не увидите, какие люди являются активными выступающими, вы делаете большую часть экрана.
...
В этом разделе рассмотрены некоторые основные показатели служб, которые используются для оценки их состояния. Постоянно оценивая показатели и движуя усилия по их обеспечению в соответствии с целевыми показателями, вы помогаете обеспечить пользователям согласованное и надежное качество звонка. В качестве отправной точки используйте предлагаемые цели в таблице ниже. При необходимости настройте целевые показатели для своих бизнес-целей.
...
-   **Сеть:** Качество звука, основное внимание уделялось метрике Poor Stream Ratio (PSR), использованию TCP, проводным и беспроводным подсетям, а также использованию прокси-каналов HTTP и VPN.
...
-   CQD не содержит потоков телефонная система аудиоконференцию
...
-   **Хорошая или неудовлетворительная:** Хороший или неудовлетворительный звонок состоит из звонка, который содержит полный набор показателей качества обслуживания, для которого служба создает и получает полный отчет о качестве обслуживания. Определение того, удовлетворительн ли поток или неудовлетворительный, [описано выше в этой статье.](#poor-stream-rate)
...
В этом отчете отображаются общее количество потоков, сбои настройки потока и коэффициент сбоев при установке потока. На указателе на любой столбец, чтобы отобразить отдельные значения. 
...
Следующий шаг для оценки состояния качества звука в организации — изучение низкой скорости потока (PSR), TCP и использования прокси-сервера. Важно помнить, что в данных CQD нет конкретной корневой причины, а вы можете начать совместную беседу с соответствующими группами по действиям по устранению проблем. 
...
Вне зависимости от ответов на вышеперечисленные вопросы у вас не будет времени для изучения с помощью подотещений для изучить все здания или подсети, которые могут потребовать исследования. Хотя общий показатель PSR может быть меньше целевого показателя, он часто находится выше метрики и требует исправлений.
...
> [!Note]
> Распространенные подсети сложно переустанавлить из-за их широкого использования. Отдельный отчет, в который отображается общедоступный IP-адрес клиента (Second Reflexive Local IP), добавлен в шаблон "Все сети", чтобы помочь в исправлении офисов, в которые используются общие сети.
...
| **Качество обслуживания (QoS)**  | QoS — это проверенный инструмент, который помогает расставить приоритеты пакетов в угрюмой сети, чтобы обеспечить их вовремя и без изменений. Думайте о внедрении QoS в организации, чтобы максимально повысить качество работы пользователей с ограниченной пропускной способностью. QoS помогает решать проблемы, которые обычно связаны с высокой потерей пакетов и (в меньшей степени) дрожания и времени кругового пути.<ul><li>[Teams Рекомендации по QoS](qos-in-teams.md)</li></ul> |
...
> [!IMPORTANT]
> В настоящее Teams клиенты автоматически распространяются и обновляются через Azure сеть доставки содержимого и будут обновляться службой. В результате вам не нужно отслеживать Teams версии клиента (если вы не отключите автоматическое обновление, которое мы не рекомендуем).
...
-   Сколько разных версий различных драйверов используется? Регулярно ли они обновляются? Регулярное исправление драйверов аудиоWi-Fi видео и видео поможет устранить их как источник проблем с качеством и сделать пользовательский интерфейс более предсказуемым и согласованным.
...
Драйверы видео также важны для обновления. Регулярное исправление видеокарт поможет исключить видеоисточники в качестве источника неудовлетворительного качества видеопотоков. Использование [сертифицированных видео устройств](https://partnersolutions.skypeforbusiness.com/solutionscatalog/personal-peripherals-pcs) обеспечит бесперебойное и высокое качество работы пользователей. Для снижения использования ЦП во время видеоконференций предпочтительнее использовать видео устройства, поддерживаюющие кодировидную кодику H.264.
