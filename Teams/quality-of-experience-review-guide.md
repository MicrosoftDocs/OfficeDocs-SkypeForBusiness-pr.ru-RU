...
<!-- Note that the link to the Word doc is intentionally NOT the aka.ms/qerquide link -->В этом руководстве — об этапе диска значение для групп Майкрософт и Скайп для бизнеса в Интернет. Вы можете [загрузить версию Word](https://github.com/MicrosoftDocs/OfficeDocs-SkypeForBusiness/blob/live/Teams/downloads/quality-of-experience-review-guide.docx?raw=true) этого руководства.
...
_В таблице 1 - показатели низкого уровня службы_
...
<tr><td rowspan="2">**All**</td><td>Internal</td><td>% 2.0 (en)</td><td>0,5%</td><td>% 2.0 (en)</td></tr>
<tr><td>Общий уровень</td><td>3.0%</td><td>1,0%</td><td>3.0%</td></tr>
<tr><td rowspan="5">**Конференц-связь**</td><td>Internal</td><td>% 2.0 (en)</td><td>0,5%</td><td>% 2.0 (en)</td></tr>
...
<tr><td>Внутренний Wi-Fi 2,4 ГГц</td><td>4.0%</td><td>0,5%</td><td>% 2.0 (en)</td></tr>
<tr><td>Общий уровень</td><td>% 2.0 (en)</td><td>0,5%</td><td>3.0%</td></tr>
<tr><td rowspan="4">**P2P**</td><td>Internal</td><td>% 2.0 (en)</td><td>0,5%</td><td>% 2.0 (en)</td></tr>
...
Другим способом на это является: _Измерение_ — это функция группировки, _мер_ является данных интересует и _Фильтр_ — Чтобы сузить результаты, которые относятся к запросу.
...
Дополнительные сведения о процессе классификации потока можно [в этой статье](/skypeforbusiness/using-call-quality-in-your-organization/stream-classification-in-call-quality-dashboard).
...
-   Последовательно выберите пункты **группами Майкрософт и Скайп по центру администрирования бизнес** и выберите ссылку на CQD, как показано на следующем рисунке.
...
_На рисунке 6: доступ к CQD с помощью групп Майкрософт & Скайп по центру администрирования бизнеса_
...
_На рисунке 7 - доступ к CQD с помощью Скайп по центру администрирования бизнеса_
...
_На рисунке 8 - переход к подробных отчетов_
...
_На рисунке 9 - страница подробных отчетов_
...
_На рисунке 10 - редактор запросов_
...
-   Определите **Сетевое имя** , используя текст «VPN» в этом поле для подсетей VPN.
  ...
  _На рисунке 11 - виртуальной частной сети с помощью сетевого имени_
...
-   Определение **Имени построения** с помощью текста «VPN» в этом поле для подсетей VPN.
  ...
  _На рисунке 12 - виртуальной частной сети с помощью имени построения_
...
2.  Выберите значок шестеренки в правом верхнем углу и выберите **Отправить данные клиента** на странице **Сводка отчеты** .
  ...
  _На рисунке 13 - меню отправка данных клиента_
...
3.  Кроме того Если это первый раз посетив CQD, будет предложено данные телеметрии построения. Вы можете выбрать **Отправить сейчас** , чтобы быстро перейти к странице **Загрузки данных клиента** .
  ...
  _На рисунке 14 - построение баннер отправляемых данных_
...
6.  После выбора, **Дата начала**, выберите **Отправить** Отправка файла CQD. <br><br>Прежде чем отправить файл проверки. Если происходит сбой проверки, сообщение об ошибке отображается разрешения на запрос, что вы правильно файл. На следующем рисунке показана ошибка во время количество столбцов в файле данных неверны.
  ...
  _Рисунок 15 - построение Ошибка передачи данных_
...
_На рисунке 16 - отсутствует стандартный отчет_
...
_Сбои программы установки на рисунке 16 - надежность звукового канала - потока_
...
_На рисунке 18 - звука неполадок с подсети_
...
_На рисунке 19 - скорость потока добавленном_
...
_На рисунке 20 – сбои размещения по подсети_
...
> [!Important]
> VPN-отчеты фильтруются с помощью измерения второй VPN. Это измерение требует, сетевой адаптер VPN должным образом был зарегистрирован как адаптер удаленного доступа.  Don'tt поставщиков VPN надежно используйте этот флаг, а вашей числа могут отличаться в зависимости от поставщика VPN, развернут в вашей организации. Следуйте указаниям, структурированного [ранее в этом руководстве](NEED LINK) по изменению VPN-Подключение отчетов, при необходимости с помощью имени построения или в сети.
...
Несмотря на то, что измерений, используемых может немного отличаться от отчета, каждый отчет будет включать меры для всего потоков, общее низкого уровня потоки, PSR и плохого качества из-за. В таблице отчеты были созданы для каждой области интересов: каналы ТСОП вызов, виртуальных Частных и собрания конференц-связи с двумя участниками. Шаблон управляемой сети включает в себя дополнительные отчеты, чтобы воспользоваться преимуществами отправляется через файл стандартные сведения о местоположении.
...
_В таблице 9 - распространенных участникам высокой PSR_
...
_на рисунке 23 – потоков звука с использованием TCP_
...
Несмотря на то, что требуется использование TCP быть как можно меньше, может появиться немного об использовании TCP, в противном случае работоспособном развертывания. Несмотря на то, что может быть всегда некоторые небольшой объем использования TCP, требуется просмотреть как маленьким использование TCP минимально необходимое для аудио, видео- и VBSS.

Также включены в отчет — скорость потока низкого уровня. Это предоставляет обзор сравнения качества между UDP и TCP для отображения как об использовании TCP влияет на общее качество.
...
Дополнительные сведения об использовании прокси-серверы с Скайп для бизнеса в Интернет и рабочих групп можно [в этой статье](/skypeforbusiness/optimizing-your-network/proxy-servers-for-skype-for-business-online).

## <a name="clients-and-devices"></a>Клиенты и устройства
...
Важной частью поездок высокого качества взаимодействия с пользователем проверка того, что управляемые клиенты используют предоставляются самые последние версии Скайп для бизнеса, помимо того что вспомогательные факторами являются в актуальном состоянии. Это предоставляет несколько преимуществ, между ними. 
...
Также важно учесть и убедитесь, что USB и драйверы звуковых устройств вносятся исправления также. Может быть легко не заметить этих драйверов и не включать их в стратегии управления исправлениями.
...
-   [Файлы для загрузки и обновления Скайп для бизнеса](/SkypeForBusiness/software-updates)
...
Отчеты устройства используются для оценки использование устройства по объему и MOS широкополосном (только со звуком), а также можно найти в соответствующие шаблоны в разделе клиентов и устройств. 
...
_На рисунке 29 - сообщить о устройства (микрофон)_
...
-   [Включение и использование панели мониторинга качества звонков для групп Майкрософт и Скайп для бизнеса в Интернет](/SkypeForBusiness/using-call-quality-in-your-organization/turning-on-and-using-call-quality-dashboard)
...
-   [Настройка вызова аналитики](/SkypeForBusiness/using-call-quality-in-your-organization/set-up-call-analytics)

-   [В чем разница между средством аналитики звонков и панелью мониторинга качества звонков?](/SkypeForBusiness/using-call-quality-in-your-organization/difference-between-call-analytics-and-call-quality-dashboard)

-   [Устранение неполадок с качеством звонков с помощью средства аналитики звонков](/SkypeForBusiness/using-call-quality-in-your-organization/use-call-analytics-to-troubleshoot-poor-call-quality)
