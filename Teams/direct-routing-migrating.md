...
- Телефонная система с локальным подключением КТСОП в Skype для бизнеса Server (для Skype для бизнеса Online)  
- Телефонная система с локальным подключением PSTN с помощью облачного соединителя (для Skype для бизнеса Online)
...
В приведенной ниже таблице показано конечное состояние для пользователя, который настроен для выбранных параметров подключения по протоколу PSTN с помощью телефонной системы. Отображаются только те атрибуты, которые относятся к голосовой связи.
