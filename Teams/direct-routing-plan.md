...
Кроме того прямой маршрутизации поддерживает подстановочные знаки в сети SAN и подстановочный знак должен соответствовать типу стандартных [RFC HTTP через TLS](https://tools.ietf.org/html/rfc2818#section-3.1). Пример будет использоваться *. contoso.com в SAN, в которой будет совпадать sbc.contoso.com SBC полное доменное имя, но не совпадает с sbc.test.contoso.com.
...
## <a name="media-traffic-codecs"></a>Трафика мультимедиа: кодеки

### <a name="leg-between-sbc-and-cloud-media-processor-or-microsoft-teams-client"></a>Ветвь между клиент SBC и облачных мультимедийный процессор или группами Майкрософт.
Применяется к media bypass case и случаев без обхода

Интерфейс прямой маршрутизации на ветвь между пограничного контроллера сеансов и облачных процессор мультимедиа (без сервера-посредника) или между клиентом групп и SBC (Если эта возможность включена сервера-посредника) можно использовать следующие кодеки:
- Non-сервера-посредника (SBC для облачных мультимедиа процессора): SILK, G.711, G.722, G, 729
- Обход сервера-посредника (SBC клиенту группами): SILK, G.711, G.722, G, 729, ОПУСЕ

Вы можете принудительно использовать кодека, определенных на пограничного контроллера сеансов, исключив нежелательный кодеки из предложения.

### <a name="leg-between-microsoft-teams-client--and-cloud-media-processor"></a>Ветвь между клиентом группами Майкрософт и облачных мультимедийный процессор
Применяется к не-сервера-посредника только случае. С помощью потоков мультимедиа сервера-посредника между группами клиента и SBC

На ветвь облачных мультимедиа процессора и клиентских рабочих групп Майкрософт использовать SILK или G.722. Выбор кодека на этом ветвь на основании Microsoft алгоритмов, которые следует принять во внимание несколько параметров. 
