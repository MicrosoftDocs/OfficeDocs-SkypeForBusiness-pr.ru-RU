...
Брандмауэр IP-адресов и портов для групп Майкрософт мультимедиа |Дополнительные сведения см в [Office 365 URL-адреса и диапазоны IP-адресов](https://support.office.com/en-us/article/Office-365-URLs-and-IP-address-ranges-8548a211-3fe7-47cb-abb1-355ea5aa88a2). |
|||
...
Если вы хотите использовать новое доменное имя. К примеру клиент имеет contoso.com как имя домена, зарегистрированное в клиент. Необходимо использовать sbc1.sip.contoso.com. Прежде чем вы можете обеспечить SBC с именем sbc1.sip.contoso.com необходимо зарегистрировать sip.contoso.com имя домена в «Домены» в вашего клиента. Если вы попытаетесь связывания sbc1.sip.contoso.com SBC появится сообщение об ошибке «Не удается используйте домен «sbc1.sip.contoso.com» как он не был настроен для клиента».
После добавления доменное имя необходимо также создать пользователя с user@sip.contoso.com имени участника-пользователя и назначение лицензии «Группы». Может потребоваться до 24 часов для полностью подготовки имя домена, после добавления в «Домены» клиента, пользователя с помощью нового создан и лицензию, назначенную для пользователя. 
