...
- **Без обхода мультимедиа** мультимедиа потоки мультимедиа будут поступать через Залив или Dublin (где развернуты центра обработки данных Майкрософт) и обратно в SBC в Сша. 
...
- **При обходе** мультимедиа мультимедиа хранятся непосредственно между Teams и SBC, как показано на следующей схеме:
...
- Стрелки и числимые значения путей соответствуют Microsoft Teams [потокам зовов](./microsoft-teams-online-call-flows.md).
...
- Прокси-сервер SIP — это компонент, который преобразует сигнал HTTP REST, Teams в SIP.    
...
- Office 365 DoD Дополнительные Office 365 средах государственных [](/office365/servicedescriptions/office-365-platform-service-description/office-365-us-government/office-365-us-government) GCC, GCC High и DoD.
...
Для **sip.pstnhub.microsoft.com,** **sip2.pstnhub.microsoft.com** и **sip3.pstnhub.microsoft.com** будут разрешены IP-адреса из следующих подсетей:
- 52.112.0.0/14
- 52.120.0.0/14
...
Требования к портам одинаковы для всех сред Office 365, где предлагается прямая маршрутная маршрутия:
- Microsoft 365 или Office 365
- Office 365 GCC
- Office 365 GCC высокая
- Office 365 DoD
...
| Трафика | От | До | Исходный порт | Конечный порт|
| :-------- | :-------- |:-----------|:--------|:---------|
| UDP/SRTP | Клиент | SBC | 50000-50019| Определено в SBC |
| UDP/SRTP | SBC | Клиент | Определено в SBC | 50000-50019  |
...
Обе связи могут указать на один и тот же SBC с одинаковым общедоступным IP-адресом. Сигнальные порты TLS на SBC должны быть другими, как показано на приведенной ниже схеме. Обратите внимание, что сертификат должен поддерживать обе стороны. В САН у вас должны быть два **имени (sbc1.contoso.com** **и sbc2.contoso.com**) или сертификат с подданными знаками.
