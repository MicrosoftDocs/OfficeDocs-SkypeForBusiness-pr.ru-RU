...
- **Без обхода** мультимедиа мультимедиа будут переходить через Дублин или Майкрософт (где развернуты центр обработки данных Майкрософт) и обратно в SBC в Facebook. 
...
Обход мультимедиа использует протоколы под названием "Интерактивная установка подключения (ICE) на клиенте Teams и ICE lite на SBC." Эти протоколы обеспечивают прямую маршрутику, чтобы использовать самый прямой путь к мультимедиа для обеспечения оптимального качества. ICE и ICE Lite — это стандарты WebRTC. Подробные сведения об этих протоколах см. в RFC 5245.


## <a name="call-flow-and-firewall-planning"></a>Планирование потока вызовов и брандмауэра

Планирование потока присоединения к звонкам и брандмауэра зависит от того, есть ли у пользователя прямой доступ к общедоступным IP-адресам SBC и находится ли пользователь внутри сети или за ее пределами.

### <a name="call-flow-if-the-user-has-direct-access-to-the-public-ip-address-of-the-sbc"></a>Поток присоединения к звонкам, если у пользователя есть прямой доступ к общедоступным IP-адресу SBC
...
> [!div class="mx-imgBorder"]
> ![Поток зовов с включенным обходом мультимедиа, клиент внутренний](media/direct-routing-media-bypass-3.png)


### <a name="call-flow-if-the-user-does-not-have-access-to-the-public-ip-address-of-the-sbc"></a>Поток присоединения к звонкам, если у пользователя нет доступа к общедоступным IP-адресу SBC
...
- Стрелки и числимые значения путей соответствуют статье о потоках вызовов [Microsoft Teams.](./microsoft-teams-online-call-flows.md)
...
На следующей схеме показаны два потока вызова: один с включенным обходом мультимедиа, а второй с отключенным обходом мультимедиа. Обратите внимание на то, что схема только иллюстрирует трафик, направляющийся с конечных пользователей или направляющийся в него.  
...
|    | Процессоры мультимедиа | Transport Relays|
| :--------------|:---------------|:------------|
В пути мультимедиа для вызовов без обхода для конечных пользователей | Всегда | Если клиенту не удается напрямую связаться с медиапереработчиком | 
В пути мультимедиа для обойденных звонков для конечных пользователей | Никогда | Если клиенту не удается связаться с SBC по адресу общего IP-адреса | 
...
Количество экземпляров по всему миру и его местонахождение | 10 итогов: 2 в восточной и западной части США; 2 в Ирландии и Ирландии; 2 в Гонконге и Сингапуре; 2 в Японии; 2 в Восточной и Юго-Восточной Австралии | Несколько
...
Для сигнального управления SIP требования к FQDN и брандмауэру одинаковы, как и для дел, не влияющих на обход. 

Прямая маршрутия предоставляется в следующих средах Microsoft 365 или Office 365:
- Microsoft 365 или Office 365
- Office 365 GCC
- Office 365 GCC High
- Office 365 DoD Подробнее об средах [Office 365](/office365/servicedescriptions/office-365-platform-service-description/office-365-us-government/office-365-us-government) и государственных органов США, таких как GCC, GCC High и DoD.
...
Для сигнального трафика необходимо открыть в брандмауэре порты для всех этих IP-адресов, чтобы разрешить входящий и исходяющий трафик, который будет входящие и исходяющие сообщения с адресов для сигнального трафика.  Если брандмауэр поддерживает имена DNS, доменные sip.pstnhub.dod.teams.microsoft.us будут разрешать их все IP-адреса. 
...
Для сигнального трафика необходимо открыть в брандмауэре порты для всех этих IP-адресов, чтобы разрешить входящий и исходяющий трафик, который будет входящие и исходяющие сообщения с адресов для сигнального трафика.  Если брандмауэр поддерживает имена DNS, доменные sip.pstnhub.gov.teams.microsoft.us будут разрешать их все IP-адреса. 
...
> 
> - v4, который может работать только с диапазоном от 50 000 до 59 999
> 
...
Диапазон портов процессоров мультимедиа (применимо для всех сред) отображается в следующей таблице:
...
Пользователи с обойденной медиауловиной связи | 20 | sbc2.contoso.com:5061 | false | 
