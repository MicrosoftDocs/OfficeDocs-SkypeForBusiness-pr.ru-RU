...
Без обхода мультимедиа, когда клиент совершает или принимает звонок, сигнальный и поток мультимедиа между SBC, системой Телефон (Майкрософт) и клиентом Teams, как показано на следующей схеме:
...
Обход мультимедиа использует протоколы,называемые интерактивным подключением (ICE) на клиенте Teams и ICE lite на SBC. Эти протоколы позволяют прямой маршрутике использовать самый прямой маршрут мультимедиа для оптимального качества. ICE и ICE Lite являются стандартами WebRTC. Подробные сведения об этих протоколах см. в описании протокола RFC 5245.
...
- Стрелки и числимые значения путей соответствуют Microsoft Teams [потокам вызовов.](./microsoft-teams-online-call-flows.md)
...
На следующей схеме показан поток зова, когда включен обход мультимедиа, клиент является внешним и не может получить доступ к общедоступным IP-адресам контроллера границы сеанса (мультимедиа передаются Teams Транспорта Ретранслятор).
...
- Процессор мультимедиа — это B2BUA. Это означает, что он может изменять кодеки (например, КОДЕКОМ с клиента Teams на MP и G.711 между MP и SBC).
...
### <a name="use-of-teams-media-processors-if-trunk-is-configured-for-media-bypass"></a>Использование процессоров Teams мультимедиа, если для обхода мультимедиа настроена та же возможность
...
- Office 365 DoD Подробнее о [средах](/office365/servicedescriptions/office-365-platform-service-description/office-365-us-government/office-365-us-government) Office 365 и государственных государственных GCC, GCC Высокая и DoD.
...
Чтобы разрешить входящий и исходяющий трафик с адресов для сигнального трафика, необходимо открыть в брандмауэре порты для всех этих диапазонов IP-адресов.  Если брандмауэр поддерживает имена DNS, доменная sip.pstnhub.dod.teams.microsoft.us ко всем этим IP-подсетям. 
...
Трафик мультимедиа передается между клиентом SBC и клиентом Teams, если прямое подключение доступно, или через Teams, если клиент не может связаться с SBC с использованием общего IP-адреса.
...
Диапазон портов ретрансляторов Teams транспорта (применимо для всех сред) показан в таблице ниже.
