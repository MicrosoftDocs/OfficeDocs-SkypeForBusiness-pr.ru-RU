...
Обход мультимедиа использует протоколы под названием "Интерактивные соединения (ICE) на клиенте Teams и ICE lite на SBC. Эти протоколы позволяют прямой маршрутике использовать самый прямой маршрут мультимедиа для оптимального качества. ICE и ICE Lite являются стандартами WebRTC. Подробные сведения об этих протоколах см. в описании протокола RFC 5245.
...
Предположим, пользователь является внешним, а администратор клиента решил не открывать общедоступный IP-адрес SBC для всех пользователей в Интернете, а только в Microsoft Cloud. Внутренние компоненты трафика могут перетекать через Teams ретрансляторов транспорта. Примите во внимание следующее:
...
- Стрелки и числимые значения путей соответствуют Microsoft Teams [потокам](./microsoft-teams-online-call-flows.md) зовов.
...
- Прокси-сервер SIP — это компонент, который преобразует сигнал http REST, используемый Teams в SIP.    
...
- Процессор мультимедиа — это B2BUA, что означает, что он может изменять кодеки (например, КОДЕКОМ с клиента Teams на MP и G.711 между MP и SBC).
...
- Office 365 DoD Подробнее о средах [Office 365](/office365/servicedescriptions/office-365-platform-service-description/office-365-us-government/office-365-us-government) государственных GCC, GCC High и DoD.
...
Диапазон портов Teams ретрансляторов транспорта (применимо для всех сред) отображается в таблице ниже.
