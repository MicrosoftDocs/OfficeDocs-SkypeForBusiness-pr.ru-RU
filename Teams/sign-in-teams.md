...
Современная проверка подлинности — это процесс, с помощью которого Teams узнает, что пользователи уже ввели свои учетные данные (например, рабочий адрес электронной почты и пароль) в другом месте, и им не требуется вводить их повторно, чтобы запустить приложение. Интерфейс зависит от нескольких факторов, например от того, используется Windows или компьютер Mac. Он также отличается в зависимости от того, какая проверка подлинности включена в вашей организации: однофакторная или многофакторная. При многофакторной проверке подлинности обычно используется проверка учетных данных с помощью телефона, предоставления уникального кода, ввода ПИН-кода или представления отпечатка пальца. Ниже приводится краткое описание каждого сценария современной проверки подлинности.
...
> [!NOTE]
>1. Эта политика ограничивает только вход. Она не ограничивает возможность пользователей быть приглашенными в качестве гостей в другие клиенты Azure AD или переключаться на другие клиенты.
...
**Политики для Windows** Файлы административных шаблонов (ADMX/ADML) доступны в [Центре загрузки](https://www.microsoft.com/download/details.aspx?id=49030). Кроме того, вы можете вручную настроить разделы в реестре Windows:
