...
## <a name="global-sign-in-and-sign-out"></a>Глобальный вход и выход

Приложение Teams для Android теперь поддерживает глобальный вход и выход, чтобы обеспечивать беспроблемный вход и выход из интерфейса для сотрудников без компьютеров. Сотрудник может выбрать устройство в пуле общих устройств и выполнить единый вход, чтобы "оно принадлежало ему" в течение смены. В конце смены у него должна быть возможность выйти, чтобы выполнить глобальный выход на устройстве. При этом будут удалены все личные и корпоративные сведения с устройства, чтобы его можно было вернуть в пул устройств. Чтобы воспользоваться этой возможностью, устройство должно находиться в общем режиме. Сведения о том, как настроить общее устройство, см. в статье [Использование режима общего устройства в Android](https://docs.microsoft.com/azure/active-directory/develop/tutorial-v2-shared-device-mode#set-up-an-android-device-in-shared-mode).

Интерфейс входа похож на стандартный интерфейс входа Teams, а интерфейс выхода похож на следующие два изображения:
...
Современная проверка подлинности доступна для всех организаций, использующих Teams. Если пользователи не могут завершить процесс, возможно, что-то не так с вашим доменом либо рабочей или учебной учетной записью Майкрософт вашей организации.
