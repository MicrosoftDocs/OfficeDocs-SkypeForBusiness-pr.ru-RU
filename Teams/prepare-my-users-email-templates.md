...
ms.reviewer: rowille
audience: admin
description: Используйте шаблоны для отправки сообщений электронной почты пользователям при реализации облачных голосовых возможностей в Teams или Skype для бизнеса.
...
---
# <a name="facilitate-user-adoption-of-your-new-online-phone-system"></a><span data-ttu-id="d06cf-103">Упрощение внедрения новой телефонной системы через Интернет</span><span class="sxs-lookup"><span data-stu-id="d06cf-103">Facilitate user adoption of your new online phone system</span></span>

<span data-ttu-id="d06cf-104">Телефон может быть жизненной линией пользователя.</span><span class="sxs-lookup"><span data-stu-id="d06cf-104">A phone can be a user's lifeline.</span></span> <span data-ttu-id="d06cf-105">Переключение с традиционной телефонной системы на телефонную систему в Сети представляет собой существенное изменение способа работы пользователей и может отрицательно сказаться на производительности.</span><span class="sxs-lookup"><span data-stu-id="d06cf-105">Switching from a traditional phone system to an online phone system represents a significant change in the way users work, and can negatively affect productivity.</span></span> <span data-ttu-id="d06cf-106">За время, необходимое для информирования, руководства и поддержки пользователей с помощью этого изменения, можно обеспечить положительное впечатление пользователей и максимально увеличить возврат инвестиций.</span><span class="sxs-lookup"><span data-stu-id="d06cf-106">Taking time to inform, guide, and support users through this change can help ensure a positive user experience and maximize your return on investment.</span></span>

<span data-ttu-id="d06cf-107">Эта статья содержит настраиваемые шаблоны сообщений электронной почты для информирования пользователей о новой телефонной системе.</span><span class="sxs-lookup"><span data-stu-id="d06cf-107">This article contains customizable email templates to help inform users about their new phone system.</span></span> <span data-ttu-id="d06cf-108">Настройте и отправьте соответствующие сообщения электронной почты для соответствующего развертывания голосовой связи.</span><span class="sxs-lookup"><span data-stu-id="d06cf-108">Customize and send the appropriate emails to align with your specific voice deployment.</span></span> <span data-ttu-id="d06cf-109">Мы использовали квадратные скобки, чтобы указать разделы, которые можно легко персонализировать, но рекомендуем добавлять сообщения для конкретной организации, чтобы сделать объявление наиболее актуальным для пользователей.</span><span class="sxs-lookup"><span data-stu-id="d06cf-109">We've used brackets to indicate sections for easy personalization, but we recommend that you add organization-specific messaging to make the announcement most relevant for your users.</span></span> <span data-ttu-id="d06cf-110">Если применимо для развертывания, замените все ссылки на Teams на "Skype для бизнеса".</span><span class="sxs-lookup"><span data-stu-id="d06cf-110">If applicable for your deployment, replace all references to "Teams" with "Skype for Business."</span></span>

## <a name="email-number-1-send-three-weeks-before-launch"></a><span data-ttu-id="d06cf-111">Номер электронной почты 1. Отправка за три недели до запуска</span><span class="sxs-lookup"><span data-stu-id="d06cf-111">Email number 1: Send three weeks before launch</span></span>

<span data-ttu-id="d06cf-112">Строка темы:</span><span class="sxs-lookup"><span data-stu-id="d06cf-112">Subject line:</span></span>

```
IMPORTANT ANNOUNCEMENT about your office desk phone
```

<span data-ttu-id="d06cf-113">Тело сообщения электронной почты:</span><span class="sxs-lookup"><span data-stu-id="d06cf-113">Body of email:</span></span>
...
## <a name="email-number-2-send-one-week-before-launch"></a><span data-ttu-id="d06cf-114">Номер электронной почты 2. Отправка за неделю до запуска</span><span class="sxs-lookup"><span data-stu-id="d06cf-114">Email number 2: Send one week before launch</span></span>

<span data-ttu-id="d06cf-115">Строка темы:</span><span class="sxs-lookup"><span data-stu-id="d06cf-115">Subject line:</span></span>

```
ACTION REQUESTED: Are you ready for your new phone system?
```

<span data-ttu-id="d06cf-116">Сообщение электронной почты:</span><span class="sxs-lookup"><span data-stu-id="d06cf-116">Body of email:</span></span>
...
## <a name="email-number-3-send-on-launch-day"></a><span data-ttu-id="d06cf-117">Номер электронной почты 3. Отправка в день запуска</span><span class="sxs-lookup"><span data-stu-id="d06cf-117">Email number 3: Send on launch day</span></span>

<span data-ttu-id="d06cf-118">Строка темы:</span><span class="sxs-lookup"><span data-stu-id="d06cf-118">Subject line:</span></span>

```
IMPORTANT NOTICE: Your new phone system is live
```

<span data-ttu-id="d06cf-119">Сообщение электронной почты:</span><span class="sxs-lookup"><span data-stu-id="d06cf-119">Body of email:</span></span>
...
## <a name="email-number-4-send-two-weeks-after-launch"></a><span data-ttu-id="d06cf-120">Номер электронной почты 4. Отправка через две недели после запуска</span><span class="sxs-lookup"><span data-stu-id="d06cf-120">Email number 4: Send two weeks after launch</span></span>

<span data-ttu-id="d06cf-121">Строка темы:</span><span class="sxs-lookup"><span data-stu-id="d06cf-121">Subject line:</span></span>

```
FEEDBACK REQUESTED: Your new phone system
```

<span data-ttu-id="d06cf-122">Тело сообщения электронной почты:</span><span class="sxs-lookup"><span data-stu-id="d06cf-122">Body of email:</span></span>
