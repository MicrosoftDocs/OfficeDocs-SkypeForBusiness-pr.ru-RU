...
Метки конфиденциальности в настоящее время неподтверчены в командах класса для клиентов, использующих skUs Teams для образования. Дополнительные информацию о лицензировании см. в описании [службы Microsoft Teams.](/office365/servicedescriptions/teams-service-description)
...
> [!NOTE]
> Если вы используете иерархические метки родительского ребенка, такие как "Конфиденциально\Финансы", то в заглавном канале будет показана только метка родительского ребенка.
...
- **Метки конфиденциальности не поддерживаются API Teams Graph, командлетами PowerShell и шаблонами**
    ...
    Пользователи не смогут указывать метки конфиденциальности при создании команд непосредственно с помощью API Teams Graph, командлетов Teams PowerShell и шаблонов Teams. Однако современные API Graph Graph и cmdlets PowerShell позволяют создавать группы с метами. Таким образом, пользователи могут сначала создавать группы с метами с помощью API Graph Groups или командлетов PowerShell, а затем преобразовывать эти группы в Teams.
