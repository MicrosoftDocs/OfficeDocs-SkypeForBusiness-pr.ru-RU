...
ms.locfileid: "60367511"
---
# <a name="set-up-cloud-voicemail"></a>Настройка облачной голосовой почты

Эта статья адресовна администратору Microsoft 365 или Office 365, как [](/microsoft-365/admin/add-users/about-admin-roles) описано в статье Роли администраторов, которые хотят настроить облачная голосовая почта для всех в компании.
...
Ниже приводится информация о настройке облачная голосовая почта для работы с пользователями, которые находятся в сети телефонная система но имеют почтовый ящик Exchange Server.

1. Сообщения голосовой почты доставляются в почтовый Exchange пользователей через SMTP, маршрутный через Exchange Online Protection. Чтобы обеспечить успешный доставку этих сообщений, убедитесь, Exchange соединители правильно настроены между серверами Exchange и Exchange Online Protection; [Настройте почтовые Flow с помощью соедините Flow.](/exchange/mail-flow-best-practices/use-connectors-to-configure-mail-flow/use-connectors-to-configure-mail-flow)
...
Дополнительные сведения о шифровании сообщений см. в [теме Шифрование электронной почты.](/microsoft-365/compliance/email-encryption)

> [!NOTE]
> Шифрование применяется к голосовой почте только в том случае, если звонок был от внутреннего пользователя. Если звонок был от внешнего пользователя, голосовая почта не будет зашифрована.
...
4. Выберите **+** **Добавить**, а затем **выберите Шифрование сообщений Office 365 и защиту прав к сообщениям**.
...
6. В **области Сделать следующее** выберите Применить шифрование сообщений Office 365 и защиту **прав** к сообщению с помощью и выберите Выберите **один**. В **RMS шаблона** выберите Не **переадваровываться**. Выберите **ОК,** а затем **Сохранить**.
    ...
    >
    > - [Настройка новых возможностей шифрования сообщений](/microsoft-365/compliance/set-up-new-message-encryption-capabilities)
...
Ниже представлены сведения об управлении настройками голосовой почты, а также другими функциями звонков в Teams.
