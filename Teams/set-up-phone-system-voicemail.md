...
Для Teams пользователей облачная голосовая почта настраивается и подготавливается автоматически. *Лицензия Телефонная система Microsoft Teams не требуется для облачная голосовая почта.*
...
(Обратите внимание, что  ->  конечные пользователи также могут настроить эти параметры в клиенте Teams, перейдя к Параметры **Calls** -> **Configure Voicemail**.)
...
Вы можете контролировать, разрешена ли маршрутизация вызовов облачная голосовая почта для Teams пользователей с помощью командлета Set-CsTeamsCallingPolicy с параметром AllowVoicemail. Дополнительные сведения см [. в разделе Set-CsTeamsCallingPolicy](/powershell/module/skype/set-csteamscallingpolicy).
...
- Для **получения дополнительных параметров электронной** почты выберите "Применить шифрование к электронной почте, полученной за пределами **организации",** и укажите Rights Management владельца.
