...
### <a name="required-permissions-for-bots-and-messaging-extensions"></a>Необходимые разрешения для ботов и расширений для обмена сообщениями

* Бот может получать сообщения от пользователей и отвечать на них. Некоторые боты отправляют только сообщения. Они называются ботами только для уведомлений, но термин не относится к тому, что разрешено или запрещено делать боту. Это означает, что бот не предлагает взаимодействие с беседами. Teams использует это поле для отключения функциональных возможностей в пользовательском интерфейсе, которые обычно включаются. Бот не ограничен тем, что ему разрешено делать по сравнению с ботами, которые предоставляют взаимодействие с беседой.
* После отправки пользователем сообщения боту бот может в любое время отправлять ей прямые или упреждающие сообщения.
* Бот, добавленный в команды, может получить список имен и идентификаторов каналов в команде.

### <a name="optional-permissions-for-bots-and-messaging-extensions"></a>Дополнительные разрешения для ботов и расширений для обмена сообщениями

* При использовании в канале бот приложения может получить доступ к основным сведениям об удостоверениях участников команды (имя, фамилия, имя участника-пользователя [имя участника-пользователя], адрес электронной почты). При использовании в личном или групповом чате, бот может получить доступ к таким же сведениям для этих пользователей.
...
* Следующие разрешения не являются явными, но подразумеваются возможностью получать сообщения и отвечать на них, а также области, в которых можно использовать ботов.
...
### <a name="considerations-for-bots-and-messaging-extensions"></a>Рекомендации по ботам и расширениям для обмена сообщениями
...
### <a name="considerations-for-tabs"></a>Рекомендации по вкладкам
...
Исходящие веб-перехватчики создаются владельцами или участниками группы. Они не являются возможностями приложений Teams, эти сведения включены для полноты.

### <a name="required-permissions-for-outgoing-webhooks"></a>Необходимые разрешения для исходящих веб-перехватчиков

RECEIVE_MESSAGE, REPLYTO_MESSAGE. Может получать сообщения от пользователей и отвечать на них.

### <a name="considerations-for-outgoing-webhooks"></a>Рекомендации по исходящим веб-перехватчикам
