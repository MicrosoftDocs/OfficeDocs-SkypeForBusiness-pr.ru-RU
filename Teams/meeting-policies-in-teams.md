...
Использование существующей политики AllowChannelMeetingScheduling для управления типами событий, которые можно создавать в календарях канала группы. Это политика для каждого пользователя, которая применяется до начала собрания. Этот параметр определяет, могут ли пользователи запланировать собрание в канале Teams. По умолчанию используется значение True.

Если этот параметр отключен, пользователи не смогут создавать новые собрания каналов. Однако существующие собрания каналов можно редактировать с помощью организатора события.
...
Это политика для организатора, позволяющая проводить собрания с телефонным подключением с незаполненными абонентами. Этот параметр определяет, могут ли пользователи с телефонным подключением присоединиться к собранию без проверки подлинности пользователя в Организации в присутствии. По умолчанию используется значение false, которое означает, что пользователи с телефонным подключением будут ждать в зале ожидания, пока пользователь, прошедший проверку подлинности, не присоединится к собранию.

> [!NOTE]
> Если вы присоединяетесь к собранию и помещаете его в зал "ложь", пользователь организации должен присоединиться к собранию с помощью клиента Teams, чтобы присвоить его пользователю в зале ожидания. У пользователей нет доступных для вас элементов управления "зал ожидания".
...
Это политика для организатора. Этот параметр определяет, будут ли пользователи, находящиеся на телефоне, присоединиться к собранию напрямую или ждать в зале ожидания, независимо от того, кто **автоматически согласен** . Значение по умолчанию — False. Если задано значение "ложь", пользователи с телефонным подключением будут ждать в зале ожидания, пока пользователь организации не присоединится к собранию с помощью клиента Teams и не будет его признать. Если установлено значение true, пользователи с телефонным подключением будут автоматически присоединяться к собранию, когда пользователь Организации присоединится к собранию.
