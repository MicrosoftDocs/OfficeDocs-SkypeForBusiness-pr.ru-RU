...
Лина, даже если она является организатором, не может выполнять запись, так как ее политика отключена. Оксана, у которой параметр политики включен, может записывать собрания, в том числе организованные Линой. Если Оксана организует собрание, она сможет записать его. Однако Лина, у которой параметр политики отключен, и Андрей, который является внешним пользователем, не могут записать это собрание.
...
> [!NOTE]
> Эта функция по-прежнему находится в разработке. Демонстрация экрана — это политика для участника, но на нее могут влиять параметры демонстрации экрана организатора, как описано в этом разделе.

Этот параметр определяет, разрешена ли демонстрация рабочего стола или окна на собрании пользователя. Участники собрания, которым не назначены политики (например, анонимные, гостевые, B2B и федеративные участники), наследуют политику организатора собрания.
...
Это политика для пользователя. Этот параметр определяет, может ли пользователь демонстрировать доску на собрании. Внешние пользователи, включая анонимных, B2B и федеративных, наследуют политику организатора собрания.
...
> [!NOTE]
> В параметрах собрания эта настройка называется "Кто может миновать «зал ожидания»". Если вы измените параметр по умолчанию для любого пользователя, он будет применяться ко всем новым собраниям, организуемым этим пользователем, и к любым предыдущим собраниям, в которых пользователь не изменил параметры.
...
|**Люди в моей организации и гости**    |Авторизованные пользователи из организации, включая гостевых пользователей, присоединяются к собранию напрямую, без помещения в "зал ожидания". Пользователи из доверенных организаций и анонимные пользователи помещаются в "зал ожидания". Это параметр по умолчанию.           |
...
|**Люди в моей организации**  |Авторизованные пользователи из организации, за исключением гостевых пользователей, присоединяются к собранию напрямую, без помещения в "зал ожидания". Гости и пользователи из доверенных организаций, а также анонимные пользователи помещаются в "зал ожидания".|
