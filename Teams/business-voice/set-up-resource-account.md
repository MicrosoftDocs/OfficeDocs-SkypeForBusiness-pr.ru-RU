...
ms.locfileid: "58630243"
---
# <a name="step-4-set-up-a-business-voice-resource-account"></a>Шаг 4. Настройка учетной записи ресурса Business Voice

Учетные записи ресурсов не назначены конкретному пользователю. Вместо этого учетные записи ресурсов, которые используют бесплатную виртуальную лицензию пользователя, используются устройствами и службами в Microsoft 365. В Microsoft Teams учетным записям ресурсов назначены номера телефонов, которые затем связываются с автосчетами и очередями звонков.
...
В разделах этой статьи поется о том, как настроить учетную запись ресурса и назначить для нее номер телефона. Позже вы соберем учетную запись ресурса с автозачетом.
...
3. Прокрутите список до конца, чтобы найти **Microsoft 365 телефонная система — лицензия виртуального** пользователя. Выберите **Сведения**, а затем **Купить**.
...
Получив лицензию на Microsoft 365 телефонная система *виртуальный* пользователь, вы можете создать учетную запись ресурса.

![Снимок экрана: добавление пользовательского интерфейса учетной записи ресурса](../media/resource-account-add.png)

1. Откройте Центр Microsoft Teams администрирования и войдите с пользователем, который является глобальным администратором (обычно это учетная запись, которую вы использовали для регистрации в Microsoft 365).
