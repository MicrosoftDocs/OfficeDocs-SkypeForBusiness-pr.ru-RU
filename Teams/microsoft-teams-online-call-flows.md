...
В этой статье описано Teams как Microsoft 365 или Office 365 в различных топологиях. Кроме того, в нем описываются уникальные Teams, которые используются для одноранговой связи с мультимедиа. В документе описаны эти потоки, их назначение, их происхождение и прекращение в сети. В этой статье предполагается следующее:

- Flow X используется локальной службой для связи с Microsoft 365 или Office 365 в облаке. Она происходит из сети клиента и завершается как конечная точка в Microsoft 365 или Office 365.

- Flow Y используется локального клиента для связи со службой в Интернете Microsoft 365 или Office 365, от которую она зависит. Оно происходит из сети клиента и завершается как конечная точка в Интернете.

В этой статье подробно описывается следующая информация:

- **Фон .** Предоставляет фоновую информацию, например о сетях, которые могут проходить потоки, типах трафика, рекомендациях по подключению из клиентской сети к конечным точкам службы Microsoft 365 или Office 365, взаимодействие со сторонними компонентами, а также принципы, используемые Teams для выбора потоков мультимедиа.
...
**Интернет**. Это сетевой сегмент, который является частью общей сети, который будет использоваться пользователями, подключающихся к Microsoft 365 или Office 365 из-за пределов клиентской сети. Он также используется некоторым трафиком из клиентской сети в Microsoft 365 или Office 365.
...
Teams требуется подключение [к Интернету.](/office365/enterprise/assessing-network-connectivity) Teams URL-адреса и диапазоны IP-адресов конечных точек перечислены в Office 365 [URL-адреса и диапазоны IP-адресов.](/office365/enterprise/urls-and-ip-address-ranges) (Обратите внимание, что требуется открытое подключение к портам TCP 80 и 443, а также к портам UDP 3478–3481.) Кроме того, Teams зависит от Skype для бизнеса Online, которая также должна быть подключена к Интернету.

Teams подключения к потокам мультимедиа реализовано с помощью стандартных процедур по подключению интерактивных подключений (ICE) IETF.

### <a name="interoperability-restrictions"></a>Ограничения на интероперируемость

**Сторонние ретрансляторы мультимедиа.** Поток Teams (то есть одна из конечных точек мультимедиа — Teams) может проходить только через Teams или Skype для бизнеса ретрансляторы мультимедиа. Не поддерживается interoperability с ретранслятором сторонних мультимедиа. (Обратите внимание, что сторонний SBC на границе с ТСОП должен прервать поток RTP/RTCP, защищенный с помощью SRTP, а не ретранслировать его на следующий переход.)

**Прокси-серверы сторонних SIP.** Диалоговое Teams сигнального SIP со сторонним SBC и /или шлюзом может проходить через Teams или Skype для бизнеса прокси-Skype для бизнеса SIP. Поддержка связи с прокси-сервером SIP сторонних сторон не поддерживается.

**стороне B2BUA (или SBC).** Поток Teams потока мультимедиа в STN и из него прерывается сторонним SBC. Однако не поддерживается interopererability с сторонним SBC в сети Teams (где сторонний SBC является медианой Teams или Skype для бизнеса конечных точек).

### <a name="technologies-that-are-not-recommended-with-microsoft-teams"></a>Технологии, не рекомендуемые для Microsoft Teams

**VPN network**. Не рекомендуется использовать трафик мультимедиа (или поток 2'). Vpn-клиент должен использовать раздельное tunneling и Teams трафик мультимедиа, как любой внешний не VPN-пользователь, как указано в разделе Включение мультимедиа Lync для обхода [VPN-канала.](https://techcommunity.microsoft.com/t5/Skype-for-Business-Blog/Enabling-Lync-Media-to-Bypass-a-VPN-Tunnel/ba-p/620210)

> [!NOTE]
> Хотя название указывает на Lync, оно также применимо Teams Lync.

**Фигураторы пакетов**. Для трафика мультимедиа не рекомендуется использовать любые виды устройств для работы с пакетами, их проверки или фигуры пакетов Teams что может привести к существенному ухудшению качества.
...
Эта топология используется клиентами, которые используют Teams из облака без локального развертывания, например Skype для бизнеса Server или телефонная система Direct Routing. Кроме того, интерфейс для Microsoft 365 или Office 365 будет проходить через Интернет без Azure Express Route.
...
- телефонная система Прямая маршрутная маршрутия (для подключения по ДНР) описана в Teams с топологией "Прямая **маршрутия".**
- Express Route описан в Teams **с помощью оптимизации Express Route.**

**Flow описаний:**

- **Flow 2.** Представляет поток, инициированный пользователем в клиентской сети, в Интернет в рамках его Teams работы. Примерами этих потоков являются DNS и одноранговая мультимедиа.
- **Flow 2'** — представляет поток, инициированный удаленной мобильной сетью Teams, с VPN в клиентской сети.
- **Flow 3.** Представляет поток, инициированный удаленной мобильной Teams для Microsoft 365 или Office 365/Teams конечных точек.
- **Flow 4.** Представляет поток, инициированный пользователем в клиентской сети для Microsoft 365 конечных точек Office 365/Teams.
...
1. Teams Пользователь A отправляет "приглашение" с соиследим, используя поток 4 для Microsoft 365 или Office 365.
...
1. Teams Пользователь Б отправляет "ответ" со словами ICE с помощью потока 4, который перенаправлется Teams пользователю A с помощью Flow 4.
...
Мультимедиа является двенаправленным. Направление потока 5 указывает на то, что одна из сторон инициирует связь с точки зрения подключения в соответствии со всеми потоками в этом документе. В этом случае не имеет значения, в каком направлении используется, так как обе конечные точки находятся в клиентской сети.
...
*Рисунок 3. Клиентская сеть для внешнего пользователя (мультимедиа, ретранслироваться Teams транспорта)*

На шаге 7 выбраны поток 4 из клиентской сети в Microsoft 365 или Office 365 и 3, от удаленного мобильного Teams до Microsoft 365 или Office 365. Эти потоки передаются Teams ретранслятором транспорта Microsoft 365 или Office 365.
...
- Прямые передачи мультимедиа удаленному мобильному пользователю (не передаются через Microsoft 365 или Office 365) необязательны. Другими словами, клиент может заблокировать этот путь, чтобы обеспечить передачу медиатрансляции через ретранслятор транспорта в Microsoft 365 или Office 365.
...
*Рисунок 5. ПОЛЬЗОВАТЕЛЬ VPN для внутреннего пользователя (мультимедиа, ретранслироваться Teams транспорта)*

Сигнальный сигнал между VPN и клиентской сетью используется поток 2'. Сигнальный сигнал между клиентской сетью Microsoft 365 или Office 365 используется поток 4. Однако мультимедиа не передаются через VPN и маршрутются с помощью потоков 3 и 4–Teams ретрансляции мультимедиа Microsoft 365 или Office 365.
...
Сигнальный сигнал между VPN и клиентской сетью используется поток 2'. Сигнальный сигнал между клиентской сетью Microsoft 365 или Office 365 используется поток 4. Однако мультимедиа пропускаются через VPN и маршрутются с помощью потока 2 из клиентской сети в Интернет.

Мультимедиа является двенаправленным. Направление потока 2 к удаленному мобильному пользователю указывает на то, что одна из сторон инициирует связь с точки зрения подключения.

**VPN-пользователь для внешнего пользователя (прямой мультимедиа)**

[![Microsoft Teams потоки вызовов, рисунок 07](media/microsoft-teams-online-call-flows-figure07-thumbnail.png)](media/microsoft-teams-online-call-flows-figure07.png)

*Рисунок 7. Пользователь VPN для внешнего пользователя (прямой мультимедиа)*

Сигнальный сигнал между пользователем VPN и клиентской сетью используется с помощью потока 2' и с помощью потока 4 для Microsoft 365 или Office 365. Однако мультимедиа обошел VPN и маршрутизируется с помощью потока 6.
...
Microsoft 365 и Office 365 есть телефонная система, которая позволяет звонить и принимать звонки из телефонной сети общего звонков .... Если с помощью плана звонков телефонная система ЗВОНКОВ соединяемая связь, особые требования к подключению для этого случая использования не предъявляются. (Если вы хотите подключить свою линию ННР к Microsoft 365 или Office 365, вы можете использовать телефонная система прямую маршрутику.)
...
Сервер аудио- и видео- и экранных услуг (VBSS) является частью Microsoft 365 и Office 365. Он имеет общедоступный IP-адрес, который должен быть заенен в клиентской сети, а для связи с клиентом Вологодского облака. У каждого клиента или конечной точки должна быть возможность подключения к серверу веб-связи.

Внутренние клиенты получают локальные, адаптивные и ретрансляторы так же, как описано для звонков по одному. Клиенты отправят этих кандидатов на сервер в приглашении. Сервер веб-связи не использует ретрансляцию, так как у него есть общедоступный IP-адрес, поэтому он отвечает с локальным IP-адресом. Клиент и сервер для подключения проверяют подключение так же, как описано для звонков "один к одному".
...
**Мультимедиа, ретрансл Teams ретранслятором транспорта в Microsoft 365 или Office 365**
...
*Рисунок 10. Мультимедиа, ретранслятором Teams транспорта в Office 365*
...
- Teams развертывается рядом с Skype для бизнеса Online, поэтому клиенты отображаются как "пользователь Teams/SFB".
...
#### <a name="use-case-teams-to-skype-for-business-one-to-one"></a>Пример использования: Teams Skype для бизнеса "один к одному"
...
Сигнальный между Teams и Skype для бизнеса мостом шлюза. Однако мультимедиа перенаправяются напрямую через одноранговую сеть клиента с помощью потока 5.
...
*Рисунок 15. Гибридная клиентская сеть с внешним Skype для бизнеса, ретранслтором Office 365*
...
- Мультимедиа передаются через ретранслятор Teams в клиентскую сеть через поток 4.
...
*Рисунок 16. Гибридная клиентская сеть с внешним Skype для бизнеса, ретранслтором локального edge*
...
Эта топология включает Teams с телефонная система прямой маршрутией.

Прямая маршрутия позволяет использовать стороннего поставщика услуг телефонной сети общего пользования (STN), связывая поддерживаемое локальное устройство пограничного контроллера сеанса (SBC) с устройством Microsoft 365 или Office 365, а затем подключая к этому устройству телефонную линию.

Для поддержки этого сценария клиенту необходимо развернуть сертифицированный SBC для прямой маршрутии от одного из сертифицированных партнеров Майкрософт. Он должен быть настроен как рекомендуется поставщиком и маршрутироваться из Microsoft 365 или Office 365 для прямого трафика UDP. Мультимедиа могут поступать непосредственно из Teams или клиента Skype для бизнеса к SBC (в обход Teams шлюза) или через Teams шлюза. Подключение к SBC при настройке соединительной связи для обхода шлюза Teams основано на ICE, где SBC поддерживает ICE-Lite, а конечная точка мультимедиа Teams/Skype для бизнеса поддерживает полную форму ICE.
...
- **Flow 5B** — представляет поток мультимедиа между Teams клиентской сетью и SBC прямой маршрутии в режиме обхода.
...
- Сигнальный и мультимедиа от клиента в клиентской сети для Microsoft 365 или Office 365 использовать поток 4.
...
- Для сигнального сигнала от SBC к Microsoft 365 или Office 365 и наоборот используется поток 4 и/или поток 4'.

- Сигнальный сигнал от клиента в Интернете для Microsoft 365 или Office 365 используется поток 3.

- Для передачи мультимедиа из клиента в Интернете на SBC в клиентской сети используются потоки 3 и 4, переданные Teams ретранслятором транспорта.
...
- Клиент не может сигнализировать и зациклить мультимедиа после того, как звонок будет соединен из STN в STN.
...
*Рисунок 24. Прямая маршрутная маршрутия (мультимедиа через Microsoft 365 или Office 365) — звонок с помощью висячьей линии связи через ПСС в двух клиентах*
...
Если развертывание и развертывание express Route были развернуты, можно перенау числеть потоки Teams из потока 4 в поток 1 и от потока 4' до потока 1'. Однако приложение Teams имеет твердую зависимость от других Microsoft 365 или Office 365 через Интернет с помощью потоков 4 и 4'; поэтому эти потоки не должны быть заблокированы.

Обратите внимание Skype для бизнеса что гибридный edge-трафик маршрутн в Интернет, а не в Express Route для связи с внешними пользователями и федерации с другими клиентами.
...
1. Teams внешний пользователь выделяет порт ретрансляции мультимедиа Teams ретрансляции транспорта с помощью потока 3.
1. Teams внешний пользователь отправляет "ответ" со Flow ICE с помощью потока 3, который перенаправлется Teams пользователю A.
1. Teams Пользователь A и Teams B вызывает проверки подключения ICE и выбирает потоки 1 и 3, которые передаются Teams ретранслятором транспорта.
1. Teams Пользователи отправляют телеметрию в Microsoft 365 или Office 365 с помощью потоков 1 и 3.

> [!NOTE]
> Flow 4 необходимо включить для поддержки зависимостей приложения Teams других микрослужб, которые поручают поток 4.
