...
- Flow X используется клиентом локальной службы для Microsoft 365 или Office 365 в облаке. Она происходит из сети клиента и завершается как конечная точка в Microsoft 365 или Office 365.
...
**Интернет**. Это сетевой сегмент, который является частью общей сети, который будет использоваться пользователями, подключающихся к Microsoft 365 или Office 365 из-за пределов клиентской сети. Он также используется некоторым трафиком из клиентской сети для Microsoft 365 или Office 365.
...
**VPN network**. Не рекомендуется использовать трафик мультимедиа (или поток 2'). В VPN-клиенте следует использовать раздельное tunneling и Teams трафик мультимедиа, как любой внешний не VPN-пользователь, как указано в разделе Включение мультимедиа Lync для обхода [VPN-канала.](https://techcommunity.microsoft.com/t5/Skype-for-Business-Blog/Enabling-Lync-Media-to-Bypass-a-VPN-Tunnel/ba-p/620210)
...
- Конференция Microsoft Teams конференции будет проводиться Microsoft 365 или Office 365 в том же регионе, где к нему присоединился первый участник. (Обратите внимание, что если в некоторых topologies есть исключения, они будут описаны в этом документе и проиллюстрированы соответствующим потоком звонка.)
...
- телефонная система Прямая маршрутная маршрутия (для подключения по ДНР) описана в Teams с топологией direct **Routing.**
...
1. Teams Пользователь B отправляет "ответ" со словами ICE с помощью потока 4, который перенаправлется Teams пользователю A с помощью Flow 4.
...
Мультимедиа является днойной. Направление потока 5 указывает на то, что одна из сторон инициирует связь с точки зрения подключения, совпадая со всеми потоками в этом документе. В этом случае не имеет значения, в каком направлении используется, так как обе конечные точки находятся в клиентской сети.
...
- Teams клиенты не могут Skype для бизнеса собрания, а Skype для бизнеса клиенты не Teams собрания.
...
- **Flow 5C** — представляет поток мультимедиа между SBC прямой маршрутизации на другой SBC прямой маршрутации в режиме обхода вызовов через ОКП.
