...
- **Background (фон** ) — в этом разделе представлены общие сведения, такие как сети, в которых потоки Office 365 могут пройти по сети, указать трафик, руководство по подключению клиента к конечным точкам служб Office 365, взаимодействие с сторонними компонентами и принципы, которые используются в Teams для выбора потоков мультимедиа.
...
1. Пользователь Teams выделит порт ретрансляции мультимедиа в Team Relay Transport с помощью потока 4
1. Пользователь Teams отправляет "пригласить" с кандидатами ICE через поток 4 в Office 365
1. Office 365 отправляет уведомление пользователю Teams B с помощью потока 4
1. Пользователь Teams B выделит порт ретрансляции мультимедиа в Team Relay Transport с помощью потока 4
1. Пользователи Teams B отправляют "Answer" с кандидатами ICE через поток 4, который пересылается обратно пользователю Teams с помощью потока 4
...
1. Пользователь Teams в сети клиента отправляет сообщение "пригласить" с кандидатами ICE через процесс 1 в Office 365
1. Office 365 отправляет уведомление внешнему пользователю Teams через поток 3
1. Внешний пользователь Teams выделит порт ретрансляции мультимедиа в Team Relay с помощью потока 3
1. Внешние пользователи Teams отправляют "Answer" с кандидатами ICE через поток 3, который пересылается назад в Teams с помощью потока 1
...
1. Пользователи Teams отправляют телеметрию в Office 365 через потоки 1 и 3
