...
Разрешения RSC дает владельцы команды согласие на доступ к данным группы и их изменение для приложения. Разрешения RSC являются детализацией и Teams, которые определяют, что приложение может делать в определенной команде. Примерами разрешений RSC является возможность создавать и удалять каналы, получать параметры для команды, а также создавать и удалять вкладки каналов. 
