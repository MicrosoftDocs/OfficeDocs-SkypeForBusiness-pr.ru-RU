...
Чтобы ИТ-администраторы могли легко настроить эту интеграцию, мы обновили подключаемый модуль Open-Source Moodle с помощью следующих возможностей:
...
Интеграция Moodle в Microsoft Teams включена в [комплект подключаемых модулей Moodle](https://github.com/Microsoft/o365-moodle)Open Source. Чтобы установить подключаемый модуль на сервере Moodle, выполните следующее:

1. Сначала скачайте [комплект подключаемого модуля Moodle](https://moodle.org/plugins/pluginversions.php?plugin=local_o365) и сохраните его на локальном компьютере. Необходимо использовать версию 3.5 или более позднюю.
...
**Важное замечание:** Страница конфигурации подключаемого модуля Microsoft 365 или Office 365 Moodle открыта на отдельной вкладке браузера, так как при этом будет возвращено на этот набор страниц в ходе этого процесса.
...
Затем требуется зарегистрировать Moodle в качестве приложения в Azure Active Directory. Мы предоставили скрипт PowerShell для выполнения этого процесса. Сценарий PowerShell подготавливает новое приложение Azure AD для своей организации Microsoft 365 или Office 365, которое будет использоваться подключаемым модулем Moodle. Сценарий будет подготавливать приложение для клиента Microsoft 365 или Office 365, настроить все необходимые URL-адреса и разрешения для подготовленного приложения и вернуть AppID и ключ. Вы можете использовать созданные AppID и ключ на странице настройки подключаемого модуля Moodle, чтобы настроить сервер Moodle с Azure AD. Подробные сведения о действиях, автоматизируемых скриптом PowerShell, см. в полной [документации для подключаемого модуля](https://docs.moodle.org/34/en/Office365#Register_your_Moodle_instance_as_an_Application).
...
1. На странице подключаемый модуль интеграции Microsoft 365 или Office 365 перейдите на вкладку **Настройка** .
...
1. Запустите скрипт PowerShell, выполнив следующие действия:
    ...
    * Войдите в свою учетную запись администратора Microsoft 365 или Office 365 во всплывающем окне.
...
1. Затем необходимо добавить идентификатор и ключ в подключаемый модуль Moodle. Вернитесь на страницу администрирования подключаемого модуля (Администрирование сайта подключаемые модули > > интеграции с Microsoft 365).
...
1. После обновления страницы отобразится еще один новый раздел **Согласие администратора и дополнительные сведения**.
    * Щелкните ссылку **предоставление согласия администратора** , введите учетные данные глобального администратора Microsoft 365 или Office 365, а затем нажмите кнопку **принять** , чтобы предоставить разрешения.
...
1. Вернитесь на страницу администрирования подключаемого модуля (Администрирование сайтов > подключаемые модули > интеграция с Microsoft 365) и выберите страницу **параметры групп** . Вам потребуется настроить некоторые параметры безопасности, чтобы включить интеграцию приложения Teams.
...
1. Вернитесь на страницу администрирования подключаемого модуля (Администрирование сайтов > подключаемые модули > интеграция с Microsoft 365) и откройте вкладку **параметры групп** .
...
После завершения развертывания ресурсов в Azure вам потребуется настроить подключаемый модуль Moodle, указав конечную точку для обмена сообщениями. Сначала вам потребуется получить конечную точку из ленты в Azure. Для этого:
...
6. В Moodle откройте страницу **Параметры группы** в своем подключаемом модуле Moodle.
...
Теперь, когда у вас есть бот, развернутый в Azure и настроенный для общения с сервером Moodle, можно развернуть приложение Microsoft Teams. Чтобы сделать это, загрузите файл манифеста, скачанный на странице Параметры группы подключаемого модуля Moodle на предыдущем этапе.
