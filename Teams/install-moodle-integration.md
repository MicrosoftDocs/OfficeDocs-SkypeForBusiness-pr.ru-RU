...
description: Узнайте, как установить и настроить приложение Moodle с открытым исходным кодом системы управления обучением (LMS) для Microsoft Teams.
...
ms.locfileid: "66789544"
---
# <a name="installing-the-moodle-integration-with-microsoft-teams"></a>Интеграция Moodle с Microsoft Teams 

> [!VIDEO https://www.youtube.com/embed/OHlPt22nKoE]

[Moodle](https://moodle.org/) — самая популярная система управления обучением (LMS) с открытым кодом в мире теперь интегрируется с Microsoft Teams! Эта интеграция помогает преподавателям и учащимся совместно работать над курсами Moodle, задавать вопросы о своих оценках и заданиях и получать уведомления прямо в Teams!

Чтобы помочь ИТ-администраторам легко настроить эту интеграцию, мы обновили подключаемый модуль Moodle с открытым кодом, используя следующие возможности:
...
Дополнительные сведения о функциях, которые предоставляет эта интеграция, см. в статье ["Установка интеграции Moodle с Microsoft Teams"](/microsoftteams/platform/resources/moodleinstructions).
...
## <a name="step-1-install-the-moodle-plugin"></a>Шаг 1. Установка подключаемого модуля Moodle

> [!VIDEO https://www.youtube.com/embed/SETEC5nzMgk]

Интеграция Moodle в Microsoft Teams выполняется на основе [открытый код-подключаемого модуля Moodle](https://github.com/Microsoft/o365-moodle). Чтобы установить подключаемый модуль на сервере Moodle, выполните следующее:

1. Сначала скачайте набор [подключаемых модулей Moodle](https://moodle.org/plugins/pluginversions.php?plugin=local_o365) и сохраните его на локальном компьютере. Необходимо использовать версию 3.5 или более позднюю.
...
1. Войдите на сервер Moodle с правами администратора и выберите " **Администрирование** сайта" на панели навигации слева.
...
1. Выберите параметр **"Отправить файл** " в области навигации слева, найдите скачанный выше файл и нажмите кнопку **"Отправить этот файл"**.
...
**Важное примечание.** Не закрывайте страницу конфигурации подключаемого модуля Microsoft 365 или Office 365 Moodle на отдельной вкладке браузера, так как вы будете возвращаться к этому набору страниц на протяжении всего процесса.
...
## <a name="step-2-configure-the-connection-between-the-microsoft-365-or-office-365-plugin-and-azure-active-directory"></a>Шаг 2. Настройка подключения между подключаемым модулем Microsoft 365 или Office 365 и Azure Active Directory

> [!VIDEO https://www.youtube.com/embed/FpGEezaJ3SA]

Затем требуется зарегистрировать Moodle в качестве приложения в Azure Active Directory. Мы предоставили скрипт PowerShell для выполнения этого процесса. Сценарий PowerShell подготавливает новое Azure AD для вашей организации Microsoft 365 или Office 365, которое будет использоваться подключаемым модулем Moodle. Скрипт подготовит приложение для клиента Microsoft 365 или Office 365, настроит все необходимые URL-адреса ответа и разрешения для подготовленного приложения и вернет AppID и ключ. Вы можете использовать созданные appID и ключ на странице настройки подключаемого модуля Moodle, чтобы настроить сервер Moodle с Azure AD. Подробные сведения о действиях, автоматизируемых скриптом PowerShell, см. в полной [документации для подключаемого модуля](https://docs.moodle.org/34/en/Office365#Register_your_Moodle_instance_as_an_Application).
...
1. На странице подключаемого модуля Microsoft 365 или Office 365 Integration выберите вкладку **"Настройка**".
...
1. Запустите скрипт PowerShell, выполнив следующие действия:
    ...
    * Во всплывающем окне войдите в учетную запись Office 365 Microsoft 365 или Office 365 администратора.
...
1. Далее необходимо добавить идентификатор и ключ в подключаемый модуль Moodle. Вернитесь на страницу администрирования подключаемого модуля (администрирование > подключаемых модулей > Интеграция Microsoft 365).
...
1. После обновления страницы отобразится еще один новый раздел **Согласие администратора и дополнительные сведения**.
    * Щелкните **ссылку "Предоставить Администратор согласия**", введите свои учетные данные глобального администратора Microsoft 365 или Office 365, а  затем примите, чтобы предоставить разрешения.
...
1. После этого вам потребуется синхронизировать пользователей между сервером Moodle и Azure Active Directory. На этом этапе вы можете выбирать разные параметры в зависимости от вашей среды. Обратите внимание, что эта конфигурация будет применяться с каждым запуском программы cron Moodle (обычно раз в день) для обеспечения синхронизации. Чтобы приступить к работе:
    ...
    * В разделе **"Ограничения на создание** пользователей" можно настроить фильтр для ограничения Azure AD пользователей, которые будут синхронизированы с Moodle.
...
1. Вернитесь на страницу администрирования подключаемого модуля (> подключаемых модулей > Microsoft 365) и выберите страницу **параметров Teams** . Вам потребуется настроить некоторые параметры безопасности, чтобы включить интеграцию приложения Teams.
...
1. Вернитесь на страницу администрирования подключаемого модуля (> подключаемых модулей > Microsoft 365) и перейдите на вкладку **"Параметры Teams** ".
...
После завершения развертывания ресурсов в Azure необходимо настроить подключаемый модуль Moodle с его конечной точкой обмена сообщениями. Сначала необходимо получить конечную точку из бота в Azure. Для этого:
...
6. В Moodle откройте **страницу параметров** команды подключаемого модуля Moodle.
...
Теперь, когда у вас есть бот, развернутый в Azure и настроенный для общения с сервером Moodle, можно развернуть приложение Microsoft Teams. Для этого загрузите файл манифеста, скачанный на странице параметров группы подключаемых модулей Moodle на предыдущем шаге.
