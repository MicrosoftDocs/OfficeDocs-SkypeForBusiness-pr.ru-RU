...
Чтобы помочь ИТ-администраторам настроить эту интеграцию, мы обновили подключаемый модуль Moodle Plugin с открытым кодом со следующими возможностями:
...
Интеграция Moodle в Microsoft Teams создана с помощью подключаемой программы Moodle с открытым [кодом.](https://github.com/Microsoft/o365-moodle) Чтобы установить подключаемый модуль на сервере Moodle, выполните следующее:
...
1. Выберите параметр **Отправить файл в** области навигации слева, найдите файл, который вы скачали выше, и нажмите **кнопку Отправить этот файл**.
...
**Важное примечание.** Не открывая страницу конфигурации Microsoft 365 или Moodle Plugin для Office 365, на отдельной вкладке браузера, так как вы будете возвращаться к этому набору страниц на протяжении всего процесса.
...
1. На странице Подключаемый модуль Microsoft 365 или Интеграция Office 365 выберите **вкладку Настройка.**
...
1. Запустите скрипт PowerShell, выполнив следующие действия:
    ...
    * Войдите в учетную запись администратора Microsoft 365 или Office 365 во всплывающее окно.
...
1. Затем нужно добавить ID и Key в подключаемый модуль Moodle. Вернись на страницу администрирования подключаемых модульов (> подключаемых > Microsoft 365 Integration).
...
1. После обновления страницы отобразится еще один новый раздел **Согласие администратора и дополнительные сведения**.
    * Щелкните **ссылку Предоставить** согласие администратора, введите свои учетные данные глобального  администратора Microsoft 365 или Office 365, а затем принять, чтобы предоставить разрешения.
...
1. Вернись на страницу администрирования подключаемых модульов (> подключаемых модульов > Интеграция с Microsoft 365) и выберите страницу **Параметры Teams.** Вам потребуется настроить некоторые параметры безопасности, чтобы включить интеграцию приложения Teams.
...
1. Вернись на страницу администрирования подключаемых модульов (администрирование сайта > подключаемых > Интеграция с Microsoft 365) и выберите вкладку **Параметры Teams.**
1. Щелкните ссылку **Портал регистрации приложений Майкрософт** и войдите с использованием идентификатора Майкрософт.
1. Введите имя приложения (EG). MoodleBot) и нажмите кнопку **Создать**.
...
Теперь, когда у вас есть бот, развернутый в Azure и настроенный для общения с сервером Moodle, можно развернуть приложение Microsoft Teams. Для этого необходимо загрузить файл манифеста, загруженный со страницы Параметры группы подключаемых модульов Moodle на предыдущем шаге.
