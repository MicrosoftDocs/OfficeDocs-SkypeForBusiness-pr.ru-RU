...
В клиенте Teams панель набора номера позволяет пользователям получать доступ к функциям телефонной сети общего присоединения . Панель набора номера доступна для пользователей с телефонная система, если они настроены правильно. Для показа панели набора номера необходимы следующие условия:
...
- Пользователь находится в Интернете, а не Skype для бизнеса локально
- Для пользователя включена Teams звонков

В следующих разделах описано, как проверить условия с помощью PowerShell. В большинстве случаев необходимо взглянуть на различные свойства в результатах Get-CsOnlineUser. В примерах $user имя-имя пользователя или SIP-адрес пользователя.
...
## <a name="user-is-homed-online-and-not-in-skype-for-business-on-premises"></a>Пользователь находится в Интернете, а не Skype для бизнеса локально
...
Для эффективной работы пользователя в TeamsCallingPolicy должно быть установлено разрешение AllowPrivateCalling на true.  По умолчанию пользователи наследуют глобальную политику, для которой по умолчанию установлено значение AllowPrivateCallingPolicy true.

Чтобы получить TeamsCallingPolicy для пользователя и проверить, что для allowPrivateCalling установлено true, используйте следующую команду:
...
-   Если вы по-прежнему не видите панель набора номера, проверьте, нет ли ошибки при подпуске с помощью следующей команды:
