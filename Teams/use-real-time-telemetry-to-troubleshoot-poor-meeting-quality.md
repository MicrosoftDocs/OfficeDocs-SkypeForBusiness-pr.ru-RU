...
- Reporting
description: Используйте телеметрию реального времени с подробными сведениями об устройствах, сетях и подключениях для устранения неполадок пользователей с Microsoft Teams собраниями.
ms.openlocfilehash: 94b303687995ac3bcd765991dbfeb41c6f1459e7
ms.sourcegitcommit: 74d3ab35c344d70b2399bc46a6ced3ab2762a470
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 10/06/2021
ms.locfileid: "60138365"
---
...
Чтобы увидеть все сведения о собрании и данные для пользователя, перейдите в центр [Teams администрирования](https://admin.teams.microsoft.com). В **области** Управление пользователями выберите пользователя и откройте вкладку Собрания  >  & **вызовы** на странице профиля пользователя. В **списке** Недавние собрания вы увидите список собраний, которые пользователь посещал в течение последних 24 часов, для которых доступна телеметрия в режиме реального *времени,* включая все собрания, которые уже ведутся. Если собрание не находится в процессе или не имеет данных телеметрии в режиме реального времени, оно отображается в области **Прошлые собрания.**
...
|Потеря пакетов |Процент |Менее 5 % |Потеря пакетов происходит, когда пакеты данных не доходят до места назначения. Процент потери пакетов зависит от общего количества отправленных пакетов. |
|Время кругового пути |Миллисекунды |Менее 500 мс |Время кругового пути — это время, необходимое для перемещения одного пакета из клиента на удаленную конечную точку и обратно в клиент. Высокое время кругового пути может привести к задержкам воспроизведения потока. Примером этого может быть то, что два человека на собрании непреднамеренно разговаривают друг с другом из-за задержки. |
...
|Частота кадров (общий доступ к приложениям) |Кадров в секунду (FPS) |Только сведения |Для общего доступа к приложениям частота кадров должна делиться контентом, чтобы при отправке кадров отправлялась необходимое количество кадров, чтобы избежать отправки фреймов, если они не нужны. Например, для общего доступа к текстовому документу на экране требуется 1 кадр в секунду, а при совместном использовании видео или содержимого с дополнительными действиями частота кадров увеличивается до 30 кадров в секунду. |
