...
> [!IMPORTANT] 
> Пользователям не нужно иметь лицензию Microsoft Stream, если вы хотите, чтобы они могли только создавать и скачивать записи. Это означает, что записи не будут храниться в потоке Microsoft, но вместо этого они будут храниться в службах мультимедиа Azure (AMS) в течение 30 дней до того, как оно будет удалено. В этом случае администратор не может ни управлять записями, ни удалять их.
...
Обратите внимание, что как организатор собрания, так и инициатор записи должны иметь разрешения на запись для записи собрания. Если пользователь не назначил пользовательскую политику, пользователи получают глобальную политику, в которой AllowCloudRecording отключен по умолчанию.
...
Подробнее о том, где данные хранятся в разных службах Office 365, см. В разделе [Где находятся ваши данные?](https://products.office.com/where-is-your-data-located?rtc=1)

### <a name="turn-on-or-turn-off-recording-transcription"></a>Включить или отключить запись транскрипции

Когда пользователи записывают свои собрания Teams, они могут подтвердить, должна ли автоматически генерироваться стенограмма после записи собрания. Если вы отключили возможность транскрипции для организатора встречи и инициатора записи, у инициатора записи не будет выбора транскрибировать записи встречи.
