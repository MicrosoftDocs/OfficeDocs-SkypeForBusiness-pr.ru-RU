...
> [!NOTE]
> Кроме того, чтобы лицо, инициирующее запись, могло выбирать, следует ли автоматически транскрибировать запись, для параметра TeamsMeetingPolicy -AllowTranscription пользователя должно быть установлено значение true

<sup>1</sup> Пользователю необходимо иметь лицензию для загрузки / выгрузки собраний в / из Microsoft Stream, однако им не нужна лицензия для записи собрания. Если вы хотите запретить пользователю записывать собрание Microsoft Teams, вы должны предоставить TeamsMeetingPolicy, для которого AllowCloudRecording установлено значение $False.
...
Размер 1-часовой записи составляет 400 МБ. Убедитесь, что вы понимаете емкость, необходимую для записанных файлов, и имеете достаточно места для хранения в Microsoft Stream.  Прочтите [эту статью](https://docs.microsoft.com/stream/license-overview), чтобы понять, какое базовое хранилище входит в подписку, и как приобрести дополнительное хранилище.
