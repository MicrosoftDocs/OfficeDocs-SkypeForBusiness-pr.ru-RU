...
Например, в следующей иерархии "Отзыв", "Розничная связь" и "Отдел кадров" можно публиковать задачи на каждом нижнем узле (группы) в иерархии, в то время как "Северо-восточная зона" может публиковать задачи только в командах "Нью-Йорк" и "Ростов". Такая иерархия позволяет отзывать сообщения, розничные коммуникации и группы кадров для публикации задач, которые относятся к всей компании, например сведений о преимуществах или сообщениях от директора. Северо-Восточная зона может публиковать задачи, такие как планирование персонала, прогноз погоды и так далее, только в командах Магазина в Нью-Йорке и Москве.
...
Централизованно классифицируя работу, группа публикации может заранее упорядоизировать список задач для всех групп, в которые были поставлены десятки, сотни или тысячи получателей. Затем группы получателей могут сортировать и фильтровать свои задачи по сегментам, чтобы сосредоточиться на области, наиболее релевантной для их работы.

При добавлении столбца сегмента обратите внимание на следующее:

- Имя столбца становится именем сегмента. Каждый из этих сегментов появится в списке "Сегменты" в приложениях Teams, которые используют иерархию. Не рекомендуется включать конфиденциальную информацию в имена сегментов. В настоящее время группы публикации не могут удалить сегмент с помощью публикации после его создания.
