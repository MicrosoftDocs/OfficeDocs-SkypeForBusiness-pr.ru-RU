...
При навигации по иерархиям важны следующие термины: Teams будет называться **узлами.**
...
* Если пользователь входит по крайней мере в одну команду, которая имеет по крайней мере одну команду с заданными в иерархии, он может публиковать публикацию в этих пользователях, не являясь участником всех команд, в которые нужно опубликовать публикацию.
...
* Если пользователь не входит в состав какой-либо команды в иерархии, он не будет видеть функции, связанные с публикацией.
...
> [!NOTE]
> облако сообщества для государственных организаций (GCC) пользователи должны использовать [](https://www.powershellgallery.com/packages/MicrosoftTeams/2.4.0-preview) предварительную версию для работы с GCC, а не в облачную среду.
...
> [!IMPORTANT]
> Для выполнения этого шага необходимо установить и использовать общедоступный модуль Teams PowerShell из [коллекции PowerShell.](https://www.powershellgallery.com/packages/MicrosoftTeams/) Действия по установке модуля см. в Teams [PowerShell.](teams-powershell-install.md)

### <a name="sample-script"></a>Пример сценария

Следующий сценарий можно использовать для создания команд и добавления .csv в Microsoft Teams клиента. Если у вас уже есть иерархия, сценарий заменит ее.
...
Проверьте, правильно ли вы используете TeamId для команды в CSV-файле схемы. TeamId должен быть таким же, как и Microsoft 365 группы, которая является ее задней частью. Вы можете найти ИД группы в центре администрирования Microsoft Teams администратора.
