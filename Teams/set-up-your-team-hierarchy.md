...
> [!NOTE]
> Для выполнения этого шага необходимо использовать командлеты PowerShell Майкрософт Teams. Следует использовать версию 4.6.0 или более позднюю из командлетов Майкрософт Teams. Это требование также относится к клиентам облака сообщества для государственных организаций (GCC).
...
> [!IMPORTANT]
> Для выполнения этого шага необходимо установить и использовать модуль PowerShell Teams из [коллекция PowerShell](https://www.powershellgallery.com/packages/MicrosoftTeams/). Инструкции по установке модуля см. в разделе [Установка модуля PowerShell Майкрософт Teams](teams-powershell-install.md).

### <a name="sample-script"></a>Пример сценария

Следующий скрипт можно использовать для создания команд и отправки файла .csv в клиент Майкрософт Teams. Если у вас есть существующая иерархия, этот скрипт заменит ее.
...
Убедитесь, что вы используете правильный TeamId для команды в CSV-файле схемы. Идентификатор TeamId должен совпадать с идентификатором группы Майкрософт 365, которая поддерживает команду. Идентификатор группы можно найти в Центре администрирования Майкрософт Teams.

1. В левой области навигации [центра администрирования Майкрософт Teams](https://admin.teams.microsoft.com/) перейдите в раздел **Управление командами** **Teams** > .
...
Убедитесь, что идентификатор TeamId в CSV-файле схемы соответствует идентификатору группы, который отображается в Центре администрирования Майкрософт Teams.
