...
CSV-файл должен содержать три следующие столбца в указанном ниже порядке, начиная с первого столбца. Узел должен быть связан с командой для получения задач. При закрытом предварительном просмотре поддерживаются узлы 2 000. По умолчанию при запуске мы рекомендуем поддерживать по крайней мере 15 000 узлов. Мы планируем работать с клиентами, чтобы увеличить это ограничение для более крупных организаций.
...
> [!IMPORTANT]
> Чтобы выполнить это действие, необходимо установить и использовать общедоступный модуль предварительной версии оболочки PowerShell из [коллекции PowerShell](https://www.powershellgallery.com/packages/MicrosoftTeams/). Инструкции по установке модуля приведены в [статье Установка оболочки PowerShell](teams-powershell-install.md).

После того как вы определили иерархию в CSV-файле схемы, вы можете отправить ее в Teams. Для этого выполните следующую команду: Для выполнения этого действия необходимо быть администратором глобального администратора или службы Teams.
...
### <a name="you-receive-an-error-message-when-you-upload-your-schema-csv-file"></a>При попытке отправить CSV-файл схемы появляется сообщение об ошибке
...
### <a name="you-receive-an-error-invalidteamid-error-message-when-you-upload-your-schema-csv-file"></a>При попытке отправить CSV-файл схемы появляется сообщение об ошибке "ошибка: InvalidTeamId"
...
Убедитесь, что вы используете правильную TeamId для группы в CSV-файле схемы. TeamId должен совпадать с ИДЕНТИФИКАТОРом группы Microsoft 365, который является резервной версией команды. Вы можете найти идентификатор группы в центре администрирования Microsoft Teams. 

1. В левой области навигации [центра администрирования Microsoft Teams](https://admin.teams.microsoft.com/)перейдите в раздел **Teams**  >  **Управление группами**.
2. Если столбец " **код группы** " не отображается в таблице, выберите **изменить столбцы** в правом верхнем углу таблицы, а затем включите **код группы**.
3. Найдите группу в списке и найдите ее идентификатор.

Убедитесь, что TeamId в CSV-файле схемы совпадает с ИДЕНТИФИКАТОРом группы, который отображается в центре администрирования Microsoft Teams. 
