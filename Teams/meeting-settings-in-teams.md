...
## <a name="allow-anonymous-users-to-interact-with-apps-in-meetings"></a>Разрешить анонимным пользователям работать с приложениями на собраниях

Анонимные пользователи теперь наследуют глобальную политику разрешений по умолчанию на уровне пользователя. После этого анонимные пользователи смогут взаимодействовать с приложениями в собраниях Teams, если это включено политикой разрешений на уровне пользователя. Обратите внимание, что анонимные пользователи могут работать только с приложениями, которые уже доступны на собрании и не могут приобрести эти приложения и управлять ими. 

> [!IMPORTANT]
> По умолчанию включен параметр, позволяющий анонимным пользователям работать с приложениями на собраниях.

![Значок с логотипом Microsoft Teams](media/teams-logo-30x30.png) **Использование центра администрирования Microsoft Teams**

Для доступа к этому параметру вы должны быть администратором службы Teams. См. [статью "Использование](https://docs.microsoft.com/microsoftteams/using-admin-roles) ролей администратора Teams для управления Teams", чтобы ознакомиться с получением ролей и разрешений администраторов.

1. Перейдите в Центр администрирования.

2. В области навигации слева выберите **Собрания** > **Параметры собрания**.

3. В **области "Участники"** можно изменить параметр для анонимных пользователей, которые могут взаимодействовать с приложениями **на** собраниях.

> [!CAUTION]
> Если вы не хотите, чтобы анонимные пользователи могли работать с приложениями на собраниях, запланированных пользователями в вашей организации, отключите этот параметр.
...
Если для приоритета сетевого трафика используется качество обслуживания (QoS), вы можете включить маркеры QoS и установить диапазоны портов для каждого типа трафика мультимедиа. Установка диапазонов портов для разных типов трафика - это только один шаг в обработке медиа в реальном времени; см. [Качество обслуживания (QoS) в Teams](qos-in-teams.md) для более подробной информации.

> [!IMPORTANT]
> Если включить QoS или изменить параметры в Центре администрирования Microsoft Teams для [](QoS-in-Teams-clients.md) службы Teams, вам также потребуется применить параметры, совпадающие со всеми пользовательскими устройствами и всеми внутренними сетевыми устройствами, чтобы полностью внедрить изменения QoS в Teams.
...
3. В разделе **Сеть** выполните указанные ниже действия.
    ...
    - Чтобы разрешить использование меток DSCP для QoS, включите **Вставить маркеры качества обслуживания (QoS) для трафика мультимедиа в реальном времени**. У вас есть только возможность использовать маркеры или нет; Вы не можете установить пользовательские маркеры для каждого типа трафика. См. [Выбор метода реализации QoS](QoS-in-Teams.md#select-a-qos-implementation-method) для получения дополнительной информации о маркерах DSCP.
        > [!NOTE]
        > По умолчанию теги DSCP осуществляется через исходные порты, а трафик UDP идет в ретранслятор транспорта с портом назначения 3478. Если вашей компании требуется пометка тегов в портах назначения, обратитесь в службу поддержки, чтобы включить связь с ретранслятором транспорта с портами UDP 3479 (звук), 3480 (видео) и 3481 (общий доступ).
    - Чтобы указать диапазоны портов, выберите **Выбрать диапазон портов для каждого типа медиа трафика в реальном времени**, выберите **Указать диапазоны портов**, а затем введите начальный и конечный порты для совместного использования звука, видео и экрана. Выбор этой опции необходим для реализации QoS. 
        > [!Note]
        > Если **маркер качества обслуживания (QoS)** для трафика мультимедиа, передаемого в режиме реального времени, необходимо управлять настройками порта. Управление ими не происходит автоматически.
...
\* Назначаемые диапазоны портов не могут перекрываться и должны быть смежными.
