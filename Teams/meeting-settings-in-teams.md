...
3. В разделе **Сеть** выполните указанные ниже действия.
    ...
    - Чтобы указать диапазоны портов, выберите **Выбрать диапазон портов для каждого типа медиатрафика в реальном времени**, выберите **Указать диапазоны портов**, а затем введите начальный и конечный порты для совместного использования звука, видео и экрана. Выбор этой опции необходим для реализации QoS. 
        > [!Note]
        > Если **маркеры качества обслуживания (QoS) для медиатрафика в реальном времени** включены, потребуется настройка параметров порта. Их настройка не выполняется автоматически.
