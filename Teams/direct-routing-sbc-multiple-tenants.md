...
> [!NOTE]
> Этот сценарий предназначен для партнеров Майкрософт и (или) операторов ТСОП, которые далее в этом документе называются операторами. Оператор продает услуги телефонии, доставленные в Microsoft Teams своим клиентам. 

Оператор:
- Развертывает SBC и управляет им в своем центре обработки данных (клиентам не нужно реализовывать SBC и они получают телефонные службы от оператора в клиенте Teams).
...
*Убедитесь, что вы понимаете основы DNS и управление доменным именем в Microsoft 365. Прежде [чем продолжить, ознакомьтесь](/microsoft-365/admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider) со справкой по доменам Microsoft 365.*
...
- Woodgrove Bank, Contoso и Adventure Works — это три клиента, которые имеют домены Microsoft 365, но получают службы телефонии от Adatum.

Поддомены должны  соответствовать имени полного доменного имени магистрали, которое будет настроено для клиента, и полное доменное имя в заголовке Contact при отправке приглашения в Microsoft 365. 

Когда вызов поступает в интерфейс прямой маршрутизации Microsoft 365, интерфейс использует заголовок Contact, чтобы найти клиент, в котором нужно найти пользователя. Прямая маршрутизация не использует поиск номеров телефонов в приглашении, так как у некоторых клиентов могут быть номера, отличные от DID, которые могут перекрываться в нескольких клиентах. Таким образом, полное доменное имя в заголовке контакта требуется для идентификации точного клиента для поиска пользователя по номеру телефона.

*Дополнительные сведения о создании доменных имен в организациях Microsoft 365 см. в справке по доменам [Microsoft 365](/microsoft-365/admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider).*
...
5. Нажмите **кнопку "** Далее", а затем на странице "Обновление параметров **DNS** " выберите "Я добавю **записи DNS самостоятельно** " и нажмите кнопку **"Далее"**.
...
После регистрации доменного имени необходимо активировать его, добавив по крайней мере одну лицензированную учетную запись пользователя или ресурса Teams. Допустимые учетные записи будут лицензированы с любым из следующих номеров SKU:
...
- Учетная запись пользователя с Microsoft 365 бизнес базовый/Standard/Premium.
- Учетная запись пользователя с телефоном общего содержимого.
- Учетная запись ресурса **с Телефонная система Microsoft Teams учетной записи** ресурса.

Кроме того, имя участника-пользователя учетной записи (имя участника-пользователя) или Skype для бизнеса локальном SIP-адресе должно использовать то же полное доменное имя, что и только что созданный домен.

Дополнительные сведения о добавлении пользователей в организации Microsoft 365 см. в справке по доменам [Microsoft 365](/microsoft-365/admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider).
...
После регистрации имени поддомена его необходимо активировать, добавив по крайней мере одну лицензированную учетную запись пользователя или ресурса Teams. Допустимые учетные записи будут лицензированы с любым из следующих номеров SKU:
...
-   Учетная запись пользователя с Microsoft 365 бизнес базовый/Standard/Premium и G3/G5
-   Учетная запись пользователя с телефоном общего содержимого
-   Учетная запись ресурса с **лицензией Телефонная система Microsoft Teams ресурса**

Кроме того, имя участника-пользователя учетной записи (имя участника-пользователя) Skype для бизнеса локальном SIP-адресе должно использовать то же полное доменное имя, что и только что созданный поддомен.

Дополнительные сведения о добавлении пользователей в организации Microsoft 365 см. в справке [по Microsoft 365](/microsoft-365/admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider).
...
- Производная магистраль, которая не требует регистрации. Это просто нужное имя узла, добавленное из магистрали оператора. Он наследует все параметры конфигурации от магистрали оператора. Связь с магистралью оператора основана на имени полного доменного имени (см. сведения ниже).
