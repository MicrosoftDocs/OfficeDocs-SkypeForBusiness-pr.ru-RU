...
1. В области управления групповыми политиками найдите контейнер, в котором нужно создать новую политику. Например, если все клиентские компьютеры расположены в OU **clients**(Клиенты), новая политика должна быть создана в клиентских клиентах.
...
1. В **диалоговом** окне QoS на открываемой странице введите имя новой политики в **поле "Имя".** Выберите **«Задать значение DSCP** и установить значение **46».** Оставьте **невыбираемой укажите исходящие** тарифы и нажмите кнопку **"Далее".**
...
1. Повторите действия 5–10, чтобы создать политики для общего доступа к видео, приложениям и рабочему столу, заменив соответствующие значения на шагах 6 и 10.
...
1. Убедитесь, что значение параметра "Имя приложения" правильно для используемого клиента, и убедитесь, что значения DSCP и локальный порт отражают параметры в объекте групповой политики.
