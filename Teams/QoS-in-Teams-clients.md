...
1. На следующей странице выберите **только приложения с именем исполняемого файла** и введите имя **Teams. exe**, а затем нажмите кнопку **Далее**. Этот параметр дает политике приоритет только для трафика, соответствующего клиенту Teams.
...
1. В командной строке введите
   ...
   Это приведет к формированию отчета об примененных объектах групповой политики и их отправке в текстовый файл с именем *GP. txt*.

   Для более удобочитаемого отчета в формате HTML с именем *GP. HTML*введите следующую команду:
...
1. Откройте редактор реестра и перейдите к разделу
   ...
   Политики\_\\программного\\обеспечения\\для\_локального компьютера\\hKey Microsoft\\Windows QoS
