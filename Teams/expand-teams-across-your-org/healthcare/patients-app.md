...
1. Служба Partner позволяет приложению пациентов создать учетную запись с партнером, которая позволяет приложению пациентов создавать и использовать client_id и client_secret, управляемые через портал регистрации авторизации на сервере аутентификации партнера.
...
Служба Partner предоставляет приложение client_id и client_secret для пациентов, управляемое через портал регистрации проверки подлинности на стороне партнера. Служба Partner предоставляет конечной точке запрос на доступ к маркеру доступа с помощью потока учетных данных клиента. Успешный ответ должен включать параметры TOKEN_TYPE, ACCESS_TOKEN и expires_in.
