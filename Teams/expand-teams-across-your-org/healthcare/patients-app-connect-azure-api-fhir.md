...
5. Нажмите **кнопку "Проверка** подлинности" и в поле "Разрешенные  ИД объектов" в поле "Разрешенные" в поле "ИД объекта", скопированные на шаге 3. Это позволит приложению "Пациенты" получать доступ к серверу FHIR. Когда вы вируете ИД объекта, Azure Active Directory проверяет его, и рядом с ней появляется зеленая метка.
