...
## <a name="conformance-minimum-required-field-set"></a>Набор полей соответствия
 ...
 - FhirVersion (Для нашего кода требуется это, чтобы понять, к какой версии следует свертить, так как поддерживается несколько версий.)
...
При поиске ресурсов используется метод POST в пункте /Patient/_search и следующие параметры:
 ...
 - Id
 - family:contains=(поиск всех пациентов, имя семьи которых содержит значение.)
 - given=\<substring>
 - name=\<substring>
