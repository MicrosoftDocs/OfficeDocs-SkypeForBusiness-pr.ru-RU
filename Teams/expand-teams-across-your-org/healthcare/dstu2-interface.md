...
- Microsoft Teams
ms.reviewer: anach
description: Узнайте о спецификации интерфейса DSTU2 в Teams, в том числе о настройке или перенастройке FHIR-сервера для работы с приложением Microsoft Teams Patients.
...
Для настройки или перенастройки FHIR-сервера для работы с приложением Microsoft Teams Patients требуется понимание данных, необходимых приложению для доступа. Сервер FHIR должен поддерживать запросы POST с использованием пакетов для следующих ресурсов:
...
> [!NOTE]
> Ресурс Patient — единственный обязательный ресурс (без которого приложение не будет загружаться). Однако партнеру рекомендуется реализовать поддержку всех указанных выше ресурсов в соответствии с указанными ниже спецификациями, чтобы лучше работать с приложением Microsoft Teams пациентов.

Запросы из приложения Microsoft Teams Patients для более чем одного ресурса выдают пакет запросов (BATCH) по URL-адресу сервера FHIR. Сервер обрабатывает каждый запрос и возвращает пакет ресурсов, которые соответствуют каждому запросу. Дополнительные сведения и примеры см. в [https://www.hl7.org/fhir/DSTU2/http.html#transaction](https://www.hl7.org/fhir/DSTU2/http.html#transaction) .
...
## <a name="conformance-minimum-required-field-set"></a>Набор полей соответствия
 ...
 - FhirVersion (код требует этого, чтобы понять, какую версию следует свертить, так как мы поддерживаем несколько версий.)
...
- MRN: это фактический идентификатор для пациента, который будет знать врач. Мы понимаем, что это mrN основано на типе идентификатора внутри ресурса идентификатора в FHIR
- Имя
- Рождения
...
Кроме того, следующие поля из профиля Core Encounter в США должны поддерживаться
