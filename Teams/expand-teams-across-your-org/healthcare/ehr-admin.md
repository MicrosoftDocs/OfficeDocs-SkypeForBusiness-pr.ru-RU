---
title: Виртуальные посещения с Teams — интеграция с EHR для ситуаций
author: LanaChin
...
description: Узнайте, как интегрировать соединитель Teams EHR, чтобы разрешить поставщикам здравоохранения в вашей организации проводить виртуальные посещения с пациентов или другими поставщиками в Teams непосредственно из системы EHR Epic.
...
- Запустите Teams виртуальных посещений из системы EHR Epic с помощью интегрированного медицинского рабочего процесса.
- Разрешите пациенту присоединяться Teams виртуальных посещений на портале пациентов или через SMS.
- Поддержка других сценариев, включая поддержку нескольких участников, посещений групп и служб интерпретаторов.
- Записывайте метаданные обратно в систему EHR о Teams виртуальных посещений для записи при подключении, отключении и включении автоматического аудита и хранения записей.
- Просмотр отчетов о потреблении данных и настраиваемых сведений о качестве звонков для посещений, подключенных к EHR.

Ознакомьтесь с этим видео, чтобы узнать, как управлять виртуальными посещениями на портале EHR.

> [!VIDEO https://www.microsoft.com/videoplayer/embed/RE4HAtn]

В этой статье описывается, как настроить соединитель Teams EHR для интеграции с платформой Epic в вашей медицинской организации. Он также содержит общие сведения о Teams виртуальных посещений из системы EHR epic.
...
1. Выберите **настройку SMS** рядом с каждым базовым URL-адресом FHIR, чтобы настроить типы SMS-уведомлений для отправки пациентов.
    ...
    - **Sms напоминания**: уведомления отправляются пациенту в соответствии с указанным интервалом времени и запланированным временем посещения.

    Нажмите кнопку **Сохранить**.

1. Выберите **Upload сертификат,** чтобы отправить сертификат открытого ключа. Для каждой среды необходимо отправить CER-сертификат в кодировке Base64 (только открытый ключ).

    Для получения сведений о встрече для отправки SMS-уведомлений требуется сертификат открытого ключа. Сертификат необходим для проверки того, что входящие сведения были из допустимого источника.
...
> [!NOTE]
> В любое время администратор Microsoft 365 может обновить любой из параметров SMS. Помните, что изменение параметров может привести к остановке службы SMS. Дополнительные сведения о просмотре отчетов SMS см. в Teams [отчетов администратора соединителя EHR](ehr-admin-reports.md).
...
Аналитик клиента Epic должен предоставить эти записи в Epic, чтобы завершить настройку виртуальных посещений в Epic. Дополнительные сведения см. в руководстве [по интеграции Epic-Microsoft Teams Telehealth](https://galaxy.epic.com/Search/GetFile?Url=1!68!100!100100357).
...
После выполнения действий по соединителю EHR и настройки Epic ваша организация готова к поддержке видео посещений с помощью Teams.
...
Поставщики услуг здравоохранения из вашей организации могут присоединяться к посещениям Teams из приложений поставщика Epic (Hyperspace, Haiku, Canto). Кнопка **Начать виртуальное посещение** внедрена в поток поставщика.
...
- Поставщики должны выполнить единый вход со своей учетной записью Microsoft 365 при первом присоединении к посещению.
...
- Поставщики могут видеть обновления участников, подключающиеся и отключающиеся в режиме реального времени для данной встречи. Поставщики могут видеть, когда пациент подключен к посещению.

  ![Взаимодействие с поставщиком посещения с пациентом.](media/ehc-provider-experience-6.png)
...
Соединитель поддерживает присоединение пациентов к посещениям через Интернет и мобильные устройства MyChart. Во время встречи пациенты могут начать посещение из MyChart с помощью кнопки **"Начать виртуальное посещение** ".
...
- Пациенты могут протестировать свои видео и микрофон в зале ожидания перед присоединением к посещению.

  ![Опыт посещения пациента.](media/ehc-virtual-visit-5.png)
