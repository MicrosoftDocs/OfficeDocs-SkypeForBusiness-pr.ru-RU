...
При работе со сменами вы можете управлять доступом к Teams, когда сотрудники, не работающие со сменами, не работают. Например, можно настроить отображение Teams, чтобы сотрудники, работающие с frontline, могли использовать Teams, если они не работают в запланированную смену.  

## <a name="scenario"></a>Сценарий

Вот пример того, как ваша организация может управлять доступом на основе смен.

У вас есть сотрудники, которые работают в вашей организации только в течение нескольких часов, запланированных и утвержденных руководителем. Им не следует платить за время, затраченное на работу вне запланированной смены, включая использование Teams приложения. Вы настроили настраиваемое сообщение "Ваше время в Teams при выходе из смены не будет учитываться в качестве оплаты в часы", которое отображается, когда сотрудники переднего экрана пытаются получить доступ к Teams при выходе из смены. Если они будут использовать Teams, он нажмет кнопку Я **принимаю** с пониманием, что оплата за это время не будет оплачена.

В вашей организации также есть информационные работники, которые получают зарплату и не работают в смену. Информационные работники могут использовать стандартные состояния присутствия в Teams при этом давая сотрудникам смены.
...
|![Сплошной красный кружок означает "Занят(а)"](../../media/flw-presence-busy.png) Занят(а)      | ![Сплошной красный кружок означает "Занят(а)"](../../media/flw-presence-busy.png) Занят(а)         |Настроить автоматически. Можно также настроить вручную, когда сотрудник в режиме смены.|

## <a name="off-shift-access-to-teams"></a>Отключение shift access для Teams

Эта функция позволяет управлять доступом к учетным Teams, когда сотрудники, работающие с приостановой связи, отключались от смены. Можно настроить отображение Teams для сотрудников, работающих с приостановой телефонной линии, Teams при выходе из смены. Перед использованием  этого сообщения сотрудникам нужно нажать кнопку Я принимаю, чтобы подтвердить Teams.
...
Администраторы могут управлять присутствием сотрудников, не работающих с телефоном, с помощью политик. Управлять этими политиками можно с помощью следующих cmdlets PowerShell:
