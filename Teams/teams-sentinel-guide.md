...
> [!NOTE]
> В одном экземпляре Azure Sentinel можно использовать несколько подписок на Microsoft 365. Это позволит выполнять [отслеживание в реальном времени](/azure/sentinel/livestream) и обнаруживать угрозы в архивных файлах журнала. Администраторы смогут выполнять поиск, используя [запросы в разных ресурсах](/azure/azure-monitor/log-query/cross-workspace-query), в одной группе ресурсов, в разных группах ресурсов или в другой подписке.

## <a name="step-1-collect-teams-logs"></a>Шаг 1. Сбор журналов Teams

Этот раздел состоит из трех частей:

1. Включение журналов аудита в **Microsoft 365**.
2. Регистрация приложения в **Microsoft Azure**, чтобы разрешить проверку подлинности и авторизацию для сбора журналов.
3. Регистрация подписки на API, которая позволит осуществлять сбор журналов через API Microsoft 365 с помощью **PowerShell**.

### <a name="enable-audit-logs-in-microsoft-365"></a>Включение журналов аудита в Microsoft 365

Так как Teams регистрирует действия через Microsoft 365, журналы аудита не собираются по умолчанию. Эту функцию можно включить, выполнив [следующие шаги](/microsoft-365/compliance/turn-audit-log-search-on-or-off?view=o365-worldwide&viewFallbackFrom=o365-worldwide%c2%a0). Данные Teams собираются для аудита Microsoft 365 в разделе *Audit.General*.
...
Завершающее действие настройки — получение и регистрация подписки на API, чтобы можно было собирать данные журналов. Для этого API действий управления Microsoft 365 должен получать вызовы REST PowerShell.
...
> [!TIP]
> Дополнительные сведения о внешнем и гостевом типах доступа в Teams см. в [этой статье](./communicate-with-users-from-other-organizations.md) или в разделе *Типы участников* в [руководстве по безопасности Teams](./teams-security-guide.md).
...
[Включение и отключение поиска в журнале аудита](/microsoft-365/compliance/turn-audit-log-search-on-or-off?view=o365-worldwide&viewFallbackFrom=o365-worldwide%c2%a0)

[Что такое Azure Sentinel](/azure/sentinel/overview)
