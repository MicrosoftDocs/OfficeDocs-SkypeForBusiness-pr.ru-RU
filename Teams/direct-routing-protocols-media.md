...
Конечная точка внутренней реализации ICE всегда будет управляющей конечной точкой и будет соответствовать стандарту "Regular" nominations для выбора конечных кандидатов, используемых для потока мультимедиа. Конечная точка ICE Lite может использовать nominations, чтобы завершить путь, который будет использоваться для мультимедиа и полных звонков.

Примечание. в случае разветвления с конечными точками одноранговой сети, отправляющей ответы 183, этот SBC должен быть готов к ответу на проверки из нескольких конечных точек, а также nominations из нескольких конечных точек, если nominations происходит перед 200OK. В зависимости от конвергенции конечного автомата ICE на конечном пути и времени ответа пользователя nominations может происходить до или после 200OK. Этот SBC должен поддерживать оба случая.
...
Параметры MKI и Length не являются обязательными.

Дополнительные сведения можно найти в [статье RFC 4568, раздел 6,1](https://tools.ietf.org/html/rfc4568#section-6.1).

## <a name="sdes-support-requirements"></a>Требования поддержки SDES

Устройство должно поддерживать SDES в формате, как описано ниже. Процессоры Microsoft Media всегда предпочитают SDES.

- Если не использовать мультимедиа, даже если клиент поддерживает DTLS, то процессоры мультимедиа будут преобразовываться в SDES.

- Если у клиента есть только DTLS (будущее рабочее состояние Google), прямая маршрутизация вставит в путь точку управления, преобразуя вызов из режима мультимедиа в режим обхода без мультимедиа. Между однокомпонентным процессором SBC и Media, SDES всегда используется.

В настоящее время нет клиента Teams, предлагающего только DTLS; Несмотря на то что компания Google объявила о том, что в какой – либо момент она прекратит поддерживать SDES.

## <a name="format-for-offer-from-sbc-in-bypass-mode"></a>Формат предложения из SBC в режиме обхода 

Предложение должно содержать SDES и может содержать DTLS необязательно в следующем формате:
...
### <a name="format-for-sdes-only-offer-to-sbc"></a>Формат для SDES только предложением для SBC
