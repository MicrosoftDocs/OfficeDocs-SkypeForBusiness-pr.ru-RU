...
3. В **области Сведения о телефонной** линии укажите внешний префикс набора, если пользователям нужно набрать одну или несколько цифр (например, 9), чтобы получить внешнюю линию. Для этого выполните следующие действия:
    ...
    2. Включите **оптимизированный набор номера устройства**. Если вы указали внешний префикс набора номера, необходимо также включить этот параметр, чтобы применить префикс, чтобы звонки можно было делать за пределами организации.
4. В **области Правила нормализации** настройте и соберите одно или несколько правил нормализации [для](what-are-dial-plans.md#normalization-rules) плана набора номера. С каждой телефонной планом должно быть связано хотя бы одно правило нормализации.  Для этого сделайте следующее:
    ...
    - Чтобы изменить правило нормализации, которое уже связано с телефонной планом, выберите его, щелкнув слева от имени правила и выбрав изменить **.** Внести нужные изменения и нажмите кнопку **Сохранить**.
...
5. Расположить правила нормализации в нужном порядке. Нажмите **кнопку Вверх** **или** Вниз, чтобы изменить положение правил в списке.
    ...
    > [!NOTE]
    > Teams список правил нормализации сверху вниз и использует первое правило, которое соответствует набратому номеру. Если вы настроили для набора номера несколько правил нормализации, убедитесь, что более строгие правила отсортировали над менее строгими.

6. Нажмите кнопку **Сохранить**.
7. Если вы хотите проверить набор номеров, **введите** номер телефона в области Проверка набора номера и нажмите кнопку **Проверить**.
...
Запустите этот запуск, чтобы удалить все назначенное tenantDialPlan для всех пользователей, у которых есть hostingProvider sipfed.online.lync.com.
```PowerShell
...
- [Метка отказов от ответственности для экстренных вызовов](https://github.com/MicrosoftDocs/OfficeDocs-SkypeForBusiness/blob/live/Teams/downloads/emergency-calling/emergency-calling-label-(en-us)-(v.1.0).zip?raw=true)
