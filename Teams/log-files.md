...
Чтобы пользователи регистрирует диагностические данные для собраний Teams, они должны включить этот параметр в клиенте Teams. Они перейдите в раздел **"Общие** >  параметры **", выберите** "Включить журналы мультимедиа" (диагностические данные для аудио **, видео и демонстрации экрана). Требуется перезапуск Teams** (требуется перезапуск Teams), перезапуск Teams и воспроизведение проблемы. Чтобы начать ведение журнала, необходимо перезапустить клиент Teams. Пользователи могут перезапустить его, щелкнув правой кнопкой мыши значок на панели задач (Mac) или панели задач (Windows) и выбрав "Выйти". После выхода они могут щелкнуть значок приложения, чтобы снова открыть Teams.
