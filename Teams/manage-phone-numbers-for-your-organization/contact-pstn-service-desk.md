---
title: Обратитесь в службу поддержки ННР
author: TonySmit
ms.author: tonysmit
manager: serdars
ms.reviewer: mikedav, roykuntz, jastark
ms.topic: conceptual
ms.tgt.pltfrm: cloud
ms.service: msteams
search.appverid: MET150
ms.collection:
- M365-voice
audience: Admin
appliesto:
- Skype for Business
- Microsoft Teams
localization_priority: Normal
f1.keywords:
- CSH
ms.custom:
- ms.teamsadmincenter.voice.pstnservicedesk
- ms.teamsadmincenter.voice.contactPSTNsupport
- Calling Plans
ROBOTS: NOINDEX, NOFOLLOW
description: При получения номеров телефонов или переносных (переносных) номеров для организации может потребоваться получить справку и поддержку в службе поддержки ННР.
ms.openlocfilehash: b0ba69aae5c25db788279ec4e28ad306514b1974
ms.sourcegitcommit: 7a9c63ee790108eaa61950ce28ae8027311039d9
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 08/13/2020
ms.locfileid: "46662099"
---
# <a name="pstn-service-desk-contact-information"></a>Контактные данные службы ННР

Чтобы связаться с командой службы ННР, отправьте сообщение электронной почты на соответствующий псевдоним, указанный в таблице ниже.

| **Я хочу перенести/перенести существующие номера**  | **Заказ новых номеров для** |
|:-----:|:------:|
| [**Напишите нам в США** или Канаде по электронной ptn@microsoft.com](mailto:ptn@microsoft.com)| [**Бельгия, Франция, Германия, Великобритания, Ирландия, Нидерланды, Испания, Швеция, Швейцария,** отправьте нам по электронной почте ptneu@microsoft.com](mailto:ptneu@microsoft.com)|
|[**За пределами США или Канады** напишите нам по электронной ptneu@microsoft.com](mailto:ptneu@microsoft.com)| [**Остальной мир**. Отправьте нам сообщение по электронной gcstnmsd@microsoft.com](mailto:gcstnmsd@microsoft.com)|
|Из-за большого пика входящих  запросов группа переноса в Северной Америке находится в неполнеемом качестве. Мы работаем над тем, чтобы очистить это невыполнение на основе функции FIFO (First In, First Out), в связи с чем наши ответы на новые запросы будут **задержаны.**|Из-за большого пика входящих запросов группа по переносу портов ЕС **не** может найти их. Мы работаем над тем, чтобы очистить это невыполнение на основе функции FIFO (First In, First Out), в связи с чем наши ответы на новые запросы будут **задержаны.**|

> [!NOTE]
> Ответы в Северной Америке могут быть задержаны из-за большого увеличения входящих запросов. Невыполнеемая невыполнена на основе данных FIFO (First In, First Out).

## <a name="transfer-or-port-your-existing-phone-numbers-to-microsoft-teams"></a>Перенос существующих номеров телефонов в Microsoft Teams
Отправьте электронную почту на соответствующий псевдоним электронной почты из таблицы выше. Включив как можно больше сведений, например:
  - Ваш [ИД организации (ИД клиента)](https://docs.microsoft.com/onedrive/find-your-office-365-tenant-id)или активные домены Майкрософт.
  - Типы номеров телефонов, по которые требуется помощь; например, пользователь, платный доступ к службе или бесплатный доступ к службе.
  - Другие важные сведения, основанные на вашем запросе.

## <a name="get-new-phone-numbers-for-microsoft-teams"></a>Получить новые номера телефонов для Microsoft Teams
Отправьте электронную почту на соответствующий псевдоним электронной почты из таблицы выше. Включив как можно больше сведений, например:
  - Ваш [ИД организации (ИД клиента)](https://docs.microsoft.com/onedrive/find-your-office-365-tenant-id) или активные домены Майкрософт.
  - Типы номеров телефонов, по которые требуется помощь; например, пользователь, платный или бесплатный номер службы.
  - Количество номеров телефонов, которые вам нужны, а также коды областей, страны или региона, из которых должны поступать новые телефонные номера.

## <a name="additional-resources"></a>Дополнительные ресурсы

- Справку по настройке и настройке номеров, лицензиям, [](https://docs.microsoft.com/microsoft-365/admin/contact-support-for-business-products?view=o365-worldwide&tabs=online)сборам или выставлению счета см. в справке для администраторов.
- Сведения о планах звонков, доступных в вашей стране или регионе, см. в сведениях о доступности стран и регионов для аудиоконференций [и планов звонков.](../country-and-region-availability-for-audio-conferencing-and-calling-plans/country-and-region-availability-for-audio-conferencing-and-calling-plans.md)
- Сведения о том, как выбрать подходящие типы телефонных номеров для организации, см. в справке по различным типам номеров, используемых в [планах звонков.](../different-kinds-of-phone-numbers-used-for-calling-plans.md)
- Сведения о том, как настроить номера телефонов для организации, см. в справке для администраторов в службе поддержки продуктов [для бизнеса.](https://docs.microsoft.com/microsoft-365/admin/contact-support-for-business-products?view=o365-worldwide&tabs=online)

## <a name="related-topics"></a>См. также:

[Типы номеров телефонов, используемые в планах звонков](../different-kinds-of-phone-numbers-used-for-calling-plans.md)

[Управление номерами телефонов организации](manage-phone-numbers-for-your-organization.md)

[Условия и положения, распространяющиеся на экстренные вызовы](../emergency-calling-terms-and-conditions.md)
  
