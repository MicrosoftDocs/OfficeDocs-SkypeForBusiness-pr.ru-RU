...
---
# <a name="how-to-use-analog-devices-with-phone-system-direct-routing"></a>Использование аналоговых устройств с прямой маршрутией телефонной системы
...
На приведенной ниже схеме Прямая маршрутия настроена таким образом, что все звонки Teams на номера между +1425 4XX XX XX и +1425 5XX XX XX должны принимать красный маршрут (пунктирная линия), а все звонки по ННР между номерами между +1425 4XX XX и любыми другими номерами, кроме диапазона +1425 5XX XX, должны проходить по синей (сплошной линии). 
...
Сведения о том, как подключить ATA к SBC и настроить его, см. в руководстве изготовителя SBC:
...
Эта команда изменяет учетную запись пользователя с помощью идентификатора exampleuser@contoso.com. В этом случае учетная запись будет изменена, чтобы включить Корпоративная голосовая связь — реализацию VoIP (Майкрософт) с включенной голосовой почтой, и назначит номер +14255000000 этому пользователю.  Эту команду следует выполнить для каждого пользователя Teams (за исключением пользователей устройств ATA) в клиенте организации.

```powershell
PS C:\> Set-CsUser -Identity "exampleuser@contoso.com" -EnterpriseVoiceEnabled $True -HostedVoiceMail $True -OnPremLineUri "tel:+14255000000"
```

## <a name="step-6-assign-the-voice-route-policy-to-a-user"></a>Шаг 6. Назначение политики голосового пути пользователю
