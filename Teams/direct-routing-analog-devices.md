...
В этой статье описано использование аналоговых устройств с телефонная система прямой маршрутии. Чтобы подключить аналоговые устройства к прямой маршрутике, необходимо использовать аналоговый адаптер телефонии (ATA), который должен поддерживаться сертифицированным поставщиком пограничного контроллера сеанса . 

Когда пользователь звонит с аналогового устройства, сигнальный поток и мультимедиа проходят через адаптер аналоговой телефонии (ATA) к SBC.  SBC отправляет звонок на конечную точку Microsoft Teams телефонную сеть общего перейти на телефонную сеть (STN) на основе внутренней таблицы маршрутов.  Когда устройство звонит, маршрут, который он принимает, зависит от политик маршрутинга, созданных для устройства.
...
> ![Схема с настройкой прямой маршрутики](media/direct-routing-analog-device.png)
...
2. Создайте службу использования ДНР.
...
## <a name="step-5-enable-the-online-user"></a>Шаг 5. Включить пользователя в Сети
...
Эта команда назначает политику сетевой маршрутизации голосовой маршрутизации "на пользователя" (AnalogInteropPolicy) пользователю с идентификаторами exampleuser@contoso.com.  Эту команду следует выполнить для каждого пользователя Teams (за исключением пользователей устройств ATA) в клиенте компании.
