...
### <a name="v160-published-on-july-22-2022"></a>Версия 1.60 опубликована 22 июля 2022 г.
...
|:---------------------------------------|:-----------------------------------------------------------------|
|Источник входящего вызова<sup>1</sup>        |Распределение вызовов по внутреннему или внешнему источнику вызовов             |
...
|Источник входящего вызова<sup>1</sup>                |Тип вызова<br>TotalCallCount                |Внешние вызовы: тип вызова — external<br>Внутренние вызовы: тип вызова является внутренним |
...
|AACallResult                            |Текст                     |Конечный результат вызова — возможные значения:<br><br>§ failed_to_establish_media (не удалось установить часть носителя вызова)<br>§ failover_to_operator (вызов, передаваемый оператору, обычно из-за системной ошибки)<br>§ oaa_chain_too_long (слишком много в AA)<br>§ oaa_session_too_long (слишком длинный сеанс AA)<br>§ service_declined (AA не принял звонок)<br>§ service_terminated (конфигурация AA отключит вызов)<br>§ terminated_automatic_selection (конфигурация AA отключит вызовы)<br>§ terminated_no_operator (вызов завершен из-за ошибки, оператор не определен) <br>§ terminated_transfer_failed (вызов прерван из-за сбоя передачи — обычно на срок действия)<br>***§ transferred_to_operator*** (вызов был передан оператору — обычно из-за ошибки ввода пользователем)<br>§ transferred_to_receptionist (то же, что и transferred_to_operator)<br>§ transferred_to_self (вызов был возвращен в начало AA — обычно из параметра объявления меню)<br>§ transferred_to_shared_voicemail (звонок был передан в общую голосовую почту)<br>§ transferred_to_user (звонок был передан пользователю — включает очереди вызовов)<br>§ неизвестно (произошла неизвестная ошибка)<br>§ user_terminated (вызывающий объект завис) |
...
|Источник входящего вызова<sup>1</sup>    |fCallQueueAnalytics — > число вызовов<br>fCallQueueAnalytics — > типа вызова    |Внешние вызовы: тип вызова — external<br>Внутренние вызовы: тип вызова является внутренним |
...
|Имя                                    |Тип данных                |Описание                            |
|:---------------------------------------|:------------------------|:--------------------------------------|
|Удостоверение очереди вызовов                     |Текст                     |Имя учетной записи ресурса, присоединенной к очереди вызовов<br><br>Если полное имя учетной записи ресурса **cq_test@microsoft.com**, это значение будет таким: **cq_test** |
...
|Результат вызова очереди звонков                  |Текст                     |Конечное состояние вызова очереди вызовов— возможные значения:<br><br>§ agent_joined_conference (ответы на звонки в режиме конференции)<br>§ отклонено<br>§ отключено<br>Ошибка §<br>Сбой §<br>§ недействительный<br>§ overflown (выполняется условие переполнения)<br>§ timed_out (выполняется условие ожидания)<br>§ transferred_to_agent (ответ на вызовы режима транскриптора {default}) |
|Удостоверение очереди вызовов                     |Текст                     |Имя учетной записи ресурса, присоединенной к очереди вызовов<br><br>Если полное имя учетной записи ресурса **cq_test@microsoft.com**, это значение будет таким: **cq_test** |
...
|Число вызовов                              |Целое число             |Summarize: Sum<br>Количество вызовов, представленных агенту     |
...
|Имя очереди вызовов                         |Текст                     |Имя учетной записи ресурса, присоединенной к очереди вызовов<br><br>Если полное имя учетной записи ресурса **cq_test@microsoft.com**, это значение будет таким: **cq_test** |
|Дата                                    |Дата                     |                                                    |
