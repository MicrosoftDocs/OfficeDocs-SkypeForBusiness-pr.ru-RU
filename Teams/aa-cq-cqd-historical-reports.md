...
ms.locfileid: "58398948"
---
# <a name="auto-attendant--call-queue-historical-report"></a>Auto Attendant & Call Queue Historical Report

Шаблон CQD Teams auto Attendant & истории очереди вызовов Power BI содержит три отчета:
...
В следующих действиях предполагается, что Power BI Desktop на компьютере уже установлен и что у вашей учетной записи есть необходимые разрешения на доступ к каналу данных CQD.
...
|AATransferAction                        |Текст                     |Целевой тип перена передачи вызовов: возможные значения:<br><br>***§ приложение — сущность голосового** приложения _<br> § <br> external_pstn_§ hunt_group - Queue *_entity_* _<br>_ * _§ orgaa - Organizational Auto Attendant entity_**<br>§ shared_voicemail<br>§ неизвестно<br>§ пользователь |
...
|Результат вызова в очереди вызовов                  |Текст                     |Конечное состояние очереди вызовов : возможные значения:<br><br>§ agent_joined_conference<br>§ отклонено<br>§ отключено<br>Ошибка §<br>§ не удалось<br>§ недопустимый<br>§ переполнение<br>§ timed_out<br>§ transferred_to_agent |
|Удостоверение очереди вызовов                     |Текст                     |Имя учетной записи ресурса, подключенной к очереди вызовов<br><br>Если полное имя учетной записи **ресурса cq_test@microsoft.com** это значение будет: **cq_test** |
...
|Имя                                    |Тип данных                |Описание                                         |
|:---------------------------------------|:------------------------|:---------------------------------------------------|
|Имя агента                              |Текст                     |Имя пользователя (UPN)<br>Если полное имя пользователя **user@microsoft.com** это значение будет: **user** |
