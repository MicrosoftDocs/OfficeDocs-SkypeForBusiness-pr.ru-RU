...
- Вам будет предложено выбрать регион конвейера обработки данных CQD. Выберите регион, в котором находится ваш клиент.
 ...
 - Регион можно увидеть с помощью cmdlet PowerShell в Skype для бизнеса Online (Get-CsTenant). Выход ServiceInstance. 
...
|Имя                                      |Описание                            |
|:---------------------------------------|:--------------------------------------|
|Источник входящих вызовов                    |Распространение зовов по внутреннему или внешнему источнику вызовов      |
...
|Timeout/Overflow call total action      |Распределение ЗВОНКА NOT forwarded(abandoned) по результатам звонка   |
|Целевые итоги для передачи и перена передачи          |Распределение переадпределяемого звонка по результатам звонка                  |
...
- В настоящее время в очереди вызовов и автоотчете вместо имен очереди вызовов и автоотетаря вы можете показывать ИД учетной записи ресурса.  Чтобы отобрать весь трафик для автозавода или очереди вызовов, необходимо выбрать все учетные записи ресурсов, которые назначены автозаводу или очереди вызовов.

- В настоящее время на панели мониторинга доступна только история за 28 дней, так как данные очереди вызовов и автозасмотра считаются личные сведения конечными пользователями и подотледят политикам хранения конфиденциальности данных.
