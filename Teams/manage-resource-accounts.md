...
### <a name="edite-resource-account-name"></a>Изменение имени учетной записи ресурса
...
![Значок с номером 3, указывающий на выноску на предыдущем снимке экрана](media/sfbcallout3.png) после создания учетной записи ресурса и назначения лицензии, вы можете выбрать команду **назначить или отменить назначение** , чтобы назначить учетной записи ресурсов номер Услуги тарифного плана, или назначить Учетная запись ресурса для автоматического ассистента или уже существующей очереди звонков. Назначение прямого номера маршрутизации может выполняться только с помощью командлетов. Если вы хотите создать очередь звонков или автосекретарь, вы можете связать ее с учетной записью ресурса. Когда все будет готово, нажмите кнопку **сохранить** .
