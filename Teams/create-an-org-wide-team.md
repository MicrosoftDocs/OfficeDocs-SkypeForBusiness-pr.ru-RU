...
С помощью групп на уровне Организации глобальные администраторы могут легко создать общедоступную группу, которая будет использовать для всех пользователей в Организации и хранить сведения о членстве в Active Directory при присоединении пользователей и выходе из нее. Только глобальные администраторы могут создавать коллективы на уровне Организации, и в настоящее время группы на уровне Организации ограничены организациями, не имеющими более 5 000 пользователей. Если эти требования соблюдены, глобальные администраторы будут видеться **** как один из вариантов, если при создании команды будет выбрано **Создание группы с нуля** . 
...
После создания группы Организации все глобальные администраторы добавляются в качестве владельцев групп, и все активные пользователи добавляются в качестве участников группы. Пользователи, отключенные для групп, гостей и пользователей, которые не добавляются в группу. Так как каталог вашей организации обновляется с учетом новых активных пользователей, а также если пользователи больше не работают в вашей компании, а их лицензия на команды отключена, изменения автоматически синхронизируются и пользователи добавляются или удаляются из команды. Участники группы не могут выйти из группы на уровне Организации. Как владелец группы вы можете вручную добавлять или удалять пользователей, если это необходимо.
...
Уменьшить шум на канале, добавив в канал общего канала только владельцев группы. Перейдите в группу и нажмите кнопку **Дополнительные параметры (...)** .  >  **Управление командой**. На вкладке **Параметры** нажмите кнопку **разрешения для участника** _гт_ выбрать **только владельцев может публиковать сообщения**.
### <a name="turn-off-team-and-team-name-mentions"></a>Отключение @team и @ [название группы] упоминания
 Уменьшите @mentions, чтобы они не были перезагружены всей Организацией. Перейдите в группу и нажмите кнопку **Дополнительные параметры (...)** .  >  **Управление командой**. На вкладке **Параметры** нажмите кнопку <strong>@mention</strong> _гт_ отключить **Отображение членов @team или @ [название группы]**. 
...
Если это не так, вы можете создать общедоступную команду с помощью PowerShell и добавить глобального администратора в качестве владельца команды. Затем попросите глобального администратора щелкнуть **̇ ̇ ̇ Дополнительно** рядом с названием группы, выбрать команду **изменить группу**, а затем изменить конфиденциальность в масштабах Организации, которые **будут автоматически добавлены**. Обратите внимание, что только владельцы группы могут получить доступ к параметру **изменить группу** , и только глобальные администраторы могут видеть параметр для **всей Организации** .
...
Глобальные администраторы могут преобразовать существующую команду в группу Организации, которая редактирует ее в клиенте Teams.
Перейдите к названию группы и нажмите кнопку Дополнительные параметры... (многоточие) _Гт_ изменить группу.
