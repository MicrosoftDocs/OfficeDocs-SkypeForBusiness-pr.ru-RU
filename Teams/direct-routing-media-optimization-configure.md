...
Примечание. Чтобы обеспечить непрерывное прерывание операций при одновременной настройке локальных операций оптимизации мультимедиа и маршрутизации Location-Based (LBR), необходимо включить для BBCs downstream SBCs LBR, задав для параметра GatewaySiteLbrEnabled (GatewaySiteLbrEnabled) $true для каждого теного SBC. (Этот параметр не является обязательным для прокси-сервера SBC.)
...
На следующей схеме показана SIP-интерфейс для входящие зовов в режиме AlwaysBypass, и пользователь находится в том же расположении, что и SBC.
