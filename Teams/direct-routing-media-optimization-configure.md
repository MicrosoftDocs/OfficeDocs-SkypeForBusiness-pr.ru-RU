...
В следующем примере к сетевым сайтам Вьетнама, Индонезии и Сингапура в регионе APAC добавляются три SBCs с режимом Всегда обходить:
...
SBC, подключенный к службе direct Routing, сообщает о том, где находится SBC, Record-Route поля контактов. На основе этих полей медиатайл вычисляется с помощью прямой маршрутии.
...
| X-MS-UserLocation: внешний | Поле указывает, что пользователь находится за пределами корпоративной сети. |
