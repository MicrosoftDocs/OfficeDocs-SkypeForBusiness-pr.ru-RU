...
Чтобы определить сетевые подсети и связать их с сетевыми сайтами, воспользуйтесь New-CsTenantNetworkSubnet-New-CsTenantNetworkSubnet. Каждую сетевую подсеть можно связывать только с одним сайтом. 
...
| X-MS-MediaPath: VNsbc.contoso.com |   Указывает, какой SBC должен проходить через клиент до целевого SBC. В этом случае у нас есть Always Bypass, а клиент является внутренним именем, которое отправляется как единственное имя в заглавном имени. | 
...
| X-MS-MediaPath: proxysbc.contoso.com, VNsbc.contoso.com    | Указывает, какой SBC должен проходить через клиент до целевого SBC. В этом случае у нас есть always Bypass, и клиент является внешним. |
...
| Режим | Пользователь | Сайт |  Направление звонка |
|:------------|:-------|:-------|:-------|
AlwaysBypass |  Внешняя |  Недоступно |   Входящий |
...
На следующей схеме показан входящий звонок в режиме OnlyForLocalUsers, и пользователь находится в том же расположении, что и SBC. Этот поток тот же, что и при входящие звонки, если пользователь находится в том же расположении, что [и SBC.](#inbound-calls-and-the-user-is-in-the-same-location-as-the-sbc-with-always-bypass)
