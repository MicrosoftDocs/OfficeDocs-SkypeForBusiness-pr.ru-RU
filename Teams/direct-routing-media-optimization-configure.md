...
> [!NOTE]
> Логика локальной оптимизации мультимедиа зависит от того, настроены ли клиентские адреса как внешние или внутренние по отношению к корпоративным сетям с внутренним интерфейсом SBC, сертифицированным для прямой маршрутизации. Расположение клиента (внутренняя или внешняя) определяется во время обработки каждого звонка путем наблюдения адреса, используемого для обращения к ретранслятору транспорта.
> 
> В сценариях vpn для разделенного канала, где ретрансляторы можно использовать через интернет-провайдера, клиентская логика наилучшего маршрута предпочитает стандартный маршрут локального интерфейса (например, общедоступный Wi-Fi). Это приводит к сигналу Корпорации Майкрософт к SBC о том, что клиент является внешним, даже если он может получить доступ к внутреннему интерфейсу SBC для прямой маршрутии клиента. Клиенты прямой маршрутизации, использующие локализованную оптимизацию мультимедиа, могут иметь нестандартное время настройки звонков и в некоторых случаях не пользоваться звуком при получении звонков из ПССО.
> 
> Чтобы избежать этого, администраторам VPN необходимо заблокировать доступ между удаленными ПОЛЬЗОВАТЕЛЯМи VPN и внутренним интерфейсом SBC прямой маршрутии.
...
Обратите внимание на следующее: 
   - Если у клиента один SBC, параметр -ProxySBC должен быть обязательным $null или FQDN SBC (Центральный SBC с централизованной связи).
