...
ms.locfileid: "50999821"
---
# <a name="local-media-optimization-for-direct-routing"></a>Оптимизация локальных мультимедиа для прямой маршрутизации

Голосовая связь телефонной сети общего перейти на телефонную сеть (ННР) считается критически важным приложением, которое ожидает высокого качества голосовой связи. Прямая маршрутия позволяет управлять потоками трафика мультимедиа, чтобы вместить большое количество сетевых топорологий и настройки локальных телефонов для различных предприятий по всему миру. 

Локальная оптимизация мультимедиа для прямой маршрутизации позволяет управлять качеством звука с помощью:

-   Контроль потока медиапотока между клиентами Teams и клиентские граничными контроллерами сеанса (SBCs).
-   Локальное сохранение мультимедиа в пределах корпоративных сетевых подсетей.
...
-   Создание топологии виртуальных сетей на основе SBCs, в которых SBCs в локальных филиалах подключены к централизованному прокси-серверу SBC, который виден и с которой можно связываться, с телефонной системой Майкрософт через ее внешний IP-адрес. В виртуальной топологии сети внутренние SBCs общаются с внутренними IPS и не видны непосредственно телефонной системе.
...
- В регионе **APAC** централизованный прокси-сервер сопряжен с маршрутикой Microsoft Direct Routing, которая направляет мультимедиа между интерфейсом Direct Routing и 9stream SBCs в локальных филиалах. 
  ...
  Перейти к верхней части SBCs в локальных филиалах не видно непосредственно для прямой маршрутики в APAC, но они связаны с помощью Set-CSOnlinePSTNGateway для создания виртуальной топологии сети в телефонной системе Майкрософт. Мультимедиа всегда остаются локальными, когда это возможно. У внешних пользователей есть поток мультимедиа между клиентом Teams и общедоступным IP-адресом прокси-сервера SBC.


## <a name="central-sbc-with-centralized-trunks"></a>Центральная SBC с централизованной магистралью
...
- Клиент Teams пользователя взаимодействует с телефонной системой напрямую через REST API, но мультимедиа, созданные во время звонка, перетекают на внутренний IP-адрес SBC. 

- SBC перенаправляет поток в телефонную систему и подключенную сеть STN. 
...
![Схема, показывающая поток трафика Local Media Optimization](media/direct-routing-media-op-1.png "Поток трафика, когда пользователь находится на домашнем сайте с централизованной SBC с подключенной централизованной связью SIP-связи")
...
![Схема, показывающая поток трафика Local Media Optimization](media/direct-routing-media-op-2.png "Поток трафика, если пользователь является внешним, если у вас есть централизованная связь SBC с подключенной централизованной связью SIP-связи")

## <a name="proxy-sbc-with-connected-downstream-sbcs"></a>Proxy SBC with connected downstream SBCs

Чтобы создать решение, в котором службы STN предоставляются во всех локальных филиалах региона APAC, где централизование линии связи TDM не является вариантом, администратор Contoso парил одну SBC (proxysbc.contoso.com), также называемую прокси-сервером SBC, со службой прямой маршрутизации. 

После этого администратор Contoso добавит несколько серверов SBCs, которые будут проходить через сервер SBC прокси-proxysbc.contoso.com. У SBCs downstream нет общедоступных IP, но их можно на назначены голосовым маршрутам. В таблице ниже показаны примеры параметров сети и конфигурации.

Когда пользователь находится в локальном филиале, где находится 9-й SBC, трафик мультимедиа передается между ним и локальной сетной SBC напрямую. Если пользователь находится за пределами офиса (в общедоступных интернет-службах), потоки мультимедиа перетекают от него на общедоступный IP-адрес прокси-сервера SBC, который прокси-сервера передается на соответствующие сетные серверы SBC.
...
-  Прокси-сервер SBC будет виден телефонной системе только по внешнему IP-адресу, а поток будет маршрутиться из направлений направлений SBC (в данном случае — местный SBC во Вьетнаме) в телефонную систему. 

- Теневая SBC в локальном филиале не видна непосредственно телефонной системе, но соповодится внутри топологии виртуальной сети, которая определена администратором Contoso при настройке local Media Optimization.
...
Дополнительные сведения о возможных режимах и соответствующих действиях см. в сведениях о настройке оптимизации локальных мультимедиа.

Схема 3. Поток трафика, когда пользователь находится в домашней сети с прокси-сервером SBC и подключенными 9-ми подключениями КБ. 
...
- В зависимости от конфигурации [](direct-routing-media-optimization-configure.md) и политик голосовой почты (подробные сведения см. в сведениях о настройке локальной оптимизации мультимедиа), прокси-сервер SBC перенаправляет поток на ходящийся вниз SBC во Вьетнаме. 
...
-  Теневая SBC в локальном филиале не видна непосредственно телефонной системе, но соповодится в топологии виртуальной сети, которая определена администратором Contoso при настройке local Media Optimization. В этом примере пользователь считается внешним, так как находится за границами корпоративной сети. 

Схема 4. Поток трафика, если пользователь является внешним с прокси-сервером SBC и подключенными вниз SBCs

![Схема, показывающая поток трафика Local Media Optimization](media/direct-routing-media-op-4.png "Поток трафика на прокси-сервере SBC с подключенными экономными SBCs при внешнем пользователе")
...
Чтобы различать режимы оптимизации локальных мультимедиа, администратор клиента должен с помощью командлета Set-CSonlinePSTNGateway установить для параметра -BypassMode параметр "Always" или "OnlyForLocalUsers". Дополнительные сведения см. в [подстройке "Настройка локальной оптимизации мультимедиа".](direct-routing-media-optimization-configure.md)  
 ...
 > [!NOTE]
  > Если пользователь является внутренним, требуется подключение мультимедиа между пользователем и SBC по внутреннему IP-адресу. В данном случае не существует каких-либо дополнительных альтернатив для ретрансляторов общественного транспорта, так как SBC предоставляет внутренний IP-адрес для подключения к мультимедиа. 
...
- Сценарий 1. Пользователь находится в том же расположении, что и SBC, определенное в политике маршрутинга голосовой почты.

- Сценарий 2. Пользователи и шлюзы находятся на разных сайтах.

#### <a name="scenario-1-the-user-is-in-the-same-location-as-the-sbc-defined-in-the-online-voice-routing-policy"></a>Сценарий 1. Пользователь находится в том же расположении, что и SBC, определенное в политике маршрутации голосовой почты Online.

SBC в Сервереве настроен как прокси-сервер SBC для локального 9-го 9-го SBC в Германии. Пользователь находится в Германии в той же подсети, что и корпоративная сеть местного SBC. Обе SBCs (прокси-сервер и последующий поток) настроены для режима "Всегда обходить". Политики сетевой маршрутизировать голосовую маршрутку определяют, что в случае звонков в Германии (с кодом города +49) они должны быть маршрутизироваться на местный номер SBC в Германии. Все другие звонки (и в случае сбой SBC в Германии) должны быть перенаправляться на прокси-сервер SBC в Швейцарии. В таблице ниже подытоживается пример конфигурации. 
...
- Клиент Teams пользователя взаимодействует с телефонной системой непосредственно через REST API. 
...
- Прокси-сервер SBC виден телефонной системе только через внешний IP-адрес и маршрутит поток от местной SBC (в данном случае местной SBC в Германии) на телефонную систему. 
...
- SBC в Германии перенаправляет поток на прокси-сервер SBC в Мичигане и в подключенную локализованную сеть STN. 
...
Например, в регионе APAC предположим, что у Contoso несколько офисов в разных странах. Во многих странах переключение на SIP не возможно, так как у компании все еще есть связи TDM во многих локальных филиалах компании. Централизация магистрали TDM в регионе APAC не является одним из вариантов. Более того, в регионе APAC есть более 50 филиалов Contoso с сотнями шлюзов. 

Чтобы создать решение, в котором службы STN предоставляются во всех локальных филиалах в регионе APAC, где централизование линии связи TDM не является возможностью, администратор Contoso примещает один региональный SBC в Сингапуре как SBC-сервер прокси-сервера к службе Прямой маршрутизации. Прямое подключение между филиалами местного филиала не плохое, но между каждым местным филиалом и региональным офисом SBC в Сингапуре существует хорошая связь. Для региональной SBC администратор выбирает режим "Всегда обходить", а для локальных сетных УАП — режим "Только для локальных пользователей".
...
- Прокси-сервер SBC будет виден телефонной системе только по внешнему IP-адресу и перенаведет поток из направлений SBC (в данном случае — местный SBC во Вьетнаме) в телефонную систему. 
...
На следующей схеме внутренний пользователь, находясь локально в индонезийском филиале, совершает прямой маршрутный телефонный звонок через Teams на номер во Вьетнаме. 
...
- SBC Downstream in Vietnam передает поток подключенной локальной сети ННП. 

- Прокси-сервер SBC будет виден телефонной системе только по внешнему IP-адресу.

- Ветвные SBCs в локальных филиалах не видны непосредственно телефонной системе, но соотноситься в виртуальной топологии сети.
...
Ниже представлен список известных проблем, которые в настоящее время присутствуют в local Media Optimization. Корпорация Майкрософт работает над решением этих проблем.
...
| Когда клиент Teams идентифицирован как внутренний, при перепадах вызовов могут происходить перепады звонков.| Отключать оптимизацию локальных мультимедиа на SBC Direct Routing.|
| Звонки между внутренними клиентами с 1 по 1 до 1 в многочасовой звонок с внешним клиентом или ресурсом в результате забросов звонков | Идет работа над исправлением. Вы также можете отключить оптимизацию локальных мультимедиа в SBC direct Routing.|
