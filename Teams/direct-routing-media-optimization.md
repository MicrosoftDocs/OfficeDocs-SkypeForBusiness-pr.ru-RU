---
title: Оптимизация локальных мультимедиа для непосредственного оха тутожения
author: CarolynRowe
ms.author: crowe
manager: serdars
ms.date: 04/07/2020
...
-   Контроль потока медиапотока между клиентами Teams и контроллерами границ сеанса клиента.
-   Сохранение локального мультимедиа в пределах подсетей корпоративной сети.
-   Разрешение потоков мультимедиа между клиентами Teams и SBCs, даже если они находятся за корпоративными брандмауэрами с личными IPS и не видны непосредственно корпорации Майкрософт.
...
-   Создание виртуальной топологии сети на веб-сайте SBCs, где SBCs в локальных филиалах подключены к централизованному прокси-серверу SBC, который виден вам, и общаются с Телефон (Майкрософт) System через свой внешний IP-адрес. В виртуальной топологии сети SBCs, отстояющие от сети, общаются через внутренние IP и не видны телефонная система.
...
- **В Европе** у Contoso есть офисы приблизительно в 30 странах. У каждого офиса есть собственный частный филиал Exchange (УАКС). 

  Contoso предлагается централизованное расположение для всех 30 офисов в Европе (в том числе Для Европы). Компания Contoso развернула SBC вТипе, предоставила достаточно пропускной способности для звонков в централизованном расположении, подключила к централизованным расположениям канал SIP и стала обслуживать все европейские расположения из Канала. 
...
## <a name="central-sbc-with-centralized-trunks"></a>Центральный SBC с централизованными туловищами

Чтобы создать решение, в котором службы STN предоставляются во все локальные филиалы через единый центральный SBC с подключенной централизованной связью SIP, администратор клиента Contoso примещает один SBC (centralsbc.contoso.com) к службе; К SBC подключена централизованная связь SIP. 
...
В локальной службе пользователю назначено локальное филиал в Германии. Пользователь звонит прямой маршрутизов через Teams.
...
- SBC перенаправляет поток на телефонная система подключенную сеть STN. 
...
На следующей схеме показан поток трафика, если пользователь не является локальной и не подключен к корпоративной сети (то есть устройство пользователя подключено к Интернету с помощью мобильного устройства или открытой сети Wi-Fi). Пользователь звонит прямой маршрутизов через Teams:

- Клиент Teams пользователя общается с телефонная система через API REST, но в данном случае мультимедиа, созданные во время звонка, перетекают на внешний IP-адрес центрального SBC. 
...
После этого администратор Contoso добавит несколько SBCs, которые будут прокси-серверами SBC через сервер SBC proxysbc.contoso.com. У SBCs downstream нет общедоступных ИП, но их можно на назначены голосовым маршрутам. В таблице ниже показаны примеры сетевых параметров и конфигурации.
...
На следующей схеме показан высокоуровневый трафик для сценария, когда пользователь находится в офисе в регионе APAC. Пользователь, который назначен локальному филиалу во Вьетнаме, совершает телефонный звонок прямой маршрутизовой маршрутизов через Teams. 

- Клиент Teams пользователя общается с телефонная система через API REST, но мультимедиа, генерируемые во время звонка, перетекают на внутренний IP-адрес локального SBC.

- Локализованный сервер SBC перенаправляет поток на прокси-сервер SBC в Сингапуре и в подключенную локовую сеть STN.

-  Прокси-сервер SBC виден телефонная система через внешний IP-адрес и маршрутит поток от SBC (в данном случае локальный SBC во Вьетнаме) на телефонная система. 
...
Схема 4. Поток трафика, когда пользователь является внешним с прокси-сервером SBC и подключенными SBCs с последующими подключениями
...
- SBC в локальном филиале не виден непосредственно телефонная система, но отображается в топологии виртуальной сети, определяемой администратором Contoso при настройке оптимизации локальных мультимедиа.
...
SBC в Во время ветви настроен как прокси-сервер SBC для локального 9-го ходящего SBC в Германии. Оба SBCs (прокси-сервер и вниз) настроены для режима всегда обходить. Внутренний пользователь во Франции, расположенный в локальном филиале, звонит в Непосредственное перенаправиние в Германия. Политики маршрутизов голосовой связи в Интернете определяют, что звонки в Германия (с кодом города +49) должны перенаться в местный SBC в Германии. Все остальные звонки ( и в случае сбой SBC в Германии — все звонки в Германии) должны перенаправиться на прокси-сервер SBC в Городе. В таблице ниже подведены итоги конфигурации примера. 
...
На следующей схеме показан высокоуровневый трафик, когда внутренний немецкий пользователь, расположенный во Франции, совершает телефонный звонок прямой маршрутизовой Teams номеру в Германии. 
...
![Схема, на диаграмме показаны потоки трафика Local Media Optimization](media/direct-routing-media-op-6.png "Поток трафика в режиме &quot;Всегда обходить&quot;, а пользователь находится не на домашнем сайте, а во внутренней сети")

### <a name="mode-2-only-for-local-users"></a>Режим 2. Только для локальных пользователей

Если между локальными филиалами есть плохие подключения, но между ними есть хорошие подключения, рекомендуется использовать режим "Только для локальных пользователей".
...
![Еще одна схема, показывающая поток трафика Local Media Optimization.](media/direct-routing-media-op-7.png "Поток трафика с режимом &quot;Только для локальных пользователей&quot;, а пользователь находится на домашнем сайте")


#### <a name="scenario-2-the-user-and-gateways-are-in-different-sites"></a>Сценарий 2. Пользователи и шлюзы находятся на разных сайтах

Предположим, что SBC в Сингапуре настроен как прокси-сервер SBC для локальных SBCs downstream во Вьетнаме и Индонезии. Внутренний пользователь в Индонезии, расположенный в локальном филиале, звонит прямой маршрутизов в Вьетнам. Политики маршрутизов голосовой связи в Интернете определяют, что звонки в Вьетнам (с кодом города +84) должны перенаться в местный SBC во Вьетнаме. Все остальные звонки ( и в случае сбой SBC во Вьетнаме ) должны быть перенаправлиться на прокси-сервер SBC в Сингапуре. Для прокси-сервера SBC в Сингапуре установлен режим "Всегда обходить", а для локального SBC во Вьетнаме — режим "Только для локальных пользователей". В таблице ниже подведены итоги конфигурации примера. 
...
На приведенной ниже схеме внутренний пользователь, находясь в филиале индонезийского филиала, совершает прямой звонок маршрутизов через Teams на номер во Вьетнаме. 
...
| Ошибка | Решение |
| :--- | :--- |
| Teams клиент не определен  как внутренний, Teams общедоступный IP-адрес клиента соответствует списку доверенных IP-адресов клиента. | Для оптимизации локальных мультимедиа требуется, чтобы Teams подсети клиента соответствует сетевой подсети, настроенной [клиентом.](/powershell/module/skype/new-cstenantnetworksubnet)|
| Когда клиент Teams как внутренний, вызовы Teams переназначаются.| Отключить оптимизацию локальных мультимедиа в SBC direct Routing.|
| При звонках между внутренними клиентами звонков (от 1 до 1) на многоэтапные звонки с внешним клиентом или ресурсом в результате пропадания звонков | Идет работа над исправлением. Вы также можете отключить оптимизацию локальных мультимедиа на SBC Direct Routing.|
