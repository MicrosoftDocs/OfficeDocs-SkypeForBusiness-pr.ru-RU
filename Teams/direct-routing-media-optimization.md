...
Локальному пользователю назначено локальное филиал в Германии. Пользователь звонит по прямой маршрутике через Teams.
...
- SBC перенаправляет поток на телефонная система и подключенную сеть STN. 
...
После этого администратор Contoso добавит несколько SBCs, которые будут прокси-серверами SBC, которые будут proxysbc.contoso.com. У SBCs downstream нет общедоступных ИП, но их можно на назначены голосовым маршрутам. В таблице ниже показаны примеры сетевых параметров и конфигурации.
...
- Клиент службы Teams пользователя общается с телефонная система через API REST, но мультимедиа, созданные во время звонка, перетекают на внутренний IP-адрес локального SBC.
...
- SBC в Германии перенаправляет поток на прокси-сервер SBC в Городе, а также в подключенную локовую сеть STN. 
...
На приведенной ниже схеме внутренний пользователь, находясь в филиале индонезийского филиала, совершает прямой звонок маршрутизов Teams маршрутизов на номер во Вьетнаме. 
