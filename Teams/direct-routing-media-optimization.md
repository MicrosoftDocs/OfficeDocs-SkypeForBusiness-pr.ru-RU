---
title: Оптимизация локальных мультимедиа для прямого перенаправления
author: CarolynRowe
ms.author: crowe
manager: serdars
ms.date: 04/07/2020
...
В локальной службе пользователю назначено локальное филиал в Германии. Пользователь звонит по прямой маршрутике через Teams.

- Клиент Teams пользователя общается с телефонная система через API REST, но мультимедиа, созданные во время звонка, перетекают на внутренний IP-адрес центрального SBC. 
...
- Клиент службы Teams пользователя общается с телефонная система через API REST, но в данном случае мультимедиа, созданные во время звонка, перетекают на внешний IP-адрес центрального SBC. 
...
На следующей схеме показан высокоуровневый трафик для сценария, когда пользователь находится в офисе в регионе APAC. Пользователь, который является локальным филиалом местного филиала во Вьетнаме, совершает телефонный звонок прямой маршрутизовой через Teams. 

- Клиент службы Teams пользователя общается с телефонная система через API REST, но мультимедиа, генерируемые во время звонка, перетекают на внутренний IP-адрес локального SBC.

- Локализованный сервер SBC перенаправляет поток на прокси-сервер SBC в Сингапуре и в подключенную локовую сеть STN.

-  Прокси-сервер SBC виден для телефонная система через внешний IP-адрес и маршрутит поток от данной SBC (в данном случае с локального SBC во Вьетнаме) на телефонная система. 
...
Чтобы различать локальные режимы оптимизации мультимедиа, администратору клиента необходимо установить для параметра -BypassMode параметр "Всегда" или "OnlyForLocalUsers" для каждого SBC с помощью Set-CSonlinePSTNGateway командлета. Дополнительные сведения см. [в настройках оптимизации локальных мультимедиа.](direct-routing-media-optimization-configure.md)  
...
На приведенной ниже схеме показан высокоуровневый трафик, в который внутренний пользователь в Германии звонит по прямому маршруту через Teams на номер в Германии. 
...
- Прокси-сервер SBC телефонная система через внешний IP-адрес и маршрутит поток от SBC (в данном случае с локального SBC в Германии) на телефонная система. 
...
На следующей схеме показан трафик высокого уровня, когда внутренний немецкий пользователь, расположенный во Франции, совершает телефонный звонок прямой маршрутизовой Teams номеру в Германии. 
...
На приведенной ниже схеме пользователь, который назначен локальному филиалу во Вьетнаме, совершает телефонный звонок прямой маршрутизовой Teams. 
...
- Прокси-сервер SBC виден телефонная система через внешний IP-адрес и маршрутит поток от данной SBC (в данном случае с локального SBC во Вьетнаме) на телефонная система. 
...
- SBCs верхней части локальных филиалов не видны непосредственно телефонная система, но соотносится с топологией виртуальной сети.
...
| Ошибка | Решение |
| :--- | :--- |
| Teams клиент не является внутренним, если Teams общедоступный IP-адрес клиента соответствует списку доверенных IP-адресов клиента.  | Для оптимизации локальных мультимедиа требуется, чтобы Teams подсети клиента соответствует сетевой подсети, настроенной [клиентом.](/powershell/module/skype/new-cstenantnetworksubnet)|
