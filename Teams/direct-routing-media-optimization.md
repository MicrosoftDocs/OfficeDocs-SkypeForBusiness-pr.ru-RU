...
Локальная оптимизация мультимедиа для прямой маршрутизации позволяет управлять качеством голоса с помощью:
...
Дополнительные сведения о возможных режимах и соответствующих действиях см. в подмносяке "Настройка оптимизации локальных мультимедиа".
...
- Ост.SBC во Вьетнаме перенаправляет поток в подключенную локализованную сеть ННП. 
...
Предположим, например, что у компании есть централизованная магистраль SIP в Москве, обслуживаемая 30 стран и отлично подключаемая к интернету между всеми 30 сайтами и локальными пользователями. Кроме того, в Германии имеется ветвь, в которой развернут локальный SBC.
...
- Прокси-сервер SBC отображается для телефонной системы только по внешнему IP-адресу и маршрутит поток из местной SBC (в данном случае — местной SBC в Германии) в телефонную систему. 
...
Чтобы создать решение, в котором службы STN предоставляются во всех локальных филиалах региона APAC, где централизование линии связи TDM не является возможностью, администратор Contoso примещает один региональный SBC в Сингапуре как SBC-сервер прокси-сервера к службе Прямой маршрутизации. Прямое подключение между филиалами местного филиала не является хорошей, но между каждым местным филиалом и региональным офисом SBC в Сингапуре существует хорошая связь. Для региональной SBC администратор выбирает режим "Всегда обходить", а для локальных сетных УАЦ — режим "Только для локальных пользователей".
...
- Верхней SBC в локальном филиале не видна непосредственно телефонной системе, а отображается в виртуальной топологии сети.
