...
- Управление потоком трафика мультимедиа между клиентами Teams и пограничными контроллерами сеансов клиента (SBC).
- Сохранение локального носителя в пределах подсетей корпоративной сети.
- Разрешение потоков мультимедиа между клиентами Teams и контроллерами SBC, даже если контроллеры SBC находятся за корпоративными брандмауэрами с частными IP-адресами и не видны корпорации Майкрософт напрямую.
...
- Создание топологии виртуальных сетей SBC, где контроллеры SBC в локальных филиалах подключены к централизованному прокси-серверу SBC, который является видимым для телефонной системы Майкрософт и взаимодействует с ней через свой внешний IP-адрес. В топологии виртуальной сети подчиненные контроллеры безопасности взаимодействуют через внутренние IP-адреса и не видны телефонной системе напрямую.
...
- **В Европе** у компании Contoso есть офисы примерно в 30 странах. Каждый офис имеет собственный обмен частными филиалами (УАТС).
...
- **В Европе** все магистрали являются централизованными и потоками мультимедиа между центральным SBC и пользователями в зависимости от расположения пользователя.
  ...
  - Если пользователь подключен к локальной подсети корпоративной сети (т. е. он является внутренним), мультимедиа передается между внутренним IP-адресом центрального SBC и клиентом Teams пользователя.

  - Если пользователь находится за пределами корпоративной сети ( например, если пользователь использует общедоступное беспроводное подключение к Интернету), то пользователь считается внешним. В этом случае мультимедиа передается между внешним IP-адресом центрального SBC и клиентом Teams.
...
- Клиент Teams пользователя взаимодействует с телефонной системой непосредственно через REST API, но мультимедиа, созданный во время вызова, передается на внутренний IP-адрес центрального SBC.
...
На следующей схеме показан поток трафика, когда пользователь не находится в локальной среде и не подключен к корпоративной сети (то есть устройство пользователя подключено к Интернету через мобильное устройство или общедоступную сеть Wi-Fi). Пользователь выполняет телефонный звонок прямой маршрутизации через Teams:

- Клиент Teams пользователя взаимодействует с телефонной системой напрямую через REST API, но в данном случае мультимедиа, созданный во время вызова, передается на внешний IP-адрес центрального SBC.
...
Пользователь, назначенный локальному филиалу в Вьетнаме, совершает телефонный звонок прямой маршрутизации через Teams.

- Клиент Teams пользователя взаимодействует с телефонной системой напрямую через REST API, но мультимедиа, созданный во время вызова, передается на внутренний IP-адрес локального SBC.

- Локальный SBC перенаправляет поток на прокси-сервер SBC в Сингапуре и в подключенную локальную сеть ТСОП.

-  Прокси-сервер SBC виден телефонной системе только через внешний IP-адрес и направляет поток из подчиненного SBC (в данном случае локального SBC в Вьетнаме) в телефонную систему.

- Подчиненный SBC в локальном филиале не виден телефонной системе напрямую, но сопоставляется с топологией виртуальной сети, определенной администратором Contoso при настройке оптимизации локальных носителей.
...
![На схеме снова показана оптимизация локального носителя для потока трафика.](media/direct-routing-media-op-3.png "Traffic flow in case of proxy SBC with connected downstream SBCs when user is in the \"home\" network")

### <a name="external-user"></a>Внешний пользователь

На следующей схеме показан поток трафика, когда пользователь находится за пределами корпоративной сети. Пользователь не находится в локальной среде (не находится в пределах корпоративной сети). Пользователь выполняет телефонный звонок прямой маршрутизации через Teams на номер телефона в Вьетнаме.

- Клиент Teams пользователя взаимодействует с телефонной системой непосредственно через REST API, но мультимедиа, созданный во время вызова, сначала передается на внешний IP-адрес прокси-сервера SBC в Сингапуре.
...
| Физическое расположение пользователя | Пользователь совершает звонок на номер | Политика сетевой маршрутизации голосовой связи | Режим, настроенный для SBC | Поток мультимедиа |
|:------------|:-------|:-------|:-------|:-------|
...
На схеме ниже показан общий поток трафика для внутреннего пользователя в Германии, выполняющего телефонный звонок прямой маршрутизации через Teams на номер в Германии.
...
![Схема, показывающая оптимизацию локального носителя для потока трафика.](media/direct-routing-media-op-5.png "Поток трафика в режиме &quot;Всегда обходить&quot;, а пользователь находится на домашнем сайте")
...
| Физическое расположение пользователя | Пользователь совершает звонок на номер | Политика сетевой маршрутизации голосовой связи | Режим, настроенный для SBC | Поток мультимедиа |
|:------------|:-------|:-------|:-------|:-------|
| Франция | +49 1 437 2800 | Приоритет 1: ^\+49(\d{8})$ -DEsbc.contoso.com <br>Приоритет 2. .* — proxysbc.contoso.com |  DEsbc.contoso.com — всегда обходить proxysbc.contoso.com — всегда обходить | Пользовательские < Teams — > DEsbc.contoso.com  |

На следующей схеме показан высокоуровневый поток трафика, когда внутренний немецкий пользователь, расположенный во Франции, совершает телефонный звонок прямой маршрутизации через Teams на номер в Германии.
...
![На схеме показана оптимизация локального носителя для потока трафика.](media/direct-routing-media-op-6.png "Поток трафика в режиме &quot;Всегда обходить&quot;, а пользователь находится не на домашнем сайте, а во внутренней сети.")
...
| Физическое расположение пользователя | Пользователь совершает звонок на номер | Политика сетевой маршрутизации голосовой связи | Режим, настроенный для SBC | Поток мультимедиа |
|:------------|:-------|:-------|:-------|:-------|
| Вьетнам | +84 4 3926 3000 | Приоритет 1: ^\+84(\d{9})$ -VNsbc.contoso.com <br>Приоритет 2. .* — proxysbc.contoso.com | VNsbc.contoso.com — только для локальных пользователей <br> proxysbc.contoso.com — всегда обходить | Пользователи Teams <–> VNsbc.contoso.com |

На следующей схеме пользователь, назначенный локальному филиалу в Вьетнаме, выполняет телефонный звонок прямой маршрутизации через Teams.
...
![Другая схема, показывающая оптимизацию локального носителя для потока трафика.](media/direct-routing-media-op-7.png "Поток трафика в режиме &quot;Только для локальных пользователей&quot;, а пользователь находится на домашнем сайте")
...
| Физическое расположение пользователя | Пользователь совершает звонок на номер | Политика сетевой маршрутизации голосовой связи | Режим, настроенный для SBC | Поток мультимедиа |
|:------------|:-------|:-------|:-------|:-------|
| Индонезия | +84 4 3926 3000 | Приоритет 1: ^\+84(\d{9})$ -VNsbc.contoso.com <br> Приоритет 2. .* — proxysbc.contoso.com |VNsbc.contoso.com — только для локальных пользователей <br> proxysbc.contoso.com — всегда обходить | Пользователи Teams <–> proxysbc.contoso.com <–> VNsbc.contoso.com |
...
- Прокси-сервер SBC в Сингапуре перенаправляет поток на внутренний IP-адрес подчиненного SBC в Вьетнаме и в телефонную систему.

- Подчиненный SBC в Вьетнаме направляет поток в подключенную локальную сеть ТСОП.

- Прокси-сервер SBC виден телефонной системе только через внешний IP-адрес.

- Подчиненные контроллеры SBC в локальных филиалах не видны телефонной системе напрямую, но сопоставляются в топологии виртуальной сети.
...
![На другой схеме показана оптимизация локального носителя для потока трафика.](media/direct-routing-media-op-8.png "Поток трафика в режиме &quot;Только для локальных пользователей&quot;, пользователь находится не на домашнем сайте, а во внутренней сети.")
...
| Ошибка | Решение |
| :--- | :--- |
| Клиент Teams не определяется как **внутренний,** если общедоступный IP-адрес клиента Teams соответствует списку надежных IP-адресов клиента. | Для оптимизации локального носителя требуется, чтобы клиентская подсеть Teams совпадала с сетевой подсетью, настроенной [клиентом.](/powershell/module/skype/new-cstenantnetworksubnet)|
| Эскалация вызовов приводит к отброшенным вызовам, когда клиент Teams определяется как внутренний.| Отключите оптимизацию локальных носителей в SBC прямой маршрутизации.|
