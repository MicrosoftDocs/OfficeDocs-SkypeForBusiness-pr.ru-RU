...
В локальной службе пользователю назначено локальное филиал в Германии. Пользователь звонит прямой маршрутизовый телефонный звонок через Teams.
...
На следующей схеме показан поток трафика, если пользователь не является локальной и не подключен к корпоративной сети (то есть устройство пользователя подключено к Интернету с помощью мобильного устройства или открытой сети Wi-Fi). Пользователь звонит прямой маршрутизовой по Teams:
...
Чтобы создать решение, в котором службы STN предоставляются во всех локальных филиалах в регионе APAC, где централизованное управление линиями связи TDM не является вариантом, администратор Contoso примещает один SBC (proxysbc.contoso.com), также называемый прокси-сервером SBC, к службе Прямой маршрутизации. 

После этого администратор Contoso добавит несколько SBCs, которые будут выводиться на сервере SBC прокси proxysbc.contoso.com. У SBCs downstream нет общедоступных ИП, но их можно на назначены голосовым маршрутам. В таблице ниже показаны примеры сетевых параметров и конфигурации.
...
На следующей схеме показан высокоуровневый трафик для сценария, когда пользователь находится в офисе в регионе APAC. Пользователь, который является локальным филиалом местного филиала во Вьетнаме, звонит по прямой маршрутике через Teams. 
...
На следующей схеме показан поток трафика, когда пользователь находится за пределами корпоративной сети. Пользователь не является локальной (не находится в пределах корпоративной сети). Пользователь звонит прямой маршрутизов через Teams на телефонный номер во Вьетнаме. 
...
На приведенной ниже схеме внутренний пользователь, находясь в филиале индонезийского филиала, звонит по прямой маршрутике Teams на номер во Вьетнаме. 
