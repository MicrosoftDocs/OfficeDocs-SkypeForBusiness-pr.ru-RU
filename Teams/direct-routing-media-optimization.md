...
-   Построение топологии виртуальных сетей на веб-сайте SBCs, в которых SBCs в локальных филиалах подключены к централизованному прокси-серверу SBC, который виден вам, и общаются с ним Телефон (Майкрософт) через внешний IP-адрес. В виртуальной топологии сети SBCs, отстояющие от сети, общаются с внутренними сетевыми сетевыми сетями и не видны телефонная система.
...
- **В Европе** все связи централизованы, а потоки мультимедиа между центральной SBC и пользователями основаны на расположении пользователей. 
  ...
  - Если пользователь находится за пределами корпоративной сети (например, если он использует беспроводное подключение к Интернету общего доступа), то он считается внешним. В этом случае потоки мультимедиа между внешним IP-адресом центрального SBC и клиентом Teams сети.
...
Чтобы создать решение, в котором службы STN предоставляются во всех локальных филиалах в регионе APAC, где централизованное управление линиями TDM не является вариантом, администратор Contoso примещает один SBC (proxysbc.contoso.com), также называемый прокси-сервером SBC, к службе Direct Routing. 
...
-  SBC в локальном филиале не виден непосредственно телефонная система, но отображается в топологии виртуальной сети, которая определена администратором Contoso при настройке оптимизации локальных мультимедиа. В этом примере пользователь считается внешним, так как он находится за пределами корпоративной сети. 
...
- SBC в локальном филиале не виден непосредственно телефонная система, но отображается в топологии виртуальной сети.
