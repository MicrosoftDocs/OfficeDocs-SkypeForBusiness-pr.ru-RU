...
> [!NOTE]
> Эта функция еще не выпущена для пользователей облако сообщества для государственных организаций (GCC), облако сообщества для государственных организаций (GCCH) и DoD.
...
- Когда администратор впервые создает шаблон для всей организации, он автоматически создает новую группу Azure Active Directory(AAD) для всех администраторов клиента, включая глобальных администраторов и администраторов служб группы. Эти администраторы будут добавлены в качестве владельцев группы, чтобы они могли совместно управлять шаблонами организации. Администраторов, которые еще не были добавлены в организацию после создания команды, необходимо вручную добавить в качестве владельцев группы, чтобы они могли управлять шаблонами для всей организации.

> [!Note]
> Если администратор удалит группу, у вас есть месяц на ее восстановление на Azure Active Directory (AAD) для восстановления всех связанных данных. Если администратор удалит эту группу из корзины через один месяц, вы потеряете все связанные данные.
...
Приложение "Утверждения" доступно по умолчанию. Вы можете отключить его в Центре администрирования Teams.
  ...
  3. Выполните поиск приложения "Утверждения".
     ...
     ![Навигация в Центре администрирования с выделенной Teams приложениями > Управление приложениями.](media/manage-approval-apps.png)
...
Для функций работы с электронной подписью "Утверждения" партнеры по подписи сторонних разработчиков по умолчанию отображаются в Teams утверждениях. Вы можете отключить определенные поставщики электронной подписи, задав параметры приложения в Teams администрирования.

1. В центре Teams администрирования в области **Управление** приложениями выберите приложение **"Утверждения"** и **Параметры**.

2. Каждый поставщик электронной подписи по умолчанию имеет для него значение в положении "вкл." (справа). Чтобы отключить определенного поставщика электронной подписи, перевиньте его влево. Если Teams отключает поставщика, конечные пользователи не будут видеть его при создании утверждения. Кроме того, конечные пользователи не смогут просматривать запросы на электронную подпись, сделанные с этим поставщиком.
