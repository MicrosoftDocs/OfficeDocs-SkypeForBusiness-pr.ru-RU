...
Если SBC в примере отправлен параметры в любой период между 11:12 и 11:15 (время звонка), он считается полезным. В этом случае SBC будет понижен с маршрута. 
...
Однако средству аналитики звонков может быть нужна только связь с внутренними компонентами прямой маршрутки и сбой. В случае проблем с парой SBC или из-за проблем с отклонением приглашения SIP (например, неправильно задано имя FQDN для связи), аналитика вызовов не поможет. В этом случае обратитесь к журналам SBC. Direct Routing отправляет подробное описание проблем в SBCs; эти проблемы можно прочитать в журналах SBC.