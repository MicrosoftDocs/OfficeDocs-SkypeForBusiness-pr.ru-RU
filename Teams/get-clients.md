...
> [!NOTE]
> Конфигурация брандмауэра Windows будет изменена. Будут созданы два правила для teams.exe для протоколов TCP и UDP. 
> - Разрешите действие, если пользователь является локальным администратором и нажимает кнопку "Разрешить доступ".
> - Блокирование действия, если пользователь не является локальным администратором и, в любом случае, при закрытии запроса с помощью параметра "Отмена".
...
> [!NOTE]
> Когда пользователи устанавливают Teams из одного из магазинов приложений Android для Китая и включают push-уведомления для Teams, корпорация Майкрософт будет предоставлять push-уведомления Teams через службу push-уведомлений для Китая.
