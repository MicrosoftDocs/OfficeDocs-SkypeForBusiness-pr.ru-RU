...
|Нет|ForwardPAI|Указывает, будет ли переадресовываться заголовок P-Asserted-Identity (PAI) одновременно с вызовом. Заголовок PAI позволяет проверить личность звонящего. Значение по умолчанию — **False** ($False).|False|True<br/>False|Логическое|
...
- Создание пользователя в локальной службе Active Directory и синхронизация пользователей в облако. В разделе [Интеграция на локальную каталогов с помощью Azure Active Directory](https://docs.microsoft.com/en-us/azure/active-directory/connect/active-directory-aadconnect).  
- Создание пользователя непосредственно в портал администратора Office 365. В разделе [Добавление пользователей по отдельности или набором для Office 365 — помощь администратора](https://support.office.com/en-us/article/Add-users-individually-or-in-bulk-to-Office-365-Admin-Help-1970f7d6-03b5-442f-b385-5880b9c256ec). 
...
## <a name="enable-calling-for-microsoft-teams"></a>Включить вызовы для групп Майкрософт

Прежде чем пользователь видит вкладке звонки в группами Майкрософт, необходимо включить частной телефонной для клиента в группах Майкрософт. Для этого выполните следующие действия:

1.  Войдите в качестве администратора клиента в центре администрирования Office 365.
2.  **Параметры и служб и надстроек** и выберите **Группами Майкрософт**. 
3.  Разверните узел **звонков и собраний** и убедитесь, что **вызов разрешить закрытый** **на**.
...
Прямое маршрутизации будет маршрутизировать вызовы только группами Майкрософт, поэтому необходимо убедитесь в том, что группами Майкрософт, является предпочтительным вызывающего клиента для пользователей. Осуществляется с помощью TeamsCallingPolicy и TeamsInteropPolicy. 

1. Во-первых используйте следующий командлет в удаленный сеанс PowerShell в Скайп для бизнеса в Интернет центра администрирования для просмотра какие политики пользователя были назначены. 

  ```
  Get-CsOnlineUser -identity <User Name> | fl *teams*
  ```
 
2. Далее просмотрите экземпляры разные политики. 
...
Перед группами Майкрософт пользователи могут использовать службу, существуют дополнительные действия, которые необходимо выполнить, чтобы применить политику вызова и разрешить вызовы.

### <a name="teams-calling-policy"></a>Политика звонков групп

Вам нужно убедитесь в том, что у пользователя есть TeamsCallingPolicy с AllowCalling = True. Эта политика может быть глобальной политики клиента или определенной политики, предоставленные пользователю. Если вам потребуется предоставить пользователю определенной политики, может использовать командлет:

```
Grant-CsTeamsCallingPolicy -PolicyName <policy> -Identity <User Name>
```

### <a name="teams-interop-policy"></a>Политики взаимодействия групп

Убедитесь в том, что у пользователя есть к равной группами Майкрософт предпочитаемый клиента. Это может выполняться двумя способами:

- Пользователь задает предпочтительную вызывающего клиента в клиенте группами Майкрософт.
- Пользователь назначена политика, которая задает предпочтительную вызывающего клиента.

Чтобы назначить политику, которая устанавливает группами Майкрософт в качестве предпочитаемого вызывающего клиента, убедитесь в том, что пользователю предоставляется политики с CallingDefaultClient = группами. Ниже показан пример командлета.
