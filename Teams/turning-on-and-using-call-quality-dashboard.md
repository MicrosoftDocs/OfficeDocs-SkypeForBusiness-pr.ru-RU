...
Панель мониторинга качества звонка (ПМКЗ) для Microsoft Teams и Skype для бизнеса Online позволяет получить представление о качестве звонков, сделанных с помощью команд Microsoft и Skype для бизнес-услуг. В этом разделе описываются действия, необходимые для выполнения начала сбора данных.
...
- Видео- и VBSS логику классификации качества потока был обновлен. Обратитесь к [Классификации потока в панели мониторинга качества звонков](stream-classification-in-call-quality-dashboard.md) для последних определений классификатор.
...
1. Войдите в организации Office 365 с помощью учетной записи администратора службы группами Майкрософт и выберите плитку **администрирования** , чтобы открыть центр администрирования.
    
2. В левой области **центра администрирования**выберите **Группами Майкрософт** , чтобы открыть центр администрирования группами Майкрософт.
    
3. В центре администрирования группами Майкрософт выберите **панель мониторинга качества звонков** в левой области.
    
  
4. На открывшейся странице вход с помощью учетной записи глобального администратора или учетная запись администратора службы Microsoft групп и затем укажите учетные данные для учетной записи при появлении запроса.
...
|Поддержка сведения конечной точки клиента  <br/> |Только в cqd.teams.microsoft.com  <br/> |Только в cqd.teams.microsoft.com  <br/> |
...
## <a name="upload-tenant-data-information"></a>Отправка данных клиента
<a name="BKMKTenantDataInformationUpload"></a>

Панель мониторинга отчетов Сводка CQD включает в себя на страницу **Загрузки данных клиента** , доступного, выбрав в меню «Параметры» в верхнем правом углу **Отправка данных клиента** . Эта страница используется для "Администраторы" для отправки их собственные сведения, такие как сопоставление IP-адреса и сведения о местоположении, сопоставление каждого беспроводного доступа и его MAC-адрес, сопоставления конечной точки, чтобы сделать/модели/тип конечной точки, и т.д.
  
![Панель мониторинга CQD](media/839c9ab4-0246-46c9-8402-aafd83a0bc63.png)
  
1. На странице **Отправка данных клиента** используйте раскрывающееся меню для выбора типа файла данных для загрузки. Тип данных файла обозначает содержимое файла (например, «Создание» означает сопоставление IP-адрес а построение, а также другие сведения о местоположении, «Конечная точка» сопоставление имя конечной точки для конечной точки сделать / / тип модели... сведения о). В настоящее время мы поддержка типов данных «Создание» и «Конечная точка» Отправка cqd.teams.microsoft.com (на этапе предварительного просмотра и недоступны официально еще), поддерживает только cqd.lync.com Отправка тип данных «Стандартные». Несколько дополнительных типов данных будет добавлен в последующих выпусках.
...
2. После выбора типа файла данных, нажмите кнопку **Обзор** , чтобы выбрать файл данных.
   ...
   - Загрузки cqd.teams.microsoft.com расширил значения предельного числа строк из 1 000 000 для увеличения производительности запросов. Мы может существует, ограничение также cqd.lync.com.
...
### <a name="tenant-data-file-format-and-structure"></a>Формат файла данных клиента и структуры
<a name="BKMKTenantDataFile"> </a>

### <a name="building-data-file"></a>Создание файла данных
CQD использует стандартные файл данных с первоначальным производными столбца подсети расширяющегося сети + NetworkRange столбца, а затем объединяющий столбец подсети столбец первого подсети Subnet/Second записи звонков для отображения построения/города/страны или региона... сведения о. Формат файла данных, загружаемых должны соответствовать следующие действия, чтобы проходит проверку до отправки.
...
|NetworkName  <br/> |String  <br/> |США/Сиэтл/СИЭТЛ SEA-1  <br/> |
|NetworkRange  <br/> |Число  <br/> |26  <br/> |
|BuildingName  <br/> |String  <br/> |СИЭТЛ SEA-1  <br/> |
|OwnershipType  <br/> |String  <br/> |Contoso  <br/> |
|BuildingType  <br/> |String  <br/> |Завершение ИТ  <br/> |
|BuildingOfficeType  <br/> |String  <br/> |Проектирование  <br/> |
|City  <br/> |String  <br/> |Сиэтл  <br/> |
|ZipCode  <br/> |String  <br/> |98001  <br/> |
|Страна  <br/> |String  <br/> |"МНЕ НРАВИТСЯ"  <br/> |
...
CQD использует файл конечной точки данных путем присоединения его столбца EndpointName записи звонков первого клиента имя конечной точки клиента конечной точки Name/Second столбца для отображения информации сделать/модели/тип конечной точки. Формат файла данных, загружаемых должны соответствовать следующие действия, чтобы проходит проверку до отправки.

- Файл должен находиться файл .tsv, который означает, что, каждой строки, столбцы, разделенных ВКЛАДКИ, или в CSV-файл с каждого столбца, разделенных точкой с запятой.

- Содержимое файла данных сюда не включены в таблице заголовки. Это означает, что первая строка файла данных должны быть реальные данные, не заголовки как «EndpointName» и т.д.

- Для каждого столбца тип данных можно только строки и на нем должно быть не более 64 символов, которая Максимальная допустимая длина.

- Для каждого столбца данных может быть пустым (но по-прежнему должны быть разделены соответствующий разделитель (то есть вкладки или запятой). Это просто присваивает значение пустое строковое значение.

- Должен быть 7 столбцов для каждой строки и столбцы должны быть в том порядке, перечисленные в следующей таблице.

- EndpointName должно быть уникальным в противном случае — что отправка завершится с ошибкой из-за повторяющуюся строку как приводят неправильные присоединения.

-  EndpointLabel1, EndpointLabel2, EndpointLable3 являются пользователя настраиваемые метки, они могут быть пустые строки или значение пользователей предпочитает использовать такие как «ИТ-отделу обозначены 2018 ноутбуков», «метка ресурсов 5678"... и т.д.

|**Имя столбца**|**Тип данных**|**Пример**|
|:-----|:-----|:-----|
|EndpointName  <br/> |String  <br/> |1409W3534  <br/> |
|EndpointMake  <br/> |String  <br/> |Fabrikam Inc  <br/> |
|EndpointModel  <br/> |String  <br/> |Модель Fabrikam 123  <br/> |
|EndpointType   <br/> |String  <br/> |Ноутбуков  <br/> |
|EndpointLabel1  <br/> |String  <br/> |Ноутбуков 2018 обозначены ИТ  <br/> |
|EndpointLabel2  <br/> |String  <br/> |Ярлык 5678  <br/> |
|EndpointLabel3  <br/> |String  <br/> |2018 покупки   <br/> |
