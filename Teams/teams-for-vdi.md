...
1. Скачайте пакет MSI для Teams, соответствующий вашей операционной системе VDI, по одной из следующих ссылок:
    ...
    > [!NOTE]
    > Для государственных облаков ознакомьтесь со ссылкой [Установка Microsoft Teams с помощью диспетчера конфигураций конечных точек Microsoft](msi-deployment.md) для загрузки ссылок на MSI-файлы.
...
3. Удалите MSI из виртуальной машины VDI. Удалить Teams можно двумя способами.
    ...
    - Сценарий PowerShell. Вы можете использовать [этот сценарий PowerShell](scripts/powershell-script-deployment-cleanup.md) для удаления команд и удаления папки "команды" для пользователя. Запустите сценарий для каждого профиля пользователя, на котором установлены команды на компьютере.
    - Командная строка: выполните указанную ниже команду.
