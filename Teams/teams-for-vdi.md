...
## <a name="install-the-teams-desktop-app-on-vdi"></a>Установка классического приложения Teams в VDI
...
|---------|---------|---------|
|вкпу   |    2 ядра     |  4, 6 или 8<br>Важно понимать основную конфигурацию неоднородного доступа к памяти (NUMA) и настраивать виртуальные компьютеры соответствующим образом.     |
...
Исключите из папки Caching teams:% AppData%/Микрософт/теамс.  За исключением этих данных, уменьшите размер кэширования пользователя, чтобы дополнительно оптимизировать несохраняемую настройку.
...
Группы с Office 365 профессиональный плюс устанавливаются для каждого пользователя. Дополнительные сведения можно найти в разделе [Установка классического приложения Teams в VDI](#install-the-teams-desktop-app-on-vdi) .
...
Кроме того, Teams также добавляется к существующим установкам Office 365 профессиональный плюс. Поскольку Office 365 ProPlus устанавливает группы только для пользователей, ознакомьтесь с разделом [Установка классического приложения Teams в VDI](#install-the-teams-desktop-app-on-vdi) .
...
3. Удаление MSI-файла из виртуальной машины VDI 
    ...
    - Командная строка: этот подход удаляет группы, но предотвращает повторную установку Teams.  
...
Помимо общения и совместной работы, Teams в VDI с поддержкой звонков и собраний доступны на платформах Citrix. Поддерживаемые функции основаны на стеке мультимедиа Вебртк и реализации Citrix. На приведенной ниже схеме представлен обзор архитектуры.
...
- Маршрутизация на основе местоположения (ЛБР)
- Парковка вызовов
- Очередь звонков
...
Teams в браузере Chrome не предоставляет замену для классического приложения Teams для VDI с оптимизацией вирусов. Работа в чате и совместной работе работают должным образом. Если требуется мультимедиа, некоторые возможности могут не соответствовать ожиданиям пользователей в браузере Chrome.
...
[**Политики звонков**](teams-calling-policy.md): команды включают встроенную политику вызовов дисалловкаллинг, в которой все функции вызова отключены. Назначьте политику Дисалловкаллинг для всех пользователей в вашей организации, которые используют команды в виртуализованной среде.

[**Политики собраний**](meeting-policies-in-teams.md): команды включают встроенную политику собраний аллофф, в которой отключены все функции собраний. Назначьте политику Аллофф для всех пользователей в вашей организации, которые используют команды в виртуализованной среде.
...
Чтобы назначить политику вызова Дисалловкаллинг и политику собрания Аллофф пользователям, выполните указанные ниже действия.
...
3. Выполните указанные ниже действия.
    1.  В разделе **Политика звонков**щелкните **дисалловкаллинг**.
    2.  В разделе **политика собраний**щелкните **аллофф**.
...
1. В левой области навигации центра администрирования Microsoft Teams выберите политику, которую вы хотите назначить. Например:
    - Перейдите к разделу**политики** **голосовой связи** > и нажмите кнопку **дисалловкаллинг**.
    - Перейдите в **** > раздел**политики собраний по собраниям**и нажмите кнопку **аллофф**.
...
Чтобы узнать больше об использовании PowerShell для управления политиками вызовов, ознакомьтесь с статьей [Set-кстеамскаллингполици](https://docs.microsoft.com/powershell/module/skype/set-csteamscallingpolicy).
...
Чтобы узнать больше об использовании PowerShell для управления политиками собраний, ознакомьтесь с статьей [Set-кстеамсмитингполици](https://docs.microsoft.com/powershell/module/skype/set-csteamsmeetingpolicy).
...
Управление [**звонками**](teams-calling-policy.md): политики вызовов в Teams управляют тем, какие функции вызова доступны пользователям. Команды включают встроенную политику вызовов Алловкаллинг, в которой включены все функции вызова. Чтобы включить все функции вызова, назначьте политику Алловкаллинг. Вы также можете создать собственную политику вызова, чтобы включить необходимые функции вызова и назначить ее пользователям. 

[**Политики для собраний**](meeting-policies-in-teams.md): политики собраний в Teams управляют типами собраний, которые пользователи могут создавать, и доступными в них функциям для участников, запланированных пользователями в Организации. Команды включают встроенную политику собраний Аллон, в которой включены все функции собраний. Чтобы включить все функции собрания, назначьте политику Аллон. Вы также можете создать собственную политику собраний, чтобы включить необходимые функции собрания и назначить их пользователям.

#### <a name="assign-policies-using-the-microsoft-teams-admin-center"></a>Назначение политик с помощью центра администрирования Microsoft Teams

Чтобы назначить политику вызова Алловкаллинг и политику собрания Аллон пользователям, выполните указанные ниже действия.

1. В левой области навигации центра администрирования Microsoft Teams перейдите к разделу **Пользователи**.
2. Выберите пользователя, щелкнув слева от его имени, а затем щелкните **изменить параметры**.
3. Выполните указанные ниже действия.
    1.  В разделе **Политика звонков**щелкните **алловкаллинг**.
    2.  В разделе **политика собраний**щелкните **Аллон**.
4. Нажмите кнопку **Применить**.
...
1. В левой области навигации центра администрирования Microsoft Teams выберите политику, которую вы хотите назначить. Например:
    - Перейдите к разделу**политики** **голосовой связи** > и нажмите кнопку **алловкаллинг**.
    - Перейдите в **** > раздел**политики собраний по собраниям**и нажмите кнопку **Аллон**.
3. Выберите **Управление пользователями**.
4. В области **Управление пользователями** найдите пользователя по отображаемому имени или по имени пользователя, выберите имя и нажмите кнопку **Добавить**. Повторите этот шаг для каждого пользователя, которого вы хотите добавить.
5. Завершив добавление пользователей, нажмите кнопку **сохранить**.
...
- При установке на компьютере команды в VDI не обновляются автоматически в соответствии с клиентами групп, не связанных с VDI. Вы должны обновить образ виртуальной машины установкой нового MSI-файла, как описано в разделе [Установка классического приложения Teams в VDI](#install-the-teams-desktop-app-on-vdi) . Вы должны удалить текущую версию, чтобы обновить ее до новой версии.
...
- Разрешение входящего и исходящего видеопотока ограничено разрешением 720p. Это Вебртк ограничение.
