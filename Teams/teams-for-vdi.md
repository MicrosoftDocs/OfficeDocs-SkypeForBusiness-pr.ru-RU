...
Для Teams оптимизации av в средах VDI тонкая конечная точка клиента должна иметь доступ к Интернету. Если на тонкой конечной точке клиента отсутствует доступ к Интернету, запуск оптимизации не будет успешным. Это означает, что пользователь находится в не оптимизированной области мультимедиа.
...
Если вы выполняете переход с Skype для бизнеса VDI на Teams VDI, кроме различий между двумя приложениями, при внедрении VDI также существуют некоторые отличия. Некоторые возможности, которые в настоящее время не поддерживаются Teams VDI, Skype для бизнеса VDI:
