...
Виртуальный рабочий стол Azure обеспечивает оптимизацию av для Teams в VDI. Подробнее о требованиях и установке см. в Teams [на виртуальном](/azure/virtual-desktop/teams-on-wvd)рабочем столе Azure.
...
Чтобы обновить версию до последней Teams, начните с процедуры удалить, а затем — с последней Teams версии.
...
Для непостояной настройки необходимо установить Teams на компьютере до золотого изображения. (Дополнительные информацию см. в разделе Установка или обновление Teams настольного приложения [на веб-сайте VDI.)](#install-or-update-the-teams-desktop-app-on-vdi) Это обеспечивает эффективный запуск приложения Teams во время сеанса пользователя.
...
Приложения Microsoft 365 для предприятий не поддерживает установку Teams. Чтобы использовать установку на компьютере, необходимо исключить Teams из Приложения Microsoft 365 для предприятий. См. [статьи Развертывание Teams на](#deploy-the-teams-desktop-app-to-the-vm) компьютере с VM и Исключение Teams в [Приложения Microsoft 365 для предприятий](#how-to-exclude-teams-deployment-through-microsoft-365-apps-for-enterprise) разделах.
...
1. Скачайте пакет Teams MSI, который соответствует вашей операционной системе VDI VM, по одной из следующих ссылок:
    ...
    > [!NOTE]
    > Ссылки на [MSI-файлы](msi-deployment.md) см. в Microsoft Teams с Microsoft Endpoint Configuration Manager с помощью Microsoft Endpoint Configuration Manager для скачивания MSI-файлов.
...
3. Удалить MSI-решение из VDI VM. Удалить эти Teams можно двумя Teams.
    ...
    - Сценарий PowerShell: с помощью этого сценария [PowerShell](scripts/powershell-script-deployment-cleanup.md) можно удалить Teams и удалить Teams папку пользователя. Запустите сценарий для каждого профиля пользователя, в котором Teams установлен на компьютере.
...
- Зависимости: к ним относятся зависимости от инфраструктуры, рабочей нагрузки и других аспектов защиты среды, не включаемой в Teams настольного приложения.
...
Если вы выполняете переход с Skype для бизнеса VDI на Teams VDI, помимо различий между двумя приложениями, при внедрении VDI также существуют некоторые отличия. Вот некоторые возможности, которые в настоящее время не поддерживаются Teams VDI, Skype для бизнеса VDI:
