...
|скриншарингмоде | Определяет режим, в котором пользователь может демонстрировать экран в звонках или собраниях. | Параметр "отключено", чтобы запретить пользователю предоставлять общий доступ к экранам. |
...
| алловекстерналпартиЦипантгиверекуестконтрол | Определяет, могут ли внешние участники запрашивать или предоставлять управление демонстрацией экрана. | Установите значение false, чтобы запретить внешнему пользователю предоставлять запрос на Управление собранием. |
...
1. Запустите сеанс Windows PowerShell с правами администратора.
1. Подключитесь к разъему Skype Online.
...
1. Просмотр списка параметров политики собраний.

      ```powershell
      Get-CsTeamsMeetingPolicy
      ```

1. Найдите параметр встроенной политики, в котором отключены все политики собраний. Оно выглядит следующим образом.
...
1. Примените встроенный параметр Аллофф ко всем пользователям, которые будут использовать команды в виртуализованной среде.
...
> [!NOTE]
> Следующие требования относятся как к классическому приложению Teams, так и к веб-приложению Teams.
...
1. Выполните следующую команду, чтобы установить MSI на виртуальной машине VDI (или завершить ее обновление).
...
1. Выполните следующую команду, чтобы удалить MSI с виртуальной машины VDI (или подготовить ее к обновлению).
...
- **Кэшированное содержимое**: Если виртуальная среда, в которой работает Teams, не является постоянной (и данные очищаются по окончании каждого пользовательского сеанса), пользователи могут заметить снижение производительности из-за того, что пользователь получал доступ к тому же содержимому в предыдущем сеансе.
