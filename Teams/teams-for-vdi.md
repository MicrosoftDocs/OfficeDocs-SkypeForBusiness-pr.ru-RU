...
Виртуальные приложения и рабочие столы Citrix (прежнее название — XenApp и XenDesktop) обеспечивают оптимизацию av для Teams в VDI. Виртуальные приложения Citrix и настольные компьютеры Teams VDI поддерживают вызовы и собрания, а также чат и совместную работу.
...
Для нестойкой настройки Teams на компьютере должно быть установлено золотое изображение. (Дополнительные информацию см. в [разделе Установка или обновление Teams на веб-сайте VDI.)](#install-or-update-the-teams-desktop-app-on-vdi) Это обеспечивает эффективный запуск приложения Teams во время сеанса пользователя.
...
Перед развертыванием Teams до Приложения Microsoft 365 для предприятий необходимо удалить все приложения Teams, если они были развернуты с помощью установки на компьютере.
...
Teams также добавляются в существующие Приложения Microsoft 365 для предприятий. Поскольку Приложения Microsoft 365 для предприятий установки Teams только для каждого пользователя, см. раздел Установка или обновление Teams на [веб-сайте VDI.](#install-or-update-the-teams-desktop-app-on-vdi)
...
1. Скачайте пакет Teams MSI, который соответствует вашей операционной системе VDI VM, по одной из следующих ссылок:
    ...
    > [!NOTE]
    > Ссылки для скачивания [MSI-файлов см.](msi-deployment.md) в Microsoft Teams с помощью Microsoft Endpoint Configuration Manager для загрузки облачных систем.
...
2. Установите MSI-решение в VDI VM с помощью одной из следующих команд:
    ...
    - Установка на компьютере
        ...
        Этот процесс добавляет на компьютер необходимый ключ реестра, который позволяет установщику Teams знать, что это экземпляр VDI.  Если не сделать этого, установщик выберет ошибку, указав: "Не удалось установить.  Не удается установить для всех пользователей, если среда VDI не обнаружена".

        ```console
        msiexec /i <path_to_msi> /l*v <install_logfile_name> ALLUSER=1 ALLUSERS=1
        ```

        Этот процесс Teams в папку Program Files (x86) в 64-битной операционной системе и в папку Program Files (Файлы программ) в 32-битной операционной системе. На этом этапе завершается установка "золотого образа". Установка Teams на компьютере необходима для непосохраняемой настройки.
...
- Зависимости: к ним относятся зависимости от инфраструктуры, рабочей нагрузки и других аспектов защиты среды вне Teams настольного приложения.
...
При переходе с Skype для бизнеса VDI на Teams VDI, помимо различий между двумя приложениями, при внедрении VDI существуют некоторые различия. Вот некоторые возможности, которые в настоящее время не поддерживаются Teams VDI, Skype для бизнеса VDI:
...
- В средах Citrix, если пользователь отключается от виртуальной машины во время Teams, обновления Teams могут привести к не оптимизированной работе пользователя с av-средой при повторном подключении. Мы рекомендуем пользователям выйти из Teams перед отключением от виртуальной машины Citrix, чтобы избежать этого.
...
Затем перезапустите VDA. Подробнее см. в этой статье поддержки Citrix Устранение неполадок с оптимизацией [HDX](https://support.citrix.com/article/CTX253754)для Teams.
