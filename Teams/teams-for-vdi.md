...
Виртуальный рабочий стол Azure обеспечивает оптимизацию av для Teams vDI. Дополнительные узнать о требованиях и установке см. в Teams [на виртуальном](/azure/virtual-desktop/teams-on-wvd)рабочем столе Azure.

### <a name="citrix-virtual-apps-and-desktops-requirements"></a>Требования к виртуальным приложениям и рабочим столам Citrix

Виртуальные приложения и рабочие столы Citrix (прежнее название — XenApp и XenDesktop) обеспечивают оптимизацию av для Teams vDI. Виртуальные приложения и настольные компьютеры Citrix Teams в VDI поддерживают вызовы и собрания, а также чат и совместную работу.
...
Последнюю версию VMware Horizon можно скачать на странице [Загрузки VMware.](https://my.vmware.com/web/vmware/downloads/#all_products) Необходимые компоненты оптимизации мультимедиа по умолчанию являются частью агентов Horizon и клиента Horizon, и для использования функции оптимизации для Teams не требуется устанавливать дополнительный подключаемый модуль.
...
Исключите из Teams кэшинга %appdata%/Microsoft/Teams. Исключая эти элементы, вы помогаете уменьшить размер кэша пользователей, чтобы оптимизировать неохраняемую настройку.
...
Teams по Приложения Microsoft 365 для предприятий устанавливается для каждого пользователя. Дополнительные информацию см. в разделе Установка или [обновление Teams настольного приложения на веб-сайте VDI.](#install-or-update-the-teams-desktop-app-on-vdi)
...
Дополнительные Teams и Приложения Microsoft 365 для предприятий см. в Teams новых установок [Приложения Microsoft 365 для предприятий](/DeployOffice/teams-install#how-to-exclude-microsoft-teams-from-new-installations-of-office-365-proplus) и Управление установкой [Teams.](/DeployOffice/teams-install#use-group-policy-to-control-the-installation-of-microsoft-teams)
...
1. Скачайте пакет Teams MSI, который соответствует вашей операционной системе VDI VM, по одной из следующих ссылок:
    ...
    > [!NOTE]
    > Ссылки на [MSI-файлы](msi-deployment.md) см. в Microsoft Teams с Microsoft Endpoint Configuration Manager с помощью Microsoft Endpoint Configuration Manager правительственных облаков.
...
- Зависимости: к ним относятся зависимости от инфраструктуры, рабочей нагрузки и других аспектов защиты среды за пределами Teams настольного приложения.
...
Teams браузере Chrome не предлагает замену для Teams для VDI с помощью av optimization. Взаимодействие и взаимодействие в чате работают так, как ожидалось. При необходимости мультимедиа в браузере Chrome некоторые интерфейсы могут не соответствовать ожиданиям пользователей.
...
[**Политики собраний:**](meeting-policies-in-teams.md)Teams включает встроенную политику собраний AllOff, в которой отключены все функции собраний. Назначьте политику AllOff всем пользователям в организации, Teams виртуализированной среде.
...
## <a name="migrate-teams-on-vdi-with-chat-and-collaboration-to-optimize-teams-with-calling-and-meetings"></a>Перенос Teams в VDI с помощью чата и совместной работы для оптимизации Teams звонками и собраниями
...
Когда пользователи подключаются с неподдерживания, пользователи находятся в запасном режиме, в котором av не оптимизирован. Вы можете отключить или включить режим отката, задав одно из следующих значений DWORD реестра:
...
- Быстрый переключение клиента может привести к проблемам со звонками в VDI. При перезапуске клиента эти проблемы устраняются.
...
#### <a name="teams-crashes-or-the-teams-sign-in-screen-is-blank"></a>Teams сбоя или Teams вход пустой
