...
- **Виртуальный рабочий** стол: стек виртуальных машин (VM), который запускается Microsoft Teams
...
Виртуальный рабочий стол Azure обеспечивает оптимизацию av для Teams vDI. Дополнительные узнать о требованиях и установке см. в Teams [на виртуальном рабочем](/azure/virtual-desktop/teams-on-wvd)столе Azure.
...
Чтобы обновить версию Teams до последней версии, начните с процедуры удалить, за которой следует последняя Teams версия.
...
|---------|---------|---------|
|vCPU   |    2 ядра     |  4,6 или 8<br>Важно разобраться в конфигурации доступа к памяти, которая не является единообразным, и соответствующим образом настроить свои VMs.     |
...
Для непостояной настройки необходимо установить Teams на компьютере до золотого изображения. (Подробнее см. в разделе Установка или обновление Teams настольного приложения [на веб-сайте VDI.)](#install-or-update-the-teams-desktop-app-on-vdi) Это обеспечивает эффективный запуск приложения Teams во время сеанса пользователя.

Для Teams неконстантной настройки также требуется диспетчер кэширования профилей, что Teams синхронизации данных во Teams времени запуска. Эффективная синхронизация данных обеспечивает кэширование соответствующих пользовательских сведений (например, данных, профилей или параметров) во время сеанса пользователя. Убедитесь, что данные в этих двух папках синхронизированы.<br>
...
##### <a name="teams-cached-content-exclusion-list-for-non-persistent-setup"></a>Teams списка исключений из кэшного контента для неохраняемой настройки
...
#### <a name="teams-deployments-through-microsoft-365-apps-for-enterprise-updates"></a>Teams с помощью Приложения Microsoft 365 для предприятий обновлений

Teams также добавляются в существующие установки Приложения Microsoft 365 для предприятий. Поскольку Приложения Microsoft 365 для предприятий установки Teams только для каждого пользователя, см. раздел Установка или обновление Teams на [веб-сайте VDI.](#install-or-update-the-teams-desktop-app-on-vdi)
...
Приложения Microsoft 365 для предприятий не поддерживает установку Teams на Teams. Чтобы использовать установку на компьютере, необходимо исключить Teams из Приложения Microsoft 365 для предприятий. См. [статьи Развертывание Teams на](#deploy-the-teams-desktop-app-to-the-vm) компьютере с VM и Исключение Teams помощью [Приложения Microsoft 365 для предприятий](#how-to-exclude-teams-deployment-through-microsoft-365-apps-for-enterprise) разделах.
...
1. Скачайте Teams MSI-пакет, который соответствует вашей операционной системе VDI VM, по одной из следующих ссылок:
    ...
    > [!NOTE]
    > Ссылки на [MSI-файлы](msi-deployment.md) см. в Microsoft Teams с помощью Microsoft Endpoint Configuration Manager для скачивания облачных систем.
...
3. Удалить MSI-решение из VDI VM. Удалить эти Teams можно двумя Teams.
    ...
    - Сценарий PowerShell: с помощью этого сценария [PowerShell](scripts/powershell-script-deployment-cleanup.md) можно удалить Teams и удалить Teams папку пользователя. Запустите сценарий для каждого профиля пользователя, в Teams был установлен на компьютере.
...
При переходе с Skype для бизнеса VDI на Teams VDI помимо различий между двумя приложениями, при внедрении VDI также существуют некоторые различия. Некоторые возможности, которые в настоящее время не поддерживаются Teams VDI, Skype для бизнеса VDI:
...
Вы можете использовать Microsoft Teams администрирования или PowerShell для назначения пользователям политик звонков и собраний. Распространение изменений политики может занять некоторое время (несколько часов). Если вы не видите изменений для учетной записи сразу, попробуйте еще раз через несколько часов.
...
Когда пользователи подключаются с неподдерживания, пользователи находятся в запасном режиме, в котором av не оптимизирован. Вы можете отключить или включить режим экономии времени, задав одно из следующих значений DWORD реестра:
...
Когда пользователи с этим параметром политики VDI -DisableCallsAndMeetings $true для Teams на VDI, они не смогут:
...
Когда пользователи с параметром политики VDI -DisableAudioVideoInCallsAndMeetings $true войти в Teams на VDI, они должны иметь возможность:
...
- Установка на компьютере Teams VDI не обновляется автоматически так, как Teams VDI. Вам необходимо обновить изображение VM, установив новый MSI-версию, как описано в разделе Установка или обновление Teams настольного приложения [на VDI.](#install-or-update-the-teams-desktop-app-on-vdi) Необходимо удалить текущую версию, чтобы обновиться до новой версии.
- В средах Citrix, если пользователь отключается от виртуальной машины во время Teams, Teams обновления могут привести к не оптимизированной работе пользователя для av при повторном подключении. Мы рекомендуем пользователям выйти Teams перед отключением от виртуальной машины Citrix, чтобы избежать этого.
...
- Быстрый переключение клиента может привести к проблемам со звонками в VDI, таким как демонстрация экрана недоступна, отображается неправильный список участников и т. д. При перезапуске клиента эти проблемы устраняются.
...
О Teams известных проблемах, не связанных с VDI, см. в Teams службы поддержки [в организации.](/MicrosoftTeams/troubleshoot/teams-welcome)
