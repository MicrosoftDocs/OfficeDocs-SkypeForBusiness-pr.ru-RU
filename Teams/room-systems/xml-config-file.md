...
|\<Скипесигнинаддресс\>   |Строковое &#x2778;  ||Имя для входа для учетной записи устройства Skype для бизнеса консоли.   |
|\<Ексчанжеаддресс\>   |Строковое &#x2778;  ||Имя для входа для учетной записи устройства Exchange консоли.   Если параметр "ExchangeAddress" (Адрес Exchange) пропущен, параметр "SkypeSignInAddress" (Адрес для входа Skype) не будет применяться автоматически.    |
...
|\<Дуалскринмоде\>  |Логическое &#x2777;  |Первый &#x2776;  |Если установлено значение true, включен режим двойного экрана. В противном случае устройство работает в режиме одного экрана.   |
|\<Сендлогс\>   |Контейнер   |Первый &#x2776;  ||
|\<Емаиладдрессфорлогсандфидбакк\>   |Строковое &#x2778;  ||Задает необязательный адрес электронной почты, на который отправляются журналы при открытии окна "Обратная связь".    |
...
|\<Микрофонефоркоммуникатион\>   |Строковое &#x2778;  ||Задает микрофон, который будет использоваться для записи во время конференц-связи.   |
...
&#x2777; логический флаг может принимать одно из следующих значений: истина, ложь, 0 или 1. Если логическое или числовое значение оставлено пустым, XML-файл будет иметь неправильный формат, в результате чего параметры не будут изменены.
