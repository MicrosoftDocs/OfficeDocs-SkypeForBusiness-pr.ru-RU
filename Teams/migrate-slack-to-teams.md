...
:::image type="content" source="media/migrate-slack-to-teams-image2.png" alt-text="Схема экспорта из Slack.":::
...
Список приложений и настраиваемых интеграций можно найти на странице <your Slack workspace>.slack.com/apps/manage. На этой странице также показано количество конфигураций, в которых используется каждое приложение. Настраиваемые интеграции различаются по возможностям переноса. Если это веб-перехватчик, обычно его можно отправить в соединитель Office 365 для перевода рабочего процесса в Teams. Для ботов и других приложений возможность переноса в Teams оценивается в индивидуальном порядке.

*Если ваш администратор ограничил использование приложений, возможно, полный список доступных приложений может не отображаться.

### <a name="users"></a>Пользователи
Схемы удостоверений, используемые в Slack, не всегда могут быть прямо воспроизведены в Office 365. Например, адреса электронной почты пользователей Slack могут на соответствовать рабочим или учебным учетным записям Office 365. Прежде чем планировать развертывание Team, необходимо составить карту соответствий пользователей и идентификаторов.
...
Приводим сценарий, который можно использовать для сравнения адресов электронной почты из экспорта Slack и из Microsoft Azure Active Directory и устранения неоднозначности имен. Кроме того, в нем сообщается, можно ли перенести пользователя в Teams. Если вам нужна помощь по использованию PowerShell, прочтите статью [Начало работы с Azure PowerShell](https://docs.microsoft.com/powershell/azure/get-started-azureps).
...
- список каналов в рабочей области с статистикой использования;
...
|Соответствие между Slack и Teams |  |
|---------|---------|
...
### <a name="channel-history"></a>История каналов
...
Ниже приведены примеры решений Slack, в которых соединитель Office 365 использовался в Teams для интеграции.
- Ansible
...
Этапы развертывания Teams в вашей организации приведены на следующей схеме. Дополнительные сведения см. в статье [Способ развертывания Teams](How-to-roll-out-teams.md).
