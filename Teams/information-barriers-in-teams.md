...
- Юридические права. Сохранение конфиденциальности данных, полученных клиентом от клиента, и предотвращение доступа к ним со стороны той же компании, которая представляет другого клиента.
...
- Пользователь делает телефонный звонок в **Teams.** Когда пользователь начинает голосовой звонок (по voIP) другому пользователю или группе пользователей, этот звонок оценивается таким образом, чтобы убедиться, что он не нарушает политики IB других участников группы. При нарушении голосовой звонок блокируется.

- **Гости в Teams** — политики IB также применяются к гостям в Teams. Если гости должны быть доступны для обнаружения в глобальном списке адресов организации, см. статью Управление гостевых доступом [в Microsoft 365 группах.](/microsoft-365/admin/create-groups/manage-guest-access-in-groups) Обнаружив гостей, вы можете определить политики [IB.](/office365/securitycompliance/information-barriers-policies)
...
- **Выбор пользователей:** заблокированные пользователи не будут видны в оке выбор людей.
    ...
    ![Снимок экрана Teams оповещение пользователя о том, что политика предотвращает отображение сведений другого пользователя](media/information-barriers-people-picker.png)

- **Вкладка Действия.** Если пользователь посещает вкладку **Действия** заблокированного пользователя, записи не отображаются. (На **вкладке Действия** отображаются только записи каналов, и не существует общих каналов между двумя пользователями.)
...
- **Карточка** "Люди": если пользователь участвует в беседе, а пользователь заблокирован, другие пользователи будут видеть сообщение об ошибке вместо карточки пользователя при наведении курсор на имя заблокированного пользователя. Действия, перечисленные на карточке (например, вызовы и чат), будут недоступны.
...
- **Skype** миграции Teams миграции из Skype для бизнеса в Teams все пользователи, даже те, которые заблокированы политиками IB, будут перенесены в Teams. После этого пользователи обрабатываются так, как описано выше.
