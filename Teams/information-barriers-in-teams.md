...
[Информационные барьеры](/microsoft-365/compliance/information-barriers) — это политики, которые администратор может настроить, чтобы запретить отдельным людям или группам общаться друг с другом. Они полезны, например, если один отдел занимается обработкой сведений, которые не следует совместно использовать с другими отделами. Они также полезны, если группу необходимо изолировать или запретить общаться с любыми внешними ее контактами. Общие каналы в Microsoft Teams поддерживаются информационными барьерами. В зависимости от типа общего доступа политики информационных барьеров могут ограничивать общий доступ определенными способами. Дополнительные сведения об общих каналах и информационных барьерах см. в раздел [Информационные барьеры и Общие каналы](information-barriers-shared-channels.md).
...
>- Сведения о поддержке барьеров для SharePoint, подключенных к Teams, см. в Microsoft Teams [сайтов](/sharepoint/information-barriers#segments-associated-with-microsoft-teams-sites).
...
- **Professional** служб: группа людей в компании может общаться только с клиентом или определенным клиентом с помощью гостевого доступа во время взаимодействия с клиентом.
...
- **Гости в Teams**: политики IB применяются и к гостям Teams в других Teams. Если гости должны быть доступны для обнаружения в глобальном списке адресов организации, см. статью Управление гостевым доступом [в Группы Microsoft 365](/microsoft-365/admin/create-groups/manage-guest-access-in-groups). Обнаружив гостей, вы можете определить политики [IB](/office365/securitycompliance/information-barriers-policies).
...
- **Карточка** "Люди": если пользователь участвует в беседе и пользователь заблокирован, другие пользователи будут видеть сообщение об ошибке вместо карточки пользователя при наведении курсор на имя заблокированного пользователя. Действия, перечисленные на карточке (например, вызовы и чат), будут недоступны.
...
- Skype миграции **на Teams**: во время миграции из Skype для бизнеса в Teams все пользователи, даже заблокированные политиками IB, будут перенесены в Teams. После этого пользователи обрабатываются так, как описано выше.
...
Teams, созданной перед активацией политики информационного барьера в клиенте, по умолчанию автоматически устанавливается  режим "Открыть". После активации политик IB в клиенте необходимо обновить существующие команды до неявных, чтобы убедиться,  что существующие команды соответствуют требованиям IB.
...
- Сведения о том, как настроить политики IB, [см. в начало работы с информационными барьерами](/office365/securitycompliance/information-barriers-policies).
- Сведения о том, как изменить или удалить политики IB, см. в статью [Управление политиками информационных барьеров](/microsoft-365/compliance/information-barriers-edit-segments-policies).
- [Информационные барьеры и общие каналы](information-barriers-shared-channels.md)

## <a name="availability"></a>Доступности

Информационные барьеры в Teams доступны в наших общедоступных облачных GCC, GCC - Высокий и DOD.
