...
2. Перейдите в **Центр администрирования Microsoft 365** > **центры администрирования** > **команды & Скайп** > **портала прежних версий** > **голосовой связи** > **телефонных номеров**.
...
### <a name="step-2---change-the-default-phone-number-of-your-conference-bridge-optional"></a>Шаг 2 - изменить номер телефона по умолчанию мост к конференции (необязательно)

Номер телефона по умолчанию к конференции моста определяет идентификатор звонящего, который будет использоваться при исходящий звонок будет произведен участника или организатором в рамках собрания.

Только бесплатный номер службы можно задать как номер по умолчанию для конференц-канала; **обслуживание бесплатных номеров службы не могут задаваться как количество конференц-канал по умолчанию**. Если вы хотите назначить новый номер по умолчанию для звукового конференц-канала при назначении бесплатный номер службы, выполните следующие действия:

1. Войдите в Office 365 со своей рабочей учетной записью.

2. Перейдите в **Центр администрирования Microsoft 365** > **центры администрирования** > **команды & Скайп** > **собраний** > **Мостов конференции**.

3. Выделите номер счета службы, который необходимо настроить значения по умолчанию.

4. Выберите **По умолчанию**.
 
### <a name="step-3---change-the-default-phone-numbers-that-are-included-in-the-meeting-invites-of-users-optional"></a>Шаг 3 - изменение телефонных номеров по умолчанию, которые включены в собрание приглашает пользователей (необязательно)

Номера телефонов по умолчанию пользователя, из них, включенных в свои собрания приглашает при составлении приглашения на собрание. Для получения дополнительных сведений, включая как номера телефонов по умолчанию назначаются для новых пользователей видеть [задать телефона, номера, находящимся на приглашает в группах Майкрософт](set-the-phone-numbers-included-on-invites-in-teams.md) или [телефона, номера, находящимся на приглашает в Скайп для бизнеса в Интернет](/SkypeForBusiness/audio-conferencing-in-office-365/set-the-phone-numbers-included-on-invites).
  
1. Войдите в Office 365 под своей учебной или рабочей учетной записью.

2. Перейдите в **Центр администрирования Microsoft 365** > **центры администрирования** > **команды & Скайп** > **портала прежних версий** > **аудиоконференции** > **пользователей**, а затем выберите пользователей в списке.
...
### <a name="step-4---update-existing-meeting-invites-of-users-using-the-meeting-migration-service-optional"></a>Шаг 4 - обновление существующих собраний приглашения пользователей с помощью службы миграции собрания (необязательно)

В первых двух шагах потребуется запустить Windows PowerShell.
  
Если обновление телефона по умолчанию, номеров, которые inlcuded на собрании приглашает для некоторых или всех пользователей, можно обновить приглашения на собрания, которые уже были отправлены для пользователей в вашей организации перед их номера телефона по умолчанию были изменены с помощью Служба миграции собрания. Для получения дополнительных сведений см [Копирование служба миграции собрания (MMS)](/SkypeForBusiness/audio-conferencing-in-office-365/setting-up-the-meeting-migration-service-mms).
...
3. Если номер телефона является номером бесплатный номер, выберите номер телефона из списка и в области действий выберите пункт **Снять**. Если номер телефона является номером счета, обратитесь в службу [поддержки Майкрософт](https://go.microsoft.com/fwlink/?linkid=2091806) иметь назначен номер телефона.

4. Если номер телефона является номером fre звонок, нажмите кнопку **Да** в окне подтверждения.
...
- Используйте командлет **Set-CsOnlineDialInConferencingUserDefaultNumber** для изменения платного или бесплатного номера по умолчанию для пользователей на основе исходного номера по умолчанию, свойственного для местоположения этих пользователей.
  ...
  ```
    ...
    > [!NOTE]
    > Расположение, используемое выше требований для сопоставления контактные данные пользователей, устанавливаемое в центре администрирования Microsoft 365.

## <a name="troubleshooting"></a>Поиск и устранение неполадок

**Отмена назначения кнопки серый масштабированием**

Чтобы отменить назначение номер, но кнопки серый масштабирования и если во время hoovering над ним, пользователь будет перенаправлен на обращение в службу поддержки следующее сообщение об ошибке _«по умолчанию или быть неназначенных из моста can´t общего числа. Чтобы отменить выделенные платные номера телефонов, обратитесь в службу поддержки._«.

Чтобы получить дополнительную информацию о bridge(s), выполните следующие Powershell:
```
Get-CsOnlineDialInConferencingBridge -Name "Conference Bridge"
```

Результат выделить другие сведения, как удостоверения, имя и регион, должен также содержать DefaultServiceNumber.

**Пример**, чтобы отменить, DefaultServiceNumber «8005551234»
```
