...
Каждая команда в Microsoft Teams имеет сайт группы в SharePoint Online, а каждый стандартный канал в команде получает папку в библиотеке документов сайта группы по умолчанию. Файлы, совместно используемые во время беседы, автоматически добавляются в библиотеку документов, а заданные в SharePoint разрешения и параметры безопасности файлов автоматически переносятся в Teams. Чтобы увидеть последствия изменения адреса сайта в SharePoint, см. [статью "Изменение адреса сайта".](https://docs.microsoft.com/sharepoint/change-site-address)

> [!NOTE]
> Эта статья относится только к стандартным каналам. Архитектура частных каналов отличается от стандартных каналов. Каждый частный канал имеет собственное веб-собрание SharePoint, отдельное от родительского сайта группы. Подробнее см. в [закрытых каналах Microsoft Teams.](private-channels.md)

Приватные файлы чата хранятся в папке OneDrive для бизнеса отправщика, и разрешения автоматически делиться файлами делиться с ними делиться данными делиться данными с их участниками автоматически.
