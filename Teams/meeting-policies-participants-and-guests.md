...
> [!NOTE]
> Если этот параметр отключен, а пользователь, входящего в телефонное окно, присоединяется к собранию первым и помещается в " организации, он должен присоединиться к собранию с помощью клиента Teams, чтобы допустить пользователя из "Teams". Для пользователей, подключающихся по телефону, недоступны элементы управления "зала ожидания".
