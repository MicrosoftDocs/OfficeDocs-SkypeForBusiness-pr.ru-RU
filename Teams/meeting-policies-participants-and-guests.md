...
> [!NOTE]
> Если этот параметр отключен, а пользователь, входящего в телефонное окно, присоединяется к собранию и помещается в " в "Teams", чтобы допустить пользователя из "Teams". Для пользователей, подключающихся по телефону, недоступны элементы управления "зала ожидания".
