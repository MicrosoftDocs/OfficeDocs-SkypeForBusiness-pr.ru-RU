...
Вы можете назначить политику собраний группе безопасности, участниками или непосредственно пользователями в рамках назначения пакетной политики. После прочтения этой статьи вы сможете следующее.
...
В этом учебнике учащиеся получат глобальную политику собраний, и мы назначим сотрудникам и преподавателям настраиваемую политику собраний с именем EducatorMeetingPolicy. Мы предполагаем, что вы влияли на глобальную [](policy-packages-edu.md) политику, чтобы настроить параметры собраний для учащихся, и создали настраиваемую политику, которая определяет параметров собраний для преподавателей и преподавателей.
...
> [!NOTE]
> В настоящее время назначение политик группам, использующим Центр администрирования Microsoft Teams, доступно только для политик Teams звонков, политики Teams звонков, политики Teams, политики трансляций Teams, политики собраний Teams и политики Teams сообщений. Для других типов политик используйте PowerShell.
...
Чтобы установить модуль [Teams PowerShell,](https://www.powershellgallery.com/packages/MicrosoftTeams) запустите следующую: Установите версию 1.0.5 или более поздней.
...
Выполните эти действия, чтобы массово назначить настраиваемую политику собраний с именем EducatorMeetingPolicy сотрудникам и преподавателям.
...
##### <a name="install-and-connect-to-the-azure-ad-powershell-for-graph-module"></a>Установка и подключение модуля Azure AD PowerShell для Graph
...
Подробнее см. в Подключение [с помощью Azure Active Directory PowerShell для Graph.](/office365/enterprise/powershell/connect-to-office-365-powershell#connect-with-the-azure-active-directory-powershell-for-graph-module)
...
Каждое массовое назначение возвращает ид операции, с помощью которой можно отслеживать ход выполнения назначений политики или выявлять сбои, которые могут возникнуть. Например, запустите следующую:
...
Вы можете изменить диапазон ид пользователей, пока не достигнете полного списка пользователей. Например, введите для первого пакета, используйте второй пакет, для третьего пакета и так ```$faculty[0..4999``` ```$faculty[5000..9999``` ```$faculty[10000..14999``` далее.
