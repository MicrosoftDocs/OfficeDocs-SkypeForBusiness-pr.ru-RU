...
> [!WARNING]
> Если вы настроили определенные команды или конкретные пользователи, которые нужно включить в сообщения канала группы или чат, и изменить их, чтобы удалить последнюю из них для расположения, конфигурация для этого **места будет восстановлена.** Убедитесь в том, что это конфигурация, которую вы собираетесь использовать перед сохранением политики.
> 
> Например, если вы указали одного пользователя в чате, чтобы включить его в политику хранения, которая настроена на удаление данных, а затем изменить политику для удаления этого пользователя, по умолчанию все пользователи будут подвергаться политике хранения, который окончательно удаляет сообщения в чате для участников группы. Это относится и к сообщениям каналов групп.
> 
> Если вы не хотите, чтобы параметр " **все** " для сообщений канала группы или сообщений в команде не подключались к политике хранения, переключите расположение. Кроме того, можно указать исключения, которые следует исключить из политики.
...
Для частных чатов (чатов в 1:1) или групповых чатов конечные пользователи будут видеть, что все сеансы старше конфигурации политики хранения удаляются, а сообщение об ошибке "мы удалили старые сообщения из-за политики хранения", которое отображается поверх еще не удаленных сообщений.
