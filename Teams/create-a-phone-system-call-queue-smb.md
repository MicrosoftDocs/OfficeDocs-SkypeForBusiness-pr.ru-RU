---
title: Создание очереди вызовов Microsoft Teams
author: DaniEASmith
ms.author: danismith
manager: serdars
...
ms.locfileid: "69763690"
---
# <a name="create-a-microsoft-teams-call-queue"></a>Создание очереди вызовов Microsoft Teams 
...
Описанные в этой статье действия по созданию очередей звонков используют Центр администрирования Teams. Инструкции по созданию очередей вызовов с помощью PowerShell см. в статье [Создание очередей вызовов с помощью командлетов PowerShell](create-a-phone-system-call-queue-via-cmdlets.md).
...
- Назначьте [лицензию Телефонная система Microsoft Teams учетной записи системных ресурсов](manage-resource-accounts.md#assign-a-license).
- Назначьте лицензию плана звонков Майкрософт, назначьте номер телефона Operator Connect или назначьте политику маршрутизации голосовой связи через Интернет для прямой маршрутизации.
- Назначьте [номер телефона учетной записи ресурса](manage-resource-accounts.md#assign-a-phone-number), если вы используете план звонков Майкрософт.
...
Чтобы [использовать канал Teams для управления очередью, выполните следующие](https://support.microsoft.com/office/9f07dabe-91c6-4a9b-a545-8ffdddd2504e) действия:
...
1. Выберите канал, который вы хотите использовать (поддерживаются только стандартные или частные каналы), а затем нажмите **кнопку Применить**.
...
> 
> Вызовы будут распределяться между всеми участниками команды, даже если частный канал содержит только подмножество участников команды.
...
1. Выберите **Добавить группы**, найдите группу, нажмите кнопку **Добавить**, а затем нажмите **кнопку Добавить**. 
    1. Вы можете использовать списки рассылки, группы безопасности и группы Microsoft 365 или Команды Microsoft Teams.
...
Учетные записи Teams агентов должны быть настроены в `TeamsOnly` режиме . Агенты, которые не соответствуют требованиям, не включаются в список маршрутизации звонков. Мы рекомендуем включить режим конференции для очередей звонков, если агенты используют совместимые клиенты.

> [!TIP]
> Рекомендуется установить **режим конференции** в **значение Включено** .

> [!NOTE]
> Режим конференции не поддерживается для вызовов, которые направляются в очередь из шлюза прямой маршрутизации, который включен для маршрутизации Location-Based.
>
> Режим конференции не поддерживается для вызовов, которые направляются в очередь из Skype для бизнеса Server.
> 
...
**Время ожидания вызова: максимальное время ожидания** указывает максимальное время, в течение времени, в течение времени, в течение всего времени ожидания звонка в очереди перед перенаправлением или отключением.
...
3. При выборе параметра Самый длительный простой для метода маршрутизации агента автоматически будет включена маршрутизация Presence-Based.
4. Невозможно задать порядок вызова агентов.
5. Режим конференции не поддерживается, если телефонные звонки направляются в очередь из шлюза прямой маршрутизации, который включен для маршрутизации Location-Based.
...
- Mac Skype для бизнеса client (версия 16.8.196 и более поздние версии).
- Android Skype для бизнеса Client (версия 6.16.0.9 и более поздние версии).
- Клиент Skype для бизнеса iPhone (версия 6.16.0 и более поздние версии).
- Клиент iPad Skype для бизнеса (версия 6.16.0 и более поздние версии).
- Клиент Microsoft Teams для Windows (32-разрядная и 64-разрядная версии).
- Клиент Microsoft Teams для Mac.
- [Инфраструктура виртуализированных рабочих столов](teams-for-vdi.md) Microsoft Teams (Виртуальный рабочий стол Windows, Citrix и VMware).
...
1. Выберите **Выполнить тесты** ниже, чтобы заполнить диагностику в Центр администрирования Microsoft 365.
...
[Вот что вы получаете с помощью системы Телефонная система Microsoft Teams](here-s-what-you-get-with-phone-system.md)
