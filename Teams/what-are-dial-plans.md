...
> [!NOTE]
> В сценарии, в котором правила нормализации телефонной программы не применяются к набраному номеру, строка набора по-прежнему будет нормализована до предварительного "+CC", где "CC" — это код страны использования пользователя, набираемого номера. Это относится к планам звонков, прямой маршрутике и конференц-связи по ЗВОНКОВ через ОКП.
