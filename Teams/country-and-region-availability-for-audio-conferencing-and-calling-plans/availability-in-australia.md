---
title: Доступность в Австралии
author: CarolynRowe
ms.author: crowe
manager: serdars
ms.reviewer: mikedav, jastark, oscarr
ms.topic: reference
ms.assetid: 98a819e4-6dd9-47ee-8c79-586aae727111
ms.tgt.pltfrm: cloud
ms.service: msteams
ms.collection:
- M365-collaboration
- M365-voice
search.appverid: MET150
audience: Admin
appliesto:
- Skype for Business
- Microsoft Teams
localization_priority: Normal
ROBOTS: NOINDEX, NOFOLLOW
f1.keywords:
- NOCSH
ms.custom:
- Audio Conferencing
- Phone System
- Calling Plans
description: Ниже приведены службы Office 365 для Skype для бизнеса и Microsoft Teams, доступные для Австралии.
ms.openlocfilehash: acbde97d1b0facf077b175650d4748318d53b591
ms.sourcegitcommit: 19f534bfafbc74dbc2d381672b0650a3733cb982
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 02/03/2020
ms.locfileid: "41695434"
---
# <a name="availability-in-australia"></a>Доступность в Австралии

[Найдите другую страну или регион](country-and-region-availability-for-audio-conferencing-and-calling-plans.md) или узнайте больше о доступности стран и регионов для аудио конференций и тарифных планов.

Ниже приведены службы Office 365 для Skype для бизнеса и Microsoft Teams, доступные для Австралии.

## <a name="audio-conferencing"></a>Аудиоконференции

|**Функция**|**Details**|
|:-----|:-----|
|Доступно ли приобретение аудиоконференций?  <br/> |Да  <br/> |
|Назначаются ли номера телефонов для аудиоконференций автоматически?  <br/> |Нет. [Свяжитесь с нами](mailto:ptnapac@microsoft.com). <br/> |
|Возможность получить обратный звонок с функцией "Звонок на мой номер"  <br/> |В сети  <br/> |

## <a name="phone-system"></a>Телефонная система

|**Функция**|**Details**|
|:-----|:-----|
|Доступно ли приобретение телефонной системы?  <br/> |Да  <br/> |
|Автосекретари <br/> |В сети  <br/> |
|Очередь звонков  <br/> |В сети  <br/> |
|Голосовая почта  <br/> |Доступно  <br/> |

## <a name="calling-plans"></a>Планы звонков

|**Функция**|**Details**|
|:-----|:-----|
|Доступно ли приобретение планов звонков?  <br/> |Да, смотрите [Telstra звонков для Office 365](https://aka.ms/TelstraVoicePlan).  <br/> |
|Доступные минуты для планов звонков  <br/> |[Дополнительные сведения о Telstra звонков для Office 365](https://aka.ms/TelstraVoicePlan). <br/> |
|Кому можно делать исходящие звонки?  <br/> |[Дополнительные сведения о Telstra звонков для Office 365](https://aka.ms/TelstraVoicePlan). <br/> |

> [!IMPORTANT]
> Чтобы удалить австралийские номера абонентов (предоставленные компанией Telstra), свяжитесь с Telstra напрямую.
> Чтобы обновить сведения об адресах для австралийского абонентского номера (предоставлено компанией Telstra), свяжитесь с Telstra напрямую.
> Для получения дополнительных сведений см. [вызов Telstra для Office 365](https://aka.ms/TelstraVoicePlan).

### <a name="if-your-tenant-is-located-outside-australia"></a>Если ваш клиент находится за пределами Австралии

Начиная с 1 сентября 2019, если ваш клиент находится за пределами Австралии, вы можете приобрести планы звонков для пользователей, которые находятся в Австралии. Эта возможность доступна для следующих стран и регионов.

|||
|---------|---------|
|Северная и Южная Америка    |США, внешние острова США      |
|Азиатско-Тихоокеанский регион     |Бангладеш, Бутан, Бруней, Камбоджа, Гонконг, Индонезия, Лаос, Макао, Малайзия, Мальдивские, Мьянма,, от|
|Европа, Ближний Восток и Африка    |Аландские острова, Андорра, Австрия, Бельгия, Буве острова, Болгария, Хорватия, Кипр, Чешская Республика, Дания, Эстония Фарерские острова, Финляндия, Франция, Германия, Гибралтар, Греция, Гренландия, Гернси, Венгерская, Исландия, Ирландия, остров, г., Нью-Йорк, г., Шпицберген, Лихтенштейн, Литва, нидерландский, Словакия, Монако, Нидерланды, Норвегия, Швеция, Португалия, Польша, Великобритания, Латвия  |

Обратите внимание, что вызов Telstra для Office 365 будет показываться в качестве надстройки только для пользователей, которые находятся в Австралии, и указанных выше стран и регионов, после того как они будут предоставлены Telstra. Для пользователей из других географических регионов они будут по мере необходимости подключены к планам звонков Microsoft и прямой маршрутизации. Для получения дополнительных сведений см. [вызов Telstra для Office 365](https://aka.ms/TelstraVoicePlan).

## <a name="toll-and-toll-free-numbers-for-services"></a>Платные и бесплатные номера служб

|**Функция**|**Details**|
|:-----|:-----|
|Для каких столиц или крупных городов можно получить платные номера?  <br/> | <ul><li>Аделаида <li>Брисбен <li>  Канберра <li>Дарвин <li>Мельбурн <li>Ньюкасл <li> Перт <li>Сидней <li>Вуллонгонг  |
|Доступны ли платные номера?  <br/> |Да  <br/> |
|Доступны ли бесплатные номера?  <br/> |Да  <br/> |

Если вы хотите получить бесплатные и платные телефонные номера для автосекретарей, очереди звонков и номера службы голосовой связи, [пожалуйста, свяжитесь с нами](mailto:ptnapac@microsoft.com).

## <a name="communications-credits"></a>Кредиты на связь

|**Функция**|**Details**|
|:-----|:-----|
|Доступны ли кредиты на связь?  <br/> |Да  <br/> |
   
Если они нужны вам, см. статью [Настройка кредитов на связь для организации](../set-up-communications-credits-for-your-organization.md).
  
## <a name="related-topics"></a>Статьи по теме

[Получение номеров телефонов служб для Skype для бизнеса и Microsoft Teams](/microsoftteams/getting-service-phone-numbers)

[Доступность аудиоконференций и планов звонков в различных странах и регионах](country-and-region-availability-for-audio-conferencing-and-calling-plans.md)
