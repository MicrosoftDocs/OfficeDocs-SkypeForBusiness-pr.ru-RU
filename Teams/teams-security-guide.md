...
Протоколы TLS и MTLS обеспечивают зашифрованный обмен данными и проверку подлинности конечной точки в Интернете. Teams использует эти два протокола для создания сети доверенных серверов и для шифрования все данных, передающихся по этой сети. Весь обмен данными между серверами осуществляется по протоколу MTLS. Сохранившиеся или устаревшие каналы обмена данными SIP между клиентом и сервером используют протокол TLS.
...
Потоки звонков в Teams основаны на модели предложения по [протоколу SDP RFC 4566](https://tools.ietf.org/html/rfc4566) и ответов по протоколу HTTPS. Когда вызываемый абонент соглашается принять входящий звонок, вызывающая сторона и вызываемая сторона согласуют параметра сеанса.
