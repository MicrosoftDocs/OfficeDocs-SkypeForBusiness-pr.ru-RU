...
- ПИН-коды могут содержать от 4 до 12 цифр, по умолчанию их число равно 5. При создании ПИН-кода используются только цифры. Буквы и специальные знаки не применяются.
    
- ПИН-код требуется организатору собрания только в том случае, если пользователь Microsoft Teams еще не начал собрание. Если все подключаются к собранию по телефону, то ПИН-код необходим организатору для начала собрания.
    
- Параметры безопасности ПИН-кода применяются ко всем номерам телефона, связанными с мостом Microsoft. Они применяются ко всем собраниям, использующим телефонные номера, которые сопоставлены с данным мостом. 
