...
Если вы не используете канал Teams для управления членством в агентах, рассмотрите возможность непосредственного настройки ИД звоняющего для участников очереди вызовов номером службы в очереди вызовов или соответствующим автослужатом. Дополнительные [сведения см.](caller-id-policies.md) в Microsoft Teams управление политиками ИД звоня.
...
> [!NOTE]
> Режим конференции не поддерживается, если телефонные звонки перенаправят в очередь из шлюза прямой маршрутации, который включен для маршрутизов с учетом расположения.
...
> 
> Агенты, которые используют Skype для бизнеса клиента, не включаются в список маршрутов зовов, если включена маршрутия на основе присутствия. Если у вас есть агенты, которые Skype для бизнеса, не в enable presence-based call routing.
...
Для агентов, которые находятся в очереди зовов, поддерживаются следующие клиенты:
  ...
  - Microsoft Teams Приложение для Android
    ...
    > [!NOTE]
    > Очереди вызовов, которые имеют прямой маршрутный номер, не поддерживают Skype для бизнеса клиентов, клиентов Lync и ip-телефонов Skype для бизнеса в качестве агентов. Клиент Teams поддерживается только в режиме Teams [.](/microsoftteams/setting-your-coexistence-and-upgrade-settings)
