---
title: Проверка работоспособности службы для Microsoft Teams
ms.reviewer: ''
author: LolaJacobsen
ms.author: lolaj
manager: serdars
ms.date: 09/25/2017
ms.topic: article
ms.service: msteams
MS.collection:
- Teams_ITAdmin_Help
- M365-collaboration
search.appverid: MET150
description: Рекомендуется проверять работоспособность службы Teams, а также других компонентов Office 365, например Exchange, SharePoint и OneDrive для бизнеса.
appliesto:
- Microsoft Teams
ms.openlocfilehash: 3e4365d253148377fc61cd9bdb6300f7c76df298
ms.sourcegitcommit: da8c037bb30abf5d5cf3b60d4b71e3a10e553402
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 03/27/2019
ms.locfileid: "30879950"
---
<a name="verify-service-health-for-microsoft-teams"></a>Проверка работоспособности службы для Microsoft Teams
===========================================

Работоспособность службы Microsoft Teams указана на главной странице портала администрирования Office 365. Ее лучше всего проверить перед устранением неполадок.

Кроме того, учтите, что служба Microsoft Teams основана на дополнительных службах Office 365, поэтому, проверяя ее работоспособность, не забудьте проверить и состояние Exchange, SharePoint и OneDrive для бизнеса. Проблемы с их работоспособностью еще не означают, что затронута служба Microsoft Teams (например, недоступно скачивание адресной книги в Exchange), однако следует проконсультироваться со справочными материалами по этим службам.

![Снимок экрана со страницей работоспособности службы.](media/Verify_service_health_for_Microsoft_Teams_image1.png)

![Снимок экрана со сведениями о том, что служба Microsoft Teams работоспособна.](media/Verify_service_health_for_Microsoft_Teams_image2.png)
